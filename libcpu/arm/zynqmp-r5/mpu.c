...
void Init_MPU(void) {
  ...
  size =
      (XPAR_PSU_R5_DDR_0_S_AXI_HIGHADDR - XPAR_PSU_R5_DDR_0_S_AXI_BASEADDR) + 1;
  if (size < 0x80000000) {
    /* Lookup the size.  */
      ...
      if (size <= region_size[i].size) {
        ...
        if (region_size[i].size > (size + Offset + 1)) {
          rt_kprintf("WARNING: DDR size mapped to Cortexr5 processor is not \
                     ...
                     region_size[i].size - 1,
                     ((u32)XPAR_PSU_R5_DDR_0_S_AXI_HIGHADDR + 1));
        }
        break;
      }
    }
...
static void Xil_SetAttribute(u32 addr, u32 reg_size, s32 reg_num, u32 attrib) {
  ...
  mtcp(XREG_CP15_MPU_REG_SIZE_EN,
       Local_reg_size); /* set the region size and enable it*/
  dsb();
