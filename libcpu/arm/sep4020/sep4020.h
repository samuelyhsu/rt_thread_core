...
#define RTC_CONFIG_CHECK                                                       \
  (RTC_BASE + 0X024) /* 配置时间确认寄存器 (在配置时间之前先写0xaaaaaaaa) */
...
#define SDC_INTERRUPT_STATUS_MASK                                              \
  (SD_BASE + 0X34) /* SDIO中断状态屏蔽寄存器                          \
                    */
...
#define USBD_BCONFIGURATIONVALUE                                               \
  (USBD_BASE + 0X010)                         /* SET_CCONFIGURATION记录 */
...
#define USBD_EP1OUTMAXPKTSIZE                                                  \
  (USBD_BASE + 0X134) /* 端点1输出最大包尺寸寄存器 */
#define USBD_EP1OUTIFNUM (USBD_BASE + 0X138) /* 端点1输出接口号寄存器 */
#define USBD_EP1OUTSTAT (USBD_BASE + 0X13C) /* 端点1输出状态寄存器 */
#define USBD_EP1OUTBMREQTYPE                                                   \
  (USBD_BASE + 0X140) /* 端点1输出SETUP事务请求类型 */
#define USBD_EP1OUTBREQUEST                                                    \
  (USBD_BASE + 0X144) /* 端点1输出SETUP事务请求内容                  \
                       */
#define USBD_EP1OUTWVALUE (USBD_BASE + 0X148) /* 端点1输出SETUP事务请求值 */
#define USBD_EP1OUTWINDX (USBD_BASE + 0X14C) /* 端点1输出SETUP事务请求索引 */
#define USBD_EP1OUTWLENGH                                                      \
  (USBD_BASE + 0X150) /* 端点1输出SETUP事务请求域长度               \
                       */
...
#define USBD_EP1INMAXPKTSIZE                                                   \
  (USBD_BASE + 0X160) /* 端点1输入最大包尺寸寄存器                 \
                       */
#define USBD_EP1INIFNUM (USBD_BASE + 0X164) /* 端点1输入接口号寄存器 */
#define USBD_EP1INSTAT (USBD_BASE + 0X168)  /* 端点1输入状态寄存器 */
#define USBD_EP1INBMREQTYPE                                                    \
  (USBD_BASE + 0X16C) /* 端点1输入SETUP事务请求类型                  \
                       */
#define USBD_EP1INBREQUEST                                                     \
  (USBD_BASE + 0X170) /* 端点1输入SETUP事务请求内容                  \
                       */
#define USBD_EP1INWVALUE (USBD_BASE + 0X174) /* 端点1输入SETUP事务请求值 */
#define USBD_EP1INWINDEX (USBD_BASE + 0X178) /* 端点1输入SETUP事务请求索引 */
#define USBD_EP1INWLENGTH                                                      \
  (USBD_BASE + 0X17C) /* 端点1输入SETUP事务请求域长度               \
                       */
...
#define USBD_EP2OUTMAXPKTSIZE                                                  \
  (USBD_BASE + 0X18C) /* 端点2输出最大包尺寸寄存器 */
#define USBD_EP2OUTIFNUM (USBD_BASE + 0X190) /* 端点2输出接口号寄存器 */
#define USBD_EP2OUTSTAT (USBD_BASE + 0X194) /* 端点2输出状态寄存器 */
#define USBD_EP2OUTBMREQTYPE                                                   \
  (USBD_BASE + 0X198) /* 端点2输出SETUP事务请求类型 */
#define USBD_EP2OUTBREQUEST                                                    \
  (USBD_BASE + 0X19C) /* 端点2输出SETUP事务请求内容                  \
                       */
#define USBD_EP2OUTWVALUE (USBD_BASE + 0X1A0) /* 端点2输出SETUP事务请求值 */
#define USBD_EP2OUTWINDEX (USBD_BASE + 0X1A4) /* 端点2输出SETUP事务请求索引 */
#define USBD_EP2OUTWLENGTH                                                     \
  (USBD_BASE + 0X1A8) /* 端点2输出SETUP事务请求域长度 */
...
#define USBD_EP2INMAXPKTSIZE                                                   \
  (USBD_BASE + 0X1B8) /* 端点2输入最大包尺寸寄存器                 \
                       */
#define USBD_EP2INIFNUM (USBD_BASE + 0X1BC) /* 端点2输入接口号寄存器 */
#define USBD_EP2INSTAT (USBD_BASE + 0X1C0)  /* 端点2输入状态寄存器 */
#define USBD_EP2INBMREQTYPE                                                    \
  (USBD_BASE + 0X1C4) /* 端点2输入SETUP事务请求类型                  \
                       */
#define USBD_EP2INBREQUEST                                                     \
  (USBD_BASE + 0X1C8) /* 端点2输入SETUP事务请求内容                  \
                       */
#define USBD_EP2INWVALUE (USBD_BASE + 0X1CC) /* 端点2输入SETUP事务请求值 */
#define USBD_EP2INWINDEX (USBD_BASE + 0X1D0) /* 端点2输入SETUP事务请求索引 */
#define USBD_EP2INWLENGTH                                                      \
  (USBD_BASE + 0X1D4) /* 端点2输入SETUP事务请求域长度               \
                       */
...
#define GPIO_DBCLK_DIV                                                         \
  (GPIO_BASE + 0X000) /* 去毛刺采用时钟分频比配置寄存器         \
                       */
#define GPIO_PORTA_DIR (GPIO_BASE + 0X004) /* A组端口输入输出方向配置寄存器 */
#define GPIO_PORTA_SEL (GPIO_BASE + 0X008) /* A组端口通用用途选择配置寄存器 */
#define GPIO_PORTA_INCTL                                                       \
  (GPIO_BASE + 0X00C) /* A组端口通用用途输入时类型配置寄存器 */
#define GPIO_PORTA_INTRCTL                                                     \
  (GPIO_BASE + 0X010) /* A组端口中断触发类型配置寄存器 */
#define GPIO_PORTA_INTRCLR                                                     \
  (GPIO_BASE + 0X014) /* A组端口通用用途中断清除配置寄存器 */
#define GPIO_PORTA_DATA                                                        \
  (GPIO_BASE + 0X018) /* A组端口通用用途数据配置寄存器           \
                       */
...
#define EMI_SDCONF2                                                            \
  (EMI_BASE + 0X01C) /* SDRAM时序配置寄存器2, SDRAM初始化用到的配置信息 */
#define EMI_REMAPCONF                                                          \
  (EMI_BASE + 0X020) /* 片选空间及地址映射REMAP配置寄存器        \
                      */
...
#define DMAC_INTTCCLEAR                                                        \
  (DMAC_BASE + 0X060) /* DMAC传输完成中断状态清除寄存器           \
                       */
#define DMAC_INTERRORSTATUS                                                    \
  (DMAC_BASE + 0X080) /* DMAC传输错误中断状态寄存器                 \
                       */
#define DMAC_INTINTERRCLR                                                      \
  (DMAC_BASE + 0X090) /* DMAC传输错误中断状态清除寄存器 */
...
#define LCDC_LGPMR                                                             \
  (LCDC_BASE + 0X040) /* 灰度调色映射寄存器组 (16个32bit寄存器) */
...
#define set_plevel(plevel)                                                     \
  *(RP)INTC_IPLR =                                                             \
      plevel // 设置普通中断的优先级门限，只有优先级大于此值的中断才能通过
#define set_int_force(intnum)                                                  \
  *(RP)INTC_IFR = (1 << intnum) // 置1后，软件强制该位对应的中断源发出中断信号
#define enable_irq(intnum)                                                     \
  *(RP)INTC_IER |= (1 << intnum) // 置1后，允许中断源的IRQ 中断信号
#define disable_irq(intnum)                                                    \
  *(RP)INTC_IER &= ~(1 << intnum) // 置0后，不允许中断源的IRQ 中断信号
#define mask_irq(intnum)                                                       \
  *(RP)INTC_IMR |= (1 << intnum) // 置1后，屏蔽对应的IRQ 中断信号
#define unmask_irq(intnum)                                                     \
  *(RP)INTC_IMR &= ~(1 << intnum) // 置0后，通过对应的IRQ 中断信号
...
#define disable_all_irq()                                                      \
  *(RP)INTC_IER = 0X00000000 // 不允许中断源的IRQ 中断信号
#define InitInt()                                                              \
  do {                                                                         \
    mask_all_irq();                                                            \
    enable_all_irq();                                                          \
