...
rt_inline rt_uint32_t cp15_rd(void) {
  ...
  __asm volatile { mrc p15, 0, i, c1, c0, 0 }
...
rt_inline void cache_enable(rt_uint32_t bit) {
  ...
  __asm volatile {
    mrc p15, 0, value, c1, c0, 0 orr value, value, bit mcr p15, 0, value, c1,
        c0, 0
  }
}
...
rt_inline void cache_disable(rt_uint32_t bit) {
  ...
  __asm volatile {
    mrc p15, 0, value, c1, c0, 0 bic value, value, bit mcr p15, 0, value, c1,
        c0, 0
  }
}
#endif
 ...
 *
 */
void rt_hw_cpu_icache_enable() { cache_enable(ICACHE_MASK); }

/**
 * disable I-Cache
 *
 */
void rt_hw_cpu_icache_disable() { cache_disable(ICACHE_MASK); }

/**
 * return the status of I-Cache
 *
 */
rt_base_t rt_hw_cpu_icache_status() { return (cp15_rd() & ICACHE_MASK); }

/**
 * enable D-Cache
 *
 */
void rt_hw_cpu_dcache_enable() { cache_enable(DCACHE_MASK); }

/**
 * disable D-Cache
 *
 */
void rt_hw_cpu_dcache_disable() { cache_disable(DCACHE_MASK); }

/**
 * return the status of D-Cache
 *
 */
rt_base_t rt_hw_cpu_dcache_status() { return (cp15_rd() & DCACHE_MASK); }
