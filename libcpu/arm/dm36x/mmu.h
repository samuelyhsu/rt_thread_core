...
#define SECT_AP_RO                                                             \
  ((0 << 10) | (0 << 11)) // supervisor=RO, user=NO Access(SR=10)

#define SECT_RW_CB                                                             \
  (SECT_AP_RW | PGD_DOMAIN(0) | PGD_SECT_WB | PGD_TYPE_SECT |                  \
   PGD_BIT4) /* Read/Write, cache, write back */
#define SECT_RW_CNB                                                            \
  (SECT_AP_RW | PGD_DOMAIN(0) | PGD_SECT_WT | PGD_TYPE_SECT |                  \
   PGD_BIT4) /* Read/Write, cache, write through */
#define SECT_RW_NCNB                                                           \
  (SECT_AP_RW | PGD_DOMAIN(0) | PGD_TYPE_SECT |                                \
   PGD_BIT4) /* Read/Write without cache and write buffer */
#define SECT_RW_FAULT                                                          \
  (SECT_AP_RW | PGD_DOMAIN(1) | PGD_TYPE_SECT |                                \
   PGD_BIT4) /* Read/Write without cache and write buffer */

#define SECT_RO_CB                                                             \
  (SECT_AP_RO | PGD_DOMAIN(0) | PGD_SECT_WB | PGD_TYPE_SECT |                  \
   PGD_BIT4) /* Read Only, cache, write back */
#define SECT_RO_CNB                                                            \
  (SECT_AP_RO | PGD_DOMAIN(0) | PGD_SECT_WT | PGD_TYPE_SECT |                  \
   PGD_BIT4) /* Read Only, cache, write through */
#define SECT_RO_NCNB                                                           \
  (SECT_AP_RO | PGD_DOMAIN(0) | PGD_TYPE_SECT |                                \
   PGD_BIT4) /* Read Only without cache and write buffer */
#define SECT_RO_FAULT                                                          \
  (SECT_AP_RO | PGD_DOMAIN(1) | PGD_TYPE_SECT |                                \
   PGD_BIT4) /* Read Only without cache and write buffer */

#define SECT_TO_PAGE                                                           \
  (PGD_DOMAIN(0) | PGD_TYPE_TABLE |                                            \
   PGD_BIT4) /* Level 2 descriptor (PTE) entry properity */
...
#define PAGE_RW_CB                                                             \
  (PAGE_AP_RW | PAGE_CB | PTE_TYPE_SMALL) /* Read/Write, cache, write back */
#define PAGE_RW_CNB                                                            \
  (PAGE_AP_RW | PAGE_CNB |                                                     \
   PTE_TYPE_SMALL) /* Read/Write, cache, write through */
#define PAGE_RW_NCNB                                                           \
  (PAGE_AP_RW | PTE_TYPE_SMALL) /* Read/Write without cache and write buffer   \
                                 */
#define PAGE_RW_FAULT                                                          \
  (PAGE_AP_RW | PTE_TYPE_SMALL) /* Read/Write without cache and write buffer   \
                                 */

#define PAGE_RO_CB                                                             \
  (PAGE_AP_RO | PAGE_CB | PTE_TYPE_SMALL) /* Read Only, cache, write back */
#define PAGE_RO_CNB                                                            \
  (PAGE_AP_RO | PAGE_CNB | PTE_TYPE_SMALL) /* Read Only, cache, write through  \
                                            */
#define PAGE_RO_NCNB                                                           \
  (PAGE_AP_RO | PTE_TYPE_SMALL) /* Read Only without cache and write buffer */
#define PAGE_RO_FAULT                                                          \
  (PAGE_AP_RO | PTE_TYPE_SMALL) /* Read Only without cache and write buffer */
