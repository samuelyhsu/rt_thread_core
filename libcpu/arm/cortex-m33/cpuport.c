...
rt_uint8_t *rt_hw_stack_init(void *tentry, void *parameter,
  ...
  stack_frame->tz = 0x00; /* trustzone thread context */
                           ...
                           * +--------+---+---+------+-------+------+-------+---+----+
                           * PREFIX [31:24]  - Indicates that this is an EXC_RETURN value. This field
                           * reads as 0b11111111.                         S      [6]      - Indicates
                           * whether registers have                         been pushed to a Secure or Non-secure stack.                         0:
                           * Non-secure stack used.                         1:                         Secure stack used.                         DCRS
                           * [5]      - Indicates whether the default stacking                         rules apply, or whether
                           * the callee registers are already on the stack.                         0:
                           * Stacking of the callee saved registers is skipped.                         1: Default rules for
                           * stacking the callee registers are followed.                         FType
                           * [4]      - In a PE with                         the Main and Floating-point Extensions:                         0: The PE
                           * allocated space on the                         stack for FP context.                         1: The
                           * PE did not allocate space on the stack for FP                         context.                         In a PE without the
                           * Floating-point Extension, this bit is                         Reserved, RES1.                         Mode   [3]      -
                           * Indicates the mode that was stacked from.                         0: Handler mode.                         1: Thread mode.
   ...
   * +---+------+------+-------+-------+
   * SFPA   [3]      - Indicates that the floating-point registers contain
   * active state that belongs to the Secure state: 0: The floating-point
   * registers do not contain state that belongs to the Secure state. 1: The
   * floating-point registers contain state that belongs to the Secure state.
   *                    This bit is not banked between Security states and
   * RAZ/WI from Non-secure state. FPCA   [2]      - Indicates whether
   * floating-point context is active: 0: No floating-point context active. 1:
   * Floating-point context active. This bit is used to determine whether to
   * preserve floating-point state when processing an exception. This bit is not
   * banked between Security states. SPSEL  [1]      - Defines the currently
   * active stack pointer: 0: MSP is the current stack pointer. 1: PSP is the
   * current stack pointer. In Handler mode, this bit reads as zero and ignores
   * writes. The CortexM33 core updates this bit automatically onexception
   * return. This bit is banked between Security states. nPRIV  [0]      -
   * Defines the Thread mode privilege level: 0: Privileged. 1: Unprivileged.
...
#define SCB_CFSR                                                               \
  (*(volatile const unsigned                                                   \
         *)0xE000ED28) /* Configurable Fault Status Register */
#define SCB_HFSR                                                               \
...
#define SCB_CFSR_MFSR                                                          \
  (*(volatile const unsigned char                                              \
         *)0xE000ED28) /* Memory-management Fault Status Register */
