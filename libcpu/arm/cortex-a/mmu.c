...
void rt_hw_init_mmu_table(struct mem_desc *mdesc, rt_uint32_t size) {
  ...
  for (; size > 0; size--) {
    ...
    rt_hw_mmu_setmtt(mdesc->vaddr_start, mdesc->vaddr_end, mdesc->paddr_start,
                     mdesc->attr);
    mdesc++;
  }
...
#ifdef RT_USING_SMART
  rt_aspace_init(&rt_kernel_space, (void *)USER_VADDR_TOP, 0 - USER_VADDR_TOP,
                 vtable);
...
void init_mm_setup(unsigned int *mtbl, unsigned int size, unsigned int pv_off) {
  ...
  for (va = 0; va < 0x1000; va++) {
    ...
    } else if (vaddr >= (KERNEL_VADDR_START + pv_off) &&
               vaddr - (KERNEL_VADDR_START + pv_off) < size) {
