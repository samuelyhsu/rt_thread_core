...
struct arm_gic_v3 {
  ...
  rt_uint32_t
      redist_hw_base[RT_CPUS_NR]; /* the pointer of the gic redistributor */
  rt_uint32_t dist_hw_base;       /* the base address of the gic distributor */
  rt_uint32_t
      cpu_hw_base[RT_CPUS_NR]; /* the base addrees of the gic cpu interface */
};
 ...
 * @out param target_list:  Target List. The set of PEs for which SGI interrupts
 * will be generated. Each bit corresponds to the PE within a cluster with an
 * Affinity 0 value equal to the bit number.
 * @return {rt_uint32_t} 0 is finish , 1 is data valid
 */
rt_weak rt_uint32_t arm_gic_cpumask_to_affval(rt_uint32_t *cpu_mask,
                                              rt_uint32_t *cluster_id,
                                              rt_uint32_t *target_list) {
  return 0;
}

rt_weak rt_uint64_t get_main_cpu_affval(void) { return 0; }
...
rt_uint32_t arm_gic_get_pending_irq(rt_uint32_t index, int irq) {
  ...
  if (irq >= 16U) {
    pend = (GIC_DIST_PENDING_SET(_gic_table[index].dist_hw_base, irq) >>
            (irq % 32U)) &
           0x1UL;
  } else {
    /* INTID 0-15 Software Generated Interrupt */
    pend = (GIC_DIST_SPENDSGI(_gic_table[index].dist_hw_base, irq) >>
            ((irq % 4U) * 8U)) &
           0xFFUL;
...
void arm_gic_set_pending_irq(rt_uint32_t index, int irq) {
  ...
  if (irq >= 16U) {
    GIC_DIST_PENDING_SET(_gic_table[index].dist_hw_base, irq) = 1U
                                                                << (irq % 32U);
  } else {
    /* INTID 0-15 Software Generated Interrupt */
...
rt_uint32_t arm_gic_get_configuration(rt_uint32_t index, int irq) {
  ...
  return (GIC_DIST_CONFIG(_gic_table[index].dist_hw_base, irq) >>
          ((irq % 16U) >> 1U));
}
...
rt_uint32_t arm_gic_get_target_cpu(rt_uint32_t index, int irq) {
  ...
  return (GIC_DIST_TARGET(_gic_table[index].dist_hw_base, irq) >>
          ((irq % 4U) * 8U)) &
         0xFFUL;
}
...
void arm_gic_set_priority(rt_uint32_t index, int irq, rt_uint32_t priority) {
  ...
  if (irq < 32U) {
    ...
    mask =
        GIC_RDISTSGI_IPRIORITYR(_gic_table[index].redist_hw_base[cpu_id], irq);
...
rt_uint32_t arm_gic_get_priority(rt_uint32_t index, int irq) {
  ...
  if (irq < 32U) {
    ...
    RT_ASSERT((cpu_id) < RT_CPUS_NR);
    return (GIC_RDISTSGI_IPRIORITYR(_gic_table[index].redist_hw_base[cpu_id],
                                    irq) >>
            ((irq % 4U) * 8U)) &
           0xFFUL;
  } else {
    return (GIC_DIST_PRI(_gic_table[index].dist_hw_base, irq) >>
            ((irq % 4U) * 8U)) &
           0xFFUL;
  }
}

void arm_gic_set_system_register_enable_mask(rt_uint32_t index,
                                             rt_uint32_t value) {
  RT_ASSERT(index < ARM_GIC_MAX_NR);
...
rt_uint32_t arm_gic_get_irq_status(rt_uint32_t index, int irq) {
  ...
  active = (GIC_DIST_ACTIVE_SET(_gic_table[index].dist_hw_base, irq) >>
            (irq % 32U)) &
           0x1UL;
  pending = (GIC_DIST_PENDING_SET(_gic_table[index].dist_hw_base, irq) >>
             (irq % 32U)) &
            0x1UL;
...
void arm_gic_send_affinity_sgi(rt_uint32_t index, int irq, rt_uint32_t cpu_mask,
                               rt_uint32_t routing_mode) {
  rt_uint64_t sgi_val;

  if (routing_mode) {
    sgi_val = (1ULL << 40) |
              ((irq & 0x0FULL) << 24); // Interrupts routed to all PEs in the
                                       // system, excluding "self".
    ...
    while (arm_gic_cpumask_to_affval(&cpu_mask, &cluster_id, &target_list)) {
      sgi_val = ((irq & 0x0FULL) << 24 | target_list |
...
rt_uint32_t arm_gic_get_group(rt_uint32_t index, int irq) {
  ...
  return (GIC_DIST_IGROUP(_gic_table[index].dist_hw_base, irq) >> (irq % 32U)) &
         0x1UL;
}
...
int arm_gic_dist_init(rt_uint32_t index, rt_uint32_t dist_base, int irq_start) {
  ...
  /*
     ...
     enable the interrupt groups and to set the routing mode. Enable Affinity
     routing (ARE bits) The ARE bits in GICD_CTLR control whether affinity
     routing is enabled. If affinity routing is not enabled, GICv3 can be
     configured for legacy operation. Whether affinity routing is enabled or not
     can be controlled separately for Secure and Non-secure state. Enables
     GICD_CTLR contains separate enable bits for Group 0, Secure Group 1 and
     Non-secure Group 1: GICD_CTLR.EnableGrp1S enables distribution of Secure
     Group 1 interrupts. GICD_CTLR.EnableGrp1NS enables distribution of
     Non-secure Group 1 interrupts. GICD_CTLR.EnableGrp0 enables distribution of
     Group 0 interrupts.
    */
...
int arm_gic_redist_address_set(rt_uint32_t index, rt_uint32_t redist_addr,
                               rt_uint32_t cpu_id) {
  RT_ASSERT(index < ARM_GIC_MAX_NR);
  RT_ASSERT((cpu_id) < RT_CPUS_NR);
...
int arm_gic_cpu_interface_address_set(rt_uint32_t index,
                                      rt_uint32_t interface_addr,
                                      rt_uint32_t cpu_id) {
  RT_ASSERT(index < ARM_GIC_MAX_NR);
  RT_ASSERT((cpu_id) < RT_CPUS_NR);
...
void arm_gic_dump_type(rt_uint32_t index) {
  ...
  rt_kprintf("GICv%d on %p, max IRQs: %d, %s security extension(%08x)\n",
             ...
             gic_type & (1U << 10U) ? "has" : "no", gic_type);
}
...
void arm_gic_dump(rt_uint32_t index) {
  ...
  for (i = 0U; i < _gic_max_irq / 32U; i++) {
    rt_kprintf("0x%08x, ",
               GIC_DIST_ENABLE_SET(_gic_table[index].dist_hw_base, i * 32U));
  }
  rt_kprintf("\n--- hw pending ---\n");
  for (i = 0U; i < _gic_max_irq / 32U; i++) {
    rt_kprintf("0x%08x, ",
               GIC_DIST_PENDING_SET(_gic_table[index].dist_hw_base, i * 32U));
  }
  rt_kprintf("\n--- hw active ---\n");
  for (i = 0U; i < _gic_max_irq / 32U; i++) {
    rt_kprintf("0x%08x, ",
               GIC_DIST_ACTIVE_SET(_gic_table[index].dist_hw_base, i * 32U));
  }
  rt_kprintf("\n");
}
