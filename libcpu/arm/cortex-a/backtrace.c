...
rt_inline void arm_get_current_stackframe(struct pt_regs *regs,
...
static int core_kernel_text(unsigned long addr) { return 1; }
...
static const struct unwind_idx *search_index(unsigned long addr,
  ...
  LOG_D("%s(%08lx, %x, %x, %x)", __func__, addr, start, origin, stop);
...
static const struct unwind_idx *
unwind_find_idx(unsigned long addr, const struct unwind_idx **origin_idx,
...
static int unwind_exec_insn(struct unwind_ctrl_block *ctrl) {
  ...
  if ((insn & 0xc0) == 0x00)
    ...
    mask = insn & 0x0fff;
    if (mask == 0) {
      LOG_W("unwind: 'Refuse to unwind' instruction %04lx", insn);
      return -URC_FAILURE;
    }
    ...
    if (mask == 0 || mask & 0xf0) {
      LOG_W("unwind: Spare encoding %04lx", (insn << 8) | mask);
      return -URC_FAILURE;
    }
 ...
 */
int unwind_frame(struct stackframe *frame, const struct unwind_idx **origin_idx,
                 const struct unwind_idx exidx_start[],
                 const struct unwind_idx exidx_end[]) {
  unsigned long low;
  ...
  ctrl.sp_high = (unsigned long)((char *)rt_c_thread->stack_addr +
                                 rt_c_thread->stack_size);

  LOG_D("%s(pc = %08lx lr = %08lx sp = %08lx)", __func__, frame->pc, frame->lr,
        frame->sp);
...
#ifdef RT_BACKTRACE_FUNCTION_NAME
  {
    char *fun_name;
    fun_name =
        unwind_get_function_name((void *)prel31_to_addr(&idx->addr_offset));
    if (fun_name) {
      rt_kprintf("0x%08x @ %s\n", frame->pc, fun_name);
    }
  }
  ...
  if ((*ctrl.insn & 0xff000000) == 0x80000000) {
    ...
    LOG_W("unwind: Unsupported personality routine %08lx at %x", *ctrl.insn,
          ctrl.insn);
    return -URC_FAILURE;
  }
