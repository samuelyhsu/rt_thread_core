 ...
 * 2021-06-03     Flybreak     Fix the crash problem after opening Oz
 * optimization on ac6.
 */
...
#define MEMITEM(ptr)                                                           \
  (struct rt_memheap_item *)((rt_uint8_t *)ptr - RT_MEMHEAP_SIZE)
...
#ifdef RT_USING_MEMTRACE
  rt_memset(header_ptr->owner_thread_name, ' ',
            sizeof(header_ptr->owner_thread_name));
#endif /* RT_USING_MEMTRACE */
 ...
 */
void _memheap_free(void *rmem) { rt_memheap_free(rmem); }
...
int memheapcheck(int argc, char *argv[]) {
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    if (name != RT_NULL &&
        rt_strncmp(name, heap->parent.name, RT_NAME_MAX) != 0)
      continue;
    /* check memheap */
    for (item = heap->block_list; item->next != heap->block_list;
         item = item->next) {
      /* check magic */
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    for (header_ptr = mh->block_list; header_ptr->next != mh->block_list;
      ...
      if (block_size < 1024)
        ...
        rt_kprintf("%2d.%dM", block_size / (1024 * 1024),
                   (block_size % (1024 * 1024) * 10) / (1024 * 1024));
      else
