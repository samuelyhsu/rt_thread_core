 ...
 * @param size is the size of buffer.
 *
 * @return the actually written size on successful, otherwise 0 will be
 * returned.
 *
 * @note the unit of size/pos is a block for block device.
 */
...
/**
 * @brief This function will set the reception indication callback function.
...
rt_device_t rt_device_create_since_driver(rt_driver_t drv, int device_id) {
  ...
  rt_snprintf(device->parent.name, sizeof(device->parent.name), "%s%d",
              drv->name, device_id);
  return device;
}
