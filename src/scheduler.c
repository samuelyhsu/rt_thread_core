 ...
 * 2010-12-13     Bernard      add defunct list initialization even if not use
 * heap. 2011-05-10     Bernard      clean scheduler debug log. 2013-12-21
 * Grissiom     add rt_critical_level 2018-11-22     Jesven       remove the
 * current task from ready queue add per cpu ready queue add
 * _scheduler_get_highest_priority_thread to find highest priority task
 *                             rt_schedule_insert_thread won't insert current
 * task to ready queue in smp version, rt_hw_context_switch_interrupt maybe
 * switch to new task directly 2022-01-07     Gabriel      Moving
 * __on_rt_xxxxx_hook to scheduler.c
 */
...
#define __on_rt_scheduler_hook(from, to)                                       \
  __ON_HOOK_ARGS(rt_scheduler_hook, (from, to))
#endif
...
/**
 * @brief This function will set a hook function, which will be invoked when
...
static void _scheduler_stack_check(struct rt_thread *thread) {
  ...
  if (lwp && ((rt_uint32_t)thread->sp > (rt_uint32_t)lwp->data_entry &&
              (rt_uint32_t)thread->sp <=
                  (rt_uint32_t)lwp->data_entry + (rt_uint32_t)lwp->data_size)) {
    return;
  }
 ...
 */
void rt_scheduler_ipi_handler(int vector, void *param) { rt_schedule(); }

/**
 * @brief This function will perform one scheduling. It will select one thread
 *        with the highest priority level in global ready queue or local ready
 * queue, then switch to it.
 */
void rt_schedule(void) {
  ...
  if ((current_thread->stat & RT_THREAD_SUSPEND_MASK) ==
  ...
  if (current_thread->scheduler_lock_nest == 1) /* whether lock scheduler */
  {
    ...
    if (rt_thread_ready_priority_group != 0 || pcpu->priority_group != 0) {
      to_thread =
          _scheduler_get_highest_priority_thread(&highest_ready_priority);
      ...
      if ((current_thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_RUNNING) {
        if (current_thread->bind_cpu == RT_CPUS_NR ||
            current_thread->bind_cpu == cpu_id) {
          if (current_thread->current_priority < highest_ready_priority) {
            to_thread = current_thread;
          } else if (current_thread->current_priority ==
                         highest_ready_priority &&
...
void rt_schedule(void) {
  ...
  if (rt_scheduler_lock_nest == 0) {
    ...
    if (rt_thread_ready_priority_group != 0) {
      ...
      if ((rt_current_thread->stat & RT_THREAD_STAT_MASK) ==
          RT_THREAD_RUNNING) {
      ...
      if (to_thread != rt_current_thread) {
        ...
        rt_current_thread->stat = RT_THREAD_RUNNING | (rt_current_thread->stat &
                                                       ~RT_THREAD_STAT_MASK);
      }
    }
  }
...
/**
 * @brief This function checks whether a scheduling is needed after an IRQ
 * context switching. If yes, it will select one thread with the highest
 * priority level, and then switch to it.
 */
#ifdef RT_USING_SMP
...
/**
 * @brief This function will insert a thread to the system ready queue. The
 * state of thread will be set as READY and the thread will be removed from
 * suspend queue.
 *
...
void rt_schedule_insert_thread(struct rt_thread *thread) {
  ...
  if (bind_cpu == RT_CPUS_NR) {
    ...
    /* there are some time slices left, inserting thread after ready list to
       schedule it firstly at next time*/
    else {
