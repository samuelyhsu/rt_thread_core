...
rt_align(RT_ALIGN_SIZE) static rt_uint8_t
    idle_thread_stack[_CPUS_NR][IDLE_THREAD_STACK_SIZE];
...
rt_align(RT_ALIGN_SIZE) static rt_uint8_t
    rt_system_stack[SYSTEM_THREAD_STACK_SIZE];
...
void rt_thread_idle_init(void) {
  ...
  for (i = 0; i < _CPUS_NR; i++) {
    ...
    rt_thread_init(&idle_thread[i],
                   ...
                   "tidle",
#endif /* RT_NAME_MAX > 0 */
                   idle_thread_entry, RT_NULL, &idle_thread_stack[i][0],
                   sizeof(idle_thread_stack[i]), RT_THREAD_PRIORITY_MAX - 1,
                   32);
#ifdef RT_USING_SMP
