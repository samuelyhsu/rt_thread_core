...
/**
 * @brief    This function will wait for the arrival of the set signal. If it
 * does not wait for this signal, the thread will be suspended until it waits
 * for this signal or the waiting time exceeds the specified timeout: timeout.
 *
 * @param    set is the set of signal values to be waited for. Use the function
 *           sigaddset() to add the signal.
 *
 * @param    si is a pointer to the received signal info. If you don't care
 * about this value, you can use RT_NULL to set.
 *
...
void rt_thread_alloc_sig(rt_thread_t tid) {
  ...
  vectors =
      (rt_sighandler_t *)RT_KERNEL_MALLOC(sizeof(rt_sighandler_t) * RT_SIG_MAX);
...
int rt_system_signal_init(void) {
  _siginfo_pool =
      rt_mp_create("signal", RT_SIG_INFO_MAX, sizeof(struct siginfo_node));
