 ...
 *
 * @note    If the spinlock is locked, the current CPU will keep polling the
 * spinlock state until the spinlock is unlocked.
 *
...
/**
 * @brief   This function will disable the local interrupt and then lock the
 * spinlock.
 *
 * @note    If the spinlock is locked, the current CPU will keep polling the
 * spinlock state until the spinlock is unlocked.
...
/**
 * @brief   This function will unlock the spinlock and then restore current cpu
 * interrupt status.
 *
 * @param   lock is a pointer to the spinlock.
 *
 ...
 */
struct rt_cpu *rt_cpu_self(void) { return &_cpus[rt_hw_cpu_id()]; }
 ...
 */
struct rt_cpu *rt_cpu_index(int index) { return &_cpus[index]; }
...
/**
 * @brief   This function will restore all cpus's scheduler and restore local
 * irq.
 *
