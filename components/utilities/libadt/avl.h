...
static inline struct util_avl_struct *
util_avl_next(struct util_avl_struct *node) {
...
static inline struct util_avl_struct *
util_avl_prev(struct util_avl_struct *node) {
  ...
  if (node) {
    if (node->avl_left) {
      ...
      while ((predecessor = node->parent) && (node == predecessor->avl_left))
        node = predecessor;
    }
  }
  return predecessor;
...
static inline struct util_avl_struct *
util_avl_first(struct util_avl_root *root) {
...
static inline struct util_avl_struct *
util_avl_last(struct util_avl_root *root) {
