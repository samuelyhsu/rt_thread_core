...
static enum rym_code _rym_recv_begin(struct rym_ctx *ctx, rt_uint8_t *buf,
                                     rt_size_t len) {
  ...
  cctx->flen =
      atoi(1 + (const char *)buf + rt_strnlen((const char *)buf, len - 1));
  if (cctx->flen == 0)
    cctx->flen = -1;
...
static enum rym_code _rym_recv_data(struct rym_ctx *ctx, rt_uint8_t *buf,
                                    rt_size_t len) {
  struct custom_ctx *cctx = (struct custom_ctx *)ctx;
...
static enum rym_code _rym_recv_end(struct rym_ctx *ctx, rt_uint8_t *buf,
                                   rt_size_t len) {
  struct custom_ctx *cctx = (struct custom_ctx *)ctx;
...
static enum rym_code _rym_send_begin(struct rym_ctx *ctx, rt_uint8_t *buf,
                                     rt_size_t len) {
  struct custom_ctx *cctx = (struct custom_ctx *)ctx;
  struct stat file_buf;
  char insert_0 = '\0';
...
static enum rym_code _rym_send_data(struct rym_ctx *ctx, rt_uint8_t *buf,
                                    rt_size_t len) {
  struct custom_ctx *cctx = (struct custom_ctx *)ctx;
  rt_size_t read_size;
  int retry_read;
...
static enum rym_code _rym_send_end(struct rym_ctx *ctx, rt_uint8_t *buf,
                                   rt_size_t len) {
  rt_memset(buf, 0, len);
...
static rt_err_t rym_download_file(rt_device_t idev, const char *file_path) {
  ...
  RT_ASSERT(idev);
  res = rym_recv_on_device(
      &ctx->parent, idev, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_INT_RX,
