...
static rt_ssize_t _rym_read_data(struct rym_ctx *ctx, rt_size_t len) {
  /* we should already have had the code */
  rt_uint8_t *buf = ctx->buf + 1;
  rt_size_t readlen = 0;

  do {
    readlen += rt_device_read(ctx->dev, 0, buf + readlen, len - readlen);
    if (readlen >= len)
      return readlen;
...
static rt_err_t _rym_send_packet(struct rym_ctx *ctx, enum rym_code code,
                                 rt_uint8_t index) {
  ...
  do {
    writelen +=
        rt_device_write(ctx->dev, 0, ctx->buf + writelen, packetlen - writelen);
...
static rt_err_t _rym_do_handshake(struct rym_ctx *ctx, int tm_sec) {
  ...
  tick = rt_tick_get();
  while (rt_tick_get() <= (tick + rt_tick_from_millisecond(100)) &&
         i < (data_sz - 1)) {
  ...
  recv_crc =
      (rt_uint16_t)(*(ctx->buf + data_sz - 2) << 8) | *(ctx->buf + data_sz - 1);
  ...
  if (ctx->on_begin &&
      ctx->on_begin(ctx, ctx->buf + 3, data_sz - 5) != RYM_CODE_ACK)
    return -RYM_ERR_CAN;

  return RT_EOK;
}

static rt_err_t _rym_do_send_handshake(struct rym_ctx *ctx, int tm_sec) {
  enum rym_code code;
  rt_size_t i;
  rt_size_t data_sz;
  rt_uint8_t index = 0;
  ...
  /* congratulations, check passed. */
  if (ctx->on_begin &&
      ctx->on_begin(ctx, ctx->buf + 3, data_sz - 5) != RYM_CODE_SOH)
    return -RYM_ERR_CODE;
...
static rt_err_t _rym_trans_data(struct rym_ctx *ctx, rt_size_t data_sz,
                                enum rym_code *code) {
...
static rt_err_t _rym_do_fin(struct rym_ctx *ctx) {
  ...
  recv_crc = (rt_uint16_t)(*(ctx->buf + _RYM_SOH_PKG_SZ - 2) << 8) |
             *(ctx->buf + _RYM_SOH_PKG_SZ - 1);
...
static rt_err_t _rym_do_send_fin(struct rym_ctx *ctx) {
  ...
  if (ctx->on_end &&
      ctx->on_end(ctx, ctx->buf + 3, data_sz - 5) != RYM_CODE_SOH)
    return -RYM_ERR_CODE;
...
static rt_err_t _rym_do_send(struct rym_ctx *ctx, int handshake_timeout) {
  rt_err_t err;
