...
static rt_err_t zwrite_file(rt_uint8_t *buf, rt_uint16_t size,
                            struct zfile *zf);
...
static rt_err_t zrec_file(rt_uint8_t *rxbuf, struct zfile *zf) {
  ...
  again:
    ...
    case ZEOF:
      ...
      return (zrec_init(
          rxbuf, zf)); /* resend ZRINIT packet,ready to receive next file */
    case ZFIN:
      zrec_ack_bibi();
      return ZCOMPL;
    case ZCAN:
...
static rt_err_t zget_file_info(char *name, struct zfile *zf) {
  ...
  if ((zf->fd = open(zf->fname, DFS_O_CREAT | DFS_O_WRONLY, 0)) <
      0) /* create or replace exist file */
  {
    zsend_can();
