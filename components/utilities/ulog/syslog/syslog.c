 ...
 *
 * @param ident is an arbitrary identification string which future syslog
 ...
 *
 * @param mask The log priority mask which generate by macro LOG_MASK and
 * LOG_UPTO.
 *
...
rt_weak rt_size_t syslog_formater(char *log_buf, int level, const char *tag,
  ...
  /* add time and priority (level) info */
  {
    ...
    rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len,
                "<%d>%s%3d %02d:%02d:%02d", level,
                get_month_str(tm->tm_mon + 1), tm->tm_mday, tm->tm_hour,
                tm->tm_min, tm->tm_sec);
#else
    rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len,
                "%s%3d %02d:%02d:%02d", get_month_str(tm->tm_mon + 1),
  ...
  /* add thread info */
  {
    ...
    if (rt_interrupt_get_nest() == 0) {
      log_len +=
          ulog_strcpy(log_len, log_buf + log_len, rt_thread_self()->name);
    } else {
