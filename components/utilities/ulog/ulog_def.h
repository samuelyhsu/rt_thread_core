...
#define ULOG_ASSERT(EXPR)                                                      \
  if (!(EXPR)) {                                                               \
    ulog_output(LOG_LVL_ASSERT, LOG_TAG, RT_TRUE,                              \
                "(%s) has assert failed at %s:%ld.", #EXPR, __FUNCTION__,      \
                __LINE__);                                                     \
    ulog_flush();                                                              \
    while (1)                                                                  \
      ;                                                                        \
...
struct ulog_backend {
  ...
  /* The filter will be call before output. It will return TRUE when the filter
   * condition is math. */
  rt_bool_t (*filter)(struct ulog_backend *backend, rt_uint32_t level,
                      const char *tag, rt_bool_t is_raw, const char *log,
                      rt_size_t len);
  rt_slist_t list;
};
typedef struct ulog_backend *ulog_backend_t;
typedef rt_bool_t (*ulog_backend_filter_t)(struct ulog_backend *backend,
                                           rt_uint32_t level, const char *tag,
