...
#if defined(ULOG_ASYNC_OUTPUT_THREAD_STACK) &&                                 \
    (ULOG_ASYNC_OUTPUT_THREAD_STACK < 2048)
...
#define SUFFIX_LEN 10
  ...
  rt_snprintf(old_path, ULOG_FILE_PATH_LEN, "%s/%s", be->cur_log_dir_path,
              be->parent.name);
  rt_snprintf(new_path, ULOG_FILE_PATH_LEN, "%s/%s", be->cur_log_dir_path,
              be->parent.name);
  ...
  for (index = be->file_max_num - 2; index >= 0; --index) {
    rt_snprintf(old_path + base_len, SUFFIX_LEN, index ? "_%d.log" : ".log",
                index - 1);
...
__exit:
  /* reopen the file */
  be->cur_log_file_fd =
      open(be->cur_log_file_path, O_CREAT | O_RDWR | O_APPEND);
...
static void ulog_file_backend_flush_with_buf(struct ulog_backend *backend) {
  ...
  if (be->cur_log_file_fd < 0) {
    ...
    /* open file */
    rt_snprintf(be->cur_log_file_path, ULOG_FILE_PATH_LEN, "%s/%s.log",
                be->cur_log_dir_path, be->parent.name);
    be->cur_log_file_fd =
        open(be->cur_log_file_path, O_CREAT | O_RDWR | O_APPEND);
    if (be->cur_log_file_fd < 0) {
...
static void ulog_file_backend_output_with_buf(struct ulog_backend *backend,
                                              rt_uint32_t level,
...
int ulog_file_backend_init(struct ulog_file_be *be, const char *name,
                           const char *dir_path, rt_size_t max_num,
...
void ulog_file_backend_enable(struct ulog_file_be *be) { be->enable = RT_TRUE; }
