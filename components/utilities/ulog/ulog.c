...
static const char *const color_output_info[] = {
    ULOG_COLOR_ASSERT, RT_NULL, RT_NULL,         ULOG_COLOR_ERROR,
    ULOG_COLOR_WARN,   RT_NULL, ULOG_COLOR_INFO, ULOG_COLOR_DEBUG,
};
#endif /* ULOG_USING_COLOR */
...
#else
    rt_kprintf("Error: Current mode not supported run in ISR. Please enable "
               "ULOG_USING_ISR_LOG.\n");
    return RT_NULL;
#endif
  }
...
rt_weak rt_size_t ulog_head_formater(char *log_buf, rt_uint32_t level,
                                     const char *tag) {
  ...
  /* add time info */
  {
    ...
    if (usec_is_support) {
      ...
      rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len, ".%03d",
                  now.tv_usec / 1000);
    }
...
rt_weak rt_size_t ulog_tail_formater(char *log_buf, rt_size_t log_len,
                                     rt_bool_t newline, rt_uint32_t level) {
  /* the caller has locker, so it can use static variable for reduce stack usage
   */
  ...
  /* overflow check and reserve some space for CSI end sign, newline sign and
   * string end sign */
#ifdef ULOG_USING_COLOR
  if (log_len + (sizeof(CSI_END) - 1) + newline_len + sizeof((char)'\0') >
      ULOG_LINE_BUF_SIZE) {
    /* using max length */
...
rt_weak rt_size_t ulog_formater(char *log_buf, rt_uint32_t level,
                                const char *tag, rt_bool_t newline,
  ...
  /* log content */
  fmt_result = rt_vsnprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len,
                            format, args);
...
rt_weak rt_size_t ulog_hex_formater(char *log_buf, const char *tag,
                                    const rt_uint8_t *buf, rt_size_t size,
                                    rt_size_t width, rt_base_t addr) {
...
static void ulog_output_to_all_backend(rt_uint32_t level, const char *tag,
                                       rt_bool_t is_raw, const char *log,
                                       rt_size_t len) {
  rt_slist_t *node;
  ...
  for (node = rt_slist_first(&ulog.backend_list); node;
...
#else
    if (backend->filter &&
        backend->filter(backend, level, tag, is_raw, log, len) == RT_FALSE) {
    ...
    if (backend->support_color || is_raw) {
      ...
      /* recalculate the log start address and log size when backend not
       * supported color */
...
static void do_output(rt_uint32_t level, const char *tag, rt_bool_t is_raw,
  ...
  if (is_raw == RT_FALSE) {
    ...
    /* allocate log frame */
    log_blk = rt_rbb_blk_alloc(
        ulog.async_rbb,
        RT_ALIGN(sizeof(struct ulog_frame) + log_buf_size, RT_ALIGN_SIZE));
    if (log_blk) {
      ...
      /* copy log data */
      rt_strncpy((char *)(log_blk->buf + sizeof(struct ulog_frame)), log_buf,
                 log_buf_size);
      /* put the block */
    ...
    rt_ringbuffer_put(ulog.async_rb, (const rt_uint8_t *)log_buf,
                      (rt_uint16_t)log_buf_size);
    /* send a notice */
    rt_sem_release(&ulog.async_notice);
  }
#else
  ...
  if (rt_interrupt_get_nest() == 0) {
    ...
    extern void ulog_console_backend_output(
...
void ulog_voutput(rt_uint32_t level, const char *tag, rt_bool_t newline,
                  const rt_uint8_t *hex_buf, rt_size_t hex_size,
                  rt_size_t hex_width, rt_base_t hex_addr, const char *format,
                  va_list args) {
  ...
  if (hex_buf == RT_NULL) {
    ...
    /* hex mode */
    log_len =
        ulog_hex_formater(log_buf, tag, hex_buf, hex_size, hex_width, hex_addr);
  }
 ...
 * @param level The filter level. When the level is LOG_FILTER_LVL_SILENT, the
 * log enter silent mode. When the level is LOG_FILTER_LVL_ALL, it will remove
 ...
 * output ulog_set_filter_lvl("example", LOG_LVL_INFO);
...
int ulog_tag_lvl_filter_set(const char *tag, rt_uint32_t level) {
  ...
  if (tag_lvl) {
    ...
    if (level != LOG_FILTER_LVL_ALL) {
      ...
      tag_lvl =
          (ulog_tag_lvl_filter_t)rt_malloc(sizeof(struct ulog_tag_lvl_filter));
      if (tag_lvl) {
 ...
 */
rt_slist_t *ulog_tag_lvl_list_get(void) { return &ulog.filter.tag_lvl_list; }

/**
 * set log global filter level
 *
 * @param level log level: LOG_LVL_ASSERT, LOG_LVL_ERROR, LOG_LVL_WARNING,
 * LOG_LVL_INFO, LOG_LVL_DBG LOG_FILTER_LVL_SILENT: disable all log output,
 * except assert level LOG_FILTER_LVL_ALL: enable all log output
 */
 ...
 *
 * @return log level: LOG_LVL_ASSERT, LOG_LVL_ERROR, LOG_LVL_WARNING,
 * LOG_LVL_INFO, LOG_LVL_DBG LOG_FILTER_LVL_SILENT: disable all log output,
 * except assert level LOG_FILTER_LVL_ALL: enable all log output
 */
rt_uint32_t ulog_global_filter_lvl_get(void) { return ulog.filter.level; }
 ...
 * @return tag
 */
const char *ulog_global_filter_tag_get(void) { return ulog.filter.tag; }
 ...
 * @return keyword
 */
const char *ulog_global_filter_kw_get(void) { return ulog.filter.keyword; }
...
static void ulog_filter(uint8_t argc, char **argv) {
  ...
  if (rt_slist_isempty(ulog_tag_lvl_list_get())) {
    ...
    for (node = rt_slist_first(ulog_tag_lvl_list_get()); node;
         node = rt_slist_next(node)) {
      tag_lvl = rt_slist_entry(node, struct ulog_tag_lvl_filter, list);
      rt_kprintf("%-*.*s: ", ULOG_FILTER_TAG_MAX_LEN, ULOG_FILTER_TAG_MAX_LEN,
                 tag_lvl->tag);
...
void ulog_async_output(void) {
  ...
  while ((log_blk = rt_rbb_blk_get(ulog.async_rbb)) != RT_NULL) {
    ...
    if (log_frame->magic == ULOG_FRAME_MAGIC) {
      ...
      ulog_output_to_all_backend(log_frame->level, log_frame->tag,
                                 log_frame->is_raw, log_frame->log,
                                 log_frame->log_len);
    }
  ...
  if (ulog.async_rb) {
    ...
    if (log) {
      rt_size_t len = rt_ringbuffer_get(ulog.async_rb, (rt_uint8_t *)log,
                                        (rt_uint16_t)log_len);
      log[log_len] = '\0';
