...
struct rt_link_session *rt_link_get_scb(void) { return rt_link_scb; }
  ...
  if (tem_list != RT_NULL) {
    do {
      ...
      if (find_frame->head.service == service->service) {
        ...
        LOG_D("remove seq(%d) serv (%d)", find_frame->head.sequence,
              service->service);
        rt_enter_critical();
...
static rt_err_t rt_link_frame_extend_config(struct rt_link_frame *frame,
                                            rt_link_frame_attr_e attribute,
...
static int rt_link_command_frame_send(rt_uint16_t serv, rt_uint8_t sequence,
                                      rt_link_frame_attr_e attribute,
                                      rt_uint16_t parameter) {
  struct rt_link_frame command_frame = {0};
  rt_uint8_t data[sizeof(command_frame.head) + sizeof(command_frame.extend)] = {
      0};
  ...
  rt_memcpy(data + data_len, &command_frame.extend,
            sizeof(command_frame.extend));
...
static void rt_link_service_send_finish(rt_link_err_e err) {
  ...
  if (frame) {
    ...
    if (rt_link_scb->service[service]->timeout_tx != RT_WAITING_NO) {
      ...
      rt_link_scb->service[service]->send_cb(rt_link_scb->service[service],
                                             buffer);
    }
  }
}
...
static rt_ssize_t frame_send(struct rt_link_frame *frame) {
  ...
  if (frame->attribute == RT_LINK_SHORT_DATA_FRAME ||
      frame->attribute == RT_LINK_LONG_DATA_FRAME) {
    rt_memcpy(data, frame->real_data, frame->data_len);
    data = data + frame->data_len;
  }
  if (frame->head.crc) {
    frame->crc = rt_link_scb->calculate_crc(RT_FALSE, rt_link_scb->sendbuffer,
                                            length - RT_LINK_CRC_LENGTH);
    rt_memcpy(data, &frame->crc, RT_LINK_CRC_LENGTH);
  }

  LOG_D("frame send seq(%d) len(%d) attr:(%d), crc:(0x%08x).",
        frame->head.sequence, length, frame->attribute, frame->crc);
...
static rt_err_t rt_link_short_handle(struct rt_link_frame *receive_frame) {
  ...
  if (rt_link_scb->rx_record.dataspace != RT_NULL) {
    ...
    rt_link_hw_copy(rt_link_scb->rx_record.dataspace, receive_frame->real_data,
                    receive_frame->data_len);
    rt_link_recv_finish(receive_frame->head.service,
                        rt_link_scb->rx_record.dataspace,
                        receive_frame->data_len);
    rt_link_scb->rx_record.dataspace = RT_NULL;
  } else {
...
static void _long_handle_first(struct rt_link_frame *receive_frame) {
  if (receive_frame->extend.parameter % RT_LINK_MAX_DATA_LENGTH == 0) {
    receive_frame->total =
        receive_frame->extend.parameter / RT_LINK_MAX_DATA_LENGTH;
  } else {
    receive_frame->total =
        receive_frame->extend.parameter / RT_LINK_MAX_DATA_LENGTH + 1;
  }
...
static void _long_handle_second(struct rt_link_frame *receive_frame) {
  ...
  receive_frame->index = rt_link_check_seq(receive_frame->head.sequence,
                                           rt_link_scb->rx_record.rx_seq) -
                         1;
  LOG_D("seq(%d), rxseq(%d), index(%d), total(%d), count(0x%x)",
        receive_frame->head.sequence, rt_link_scb->rx_record.rx_seq,
        receive_frame->index, receive_frame->total,
        rt_link_scb->rx_record.long_count);

  if ((receive_frame->index > RT_LINK_FRAMES_MAX) ||
      (rt_link_scb->rx_record.long_count & (0x01 << receive_frame->index))) {
    LOG_D("ERR:index %d, rx_seq %d", receive_frame->index,
          rt_link_scb->rx_record.rx_seq);
  } else if (rt_link_scb->rx_record.dataspace != RT_NULL) {
    rt_link_scb->rx_record.long_count |= (0x01 << receive_frame->index);
    offset = RT_LINK_MAX_DATA_LENGTH * receive_frame->index;
    rt_link_hw_copy(rt_link_scb->rx_record.dataspace + offset,
                    receive_frame->real_data, receive_frame->data_len);

    if (receive_frame->head.ack) {
      if (rt_link_utils_num1(rt_link_scb->rx_record.long_count) ==
          rt_link_scb->rx_record.total) {
        rt_link_command_frame_send(
    ...
    if (rt_link_utils_num1(rt_link_scb->rx_record.long_count) ==
      ...
      rt_exit_critical();
    } else if (rt_link_hw_recv_len(rt_link_scb->rx_buffer) <
               (receive_frame->data_len % RT_LINK_MAX_DATA_LENGTH)) {
      rt_int32_t timeout = RT_LINK_LONG_FRAME_TIMEOUT;
      rt_timer_control(&rt_link_scb->longframetimer, RT_TIMER_CTRL_SET_TIME,
                       &timeout);
...
static rt_err_t rt_link_handshake_handle(struct rt_link_frame *receive_frame) {
  LOG_D("HANDSHAKE: seq(%d) param(%d)", receive_frame->head.sequence,
        receive_frame->extend.parameter);
  rt_link_scb->state = RT_LINK_CONNECT;
...
static rt_err_t
rt_link_session_end_handle(struct rt_link_frame *receive_frame) {
...
static void rt_link_frame_check(void) {
  ...
  while (recv_len > 0) {
    ...
    case FIND_FRAME_HEAD: {
      ...
      if ((*rt_link_scb->rx_buffer->read_point & RT_LINK_FRAME_HEAD_MASK) ==
          RT_LINK_FRAME_HEAD) {
    ...
    case PARSE_FRAME_HEAD: {
      ...
      rt_link_hw_copy((rt_uint8_t *)&receive_frame.head, data,
                      sizeof(struct rt_link_frame_head));
      rt_link_hw_buffer_point_shift(&data, sizeof(struct rt_link_frame_head));

      LOG_D("HEAD: seq(%d) serv(%d) ack(%d) crc(%d) ext(%d) len(%d) "
            "attr(%d)(0x%x)",
            receive_frame.head.sequence, receive_frame.head.service,
            receive_frame.head.ack, receive_frame.head.crc,
            receive_frame.head.extend, receive_frame.head.length,
            receive_frame.extend.attribute, receive_frame.extend.parameter);
    ...
    case PARSE_FRAME_EXTEND: {
      if (receive_frame.head.extend) {
        if (recv_len < buff_len) {
          ...
          timeout = 50;
          rt_timer_control(&rt_link_scb->recvtimer, RT_TIMER_CTRL_SET_TIME,
                           &timeout);
          rt_timer_start(&rt_link_scb->recvtimer);
          return;
        }
        rt_timer_stop(&rt_link_scb->recvtimer);
        rt_link_hw_copy((rt_uint8_t *)&receive_frame.extend, data,
                        sizeof(struct rt_link_extend));
    ...
    case PARSE_FRAME_SEQ: {
      ...
      case RT_LINK_RESEND_FRAME: {
        ...
        offset =
            rt_link_check_seq(receive_frame.head.sequence, rt_link_scb->tx_seq);
        if (rt_slist_first(&rt_link_scb->tx_data_slist) != RT_NULL) {
          send_frame = rt_container_of(rt_link_scb->tx_data_slist.next,
                                       struct rt_link_frame, slist);
          if (offset > send_frame->total) {
            /* exceptional frame, ignore it */
            LOG_D("seq (%d) failed, tx_seq (%d).offset=(%d) total= (%d)",
                  receive_frame.head.sequence, rt_link_scb->tx_seq, offset,
                  send_frame->total);
            rt_link_hw_buffer_point_shift(&rt_link_scb->rx_buffer->read_point,
                                          1);
            goto __find_head;
          }
        }
      ...
      case RT_LINK_SESSION_END: {
        ...
        offset = rt_link_check_seq(receive_frame.head.sequence,
                                   rt_link_scb->rx_record.rx_seq) -
                 1;
        if (offset > RT_LINK_FRAMES_MAX) {
          /* exceptional frame, ignore it */
          LOG_D(
              "seq (%d) failed, rx_seq (%d) offset=(%d) attr= (%d) status (%d)",
              receive_frame.head.sequence, rt_link_scb->rx_record.rx_seq,
              offset, receive_frame.attribute, rt_link_scb->state);
          rt_link_hw_buffer_point_shift(&rt_link_scb->rx_buffer->read_point, 1);
          goto __find_head;
        }
      }
    ...
    case CHECK_FRAME_CRC: {
      if (receive_frame.head.crc) {
        ...
        rt_link_hw_copy((rt_uint8_t *)&receive_frame.crc, data,
                        RT_LINK_CRC_LENGTH);
        temporary_crc = rt_link_scb->calculate_crc(
            RT_TRUE, rt_link_scb->rx_buffer->read_point,
            buff_len - RT_LINK_CRC_LENGTH);
        if (receive_frame.crc != temporary_crc) {
          /* check failed. ready resent */
          LOG_D("CRC: calc:(0x%08x) ,recv:(0x%08x).", temporary_crc,
                receive_frame.crc);
          rt_link_hw_buffer_point_shift(&rt_link_scb->rx_buffer->read_point, 1);
          goto __find_head;
        }
    ...
    case HEADLE_FRAME_DATA: {
      ...
      LOG_D("PARSE: buff_len (%d) r (0x%p)  w (0x%p)", buff_len,
            rt_link_scb->rx_buffer->read_point,
...
static void rt_link_send_ready(void) {
  ...
  if (rt_slist_next(&rt_link_scb->tx_data_slist)) {
    frame = rt_container_of(rt_slist_next(&rt_link_scb->tx_data_slist),
                            struct rt_link_frame, slist);
  }
...
static void rt_link_send_timeout(void) {
  ...
  if ((rt_uint32_t)rt_link_scb->sendtimer.parameter >= 5) {
    ...
    if (rt_slist_next(&rt_link_scb->tx_data_slist)) {
      struct rt_link_frame *frame =
          rt_container_of(rt_slist_next(&rt_link_scb->tx_data_slist),
                          struct rt_link_frame, slist);
      frame->issent = RT_LINK_FRAME_NOSEND;
      rt_link_command_frame_send(RT_LINK_SERVICE_RTLINK, frame->head.sequence,
                                 RT_LINK_HANDSHAKE_FRAME,
                                 rt_link_scb->rx_record.rx_seq);
    }
  }
...
void rt_link_thread(void *parameter) {
  ...
  while (1) {
    rt_event_recv(
        &rt_link_scb->event,
        RT_LINK_READ_CHECK_EVENT | RT_LINK_SEND_READY_EVENT |
            RT_LINK_SEND_TIMEOUT_EVENT | RT_LINK_RECV_TIMEOUT_FRAME_EVENT |
 ...
 * */
rt_size_t rt_link_send(struct rt_link_service *service, const void *data,
                       rt_size_t size) {
  RT_ASSERT(service != RT_NULL);
  ...
  do {
    ...
    if (attribute == RT_LINK_LONG_DATA_FRAME) {
      ...
      rt_link_frame_extend_config(send_frame, RT_LINK_LONG_DATA_FRAME,
                                  (rt_uint16_t)size);
    } else {
    ...
    LOG_D("append send slist, seq(%d), len(%d)", send_frame->head.sequence,
          send_frame->data_len);
  ...
  if (service->timeout_tx != RT_WAITING_NO) {
    ...
    rt_err_t ret = rt_event_recv(
        &rt_link_scb->sendevent, (0x01 << service->service),
        RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, service->timeout_tx, &recved);
    if (ret == -RT_ETIMEOUT) {
...
void rtlink_status(void) {
  ...
  if (rt_link_scb != RT_NULL) {
    ...
    rt_tick_t state = 0;
    rt_timer_control(&rt_link_scb->longframetimer, RT_TIMER_CTRL_GET_STATE,
                     &state);
    ...
    if (rt_link_scb->tx_data_slist.next) {
      ...
      rt_slist_for_each(data, &rt_link_scb->tx_data_slist) {
        rt_kprintf("\tsend data list: serv %u\t",
                   ((struct rt_link_frame_head *)data)->service);
...
int rt_link_init(void) {
  ...
  rt_timer_init(&rt_link_scb->longframetimer, "rxl_time",
                rt_link_receive_long_frame_callback, RT_NULL, 0,
  ...
  thread = rt_thread_create(RT_LINK_THREAD_NAME, rt_link_thread, RT_NULL,
                            RT_LINK_THREAD_STACK_SIZE, RT_LINK_THREAD_PRIORITY,
                            RT_LINK_THREAD_TICK);
  if (thread == RT_NULL) {
    result = -RT_ENOMEM;
    goto __exit;
...
rt_err_t rt_link_service_attach(struct rt_link_service *serv) {
  ...
  rt_link_command_frame_send(serv->service, seq, RT_LINK_HANDSHAKE_FRAME,
                             rt_link_scb->rx_record.rx_seq);
  return RT_EOK;
}
...
rt_err_t rt_link_service_detach(struct rt_link_service *serv) {
  ...
  rt_link_command_frame_send(serv->service, rt_link_scb->tx_seq,
