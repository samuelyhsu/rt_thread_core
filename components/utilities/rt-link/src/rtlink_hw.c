 ...
 * 2021-05-08     Sherman      Optimize the operation function on the
...
struct rt_link_receive_buffer *rt_link_hw_buffer_init(void *parameter) {
  ...
  if (rx_buffer != RT_NULL) {
    ...
    rx_buffer->end_point =
        rx_buffer->data +
        RT_LINK_RECEIVE_BUFFER_LENGTH; /* Point to memory that has no access
                                          rights */
  } else {
...
static rt_ssize_t rt_link_hw_buffer_write(void *data, rt_size_t count) {
  ...
  if (rx_buffer->write_point >= rx_buffer->read_point) {
    ...
    surplus = RT_LINK_RECEIVE_BUFFER_LENGTH -
              (rx_buffer->write_point - rx_buffer->read_point);
    count = count > surplus ? surplus : count;
    if (count >= w2end) {
      rt_memcpy(rx_buffer->write_point, data, w2end);
      rx_buffer->write_point = rx_buffer->data;

      rt_memcpy(rx_buffer->write_point, (rt_uint8_t *)data + w2end,
                (count - w2end));
...
rt_size_t rt_link_hw_recv_len(struct rt_link_receive_buffer *buffer) {
  ...
  if (buffer->write_point >= buffer->read_point) {
    ...
    return (RT_LINK_RECEIVE_BUFFER_LENGTH -
            (buffer->read_point - buffer->write_point));
  }
}
...
rt_uint32_t rt_link_get_crc(rt_uint8_t using_buffer_ring, rt_uint8_t *data,
                            rt_size_t size) {
