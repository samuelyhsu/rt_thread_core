...
int rtlink_fops_open(struct dfs_fd *fd) {
  ...
  if (fd->flags & O_NONBLOCK) {
    rt_device_control(device, RT_LINK_TX_NONBLOCKING | RT_LINK_RX_NONBLOCKING,
                      RT_NULL);
  }
...
int rtlink_fops_ioctl(struct dfs_fd *fd, int cmd, void *args) {
  ...
  if (cmd == O_NONBLOCK) {
    return rt_device_control(
        device, RT_LINK_TX_NONBLOCKING | RT_LINK_RX_NONBLOCKING, RT_NULL);
  } else {
...
const static struct dfs_file_ops _rtlink_fops = {
    rtlink_fops_open, rtlink_fops_close, rtlink_fops_ioctl,
    rtlink_fops_read, rtlink_fops_write, RT_NULL, /* flush */
    RT_NULL,                                      /* lseek */
    RT_NULL,                                      /* getdents */
    rtlink_fops_poll,
};
...
static rt_err_t rt_link_event_recv(struct rt_link_service *service) {
  ...
  rt_err_t ret =
      rt_event_recv(&recv_event, set, RT_EVENT_FLAG_AND | RT_EVENT_FLAG_CLEAR,
...
static void recv_cb(struct rt_link_service *service, void *data,
                    rt_size_t size) {
  RT_ASSERT(service != RT_NULL);
  struct rt_link_device *rtlink = (struct rt_link_device *)service->user_data;
...
rt_ssize_t rt_link_dev_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
  RT_ASSERT(dev != RT_NULL);
  RT_ASSERT(buffer != RT_NULL);
  RT_ASSERT(size != 0);
  ...
  if (rt_slist_first(&rtlink->recv_head) != RT_NULL) {
    node = rt_container_of(rt_slist_next(&rtlink->recv_head),
                           struct rtlink_recv_list, list);
...
rt_ssize_t rt_link_dev_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  RT_ASSERT(dev != RT_NULL);
  RT_ASSERT(buffer != RT_NULL);
  RT_ASSERT(size != 0);
...
const static struct rt_device_ops rtlink_ops = {
    rt_link_dev_init, rt_link_dev_open,  rt_link_dev_close,
    rt_link_dev_read, rt_link_dev_write, rt_link_dev_control};
#endif /* RT_USING_DEVICE_OPS */

/*
 * rtlink device register
 */
rt_err_t rt_link_dev_register(struct rt_link_device *rtlink, const char *name,
