...
static rt_used const struct ve_exporter
    __ve_table_start rt_section("0."
                                "VarExpTab") = {"ve_start", "ve_start", 0};

static rt_used const struct ve_exporter
    __ve_table_end rt_section("2."
                              "VarExpTab") = {"ve_end", "ve_end", 2};
#endif

/* for MS VC++ compiler */
#if defined(_MSC_VER)
#pragma section("VarExpTab$a", read)
__declspec(allocate("VarExpTab$a")) rt_used const
    struct ve_exporter __ve_table_start = {"ve_start", "ve_start", 0};

#pragma section("VarExpTab$z", read)
__declspec(allocate("VarExpTab$z")) rt_used const
    struct ve_exporter __ve_table_end = {"ve_end", "ve_end", 2};

/* Find var objects in VarExpTab segments */
static int ve_init_find_obj(unsigned int *begin, unsigned int *end,
                            ve_exporter_t *table) {
  int obj_count = 0;
...
int var_export_init(void) {
  ...
  /* Find var objects in custom segments to solve the problem of holes in
  ...
  RT_ASSERT(ve_exporter_num <
            (sizeof(ve_exporter_tab) / sizeof(ve_exporter_t)));

  /* bubble sort algorithms */
  for (index_i = 0; index_i < (ve_exporter_num - 1); index_i++) {
    for (index_j = 0; index_j < ((ve_exporter_num - 1) - index_i); index_j++) {
      /* splice ve_exporter's module and ve_exporter's identifier into a
       * complete string */
      rt_snprintf(__vexp_strbuf1, sizeof(__vexp_strbuf1), "%s%s",
                  ve_exporter_tab[index_j].module,
                  ve_exporter_tab[index_j].identifier);
      rt_snprintf(__vexp_strbuf2, sizeof(__vexp_strbuf2), "%s%s",
...
const ve_exporter_t *ve_binary_search(ve_module_t *mod,
  ...
  while ((ve_low_num <= ve_high_num) && (ve_high_num >= 0) &&
         (ve_low_num >= 0)) {
...
rt_size_t ve_value_count(ve_module_t *mod) { return mod->end - mod->begin + 1; }
