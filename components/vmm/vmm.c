...
void vmm_thread_init(struct rt_thread *thread, const char *name) {
  ...
  rt_thread_init(thread, name, RT_NULL, RT_NULL, &vmm_stack_start,
                 &vmm_stack_end - &vmm_stack_start, RT_THREAD_PRIORITY_MAX - 1,
                 10);
...
static void vmm_create_monitor(void) {
  ...
  tid = rt_thread_create("vmon", _verify_guest, RT_NULL, 1024, 8, 20);
  if (tid)
...
static void vmm_entry_glue(rt_uint32_t level, unsigned int vmm_domain,
                           unsigned int kernel_domain)
    /* inline would make the section setting meaningless */
    __attribute__((noinline)) rt_section(".vmm_glue");
static void vmm_entry_glue(rt_uint32_t level, unsigned int vmm_domain,
                           unsigned int kernel_domain) {
  rt_schedule();
...
void vmm_entry(struct vmm_entry_param *param) {
  ...
  rt_kprintf("heap: 0x%p - 0x%p, %dKi bytes\n", (void *)HEAP_BEGIN,
             (void *)HEAP_END, ((int)HEAP_END - (int)HEAP_BEGIN) / 1024);
