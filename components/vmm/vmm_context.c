...
void vmm_context_init_domain(struct vmm_domain *domain) {
  ...
  rt_kprintf("Linux domain: kernel: %d, user: %d, io: %d\n"
             ...
             domain->kernel, domain->user, domain->io, domain->vmm,
             domain->vmm_share);

  if (domain->kernel == domain->vmm || domain->io == domain->vmm) {
...
void vmm_virq_update(void) {
  if ((!_vmm_context->virq_status) && (_vmm_context->virq_pended)) {
...
void vmm_dump_virq(void) {
  ...
  for (s = 0, i = 0; i < ARRAY_SIZE(_vmm_context->virq_pending); i++) {
    s += rt_snprintf(_vmbuf + s, sizeof(_vmbuf) - s, "%08x, ",
                     _vmm_context->virq_pending[i]);
  }
...
int vmm_virq_coherence_ok(void) {
  ...
  if (!res) {
    vmm_info("--- %x %x, %x\n", _vmm_context->virq_pended, should_pend,
             !!should_pend);
  }
