 ...
 * 2021-02-19     lizhirui     add riscv64 support for lwp_user_accessable and
 * lwp_get_from_user 2021-06-07     lizhirui     modify user space bound check
 */
...
int lwp_user_space_init(struct rt_lwp *lwp, rt_bool_t is_fork) {
  ...
  if (lwp->lwp_obj) {
    ...
    if (!is_fork && err == RT_EOK) {
      ...
      err =
          rt_aspace_map(lwp->aspace, &addr, USER_STACK_VEND - USER_STACK_VSTART,
    ...
    if (paddr != ARCH_MAP_FAILED) {
      ...
      if (!(varea->flag & MMF_TEXT)) {
        ...
        if (cp) {
          ...
          LOG_W("%s: page alloc failed at %p", __func__, varea->start);
        }
      } else {
...
int lwp_dup_user(rt_varea_t varea, void *arg) {
  ...
  if (!mem_obj) {
    ...
    err = rt_aspace_map_phy(new_lwp->aspace, &hint, varea->attr,
                            MM_PA_TO_OFF(pa), &va);
    if (err != RT_EOK) {
      LOG_W("%s: aspace map failed at %p with size %p", __func__, varea->start,
            varea->size);
    }
  } else {
    ...
    err = rt_aspace_map(new_lwp->aspace, &va, varea->size, varea->attr,
                        varea->flag, &new_lwp->lwp_obj->mem_obj, varea->offset);
    if (err != RT_EOK) {
      LOG_W("%s: aspace map failed at %p with size %p", __func__, varea->start,
            varea->size);
    } else {
...
void *lwp_map_user_phy(struct rt_lwp *lwp, void *map_va, void *map_pa,
  ...
  err = rt_aspace_map_phy(lwp->aspace, &hint, attr, MM_PA_TO_OFF(map_pa), &va);
  if (err != RT_EOK) {
    va = RT_NULL;
    LOG_W("%s", __func__);
  }
