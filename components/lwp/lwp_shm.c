...
static const char *get_shm_name(rt_varea_t varea) { return "user.shm"; }
...
static void on_shm_page_fault(struct rt_varea *varea,
...
static int _lwp_shmget(size_t key, size_t size, int create) {
  ...
  if (create) {
    ...
    page_addr_p =
        (void *)((char *)page_addr + PV_OFFSET); /* physical address */
    ...
    node_key =
        (struct lwp_avl_struct *)rt_malloc(sizeof(struct lwp_avl_struct) * 2);
    if (!node_key) {
      goto err;
    }
...
/* Free the shared pages, the shared-memory structure and its binary tree
 * node_key. */
...
static void *_lwp_shmat(int id, void *shm_vaddr) {
   ...
   * shared-memory structure linked to the node_key. We don't use the id to
   * refer to the shared-memory structure directly, because the binary tree is
   * used to verify the structure is really in use.
   */
  node_key = shm_id_to_node(id);
  if (!node_key) {
    return RT_NULL;
...
static struct lwp_shm_struct *_lwp_shm_struct_get(struct rt_lwp *lwp,
                                                  void *shm_vaddr) {
  void *pa = RT_NULL;
  struct lwp_avl_struct *node_pa = RT_NULL;
