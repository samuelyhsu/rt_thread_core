...
int arch_expand_user_stack(void *addr) {
  ...
  if ((stack_addr >= (size_t)USER_STACK_VSTART) &&
    ...
    void *map =
        lwp_map_user(lwp_self(), (void *)stack_addr, PAGE_SIZE, RT_FALSE);
...
void *get_thread_kernel_stack_top(rt_thread_t thread) { return RT_NULL; }

/**
 * don't support this in i386, it's ok!
 */
void *arch_get_user_sp() { return RT_NULL; }
...
int arch_user_space_init(struct rt_lwp *lwp) {
  ...
  memset((rt_uint8_t *)mmu_table + ARCH_PAGE_SIZE / 4, 0,
         ARCH_PAGE_SIZE / 4 * 3);
  rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, mmu_table, ARCH_PAGE_SIZE);
  if (rt_hw_mmu_map_init(&lwp->mmu_info, (void *)USER_VADDR_START,
...
void *arch_kernel_mmu_table_get(void) { return (void *)((char *)g_mmu_table); }
 ...
 * then pop then into register, final use iret to switch kernel mode to user
 * mode.
 */
...
void lwp_exec_user(void *args, void *kernel_stack, void *user_entry) {
  arch_start_umode(args, (const void *)user_entry, (void *)USER_STACK_VEND,
                   kernel_stack);
}
...
/**
 * when called sys_thread_create, need create a thread, after thread stared,
 * will come here, like arch_start_umode, will enter user mode, but we must set
 * thread exit function. it looks like: void func(void *arg)
 * {
 *      ...
 * }
 * when thread func return, we must call exit code to exit thread, or not the
 * program runs away. so we need copy exit code to user and call exit code when
 * func return.
 */
void arch_crt_start_umode(void *args, const void *text, void *ustack,
  ...
  unsigned long *retcode =
      lwp_copy_return_code_to_user_stack(ustack); /* copy ret code */
  unsigned long *retstack =
      (unsigned long *)RT_ALIGN_DOWN(((rt_ubase_t)retcode), sizeof(rt_ubase_t));
...
rt_thread_t rt_thread_sp_to_thread(void *spmember_addr) {
  return (rt_thread_t)(((rt_ubase_t)spmember_addr) -
 ...
 * user_stack(unused)
 */
void arch_set_thread_context(void *exit_addr, void *new_thread_stack,
   ...
   */
  rt_hw_stack_frame_t *frame =
      (rt_hw_stack_frame_t *)((rt_ubase_t)new_thread_stack -
                              sizeof(rt_hw_stack_frame_t));

  frame->eax = 0; /* child return 0 */

  rt_hw_context_t *context =
      (rt_hw_context_t *)(((rt_uint32_t *)frame) - HW_CONTEXT_MEMBER_NR);
  context->eip = (void *)exit_addr; /* when thread started, jump to intr exit
                                       for enter user mode */
...
void lwp_try_do_signal(rt_hw_stack_frame_t *frame) {
  ...
  int signal = lwp_signal_backup((void *)frame->esp, (void *)frame->eip,
                                 (void *)frame->eflags);
  ...
  /* 3. backup frame */
  rt_signal_frame_t *sig_frame =
      (rt_signal_frame_t *)((frame->esp - sizeof(rt_signal_frame_t)) & -8UL);
   ...
   * ----------
   */
  size_t ret_code_size =
      (size_t)lwp_signal_return_end - (size_t)lwp_signal_return;
...
void lwp_signal_do_return(rt_hw_stack_frame_t *frame) {
  /**
   * ASSUME: in x86, each stack push and pop element is 4 byte. so
   * STACK_ELEM_SIZE = sizeof(int) => 4. when signal handler return, the stack
   * move to the buttom of signal frame. but return will pop eip from esp, then
   * {esp += STACK_ELEM_SIZE}, thus {esp = (signal frame) + STACK_ELEM_SIZE}. so
   * {(signal frame) = esp - STACK_ELEM_SIZE}
   */
  rt_signal_frame_t *sig_frame =
      (rt_signal_frame_t *)(frame->esp - sizeof(rt_uint32_t));
