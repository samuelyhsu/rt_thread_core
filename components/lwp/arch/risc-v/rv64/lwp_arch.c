...
void *lwp_copy_return_code_to_user_stack() {
  ...
  if (tid->user_stack != RT_NULL) {
    rt_size_t size =
        (rt_size_t)lwp_thread_return_end - (rt_size_t)lwp_thread_return;
    rt_size_t userstack =
        (rt_size_t)tid->user_stack + tid->user_stack_size - size;
...
void *get_thread_kernel_stack_top(rt_thread_t thread) {
  return (void *)(((rt_size_t)thread->stack_addr) +
                  ((rt_size_t)thread->stack_size));
}
...
void *arch_get_user_sp(void) {
  ...
  struct rt_hw_stack_frame *frame =
      (struct rt_hw_stack_frame *)(stack_top -
                                   sizeof(struct rt_hw_stack_frame));
...
void *arch_kernel_mmu_table_get(void) { return (void *)((char *)MMUTable); }
...
long sys_clone(void *arg[]) { return _sys_clone(arg); }

long _sys_fork(void);
long sys_fork(void) { return _sys_fork(); }

long _sys_vfork(void);
long sys_vfork(void) { return _sys_fork(); }
...
int arch_set_thread_context(void (*exit)(void), void *new_thread_stack,
  ...
  /* reset thread area */
  rt_thread_t thread =
      rt_container_of((unsigned long)thread_sp, struct rt_thread, sp);
  syscall_frame->tp = (rt_ubase_t)thread->thread_idr;

#ifdef ARCH_USING_NEW_CTX_SWITCH
  extern void *_rt_hw_stack_init(rt_ubase_t * sp, rt_ubase_t ra,
                                 rt_ubase_t sstatus);
...
void *arch_get_usp_from_uctx(struct rt_user_context *uctx) { return uctx->sp; }
