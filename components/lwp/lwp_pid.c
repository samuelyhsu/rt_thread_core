...
struct lwp_avl_struct *lwp_get_pid_ary(void) { return lwp_pid_ary; }
...
static rt_err_t _object_node_delete(struct rt_lwp *lwp,
                                    struct lwp_avl_struct *node) {
  rt_err_t ret = -1;
  rt_object_t object;
...
int lwp_getpid(void) { return ((struct rt_lwp *)rt_thread_self()->lwp)->pid; }
...
static void print_thread_info(struct rt_thread *thread, int maxlen) {
  ...
  if (thread->oncpu != RT_CPU_DETACHED)
    rt_kprintf("%-*.*s %3d %3d ", maxlen, RT_NAME_MAX, thread->name,
               thread->oncpu, thread->current_priority);
  else
    rt_kprintf("%-*.*s N/A %3d ", maxlen, RT_NAME_MAX, thread->name,
  ...
  rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
             ...
             thread->stack_size,
             ((rt_uint32_t)ptr - (rt_uint32_t)thread->stack_addr) * 100 /
                 thread->stack_size,
  ...
  rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
             (thread->stack_size + (rt_uint32_t)(rt_size_t)thread->stack_addr -
              (rt_uint32_t)(rt_size_t)thread->sp),
             thread->stack_size,
             (thread->stack_size + (rt_uint32_t)(rt_size_t)thread->stack_addr -
              (rt_uint32_t)(rt_size_t)ptr) *
...
long list_process(void) {
  ...
  maxlen = RT_NAME_MAX;
#ifdef RT_USING_SMP
  rt_kprintf("%-*.s %-*.s %-*.s cpu pri  status      sp     stack size max "
             "used left tick  error\n",
  ...
  rt_kprintf(
      "--- ---  ------- ---------- ----------  ------  ---------- ---\n");
#else
  rt_kprintf("%-*.s %-*.s %-*.s pri  status      sp     stack size max used "
             "left tick  error\n",
             4, "PID", maxlen, "CMD", maxlen, item_title);
  object_split(4);
  rt_kprintf(" ");
  ...
  if (count > 0) {
    ...
    if (threads) {
      index = rt_object_get_pointers(RT_Object_Class_Thread,
                                     (rt_object_t *)threads, count);
  ...
  for (index = 0; index < RT_LWP_MAX_NR; index++) {
    ...
    if (lwp) {
      ...
      for (node = list->next; node != list; node = node->next) {
        ...
        rt_kprintf("%4d %-*.*s ", lwp_to_pid(lwp), maxlen, RT_NAME_MAX,
                   lwp->cmd);
...
void lwp_wait_subthread_exit(void) {
  ...
  while (1) {
    ...
    if (!subthread_is_terminated) {
      ...
      for (list = thread->sibling.prev; list != &lwp->t_grp;
           list = list->prev) {
