...
int futex_wait(struct rt_futex *futex, int value,
...
int sys_futex(int *uaddr, int op, int val, const struct timespec *timeout,
   ...
   * was TRUE `timeout` should be ignored by implementation, according to POSIX
   * futex(2) manual. since only FUTEX_WAKE is implemented in rt-smart, only
   * FUTEX_WAKE was omitted currently
   */
