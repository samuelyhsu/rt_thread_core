...
struct termios *get_old_termios(void) { return &old_stdin_termios; }
...
uint32_t *lwp_get_kernel_sp(void) {
  ...
  if (rt_thread_switch_interrupt_flag) {
    kernel_sp =
        (uint32_t *)((rt_thread_t)rt_container_of(rt_interrupt_from_thread,
                                                  struct rt_thread, sp))
            ->kernel_sp;
  } else {
...
struct process_aux *lwp_argscopy(struct rt_lwp *lwp, int argc, char **argv,
  ...
  args = (int *)lwp_map_user(lwp, (void *)(USER_VADDR_TOP - ARCH_PAGE_SIZE),
                             size, 0);
  if (args == RT_NULL) {
    return RT_NULL;
  }
  ...
  str =
      (char *)((size_t)args + (argc + 2 + i + 1 + AUX_ARRAY_ITEMS_NR * 2 + 1) *
                                  sizeof(size_t));
  str_k = (char *)((size_t)args_k +
                   (argc + 2 + i + 1 + AUX_ARRAY_ITEMS_NR * 2 + 1) *
                       sizeof(size_t));
...
#else
static struct process_aux *lwp_argscopy(struct rt_lwp *lwp, int argc,
                                        char **argv, char **envp) {
#ifdef ARCH_MM_MMU
  ...
  /* argc, argv[], 0, envp[], 0 */
  str = (char *)((size_t)args +
                 (argc + 2 + i + 1 + AUX_ARRAY_ITEMS_NR * 2 + 1) * sizeof(int));
#else
  args = (int *)rt_malloc(size);
  if (args == RT_NULL) {
    return RT_NULL;
...
static int map_range_ckeck(struct map_range *m1, struct map_range *m2) {
  ...
  void *m1_end = (void *)((((size_t)m1->start + m1->size) + ARCH_PAGE_MASK) &
                          ~ARCH_PAGE_MASK);
  void *m2_start = (void *)((size_t)m2->start & ~ARCH_PAGE_MASK);
  void *m2_end = (void *)((((size_t)m2->start + m2->size) + ARCH_PAGE_MASK) &
                          ~ARCH_PAGE_MASK);
...
static int load_elf(int fd, int len, struct rt_lwp *lwp, uint8_t *load_addr,
                    struct process_aux *aux) {
  uint32_t i;
  uint32_t off = 0;
  size_t load_off = 0;
...
#ifdef ARCH_MM_MMU
  struct map_range user_area[2] = {{NULL, 0},
                                   {NULL, 0}}; /* 0 is text, 1 is data */
  void *pa, *va;
  void *va_self;
  ...
  if (eheader.e_ident[4] != 1) { /* not 32bit */
    LOG_E("elf not 32bit, %d!", eheader.e_ident[4]);
    return -RT_ERROR;
  }
#else
  if (eheader.e_ident[4] != 2) { /* not 64bit */
    LOG_E("elf not 64bit, %d!", eheader.e_ident[4]);
    return -RT_ERROR;
  }
#endif

  if (eheader.e_ident[6] != 1) { /* ver not 1 */
  ...
  { /* load aux */
    ...
    va = (uint8_t *)lwp_map_user(lwp,
                                 (void *)(USER_VADDR_TOP - ARCH_PAGE_SIZE * 2),
                                 process_header_size, 0);
    if (!va) {
...
#else
    process_header =
        (uint8_t *)rt_malloc(process_header_size + sizeof(char[16]));
    if (!process_header) {
      LOG_E("process_header malloc failed, size:0x%x!",
            process_header_size + sizeof(char[16]));
      return -RT_ERROR;
    }
#endif
    check_off(off, len);
...
#ifdef ARCH_MM_MMU
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, process_header,
                         process_header_size);
#endif
  ...
  if (load_addr) {
    ...
    for (i = 0; i < eheader.e_shnum; i++, off += sizeof sheader) {
      ...
      case SHT_PROGBITS:
        if ((sheader.sh_flags & SHF_WRITE) == 0) {
          expand_map_range(&user_area[0], (void *)sheader.sh_addr,
                           sheader.sh_size);
        } else {
          expand_map_range(&user_area[1], (void *)sheader.sh_addr,
                           sheader.sh_size);
        }
        break;
      case SHT_NOBITS:
    ...
    if (load_off == 0) {
      ...
      LOG_D("lwp text malloc : %p, size: %d!", (void *)load_off,
            lwp->text_size);
    }
  ...
  for (i = 0; i < eheader.e_phnum; i++, off += sizeof pheader) {
    ...
    if (pheader.p_type == PT_LOAD) {
      if (pheader.p_filesz > pheader.p_memsz) {
        LOG_E("pheader.p_filesz > pheader.p_memsz, p_filesz:0x%x;p_memsz:0x%x!",
...
#else
      read_len = load_fread((void *)(pheader.p_vaddr + load_off), 1,
                            pheader.p_filesz, fd);
#endif
      ...
      if (pheader.p_filesz < pheader.p_memsz) {
        ...
        va = (void *)((pheader.p_vaddr + pheader.p_filesz + load_off) &
                      ~ARCH_PAGE_MASK);
        while (size) {
          ...
          rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH,
                               (void *)((char *)va_self + off), size_s);
          off = 0;
          size -= size_s;
          va = (void *)((char *)va + ARCH_PAGE_SIZE);
        }
#else
        memset((uint8_t *)pheader.p_vaddr + pheader.p_filesz + load_off, 0,
               (size_t)(pheader.p_memsz - pheader.p_filesz));
#endif
      }
    }
  }
...
#ifdef ARCH_MM_MMU
    arch_elf_reloc(lwp->aspace, (void *)load_off, rel_dyn_start, rel_dyn_size,
                   got_start, got_size, dynsym);
#else
    arch_elf_reloc((void *)load_off, rel_dyn_start, rel_dyn_size, got_start,
                   got_size, dynsym);

    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, lwp->text_entry, lwp->text_size);
    rt_hw_cpu_icache_ops(RT_HW_CACHE_INVALIDATE, lwp->text_entry,
                         lwp->text_size);
#endif
  }
...
rt_weak int lwp_load(const char *filename, struct rt_lwp *lwp,
                     uint8_t *load_addr, size_t addr_size,
                     struct process_aux *aux) {
  uint8_t *ptr;
  int ret = -1;
  int len;
...
#else
  arch_start_umode(lwp->args, lwp->text_entry, lwp->data_entry,
                   (void *)((uint32_t)lwp->data_entry + lwp->data_size));
#endif /* ARCH_MM_MMU */
}
...
pid_t lwp_execve(char *filename, int debug, int argc, char **argv,
                 char **envp) {
  int result;
  rt_base_t level;
  struct rt_lwp *lwp;
  ...
  if (result == RT_EOK) {
    ...
    if (thread != RT_NULL) {
      ...
      thread->user_stack =
          app_head->stack_offset
              ? (void *)(app_head->stack_offset - app_head->data_offset +
                         (uint32_t)lwp->data_entry)
              : RT_NULL;
