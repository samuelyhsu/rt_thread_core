...
static void lwp_avl_rebalance(struct lwp_avl_struct ***nodeplaces_ptr,
                              int count) {
  for (; count > 0; count--) {
...
void lwp_avl_remove(struct lwp_avl_struct *node_to_delete,
                    struct lwp_avl_struct **ptree) {
...
struct lwp_avl_struct *lwp_avl_find(avl_key_t key,
                                    struct lwp_avl_struct *ptree) {
  for (;;) {
    if (ptree == AVL_EMPTY) {
...
int lwp_avl_traversal(struct lwp_avl_struct *ptree,
                      int (*fun)(struct lwp_avl_struct *, void *), void *arg) {
  int ret;
...
rt_weak struct lwp_avl_struct *
lwp_map_find_first(struct lwp_avl_struct *ptree) {
  if (ptree == AVL_EMPTY) {
    return (struct lwp_avl_struct *)0;
  }
  while (1) {
