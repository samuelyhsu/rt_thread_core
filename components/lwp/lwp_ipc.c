...
static rt_ipc_msg_t _ipc_msg_free_list =
    (rt_ipc_msg_t)RT_NULL;      /* released chain */
static int rt_ipc_msg_used = 0; /* first unallocated entry */
static struct rt_ipc_msg
    ipc_msg_pool[RT_CH_MSG_MAX_NR]; /* initial message array */
...
static rt_ipc_msg_t _ipc_msg_alloc(void) {
  ...
  if (_ipc_msg_free_list) /* use the released chain first */
  {
    ...
    _ipc_msg_free_list =
        (rt_ipc_msg_t)p->msg.sender; /* emtry payload as a pointer */
 ...
 */
static void rt_ipc_msg_init(rt_ipc_msg_t msg, struct rt_channel_msg *data,
                            rt_uint8_t need_reply) {
 ...
 */
rt_inline rt_err_t rt_channel_list_suspend(rt_list_t *list,
                                           struct rt_thread *thread) {
  /* suspend thread */
...
rt_err_t rt_raw_channel_close(rt_channel_t ch) {
  ...
  if (ch->ref == 0) {
    ...
    rt_object_delete(
        &ch->parent.parent); /* release the IPC channel structure */
  }
  rt_hw_interrupt_enable(temp);
  return RT_EOK;
}

static rt_err_t wakeup_sender_wait_recv(void *object,
                                        struct rt_thread *thread) {
  rt_channel_t ch;
...
static rt_err_t wakeup_sender_wait_reply(void *object,
                                         struct rt_thread *thread) {
  rt_channel_t ch;
 ...
 */
static rt_err_t _rt_raw_channel_send_recv_timeout(rt_channel_t ch,
  ...
  case RT_IPC_STAT_ACTIVE:
    if (need_reply) {
      ...
      if (time > 0) {
        rt_timer_control(&(thread_send->thread_timer), RT_TIMER_CTRL_GET_FUNC,
                         &old_timeout_func);
        rt_timer_control(&(thread_send->thread_timer), RT_TIMER_CTRL_SET_FUNC,
                         sender_timeout);
        /* reset the timeout of thread timer and start it */
        rt_timer_control(&(thread_send->thread_timer), RT_TIMER_CTRL_SET_TIME,
                         &time);
  ...
  case RT_IPC_STAT_WAIT:
    ...
    thread_recv =
        rt_list_entry(ch->parent.suspend_thread.next, struct rt_thread, tlist);
...
rt_err_t rt_raw_channel_send_recv(rt_channel_t ch, rt_channel_msg_t data,
                                  rt_channel_msg_t data_ret) {
  return _rt_raw_channel_send_recv_timeout(ch, data, 1, data_ret,
                                           RT_WAITING_FOREVER);
}
 ...
 */
static rt_err_t _rt_raw_channel_recv_timeout(rt_channel_t ch,
                                             rt_channel_msg_t data,
                                             rt_int32_t time) {
  struct rt_thread *thread;
  ...
  if (ch->wait_msg.next != &ch->wait_msg) /* there exist unhandled messages */
  {
    ...
    *data = msg_ret->msg;   /* extract the transferred data */
    ...
    thread->error = RT_EOK;
    if (time > 0) {
      rt_timer_control(&(thread->thread_timer), RT_TIMER_CTRL_GET_FUNC,
                       &old_timeout_func);
      rt_timer_control(&(thread->thread_timer), RT_TIMER_CTRL_SET_FUNC,
                       receiver_timeout);
      /* reset the timeout of thread timer and start it */
...
rt_err_t lwp_channel_send_recv_timeout(int fdt_type, int fd,
                                       rt_channel_msg_t data,
                                       rt_channel_msg_t data_ret,
                                       rt_int32_t time) {
  rt_channel_t ch;
...
rt_err_t lwp_channel_recv_timeout(int fdt_type, int fd, rt_channel_msg_t data,
                                  rt_int32_t time) {
  rt_channel_t ch;
  ch = fd_2_channel(fdt_type, fd);
  if (ch) {
...
rt_err_t rt_channel_recv(int fd, rt_channel_msg_t data) {
  return lwp_channel_recv_timeout(FDT_TYPE_KERNEL, fd, data,
                                  RT_WAITING_FOREVER);
}
