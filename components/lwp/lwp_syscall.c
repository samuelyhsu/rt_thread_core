 ...
 * 2021-02-03     lizhirui     add limit condition for network syscall and add
 * 64-bit arch support 2021-02-06     lizhirui     fix some bugs 2021-02-12
 * lizhirui     add 64-bit support for sys_brk 2021-02-20     lizhirui     fix
 * some warnings
 */
#define _GNU_SOURCE
...
int sys_futex(int *uaddr, int op, int val, void *timeout, void *uaddr2,
              int val3);
int sys_pmutex(void *umutex, int op, void *arg);
int sys_cacheflush(void *addr, int len, int cache);
static void *kmem_get(size_t size) { return rt_malloc(size); }

static void kmem_put(void *kptr) { rt_free(kptr); }
#else
...
static void sockaddr_tolwip(const struct musl_sockaddr *std,
                            struct sockaddr *lwip) {
  if (std && lwip) {
...
static void sockaddr_tomusl(const struct sockaddr *lwip,
                            struct musl_sockaddr *std) {
  if (std && lwip) {
...
#ifdef ARCH_MM_MMU
  arch_crt_start_umode(parameter, tid->user_entry, (void *)user_stack,
                       tid->stack_addr + tid->stack_size);
#else
  set_user_context((void *)user_stack);
  arch_start_umode(parameter, tid->user_entry,
                   ((struct rt_lwp *)tid->lwp)->data_entry, (void *)user_stack);
#endif /* ARCH_MM_MMU */
}
...
/* exit group */
void sys_exit_group(int status) { return; }
...
#define INTF_POLLIN_MASK                                                       \
  (INTF_POLLIN | INTF_POLLRDNORM | INTF_POLLRDBAND | INTF_POLLPRI)
...
/* syscall: "nanosleep" ret: "int" args: "const struct timespec *" "struct
 * timespec *" */
int sys_nanosleep(const struct timespec *rqtp, struct timespec *rmtp) {
  ...
  if ((ret != -1 || rt_get_errno() == EINTR) && rmtp &&
      lwp_user_accessable((void *)rmtp, sizeof *rmtp)) {
...
int sys_gettimeofday(struct timeval *tp, struct timezone *tzp) {
  ...
  if (tp) {
    ...
    t_k.tv_usec =
        (rt_tick_get() % RT_TICK_PER_SECOND) * (1000000 / RT_TICK_PER_SECOND);
...
int sys_getpid(void) { return lwp_getpid(); }
...
rt_err_t sys_sem_release(rt_sem_t sem) { return rt_sem_release(sem); }
...
rt_err_t sys_mutex_release(rt_mutex_t mutex) { return rt_mutex_release(mutex); }

#ifdef ARCH_MM_MMU
/* memory allocation */
rt_base_t sys_brk(void *addr) { return lwp_brk(addr); }
...
int sys_munmap(void *addr, size_t length) { return lwp_munmap(addr); }
...
int sys_madvise(void *addr, size_t len, int behav) { return -ENOSYS; }
#endif
...
rt_err_t sys_event_recv(rt_event_t event, rt_uint32_t set, rt_uint8_t opt,
                        rt_int32_t timeout, rt_uint32_t *recved) {
  if ((recved != NULL) &&
      !lwp_user_accessable((void *)recved, sizeof(rt_uint32_t *))) {
    return -EFAULT;
  }
...
rt_err_t sys_mb_send_wait(rt_mailbox_t mb, rt_ubase_t value,
                          rt_int32_t timeout) {
...
rt_mq_t sys_mq_create(const char *name, rt_size_t msg_size, rt_size_t max_msgs,
                      rt_uint8_t flag) {
...
rt_err_t sys_mq_recv(rt_mq_t mq, void *buffer, rt_size_t size,
                     rt_int32_t timeout) {
  if (!lwp_user_accessable((void *)buffer, size)) {
    return -EFAULT;
  }
...
rt_timer_t sys_rt_timer_create(const char *name, void *data, rt_tick_t time,
                               rt_uint8_t flag) {
  rt_timer_t timer =
      rt_timer_create(name, timer_timeout_callback, (void *)data, time, flag);
...
rt_err_t sys_rt_timer_start(rt_timer_t timer) { return rt_timer_start(timer); }

rt_err_t sys_rt_timer_stop(rt_timer_t timer) { return rt_timer_stop(timer); }

rt_err_t sys_rt_timer_control(rt_timer_t timer, int cmd, void *arg) {
  return rt_timer_control(timer, cmd, arg);
}

rt_err_t sys_timer_create(clockid_t clockid, struct sigevent *restrict sevp,
...
rt_thread_t sys_thread_create(void *arg[]) {
  ...
  thread =
      rt_thread_create((const char *)arg[0], _crt_thread_entry, (void *)arg[2],
                       ALLOC_KERNEL_STACK_SIZE, (rt_uint8_t)(size_t)arg[4],
  ...
  if (kstack_size < ALLOC_KERNEL_STACK_SIZE_MIN) {
    ...
    kstack_size =
        kstack_size ? ALLOC_KERNEL_STACK_SIZE_MIN : ALLOC_KERNEL_STACK_SIZE;
  ...
  thread = rt_thread_create((const char *)arg[0], _crt_thread_entry,
                            (void *)arg[2], kstack_size,
                            (rt_uint8_t)(size_t)arg[5], (rt_uint32_t)arg[6]);
  if (!thread) {
    goto fail;
  }
  thread->cleanup = lwp_cleanup;
...
long _sys_clone(void *arg[]) {
  ...
  thread = rt_thread_create(self->name, RT_NULL, RT_NULL, self->stack_size,
                            self->init_priority, self->init_tick);
  if (!thread) {
    goto fail;
  }
  ...
  arch_set_thread_context(
      arch_clone_exit,
      (void *)((char *)thread->stack_addr + thread->stack_size), user_stack,
      &thread->sp);
...
rt_weak long sys_clone(void *arg[]) { return _sys_clone(arg); }
...
static void lwp_struct_copy(struct rt_lwp *dst, struct rt_lwp *src) {
  ...
  rt_memcpy(dst->signal_handler, src->signal_handler,
            sizeof dst->signal_handler);
}
...
int _sys_fork(void) {
  ...
  thread = rt_thread_create(self_thread->name, RT_NULL, RT_NULL,
                            self_thread->stack_size, self_thread->init_priority,
  ...
  /* copy origin stack */
  rt_memcpy(thread->stack_addr, self_thread->stack_addr,
  ...
  arch_set_thread_context(
      arch_fork_exit, (void *)((char *)thread->stack_addr + thread->stack_size),
...
rt_weak int sys_fork(void) { return _sys_fork(); }

rt_weak int sys_vfork(void) { return sys_fork(); }

struct process_aux *lwp_argscopy(struct rt_lwp *lwp, int argc, char **argv,
                                 char **envp);
int lwp_load(const char *filename, struct rt_lwp *lwp, uint8_t *load_addr,
             size_t addr_size, struct process_aux *aux);
...
static char *_insert_args(int new_argc, char *new_argv[],
                          struct lwp_args_info *args) {
  void *page = NULL;
  int err = 0;
  char **nargv;
  char **nenvp;
...
int load_ldso(struct rt_lwp *lwp, char *exec_name, char *const argv[],
              char *const envp[]) {
  int ret = -1;
  int i;
  void *page;
  ...
  if ((aux = lwp_argscopy(lwp, args_info.argc, args_info.argv,
                          args_info.envp)) == NULL) {
    SET_ERRNO(ENOMEM);
    goto quit;
  }
...
int sys_execve(const char *path, char *const argv[], char *const envp[]) {
  ...
  /* now load elf */
  if ((aux = lwp_argscopy(new_lwp, args_info.argc, args_info.argv,
                          args_info.envp)) == NULL) {
    SET_ERRNO(ENOMEM);
    goto quit;
  }
  ...
  if (ret == RT_EOK) {
    ...
    lwp_ref_dec(new_lwp);
    arch_start_umode(lwp->args, lwp->text_entry, (void *)USER_STACK_VEND,
...
rt_thread_t sys_thread_self(void) { return rt_thread_self(); }
...
rt_err_t sys_channel_send_recv_timeout(int fd, rt_channel_msg_t data,
...
rt_err_t sys_channel_recv_timeout(int fd, rt_channel_msg_t data,
                                  rt_int32_t time) {
...
void sys_exit_critical(void) { rt_sem_release(&critical_lock); }
...
int sys_notimpl(void) { return -ENOSYS; }

uint32_t sys_hw_interrupt_disable(void) { return rt_hw_interrupt_disable(); }

void sys_hw_interrupt_enable(uint32_t level) { rt_hw_interrupt_enable(level); }
...
int sys_shmrm(int id) { return lwp_shmrm(id); }

void *sys_shmat(int id, void *shm_vaddr) { return lwp_shmat(id, shm_vaddr); }

int sys_shmdt(void *shm_vaddr) { return lwp_shmdt(shm_vaddr); }
...
/* device interfaces */
rt_err_t sys_device_init(rt_device_t dev) { return rt_device_init(dev); }

rt_err_t sys_device_register(rt_device_t dev, const char *name,
                             rt_uint16_t flags) {
...
rt_device_t sys_device_find(const char *name) { return rt_device_find(name); }

rt_err_t sys_device_open(rt_device_t dev, rt_uint16_t oflag) {
  return rt_device_open(dev, oflag);
}

rt_err_t sys_device_close(rt_device_t dev) { return rt_device_close(dev); }

rt_ssize_t sys_device_read(rt_device_t dev, rt_off_t pos, void *buffer,
                           rt_size_t size) {
  return rt_device_read(dev, pos, buffer, size);
}

rt_ssize_t sys_device_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                            rt_size_t size) {
...
int sys_shutdown(int socket, int how) { return shutdown(socket, how); }

int sys_getpeername(int socket, struct musl_sockaddr *name,
                    socklen_t *namelen) {
  int ret = -1;
  struct sockaddr sa;
  struct musl_sockaddr kname;
...
int sys_getsockname(int socket, struct musl_sockaddr *name,
                    socklen_t *namelen) {
  int ret = -1;
  struct sockaddr sa;
  struct musl_sockaddr kname;
...
int sys_getsockopt(int socket, int level, int optname, void *optval,
                   socklen_t *optlen) {
  int ret;
...
int sys_setsockopt(int socket, int level, int optname, const void *optval,
                   socklen_t optlen) {
  int ret;
  convert_sockopt(&level, &optname);
...
int sys_connect(int socket, const struct musl_sockaddr *name,
                socklen_t namelen) {
  int ret;
  struct sockaddr sa;
  struct musl_sockaddr kname;
...
int sys_listen(int socket, int backlog) { return listen(socket, backlog); }
...
int sys_closesocket(int socket) { return closesocket(socket); }

#endif

rt_thread_t sys_thread_find(char *name) { return rt_thread_find(name); }

rt_tick_t sys_tick_get(void) { return rt_tick_get(); }

rt_err_t sys_thread_mdelay(rt_int32_t ms) { return rt_thread_mdelay(ms); }
...
#ifdef ARCH_MM_MMU
  if (ret == 0 && oact) {
    lwp_put_to_user(&oact->handler, &pkoact->__sa_handler._sa_handler,
                    sizeof(void (*)(int)));
    lwp_put_to_user(&oact->mask, &pkoact->sa_mask, sigsetsize);
    lwp_put_to_user(&oact->flags, &pkoact->sa_flags, sizeof(int));
    lwp_put_to_user(&oact->restorer, &pkoact->sa_restorer,
                    sizeof(void (*)(void)));
  }
#endif /* ARCH_MM_MMU */
out:
...
int sys_sigprocmask(int how, const sigset_t *sigset, sigset_t *oset,
                    size_t size) {
  int ret = -1;
...
int sys_thread_sigprocmask(int how, const lwp_sigset_t *sigset,
                           lwp_sigset_t *oset, size_t size) {
  int ret = -1;
  lwp_sigset_t *pnewset = RT_NULL, *poldset = RT_NULL;
#ifdef ARCH_MM_MMU
  lwp_sigset_t newset, oldset;
...
int sys_gethostbyname2_r(const char *name, int af, struct hostent *ret,
  ...
  *result = ret;
  ...
  sal_ret = sal_gethostbyname_r(k_name, &sal_he, sal_buf, HOSTENT_BUFSZ,
                                &sal_result, &sal_err);
  if (sal_ret == 0) {
    int index = 0, cnt = 0;
    char *ptr = buf;
...
rt_err_t sys_get_errno(void) { return rt_get_errno(); }
#ifdef ARCH_MM_MMU
...
int sys_gettid(void) { return rt_thread_self()->tid; }
...
int sys_clock_nanosleep(clockid_t clk, int flags, const struct timespec *rqtp,
...
int sys_prlimit64(pid_t pid, unsigned int resource,
...
int sys_sched_setparam(pid_t pid, void *param) {
  ...
  if (lwp) {
    main_thread = rt_list_entry(lwp->t_grp.prev, struct rt_thread, sibling);
    return rt_thread_control(main_thread, RT_THREAD_CTRL_CHANGE_PRIORITY,
                             (void *)&sched_param->sched_priority);
  }
  return ret;
}
...
mqd_t sys_mq_open(const char *name, int flags, mode_t mode,
                  struct mq_attr *attr) {
  mqd_t mqdes;
  rt_err_t ret = 0;
#ifdef ARCH_MM_MMU
...
int sys_mq_timedsend(mqd_t mqd, const char *msg, size_t len, unsigned prio,
...
int sys_mq_timedreceive(mqd_t mqd, char *restrict msg, size_t len,
...
int sys_mq_getsetattr(mqd_t mqd, const struct mq_attr *restrict new,
...
const static void *func_table[] = {
    ...
    SYSCALL_SIGN(
        sys_notimpl), // SYSCALL_SIGN(sys_hw_interrupt_disable), /* 95 */
