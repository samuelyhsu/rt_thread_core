 ...
 * 2018-12-27     Jesven       Fix the problem that disable interrupt too long
 * in list_thread Provide protection for the "first layer of objects" when
 * list_* 2020-04-07     chenhui      add clear 2022-07-02     Stanley Lwin add
 * list command
 */
...
#ifdef RT_USING_SMP
  rt_kprintf("%-*.*s cpu bind pri  status      sp     stack size max used left "
             "tick  error\n",
      ...
      for (i = 0; i < find_arg.nr_out; i++) {
        ...
        thread = (struct rt_thread *)obj;
        {
          ...
          if (thread->oncpu != RT_CPU_DETACHED)
            rt_kprintf("%-*.*s %3d %3d %4d ", maxlen, RT_NAME_MAX, thread->name,
                       thread->oncpu, thread->bind_cpu,
                       thread->current_priority);
          else
            rt_kprintf("%-*.*s N/A %3d %4d ", maxlen, RT_NAME_MAX, thread->name,
                       thread->bind_cpu, thread->current_priority);
    ...
    next = list_get_next(next, &find_arg);
    {
      ...
      for (i = 0; i < find_arg.nr_out; i++) {
        ...
        if (!rt_list_isempty(&m->parent.suspend_thread)) {
          rt_kprintf("%-*.*s %-8.*s %04d %8d  %04d ", maxlen, RT_NAME_MAX,
                     m->parent.parent.name, RT_NAME_MAX, m->owner->name,
                     m->hold, m->priority,
  ...
  do {
    next = list_get_next(next, &find_arg);
    {
      ...
      for (i = 0; i < find_arg.nr_out; i++) {
        ...
        rt_list_for_each(node, &mp->suspend_thread) { suspend_thread_count++; }
