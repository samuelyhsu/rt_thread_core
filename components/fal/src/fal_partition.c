 ...
 * When this option is disable, it will auto find the partition table on a
 * specified location in flash partition.
 */
...
static struct part_flash_info part_flash_cache[sizeof(partition_table_def) /
                                               sizeof(partition_table_def[0])] =
    {0};
...
void fal_show_part_table(void) {
  ...
  log_i("| %-*.*s | %-*.*s |   offset   |    length  |", part_name_max,
        FAL_DEV_NAME_MAX, item1, flash_dev_name_max, FAL_DEV_NAME_MAX, item2);
  log_i("-------------------------------------------------------------");
  for (i = 0; i < partition_table_len; i++) {
    ...
    log_i("| %-*.*s | %-*.*s | 0x%08lx | 0x%08x |", part_name_max,
          FAL_DEV_NAME_MAX, part->name, flash_dev_name_max, FAL_DEV_NAME_MAX,
          part->flash_name, part->offset, part->len);
  }
  log_i("=============================================================");
}

static int check_and_update_part_cache(const struct fal_partition *table,
                                       size_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (table[i].offset >= (long)flash_dev->len) {
      log_e("Initialize failed! Partition(%s) offset address(%ld) out of flash "
            "bound(<%d).",
...
int fal_partition_init(void) {
  ...
  partition_table_len =
      sizeof(partition_table_def) / sizeof(partition_table_def[0]);
#else
  /* load partition table from the end address FAL_PART_TABLE_END_OFFSET, error
   * return 0 */
  ...
  if (flash_dev == NULL) {
    log_e("Initialize failed! Flash device (%s) NOT found.",
  ...
  if (part_table_offset < 0 || part_table_offset >= (long)flash_dev->len) {
    log_e("Setting partition table end offset address(%ld) out of flash "
          "bound(<%d).",
  ...
  /* find partition table location */
  {
    ...
    while (part_table_offset >= 0) {
      if (flash_dev->ops.read(part_table_offset, read_buf, sizeof(read_buf)) >
          0) {
        ...
        for (i = 0; i < sizeof(read_buf) - sizeof(read_magic_word) + 1; i++) {
          ...
          if (read_magic_word ==
              ((FAL_PART_MAGIC_WORD_H << 16) + FAL_PART_MAGIC_WORD_L)) {
  ...
  while (part_table_find_ok) {
    ...
    if (flash_dev->ops.read(part_table_offset - table_item_size * (table_num),
      ...
      log_e("Initialize failed! Flash device (%s) read error!",
            flash_dev->name);
      table_num = 0;
      break;
    }

    if (new_part->magic_word !=
        ((FAL_PART_MAGIC_WORD_H << 16) + FAL_PART_MAGIC_WORD_L)) {
      break;
    }

    partition_table = (fal_partition_t)FAL_REALLOC(
        partition_table, table_item_size * (table_num + 1));
  ...
  if (table_num == 0) {
    log_e(
        "Partition table NOT found on flash: %s (len: %d) from offset: 0x%08x.",
        FAL_PART_TABLE_FLASH_DEV_NAME, FAL_DEV_NAME_MAX,
...
static const struct fal_flash_dev *
flash_device_find_by_part(const struct fal_partition *part) {
...
int fal_partition_read(const struct fal_partition *part, uint32_t addr,
  ...
  if (flash_dev == NULL) {
    log_e("Partition read error! Don't found flash device(%s) of the "
          "partition(%s).",
...
int fal_partition_write(const struct fal_partition *part, uint32_t addr,
  ...
  flash_dev = flash_device_find_by_part(part);
  if (flash_dev == NULL) {
    log_e("Partition write error!  Don't found flash device(%s) of the "
          "partition(%s).",
          part->flash_name, part->name);
    return -1;
  }
...
int fal_partition_erase(const struct fal_partition *part, uint32_t addr,
  ...
  flash_dev = flash_device_find_by_part(part);
  if (flash_dev == NULL) {
    log_e("Partition erase error! Don't found flash device(%s) of the "
          "partition(%s).",
          part->flash_name, part->name);
    return -1;
  }
