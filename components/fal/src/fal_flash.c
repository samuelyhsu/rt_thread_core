...
#error                                                                         \
    "You must defined flash device table (FAL_FLASH_DEV_TABLE) on 'fal_cfg.h'"
#endif

static const struct fal_flash_dev *const device_table[] = FAL_FLASH_DEV_TABLE;
static const size_t device_table_len =
...
int fal_flash_init(void) {
  ...
  for (i = 0; i < device_table_len; i++) {
    ...
    log_d("Flash device | %*.*s | addr: 0x%08lx | len: 0x%08x | blk_size: "
          "0x%08x |initialized finish.",
          FAL_DEV_NAME_MAX, FAL_DEV_NAME_MAX, device_table[i]->name,
          device_table[i]->addr, device_table[i]->len,
