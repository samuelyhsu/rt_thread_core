...
static rt_ssize_t blk_dev_read(rt_device_t dev, rt_off_t pos, void *buffer,
                               rt_size_t size) {
  int ret = 0;
  struct fal_blk_device *part = (struct fal_blk_device *)dev;
  ...
  ret = fal_partition_read(part->fal_part, pos * part->geometry.block_size,
                           buffer, size * part->geometry.block_size);
...
static rt_ssize_t blk_dev_write(rt_device_t dev, rt_off_t pos,
...
struct rt_device *fal_blk_device_create(const char *parition_name) {
  ...
  if (blk_dev) {
    ...
    rt_device_register(RT_DEVICE(blk_dev), fal_part->name,
...
static rt_ssize_t mtd_nor_dev_read(struct rt_mtd_nor_device *device,
...
static rt_ssize_t mtd_nor_dev_write(struct rt_mtd_nor_device *device,
                                    rt_off_t offset, const rt_uint8_t *data,
                                    rt_uint32_t length) {
  int ret = 0;
...
static rt_err_t mtd_nor_dev_erase(struct rt_mtd_nor_device *device,
                                  rt_off_t offset, rt_uint32_t length) {
  int ret = 0;
  struct fal_mtd_nor_device *part;
...
struct rt_device *fal_mtd_nor_device_create(const char *parition_name) {
  ...
  mtd_nor_dev =
      (struct fal_mtd_nor_device *)rt_malloc(sizeof(struct fal_mtd_nor_device));
  if (mtd_nor_dev) {
...
/* RT-Thread device interface */
static rt_ssize_t char_dev_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                rt_size_t size) {
  int ret = 0;
...
static rt_ssize_t char_dev_write(rt_device_t dev, rt_off_t pos,
                                 const void *buffer, rt_size_t size) {
  int ret = 0;
...
const static struct rt_device_ops char_dev_ops = {
    RT_NULL, RT_NULL, RT_NULL, char_dev_read, char_dev_write, RT_NULL};
#endif
...
static const struct dfs_file_ops char_dev_fops = {
    char_dev_fopen, RT_NULL,         RT_NULL,
    char_dev_fread, char_dev_fwrite, RT_NULL, /* flush */
    RT_NULL,                                  /* lseek */
    RT_NULL,                                  /* getdents */
    RT_NULL,
};
...
struct rt_device *fal_char_device_create(const char *parition_name) {
  ...
  char_dev =
      (struct fal_char_device *)rt_malloc(sizeof(struct fal_char_device));
  if (char_dev) {
    ...
    rt_device_register(RT_DEVICE(char_dev), fal_part->name,
                       RT_DEVICE_FLAG_RDWR);
...
#define CMD_BENCH_INDEX 4
  ...
  const char *help_info[] = {
      [CMD_PROBE_INDEX] = "fal probe [dev_name|part_name]   - probe flash "
                          "device or partition by given name",
      [CMD_READ_INDEX] = "fal read addr size               - read 'size' bytes "
                         "starting at 'addr'",
      [CMD_WRITE_INDEX] = "fal write addr data1 ... dataN   - write some bytes "
                          "'data' starting at 'addr'",
      [CMD_ERASE_INDEX] = "fal erase addr size              - erase 'size' "
                          "bytes starting at 'addr'",
      [CMD_BENCH_INDEX] = "fal bench <blk_size>             - benchmark test "
                          "with per block size",
  };
  ...
  if (argc < 2) {
    ...
    if (!strcmp(operator, "probe")) {
      ...
      if (flash_dev) {
        ...
        rt_kprintf("Probed a flash partition | %s | flash_dev: %s | offset: "
                   "%ld | len: %d |.\n",
                   part_dev->name, part_dev->flash_name, part_dev->offset,
                   part_dev->len);
      } else {
      ...
      if (!flash_dev && !part_dev) {
        rt_kprintf("No flash device or partition was probed. Please run 'fal "
                   "probe'.\n");
        return;
      }
      if (!rt_strcmp(operator, "read")) {
        ...
        if (argc < 4) {
          ...
          if (data) {
            ...
            if (result >= 0) {
              rt_kprintf(
                  "Write data success. Start from 0x%08X, size is %ld.\n", addr,
                  size);
        ...
        if (argc < 3) {
          ...
          rt_kprintf("DANGER: It will erase full chip or partition! Please run "
                     "'fal bench %d yes'.\n",
        ...
        size_t write_size = strtol(argv[2], NULL, 0),
               read_size = strtol(argv[2], NULL, 0), cur_op_size;
        uint8_t *write_data = (uint8_t *)rt_malloc(write_size),
                *read_data = (uint8_t *)rt_malloc(read_size);
        ...
        if (write_data && read_data) {
          ...
          for (i = 0; i < size; i += write_size) {
            ...
            if (flash_dev) {
              ...
              result =
                  fal_partition_write(part_dev, i, write_data, cur_op_size);
            }
            if (result < 0) {
              break;
            }
