...
static void
virtio_console_send_ctrl(struct virtio_console_device *virtio_console_dev,
  ...
  rt_memcpy(&virtio_console_dev->info[id].tx_ctrl, ctrl,
  ...
  virtio_fill_desc(virtio_dev, VIRTIO_CONSOLE_QUEUE_CTRL_TX, id,
                   virtio_console_dev->info[id].tx_ctrl_paddr,
                   sizeof(struct virtio_console_control), 0, 0);
...
static rt_ssize_t virtio_console_port_read(rt_device_t dev, rt_off_t pos,
                                           void *buffer, rt_size_t size);
static rt_ssize_t virtio_console_port_write(rt_device_t dev, rt_off_t pos,
                                            const void *buffer, rt_size_t size);
static rt_err_t virtio_console_port_control(rt_device_t dev, int cmd,
                                            void *args);

#ifdef RT_USING_DEVICE_OPS
const static struct rt_device_ops virtio_console_port_ops = {
    virtio_console_port_init,  virtio_console_port_open,
    virtio_console_port_close, virtio_console_port_read,
    virtio_console_port_write, virtio_console_port_control};
#endif

static rt_err_t
virtio_console_port_create(struct virtio_console_device *virtio_console_dev) {
  rt_uint32_t port_id;
  char dev_name[RT_NAME_MAX];
  struct port_device *port_dev, *prev_port_dev = RT_NULL;
  struct virtio_device *virtio_dev = &virtio_console_dev->virtio_dev;

  if (virtio_console_dev->port_nr > 0 &&
      !virtio_has_feature(virtio_dev, VIRTIO_CONSOLE_F_MULTIPORT)) {
    return -RT_ENOSYS;
  }
  ...
  port_dev->queue_rx_index = VIRTIO_CONSOLE_PORT_QUEUE_INDEX(
      port_dev->port_id, VIRTIO_CONSOLE_QUEUE_DATA_RX);
  port_dev->queue_tx_index = VIRTIO_CONSOLE_PORT_QUEUE_INDEX(
      port_dev->port_id, VIRTIO_CONSOLE_QUEUE_DATA_TX);
  ...
  rt_snprintf(dev_name, RT_NAME_MAX, "vport%dp%d",
              virtio_console_dev->console_id, port_id);

  if (rt_device_register((rt_device_t)port_dev, dev_name,
...
static void
virtio_console_port_destroy(struct virtio_console_device *virtio_console_dev,
...
static rt_err_t virtio_console_port_init(rt_device_t dev) {
  ...
  rx_queue_index = VIRTIO_CONSOLE_PORT_QUEUE_INDEX(
      port_dev->port_id, VIRTIO_CONSOLE_QUEUE_DATA_RX);
  tx_queue_index = VIRTIO_CONSOLE_PORT_QUEUE_INDEX(
      port_dev->port_id, VIRTIO_CONSOLE_QUEUE_DATA_TX);
  ...
  for (id = 0; id < queue_rx->num; ++id) {
    ...
    virtio_fill_desc(virtio_dev, rx_queue_index, id, VIRTIO_VA2PA(addr),
                     sizeof(char), VIRTQ_DESC_F_WRITE, 0);
...
static rt_err_t virtio_console_port_open(rt_device_t dev, rt_uint16_t oflag) {
  ...
  if (port_dev->port_id == 0 &&
      virtio_has_feature(&port_dev->console->virtio_dev,
...
static rt_ssize_t virtio_console_port_write(rt_device_t dev, rt_off_t pos,
  ...
  while (i < size || ch == '\r') {
    ...
    virtio_fill_desc(virtio_dev, queue_tx_index, id,
                     VIRTIO_VA2PA(&port_dev->info[id].tx_char), sizeof(char), 0,
                     0);
...
static rt_err_t virtio_console_port_control(rt_device_t dev, int cmd,
  ...
  case RT_DEVICE_CTRL_NOTIFY_SET:
    ...
    rt_memcpy(&port_dev->rx_notify_helper, args,
              sizeof(port_dev->rx_notify_helper));
    break;
...
static rt_err_t virtio_console_init(rt_device_t dev) {
  struct virtio_console_device *virtio_console_dev =
      (struct virtio_console_device *)dev;
  struct virtio_device *virtio_dev = &virtio_console_dev->virtio_dev;
  ...
  if (virtio_has_feature(virtio_dev, VIRTIO_CONSOLE_F_MULTIPORT)) {
    ...
    virtio_alloc_desc_chain(virtio_dev, VIRTIO_CONSOLE_QUEUE_CTRL_RX,
                            queue_ctrl_rx->num, idx);
    virtio_alloc_desc_chain(virtio_dev, VIRTIO_CONSOLE_QUEUE_CTRL_TX,
                            queue_ctrl_tx->num, idx);

    for (id = 0; id < queue_ctrl_rx->num; ++id) {
      void *addr = &virtio_console_dev->info[id].rx_ctrl;

      virtio_fill_desc(
          virtio_dev, VIRTIO_CONSOLE_QUEUE_CTRL_RX, id, VIRTIO_VA2PA(addr),
          sizeof(struct virtio_console_control), VIRTQ_DESC_F_WRITE, 0);
    ...
    for (id = 0; id < queue_ctrl_tx->num; ++id) {
      virtio_console_dev->info[id].tx_ctrl_paddr =
          VIRTIO_VA2PA(&virtio_console_dev->info[id].tx_ctrl);
    }
...
const static struct rt_device_ops virtio_console_ops = {
    virtio_console_init,   RT_NULL, RT_NULL, RT_NULL, RT_NULL,
...
static void virtio_console_isr(int irqno, void *param) {
  ...
  struct port_device *port_dev;
  struct virtio_console_device *virtio_console_dev =
      (struct virtio_console_device *)param;
  struct virtio_device *virtio_dev = &virtio_console_dev->virtio_dev;
...
rt_err_t rt_virtio_console_init(rt_ubase_t *mmio_base, rt_uint32_t irq) {
  ...
  virtio_console_dev->config =
      (struct virtio_console_config *)virtio_dev->mmio_config->config;
  ...
  if (!virtio_has_feature(virtio_dev, VIRTIO_CONSOLE_F_MULTIPORT)) {
    ...
    if (virtio_console_dev->config->max_nr_ports >
        RT_USING_VIRTIO_CONSOLE_PORT_MAX_NR) {
      virtio_console_dev->max_port_nr = RT_USING_VIRTIO_CONSOLE_PORT_MAX_NR;
      virtio_console_dev->config->max_nr_ports =
          virtio_console_dev->max_port_nr;
    } else {
      virtio_console_dev->max_port_nr =
          virtio_console_dev->config->max_nr_ports;
    }

    queues_num = VIRTIO_CONSOLE_PORT_QUEUE_INDEX(
        virtio_console_dev->max_port_nr, VIRTIO_CONSOLE_QUEUE_DATA_RX);
  }
  ...
  rt_hw_interrupt_install(irq, virtio_console_isr, virtio_console_dev,
                          dev_name);
  rt_hw_interrupt_umask(irq);

  return rt_device_register((rt_device_t)virtio_console_dev, dev_name,
                            RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX);
