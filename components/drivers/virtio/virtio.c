...
rt_bool_t virtio_has_feature(struct virtio_device *dev,
                             rt_uint32_t feature_bit) {
  _virtio_dev_check(dev);
...
rt_err_t virtio_queue_init(struct virtio_device *dev, rt_uint32_t queue_index,
                           rt_size_t ring_size) {
  int i;
  void *pages;
  ...
  pages_total_size = VIRTIO_PAGE_ALIGN(VIRTQ_DESC_TOTAL_SIZE(ring_size) +
                                       VIRTQ_AVAIL_TOTAL_SIZE(ring_size)) +
                     VIRTQ_USED_TOTAL_SIZE(ring_size);
  ...
  queue->avail = (struct virtq_avail *)(((rt_ubase_t)pages) +
                                        VIRTQ_DESC_TOTAL_SIZE(ring_size));
...
void virtio_submit_chain(struct virtio_device *dev, rt_uint32_t queue_index,
                         rt_uint16_t desc_index) {
  rt_size_t ring_size;
  struct virtq *queue;
...
rt_uint16_t virtio_alloc_desc(struct virtio_device *dev,
                              rt_uint32_t queue_index) {
  int i;
  struct virtq *queue;
...
void virtio_free_desc(struct virtio_device *dev, rt_uint32_t queue_index,
                      rt_uint16_t desc_index) {
  struct virtq *queue;
...
rt_err_t virtio_alloc_desc_chain(struct virtio_device *dev,
                                 rt_uint32_t queue_index, rt_size_t count,
                                 rt_uint16_t *indexs) {
  int i, j;
...
void virtio_free_desc_chain(struct virtio_device *dev, rt_uint32_t queue_index,
                            rt_uint16_t desc_index) {
...
void virtio_fill_desc(struct virtio_device *dev, rt_uint32_t queue_index,
                      rt_uint16_t desc_index, rt_uint64_t addr, rt_uint32_t len,
                      rt_uint16_t flags, rt_uint16_t next) {
  struct virtq_desc *desc;
