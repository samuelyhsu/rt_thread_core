...
static rt_ubase_t _pixel_format_convert(rt_ubase_t format,
                                        rt_bool_t to_virtio_gpu_format) {
  rt_ubase_t ret = 0;
...
static void
virtio_gpu_ctrl_send_command(struct virtio_gpu_device *virtio_gpu_dev,
                             const void *cmd, rt_size_t cmd_len, void *res,
                             rt_size_t res_len) {
  rt_uint16_t idx[2];
  ...
  virtio_fill_desc(virtio_dev, VIRTIO_GPU_QUEUE_CTRL, idx[1],
                   VIRTIO_VA2PA(addr) + cmd_len, res_len, VIRTQ_DESC_F_WRITE,
                   0);
...
static void
virtio_gpu_cursor_send_command(struct virtio_gpu_device *virtio_gpu_dev,
  ...
  while ((id = virtio_alloc_desc(virtio_dev, VIRTIO_GPU_QUEUE_CURSOR)) ==
  ...
  virtio_fill_desc(virtio_dev, VIRTIO_GPU_QUEUE_CURSOR, id, VIRTIO_VA2PA(addr),
                   cmd_len, 0, 0);
...
static rt_err_t virtio_gpu_create_2d_resource(
    struct virtio_gpu_device *virtio_gpu_dev, enum virtio_gpu_formats format,
  ...
  *resource_id = ++virtio_gpu_dev->next_resource_id;
  ...
  virtio_gpu_ctrl_send_command(virtio_gpu_dev, &req, sizeof(req), &res,
                               sizeof(res));
...
static rt_err_t
virtio_gpu_unref_resource(struct virtio_gpu_device *virtio_gpu_dev,
...
static rt_err_t
virtio_gpu_attach_backing_resource(struct virtio_gpu_device *virtio_gpu_dev,
                                   rt_uint32_t resource_id, void *buffer,
                                   rt_size_t size) {
  struct virtio_gpu_ctrl_hdr res;
  struct {
...
static rt_err_t virtio_gpu_set_scanout(struct virtio_gpu_device *virtio_gpu_dev,
...
static rt_err_t
virtio_gpu_flush_resource(struct virtio_gpu_device *virtio_gpu_dev,
...
static rt_err_t
virtio_gpu_transfer_to_host_2d(struct virtio_gpu_device *virtio_gpu_dev,
                               rt_uint32_t resource_id, rt_uint32_t x,
                               rt_uint32_t y, rt_uint32_t width,
                               rt_uint32_t height, rt_uint32_t offset) {
  struct virtio_gpu_ctrl_hdr res;
...
static rt_err_t
virtio_gpu_gfx_flush_2d(struct virtio_gpu_device *virtio_gpu_dev,
                        rt_uint32_t resource_id, rt_uint32_t x, rt_uint32_t y,
                        rt_uint32_t width, rt_uint32_t height) {
  rt_err_t status = virtio_gpu_transfer_to_host_2d(virtio_gpu_dev, resource_id,
                                                   x, y, width, height, 0);

  if (status == RT_EOK) {
    status = virtio_gpu_flush_resource(virtio_gpu_dev, resource_id, x, y, width,
                                       height);
  }
...
static rt_err_t
virtio_gpu_update_cursor(struct virtio_gpu_device *virtio_gpu_dev,
...
static rt_err_t virtio_gpu_cursor_move(struct virtio_gpu_device *virtio_gpu_dev,
                                       rt_uint32_t scanout_id,
                                       rt_uint32_t resource_id, rt_uint32_t x,
                                       rt_uint32_t y) {
  struct virtio_gpu_update_cursor req;
...
static rt_err_t
virtio_gpu_cursor_set_img(struct virtio_gpu_device *virtio_gpu_dev, void *img) {
  rt_err_t status;
  ...
  status = virtio_gpu_attach_backing_resource(
      ...
      virtio_gpu_dev->cursor_img, VIRTIO_GPU_CURSOR_IMG_SIZE);

  if (status != RT_EOK) {
    return status;
  }

  status = virtio_gpu_transfer_to_host_2d(
      virtio_gpu_dev, virtio_gpu_dev->cursor_resource_id, 0, 0,
      VIRTIO_GPU_CURSOR_WIDTH, VIRTIO_GPU_CURSOR_HEIGHT, 0);

  return status;
}

static rt_err_t
virtio_gpu_get_display_info(struct virtio_gpu_device *virtio_gpu_dev) {
  int i;
  ...
  virtio_gpu_ctrl_send_command(virtio_gpu_dev, &req, sizeof(req), &info,
                               sizeof(info));
  ...
  for (i = 0; i < VIRTIO_GPU_MAX_SCANOUTS; ++i) {
    if (info.pmodes[i].enabled) {
      if (virtio_gpu_dev->pmode_id == VIRTIO_GPU_INVALID_PMODE_ID) {
        rt_memcpy(&virtio_gpu_dev->pmode, &info.pmodes[i],
                  sizeof(virtio_gpu_dev->pmode));
...
static rt_err_t virtio_gpu_init(rt_device_t dev) {
  ...
  if (virtio_gpu_dev->pmode_id != VIRTIO_GPU_INVALID_PMODE_ID &&
      _primary_virtio_gpu_dev == RT_NULL) {
...
static rt_ssize_t virtio_gpu_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                  rt_size_t size) {
  struct virtio_gpu_device *virtio_gpu_dev = (struct virtio_gpu_device *)dev;

  if (virtio_gpu_dev->framebuffer == RT_NULL ||
      pos + size >= virtio_gpu_dev->smem_len) {
    return 0;
  }
...
static rt_ssize_t virtio_gpu_write(rt_device_t dev, rt_off_t pos,
...
static rt_err_t virtio_gpu_control(rt_device_t dev, int cmd, void *args) {
  ...
  case RTGRAPHIC_CTRL_RECT_UPDATE: {
    ...
    status = virtio_gpu_gfx_flush_2d(
        virtio_gpu_dev, virtio_gpu_dev->display_resource_id, info->x, info->y,
  ...
  case VIRTIO_DEVICE_CTRL_GPU_CREATE_2D:
    ...
    if (virtio_gpu_dev->format == 0 ||
        virtio_gpu_dev->pmode_id == VIRTIO_GPU_INVALID_PMODE_ID) {
      status = -RT_ERROR;
      break;
    }

    status = virtio_gpu_create_2d_resource(
        virtio_gpu_dev, virtio_gpu_dev->format,
        &virtio_gpu_dev->display_resource_id, virtio_gpu_dev->pmode.r.width,
        virtio_gpu_dev->pmode.r.height);

    if (status != RT_EOK) {
      break;
    }

    virtio_gpu_dev->smem_len = virtio_gpu_dev->pmode.r.width *
                               virtio_gpu_dev->pmode.r.height *
                               VIRTIO_GPU_FORMAT_PIXEL;
    virtio_gpu_dev->smem_len =
        RT_ALIGN(virtio_gpu_dev->smem_len, VIRTIO_PAGE_SIZE);
    virtio_gpu_dev->framebuffer =
        rt_malloc_align(virtio_gpu_dev->smem_len, VIRTIO_PAGE_SIZE);

    if (virtio_gpu_dev->framebuffer == RT_NULL) {
      virtio_gpu_unref_resource(virtio_gpu_dev,
                                virtio_gpu_dev->display_resource_id);
    ...
    status = virtio_gpu_attach_backing_resource(
        virtio_gpu_dev, virtio_gpu_dev->display_resource_id,
        virtio_gpu_dev->framebuffer, virtio_gpu_dev->smem_len);

    if (status != RT_EOK) {
      break;
    }

    status = virtio_gpu_set_scanout(virtio_gpu_dev, virtio_gpu_dev->pmode_id,
                                    virtio_gpu_dev->display_resource_id,
                                    virtio_gpu_dev->pmode.r.width,
                                    virtio_gpu_dev->pmode.r.height);
  ...
  case VIRTIO_DEVICE_CTRL_CURSOR_SETUP:
    ...
    status = virtio_gpu_create_2d_resource(
        virtio_gpu_dev, virtio_gpu_dev->format,
        &virtio_gpu_dev->cursor_resource_id, VIRTIO_GPU_CURSOR_WIDTH,
    ...
    status = virtio_gpu_update_cursor(virtio_gpu_dev, virtio_gpu_dev->pmode_id,
                                      virtio_gpu_dev->cursor_resource_id,
                                      virtio_gpu_dev->cursor_x,
                                      virtio_gpu_dev->cursor_y);
  ...
  case VIRTIO_DEVICE_CTRL_CURSOR_MOVE:
    ...
    status = virtio_gpu_cursor_move(virtio_gpu_dev, virtio_gpu_dev->pmode_id,
                                    virtio_gpu_dev->cursor_resource_id,
                                    virtio_gpu_dev->cursor_x,
                                    virtio_gpu_dev->cursor_y);
...
const static struct rt_device_ops virtio_gpu_ops = {
    virtio_gpu_init, RT_NULL,          RT_NULL,
    virtio_gpu_read, virtio_gpu_write, virtio_gpu_control};
#endif
...
static void virtio_gpu_draw_hline(const char *pixel, int x1, int x2, int y) {
  ...
  if (virtio_gpu_dev == RT_NULL ||
      virtio_gpu_dev->pmode_id == VIRTIO_GPU_INVALID_PMODE_ID || x1 < 0 ||
      x2 < 0 || y < 0) {
    return;
  }
...
static void virtio_gpu_draw_vline(const char *pixel, int x, int y1, int y2) {
  ...
  if (virtio_gpu_dev == RT_NULL ||
      virtio_gpu_dev->pmode_id == VIRTIO_GPU_INVALID_PMODE_ID || x < 0 ||
      y1 < 0 || y2 < 0) {
    return;
  }
...
static void virtio_gpu_blit_line(const char *pixel, int x, int y,
                                 rt_size_t size) {
  int i;
  rt_uint8_t *fb;
...
static struct rt_device_graphic_ops virtio_gpu_graphic_ops = {
    virtio_gpu_set_pixel, virtio_gpu_get_pixel, virtio_gpu_draw_hline,
    virtio_gpu_draw_vline, virtio_gpu_blit_line};
...
static void virtio_gpu_isr(int irqno, void *param) {
  ...
  while (queue_cursor->used_idx != queue_cursor->used->idx) {
    rt_hw_dsb();
    id =
        queue_cursor->used->ring[queue_cursor->used_idx % queue_cursor->num].id;
...
rt_err_t rt_virtio_gpu_init(rt_ubase_t *mmio_base, rt_uint32_t irq) {
  ...
  virtio_dev->mmio_config->driver_features =
      virtio_dev->mmio_config->device_features &
      ~((1 << VIRTIO_F_RING_EVENT_IDX) | (1 << VIRTIO_F_RING_INDIRECT_DESC));
  ...
  if (virtio_queue_init(virtio_dev, VIRTIO_GPU_QUEUE_CTRL,
                        VIRTIO_GPU_QUEUE_SIZE) != RT_EOK) {
    goto _alloc_fail;
  }

  if (virtio_queue_init(virtio_dev, VIRTIO_GPU_QUEUE_CURSOR,
                        VIRTIO_GPU_QUEUE_SIZE) != RT_EOK) {
  ...
  return rt_device_register((rt_device_t)virtio_gpu_dev, dev_name,
                            RT_DEVICE_FLAG_RDWR);
