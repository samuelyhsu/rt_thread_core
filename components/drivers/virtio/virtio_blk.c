...
static void virtio_blk_rw(struct virtio_blk_device *virtio_blk_dev,
                          rt_off_t pos, void *buffer, rt_size_t count,
                          int flags) {
  rt_uint16_t idx[3];
  ...
  virtio_blk_dev->info[idx[0]].req.sector =
      pos * (virtio_blk_dev->config->blk_size / 512);

  flags = flags == VIRTIO_BLK_T_OUT ? 0 : VIRTQ_DESC_F_WRITE;

  virtio_fill_desc(virtio_dev, VIRTIO_BLK_QUEUE, idx[0],
                   VIRTIO_VA2PA(&virtio_blk_dev->info[idx[0]].req),
                   sizeof(struct virtio_blk_req), VIRTQ_DESC_F_NEXT, idx[1]);

  virtio_fill_desc(virtio_dev, VIRTIO_BLK_QUEUE, idx[1], VIRTIO_VA2PA(buffer),
                   size, flags | VIRTQ_DESC_F_NEXT, idx[2]);

  virtio_fill_desc(virtio_dev, VIRTIO_BLK_QUEUE, idx[2],
                   VIRTIO_VA2PA(&virtio_blk_dev->info[idx[0]].status),
                   sizeof(rt_uint8_t), VIRTQ_DESC_F_WRITE, 0);
...
static rt_ssize_t virtio_blk_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                  rt_size_t count) {
  virtio_blk_rw((struct virtio_blk_device *)dev, pos, buffer, count,
                VIRTIO_BLK_T_IN);

  return count;
}

static rt_ssize_t virtio_blk_write(rt_device_t dev, rt_off_t pos,
                                   const void *buffer, rt_size_t count) {
  virtio_blk_rw((struct virtio_blk_device *)dev, pos, (void *)buffer, count,
                VIRTIO_BLK_T_OUT);
...
const static struct rt_device_ops virtio_blk_ops = {
    ...
    virtio_blk_read, virtio_blk_write, virtio_blk_control};
#endif
...
rt_err_t rt_virtio_blk_init(rt_ubase_t *mmio_base, rt_uint32_t irq) {
  ...
  virtio_blk_dev->config =
      (struct virtio_blk_config *)virtio_dev->mmio_config->config;
  ...
  virtio_dev->mmio_config->driver_features =
      ...
      ~((1 << VIRTIO_BLK_F_RO) | (1 << VIRTIO_BLK_F_MQ) |
        (1 << VIRTIO_BLK_F_SCSI) | (1 << VIRTIO_BLK_F_CONFIG_WCE) |
        (1 << VIRTIO_F_ANY_LAYOUT) | (1 << VIRTIO_F_RING_EVENT_IDX) |
  ...
  return rt_device_register((rt_device_t)virtio_blk_dev, dev_name,
