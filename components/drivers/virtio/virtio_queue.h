...
#define VIRTQ_DESC_F_WRITE                                                     \
  2 /* This marks a buffer as write-only (otherwise read-only). */
#define VIRTQ_DESC_F_INDIRECT                                                  \
  4 /* This means the buffer contains a list of buffer descriptors. */
...
struct virtq_avail {
  ...
  rt_uint16_t idx;   /* Where the driver would put the next descriptor entry in
...
struct virtq_used_elem {
  ...
  rt_uint32_t
      len; /* Total length of the descriptor chain which was written to. */
};
...
#define VIRTQ_DESC_TOTAL_SIZE(ring_size)                                       \
  (sizeof(struct virtq_desc) * (ring_size))
/* flags, idx, used_event + ring * ring_size */
#define VIRTQ_AVAIL_TOTAL_SIZE(ring_size)                                      \
  (sizeof(rt_uint16_t) * 3 + sizeof(rt_uint16_t) * (ring_size))
/* flags, idx, avail_event + ring * ring_size */
#define VIRTQ_USED_TOTAL_SIZE(ring_size)                                       \
  (sizeof(rt_uint16_t) * 3 + sizeof(struct virtq_used_elem) * (ring_size))
