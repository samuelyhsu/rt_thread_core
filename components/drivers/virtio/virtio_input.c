...
static rt_ssize_t
virtio_input_cfg_select(struct virtio_input_device *virtio_input_dev,
...
static void virtio_input_cfg_bits(struct virtio_input_device *virtio_input_dev,
                                  rt_uint8_t select, rt_uint8_t subsel,
                                  rt_ubase_t *bits, rt_uint32_t bitcount) {
  int i;
  rt_uint32_t bit;
  rt_uint8_t bytes;
...
static rt_err_t virtio_input_init(rt_device_t dev) {
  ...
  virtio_input_cfg_bits(virtio_input_dev, VIRTIO_INPUT_CFG_EV_BITS, EV_KEY,
                        virtio_input_dev->key_bit, KEY_CNT);
  virtio_input_cfg_bits(virtio_input_dev, VIRTIO_INPUT_CFG_EV_BITS, EV_REL,
                        virtio_input_dev->rel_bit, REL_CNT);
  virtio_input_cfg_bits(virtio_input_dev, VIRTIO_INPUT_CFG_EV_BITS, EV_ABS,
                        virtio_input_dev->abs_bit, ABS_CNT);

  queue_event = &virtio_dev->queues[VIRTIO_INPUT_QUEUE_EVENT];
  queue_status = &virtio_dev->queues[VIRTIO_INPUT_QUEUE_STATUS];

  virtio_alloc_desc_chain(virtio_dev, VIRTIO_INPUT_QUEUE_EVENT,
                          queue_event->num, idx);
  virtio_alloc_desc_chain(virtio_dev, VIRTIO_INPUT_QUEUE_STATUS,
                          queue_status->num, idx);
  ...
  for (i = 0; i < queue_event->num; ++i) {
    ...
    virtio_fill_desc(virtio_dev, VIRTIO_INPUT_QUEUE_EVENT, id,
                     VIRTIO_VA2PA(addr), sizeof(struct virtio_input_event),
...
static rt_ssize_t virtio_input_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                    rt_size_t size) {
  struct virtio_input_device *virtio_input_dev =
      (struct virtio_input_device *)dev;
  ...
  if (buffer == RT_NULL ||
      pos + size >=
          virtio_input_dev->virtio_dev.queues[VIRTIO_INPUT_QUEUE_EVENT].num) {
    return 0;
  }
...
static rt_ssize_t virtio_input_write(rt_device_t dev, rt_off_t pos,
...
static rt_err_t virtio_input_control(rt_device_t dev, int cmd, void *args) {
  ...
  case VIRTIO_DEVICE_CTRL_INPUT_GET_ABS_X_INFO:
    ...
    virtio_input_cfg_select(virtio_input_dev, VIRTIO_INPUT_CFG_ABS_INFO,
  ...
  case VIRTIO_DEVICE_CTRL_INPUT_SET_STATUS: {
    ...
    virtio_fill_desc(virtio_dev, VIRTIO_INPUT_QUEUE_STATUS, id,
                     VIRTIO_VA2PA(addr), sizeof(struct virtio_input_event), 0,
                     0);
  ...
  case VIRTIO_DEVICE_CTRL_INPUT_GET_KEY_BIT:
    ...
    rt_memcpy(args, virtio_input_dev->key_bit,
              sizeof(virtio_input_dev->key_bit));

    break;
  case VIRTIO_DEVICE_CTRL_INPUT_GET_REL_BIT:

    rt_memcpy(args, virtio_input_dev->rel_bit,
              sizeof(virtio_input_dev->rel_bit));

    break;
  case VIRTIO_DEVICE_CTRL_INPUT_GET_ABS_BIT:

    rt_memcpy(args, virtio_input_dev->abs_bit,
              sizeof(virtio_input_dev->abs_bit));
...
const static struct rt_device_ops virtio_input_ops = {virtio_input_init,
                                                      RT_NULL,
                                                      RT_NULL,
                                                      virtio_input_read,
                                                      virtio_input_write,
...
static void virtio_input_isr(int irqno, void *param) {
  struct virtio_input_device *virtio_input_dev =
      (struct virtio_input_device *)param;
  struct virtio_device *virtio_dev = &virtio_input_dev->virtio_dev;
  ...
  while (event_queue->used_idx != event_queue->used->idx) {
    rt_uint16_t id =
        event_queue->used->ring[event_queue->used_idx % event_queue->num].id;
    rt_uint32_t len =
        event_queue->used->ring[event_queue->used_idx % event_queue->num].len;

    if (len == sizeof(struct virtio_input_event)) {
      struct virtio_input_event *recv_events =
          &virtio_input_dev->recv_events[id];
      struct virtio_input_event *bcst_events =
          &virtio_input_dev->bcst_events[id];
...
rt_err_t rt_virtio_input_init(rt_ubase_t *mmio_base, rt_uint32_t irq) {
  ...
  virtio_input_dev->config =
      (struct virtio_input_config *)virtio_dev->mmio_config->config;
  ...
  if (virtio_queue_init(virtio_dev, VIRTIO_INPUT_QUEUE_EVENT,
                        VIRTIO_INPUT_EVENT_QUEUE_SIZE) != RT_EOK) {
    goto _alloc_fail;
  }

  if (virtio_queue_init(virtio_dev, VIRTIO_INPUT_QUEUE_STATUS,
                        VIRTIO_INPUT_STATUS_QUEUE_SIZE) != RT_EOK) {
