...
static rt_err_t virtio_net_tx(rt_device_t dev, struct pbuf *p) {
  ...
  virtio_fill_desc(virtio_dev, VIRTIO_NET_QUEUE_TX, id,
                   VIRTIO_VA2PA(&virtio_net_dev->info[id].hdr),
                   VIRTIO_NET_HDR_SIZE, VIRTQ_DESC_F_NEXT, id + 1);

  virtio_fill_desc(virtio_dev, VIRTIO_NET_QUEUE_TX, id + 1,
                   VIRTIO_VA2PA(virtio_net_dev->info[id].rx_buffer), p->tot_len,
                   0, 0);
...
static struct pbuf *virtio_net_rx(rt_device_t dev) {
  ...
  while (queue_rx->used_idx != queue_rx->used->idx) {
    ...
    rt_base_t level = rt_spin_lock_irqsave(&virtio_dev->spinlock);
#endif
    id = (queue_rx->used->ring[queue_rx->used_idx % queue_rx->num].id + 1) %
         queue_rx->num;
...
static rt_err_t virtio_net_init(rt_device_t dev) {
  ...
  for (i = 0; i < queue_rx->num; ++i) {
    ...
    virtio_fill_desc(virtio_dev, VIRTIO_NET_QUEUE_RX, id, VIRTIO_VA2PA(addr),
                     VIRTIO_NET_HDR_SIZE,
                     VIRTQ_DESC_F_NEXT | VIRTQ_DESC_F_WRITE, id + 1);

    /* Descriptor for data */
    virtio_fill_desc(virtio_dev, VIRTIO_NET_QUEUE_RX, id + 1,
                     VIRTIO_VA2PA(addr) + VIRTIO_NET_HDR_SIZE, VIRTIO_NET_MSS,
...
static rt_err_t virtio_net_control(rt_device_t dev, int cmd, void *args) {
  ...
  case NIOCTL_GADDR:
    ...
    rt_memcpy(args, virtio_net_dev->config->mac,
              sizeof(virtio_net_dev->config->mac));
    break;
  default:
    status = -RT_EINVAL;
    break;
...
const static struct rt_device_ops virtio_net_ops = {
    virtio_net_init, RT_NULL, RT_NULL, RT_NULL, RT_NULL, virtio_net_control};
#endif
...
rt_err_t rt_virtio_net_init(rt_ubase_t *mmio_base, rt_uint32_t irq) {
  ...
  virtio_net_dev->config =
      (struct virtio_net_config *)virtio_dev->mmio_config->config;
  ...
  virtio_dev->mmio_config->driver_features =
      ...
      ~((1 << VIRTIO_NET_F_CTRL_VQ) | (1 << VIRTIO_F_RING_EVENT_IDX));
  ...
  if (virtio_queue_init(virtio_dev, VIRTIO_NET_QUEUE_RX,
                        VIRTIO_NET_RTX_QUEUE_SIZE) != RT_EOK) {
    goto _alloc_fail;
  }

  if (virtio_queue_init(virtio_dev, VIRTIO_NET_QUEUE_TX,
                        VIRTIO_NET_RTX_QUEUE_SIZE) != RT_EOK) {
