...
/*
 * Introduction:
 * The rbb is the ring buffer which is composed with many blocks. It is
 * different from the ring buffer. The ring buffer is only composed with chars.
 * The rbb put and get supported zero copies. So the rbb is very suitable for
 * put block and get block by a certain order. Such as DMA block transmit,
...
/**
 * Rbb block queue: the blocks (from block1->buf to blockn->buf) memory which on
 * this queue is continuous.
 */
...
/* rbb block queue API */
rt_size_t rt_rbb_blk_queue_get(rt_rbb_t rbb, rt_size_t queue_data_len,
