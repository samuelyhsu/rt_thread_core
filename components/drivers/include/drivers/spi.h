...
#define RT_SPI_MODE_MASK                                                       \
  (RT_SPI_CPHA | RT_SPI_CPOL | RT_SPI_MSB | RT_SPI_SLAVE | RT_SPI_CS_HIGH |    \
   RT_SPI_NO_CS | RT_SPI_3WIRE | RT_SPI_READY)
 ...
 */
struct rt_spi_ops {
  rt_err_t (*configure)(struct rt_spi_device *device,
                        struct rt_spi_configuration *configuration);
  rt_uint32_t (*xfer)(struct rt_spi_device *device,
...
struct rt_qspi_message {
  ...
  /* number of lines in qspi data stage, the other configuration items are in
   * parent */
 ...
 * @param device the SPI device attached to SPI bus
 *
 * @return RT_EOK on release SPI bus successfully. others on taken SPI bus
 * failed.
 */
...
rt_err_t rt_spi_send_then_send(struct rt_spi_device *device,
                               const void *send_buf1, rt_size_t send_length1,
                               const void *send_buf2, rt_size_t send_length2);

rt_uint16_t rt_spi_sendrecv16(struct rt_spi_device *device, rt_uint16_t data);
...
rt_inline rt_size_t rt_spi_recv(struct rt_spi_device *device, void *recv_buf,
                                rt_size_t length) {
 ...
 * @return the actual length of transmitted.
 */
rt_err_t rt_qspi_bus_register(struct rt_spi_bus *bus, const char *name,
                              const struct rt_spi_ops *ops);
 ...
 * @return the status of transmit.
 */
rt_err_t rt_qspi_send(struct rt_qspi_device *device, const void *send_buf,
                      rt_size_t length);
