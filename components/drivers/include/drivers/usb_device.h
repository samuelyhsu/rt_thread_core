...
struct udcd_ops {
  ...
  rt_size_t (*ep_read_prepare)(rt_uint8_t address, void *buffer,
                               rt_size_t size);
...
rt_inline rt_size_t dcd_ep_read_prepare(udcd_t dcd, rt_uint8_t address,
...
rt_inline void usbd_os_proerty_descriptor_send(ufunction_t func, ureq_t setup,
  ...
  if (data == RT_NULL) {
    ...
    pdata = data;
    rt_memcpy((void *)pdata, (void *)&header,
              sizeof(struct usb_os_property_header));
