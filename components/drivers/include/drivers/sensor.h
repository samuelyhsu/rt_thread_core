...
#define rt_sensor_get_ts()                                                     \
  time(RT_NULL) /* API for the sensor to get the timestamp */
#else
#define rt_sensor_get_ts()                                                     \
  rt_tick_get() /* API for the sensor to get the timestamp */
#endif

#define RT_DEVICE_FLAG_FIFO_RX                                                 \
  0x200 /* Flag to use when the sensor is open by fifo mode */

#define RT_SENSOR_MODULE_MAX                                                   \
  (3) /* The maximum number of members of a sensor module */
...
#define RT_SENSOR_UNIT_PERCENTAGE                                              \
  (10) /* Relative Humidity       unit: percentage */
#define RT_SENSOR_UNIT_PERCENTAGE_STR "%"
#define RT_SENSOR_UNIT_PERMILLAGE                                              \
  (11) /* Relative Humidity       unit: permillage */
#define RT_SENSOR_UNIT_PERMILLAGE_STR "‰"
#define RT_SENSOR_UNIT_CELSIUS                                                 \
  (12) /* Temperature             unit: Celsius ℃ */
#define RT_SENSOR_UNIT_CELSIUS_STR "℃"
#define RT_SENSOR_UNIT_FAHRENHEIT                                              \
  (13) /* Temperature             unit: Fahrenheit ℉ */
#define RT_SENSOR_UNIT_FAHRENHEIT_STR "℉"
#define RT_SENSOR_UNIT_KELVIN                                                  \
  (14) /* Temperature             unit: Kelvin K   */
...
#define RT_SENSOR_MODE_GET_ACCURACY(mode)                                      \
  (rt_uint8_t)((mode >> RT_SENSOR_MODE_ACCURACY_BIT_OFFSET) & 0x0F)
#define RT_SENSOR_MODE_GET_POWER(mode)                                         \
  (rt_uint8_t)((mode >> RT_SENSOR_MODE_POWER_BIT_OFFSET) & 0x0F)
#define RT_SENSOR_MODE_GET_FETCH(mode)                                         \
  (rt_uint8_t)((mode >> RT_SENSOR_MODE_FETCH_BIT_OFFSET) & 0x0F)

#define RT_SENSOR_MODE_CLEAR_ACCURACY(mode)                                    \
  (mode &=                                                                     \
   ((rt_uint16_t) ~((rt_uint16_t)0x0F << RT_SENSOR_MODE_ACCURACY_BIT_OFFSET)))
#define RT_SENSOR_MODE_CLEAR_POWER(mode)                                       \
  (mode &=                                                                     \
   ((rt_uint16_t) ~((rt_uint16_t)0x0F << RT_SENSOR_MODE_POWER_BIT_OFFSET)))
#define RT_SENSOR_MODE_CLEAR_FETCH(mode)                                       \
  (mode &=                                                                     \
   ((rt_uint16_t) ~((rt_uint16_t)0x0F << RT_SENSOR_MODE_FETCH_BIT_OFFSET)))

#define RT_SENSOR_MODE_SET_ACCURACY(mode, accuracy_mode)                       \
  RT_SENSOR_MODE_CLEAR_ACCURACY(mode);                                         \
  (mode |= (accuracy_mode << RT_SENSOR_MODE_ACCURACY_BIT_OFFSET))
#define RT_SENSOR_MODE_SET_POWER(mode, power_mode)                             \
  RT_SENSOR_MODE_CLEAR_POWER(mode);                                            \
  (mode |= (power_mode << RT_SENSOR_MODE_POWER_BIT_OFFSET))
#define RT_SENSOR_MODE_SET_FETCH(mode, fetch_mode)                             \
  RT_SENSOR_MODE_CLEAR_FETCH(mode);                                            \
  (mode |= (fetch_mode << RT_SENSOR_MODE_FETCH_BIT_OFFSET))
...
#define RT_SENSOR_MODE_FETCH_INT                                               \
  (1) /* TODO: One shot interrupt only read a data */
#define RT_SENSOR_MODE_FETCH_INT_STR "Interrupt Mode"
#define RT_SENSOR_MODE_FETCH_FIFO                                              \
  (2) /* TODO: One shot interrupt read all fifo data */
#define RT_SENSOR_MODE_FETCH_FIFO_STR "FIFO Mode"

/* Sensor control cmd types */
#define RT_SENSOR_CTRL_GET_ID                                                  \
  (RT_DEVICE_CTRL_BASE(Sensor) + 0) /* Get device id */
#define RT_SENSOR_CTRL_SELF_TEST                                               \
  (RT_DEVICE_CTRL_BASE(Sensor) + 1) /* Take a self test */
#define RT_SENSOR_CTRL_SOFT_RESET                                              \
  (RT_DEVICE_CTRL_BASE(Sensor) + 2) /* soft reset sensor */
#define RT_SENSOR_CTRL_SET_FETCH_MODE                                          \
  (RT_DEVICE_CTRL_BASE(Sensor) + 3) /* set fetch data mode */
#define RT_SENSOR_CTRL_SET_POWER_MODE                                          \
  (RT_DEVICE_CTRL_BASE(Sensor) + 4) /* set power mode */
#define RT_SENSOR_CTRL_SET_ACCURACY_MODE                                       \
  (RT_DEVICE_CTRL_BASE(Sensor) + 5) /* set accuracy mode */

#define RT_SENSOR_CTRL_USER_CMD_START                                          \
  0x100 /* User commands should be greater than 0x100 */
...
struct rt_sensor_scale {
  rt_sensor_float_t
      range_max; /* maximum range of this sensor's value. unit is 'unit' */
  rt_sensor_float_t
      range_min; /* minimum range of this sensor's value. unit is 'unit' */
};
...
struct rt_sensor_info {
  ...
  rt_uint8_t fifo_max;
  rt_sensor_float_t acquire_min; /* minimum acquirement period, unit:ms. zero =
                                    not a constant rate */
...
struct rt_sensor_intf {
  ...
  void *arg; /* Interface argument for the sensor. ex. i2c addr,spi cs,control
                I/O */
};

struct rt_sensor_config {
  struct rt_sensor_intf intf;        /* sensor interface config */
  struct rt_device_pin_mode irq_pin; /* Interrupt pin, The purpose of this pin
                                        is to notification read data */
};
...
struct rt_sensor_device {
  ...
  rt_err_t (*irq_handle)(
      rt_sensor_t sensor); /* Called when an interrupt is generated, registered
                              by the driver */
};
...
struct rt_sensor_module {
  ...
  rt_sensor_t
      sen[RT_SENSOR_MODULE_MAX]; /* The module contains a list of sensors */
...
struct rt_sensor_ops {
  rt_ssize_t (*fetch_data)(rt_sensor_t sensor, rt_sensor_data_t buf,
                           rt_size_t len);
