...
struct uhcd_ops {
  ...
  int (*pipe_xfer)(upipe_t pipe, rt_uint8_t token, void *buffer, int nbytes,
                   int timeout);
...
rt_err_t rt_usbh_get_descriptor(struct uinstance *device, rt_uint8_t type,
...
rt_err_t rt_usbh_clear_feature(struct uinstance *device, int endpoint,
                               int feature);
...
rt_inline upipe_t rt_usb_instance_find_pipe(uinst_t inst,
  ...
  for (l = inst->pipe.next; l != &inst->pipe; l = l->next) {
    if (rt_list_entry(l, struct upipe, list)->ep.bEndpointAddress ==
        ep_address) {
...
int rt_usb_hcd_pipe_xfer(uhcd_t hcd, upipe_t pipe, void *buffer, int nbytes,
                         int timeout);
rt_inline int rt_usb_hcd_setup_xfer(uhcd_t hcd, upipe_t pipe, ureq_t setup,
                                    int timeout) {
