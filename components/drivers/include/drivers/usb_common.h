...
/*
    The HcRhPortStatus[1:NDP] register is used to control and report port events
   on a per-port basis. NumberDownstreamPorts represents the number of
   HcRhPortStatus registers that are implemented in hardware.  The lower word is
   used to reflect the port status, whereas the upper word reflects the status
   change bits.  Some status bits are implemented with special write behavior
    (see below).  If a transaction (token through handshake) is in progress when
   a write to change port status occurs, the resulting port status change must
   be postponed until the transaction completes.  Reserved bits should always be
   written '0'.
*/
#define PORT_CCS                                                               \
  0x00000001UL /* R:CurrentConnectStatus - W:ClearPortEnable    */
#define PORT_PES                                                               \
  0x00000002UL /* R:PortEnableStatus - W:SetPortEnable             */
#define PORT_PSS                                                               \
  0x00000004UL /* R:PortSuspendStatus - W:SetPortSuspend        */
#define PORT_POCI                                                              \
  0x00000008UL /* R:PortOverCurrentIndicator - W:ClearSuspendStatus    */
#define PORT_PRS                                                               \
  0x00000010UL /* R:PortResetStatus - W: SetPortReset            */
#define PORT_PPS                                                               \
  0x00000100UL /* R:PortPowerStatus - W: SetPortPower            */
#define PORT_LSDA                                                              \
  0x00000200UL /* R:LowSpeedDeviceAttached - W:ClearPortPower    */
...
//  2       A NULL-terminated Unicode String that includes environment variables
//  (REG_EXPAND_SZ) 3       Free-form binary (REG_BINARY) 4       A
//  little-endian 32-bit integer (REG_DWORD_LITTLE_ENDIAN) 5       A big-endian
//  32-bit integer (REG_DWORD_BIG_ENDIAN) 6       A NULL-terminated Unicode
//  string that contains a symbolic link (REG_LINK) 7       Multiple
//  NULL-terminated Unicode strings (REG_MULTI_SZ)
