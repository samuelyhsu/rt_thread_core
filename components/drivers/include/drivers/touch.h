...
#define rt_touch_get_ts()                                                      \
  time(RT_NULL) /* API for the touch to get the timestamp */
#else
#define rt_touch_get_ts()                                                      \
...
#define RT_TOUCH_CTRL_GET_ID                                                   \
  (RT_DEVICE_CTRL_BASE(Touch) + 0) /* Get device id */
#define RT_TOUCH_CTRL_GET_INFO                                                 \
  (RT_DEVICE_CTRL_BASE(Touch) + 1) /* Get touch info */
#define RT_TOUCH_CTRL_SET_MODE                                                 \
  (RT_DEVICE_CTRL_BASE(Touch) +                                                \
   2) /* Set touch's work mode. ex. RT_TOUCH_MODE_POLLING,RT_TOUCH_MODE_INT */
#define RT_TOUCH_CTRL_SET_X_RANGE                                              \
  (RT_DEVICE_CTRL_BASE(Touch) + 3) /* Set x coordinate range */
#define RT_TOUCH_CTRL_SET_Y_RANGE                                              \
  (RT_DEVICE_CTRL_BASE(Touch) + 4) /* Set y coordinate range */
#define RT_TOUCH_CTRL_SET_X_TO_Y                                               \
  (RT_DEVICE_CTRL_BASE(Touch) + 5) /* Set X Y coordinate exchange */
#define RT_TOUCH_CTRL_DISABLE_INT                                              \
  (RT_DEVICE_CTRL_BASE(Touch) + 6) /* Disable interrupt */
#define RT_TOUCH_CTRL_ENABLE_INT                                               \
  (RT_DEVICE_CTRL_BASE(Touch) + 7) /* Enable interrupt */
#define RT_TOUCH_CTRL_POWER_ON                                                 \
  (RT_DEVICE_CTRL_BASE(Touch) + 8) /* Touch Power On */
#define RT_TOUCH_CTRL_POWER_OFF                                                \
  (RT_DEVICE_CTRL_BASE(Touch) + 9) /* Touch Power Off */
#define RT_TOUCH_CTRL_GET_STATUS                                               \
  (RT_DEVICE_CTRL_BASE(Touch) + 10) /* Get Touch Power Status */
...
struct rt_touch_device {
  ...
  rt_err_t (*irq_handle)(
      rt_touch_t touch); /* Called when an interrupt is generated, registered by
                            the driver */
};
...
struct rt_touch_ops {
  rt_size_t (*touch_readpoint)(struct rt_touch_device *touch, void *buf,
                               rt_size_t touch_num);
  rt_err_t (*touch_control)(struct rt_touch_device *touch, int cmd, void *arg);
};

int rt_hw_touch_register(rt_touch_t touch, const char *name, rt_uint32_t flag,
                         void *data);

/* if you doesn't use pin device. you must call this function in your touch irq
 * callback */
