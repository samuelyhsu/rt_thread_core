...
struct rt_can_filter_item {
  ...
  rt_uint32_t mode : 1;
  rt_uint32_t mask;
  rt_int32_t hdr_bank; /*Should be defined as:rx.FilterBank,which should be
                          changed to rt_int32_t hdr_bank*/
  rt_uint32_t
      rxfifo; /*Add a configuration item that CAN_RX_FIFO0/CAN_RX_FIFO1*/
...
struct can_configure {
  ...
  rt_uint32_t
      use_bit_timing : 8; /* Use the bit timing for CAN timing configuration */
  ...
  struct rt_can_bit_timing
      can_timing; /* CAN bit-timing /CANFD bit-timing for arbitration phase */
...
struct rt_can_msg {
  ...
  rt_uint32_t len : 8;
  rt_uint32_t priv : 8;
  rt_int32_t hdr_index : 8; /*Should be defined as:rx.FilterMatchIndex,which
                               should be changed to rt_int32_t hdr_index : 8*/
#ifdef RT_CAN_USING_CANFD
