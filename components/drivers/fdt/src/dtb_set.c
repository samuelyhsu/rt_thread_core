...
size_t dtb_node_set_linux_cmdline(void *fdt, char *cmdline) {
  ...
  if (chosen_offset >= 0 || chosen_offset == -FDT_ERR_EXISTS) {
    if (fdt_setprop(fdt, chosen_offset, "bootargs", cmdline, cmdline_size) <
        0) {
...
size_t dtb_node_set_linux_initrd(void *fdt, uint64_t initrd_addr,
  ...
  if (chosen_offset >= 0 || chosen_offset == -FDT_ERR_EXISTS) {
    ...
    if (IN_64BITS_MODE) {
      ...
      fdt_setprop_u64(fdt, chosen_offset, "linux,initrd-end",
                      initrd_addr + initrd_size);
    } else {
      fdt_setprop_u32(fdt, chosen_offset, "linux,initrd-start", initrd_addr);
      fdt_setprop_u32(fdt, chosen_offset, "linux,initrd-end",
                      initrd_addr + initrd_size);
    }
  }
