...
rt_bool_t dtb_node_read_bool(const struct dtb_node *node,
                             const char *propname) {
  const void *prop;
...
const void *dtb_node_read_prop(const struct dtb_node *node,
  ...
  struct dtb_property *prop =
      dtb_node_get_dtb_node_property(node, propname, &len);
...
const struct dtb_node *dtb_node_find_subnode(const struct dtb_node *node,
  ...
  debug("%s\n",
        dtb_node_valid(subnode) ? dtb_node_get_name(subnode) : "<none>");
...
int dtb_node_read_size(const struct dtb_node *node, const char *propname) {
  struct dtb_property *prop =
      dtb_node_get_dtb_node_property(node, propname, NULL);
...
size_t dtb_node_get_addr_index(const struct dtb_node *node, int index) {
  ...
  prop_val = dtb_node_get_address(node, index, (uint64_t *)&size, &flags);
  if (!prop_val)
    return -1;
...
size_t dtb_node_get_addr_size(const struct dtb_node *node, const char *property,
  ...
  int psize;
  const uint32_t *prop =
      dtb_node_get_dtb_node_property_value(node, property, &psize);
...
const uint8_t *dtb_node_read_u8_array_ptr(const struct dtb_node *node,
  ...
  int psize;
  const uint32_t *prop =
      dtb_node_get_dtb_node_property_value(node, propname, &psize);
...
/**
 * dtb_node_irq_find_parent - Given a device node, find its interrupt parent
 * node
...
int dtb_node_irq_get(struct dtb_node *dev, int index) {
  ...
  for (i = 0; i < intsize; i++) {
    res = dtb_node_read_u32_index(dev, "interrupts", (index * 3 + i),
                                  out_irq.args + i);
    if (res)
      goto out;
  }
...
/**
 * dtb_node_irq_get_byname - Decode a node's IRQ and return it as a Linux IRQ
 * number
