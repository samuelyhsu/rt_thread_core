...
int dtb_node_get_exec_status() { return fdt_exec_status; }

static int _dtb_node_get_dtb_properties_list(struct dtb_property *dtb_property,
                                             off_t node_off) {
  ...
  for (;;) {
    fdt_property = (struct fdt_property *)fdt_get_property_by_offset(
        current_fdt, property_off, &dtb_property->size);
    if (fdt_property != NULL) {
      dtb_property->name =
          fdt_string(current_fdt, fdt32_to_cpu(fdt_property->nameoff));
    ...
    if (property_off >= 0) {
      dtb_property->next =
          (struct dtb_property *)malloc(sizeof(struct dtb_property));
...
static int _dtb_node_get_dtb_nodes_list(struct dtb_node *dtb_node_head,
    ...
    for (;;) {
      ...
      if (paths_buf.cur + pathname_sz + node_name_sz + 2 < paths_buf.end) {
        ...
        *paths_buf.cur++ = '\0';
        ...
        dtb_node->path = NULL;
        rt_kprintf("\033[31m\rERROR: `FDT_DTB_ALL_NODES_PATH_SIZE' = %d bytes "
                   "is configured too low.\033[0m\n",
      ...
      fdt_exec_status =
          _dtb_node_get_dtb_properties_list(dtb_node->properties, node_off);
      ...
      fdt_exec_status = _dtb_node_get_dtb_nodes_list(dtb_node, dtb_node->child,
                                                     dtb_node->path);
      if (fdt_exec_status == FDT_RET_GET_EMPTY) {
        free(dtb_node->child);
        dtb_node->child = NULL;
...
struct dtb_node *dtb_node_get_dtb_list(void *fdt) {
  ...
  if ((dtb_node_head = (struct dtb_node *)malloc(sizeof(struct dtb_node))) ==
      NULL) {
  ...
  if ((dtb_node_head->header = malloc(sizeof(struct dtb_header))) == NULL) {
    ...
    ((struct dtb_header *)dtb_node_head->header)->memreserve_sz =
        fdt_num_mem_rsv(fdt);
    ...
    if (dtb_node_head->header->memreserve_sz > 0) {
      ...
      struct fdt_reserve_entry *rsvmap =
          (struct fdt_reserve_entry *)((char *)fdt + off_mem_rsvmap);

      ((struct dtb_header *)dtb_node_head->header)->memreserve =
          malloc(sizeof(struct dtb_memreserve) * memreserve_sz);
      if (dtb_node_head->header->memreserve == NULL) {
        fdt_exec_status = FDT_RET_NO_MEMORY;
        goto fail;
      }
      for (i = 0; i < memreserve_sz; ++i) {
        ((struct dtb_header *)dtb_node_head->header)->memreserve[i].address =
            fdt64_to_cpu(rsvmap[i].address);
        ((struct dtb_header *)dtb_node_head->header)->memreserve[i].size =
            fdt64_to_cpu(rsvmap[i].size);
      }
    } else {
  ...
  *paths_buf.cur++ = '\0';
  ...
  if ((fdt_exec_status = _dtb_node_get_dtb_properties_list(
           dtb_node_head->properties, root_off)) != FDT_RET_GET_OK) {
    goto fail;
  }

  if ((fdt_exec_status = _dtb_node_get_dtb_nodes_list(
           dtb_node_head, dtb_node_head->child, dtb_node_head->path)) !=
      FDT_RET_GET_OK) {
    goto fail;
  }
...
void dtb_node_free_dtb_list(struct dtb_node *dtb_node_head) {
  ...
  if (dtb_node_head->parent == NULL ||
      (dtb_node_head->path != NULL && !strcmp(dtb_node_head->path, "/"))) {
...
void dtb_node_get_dts_dump(struct dtb_node *dtb_node_head) {
  if (dtb_node_head != NULL) {
    ...
    rt_kputs("/dts-v1/;\n");
    while (i-- > 0) {
      rt_kprintf("\n/memreserve/\t0x%lx 0x%zx;",
                 dtb_node_head->header->memreserve[i].address,
                 dtb_node_head->header->memreserve[i].size);
    }
...
static void
_dtb_node_get_enum_dtb_node(struct dtb_node *dtb_node,
...
struct dtb_node *dtb_node_get_dtb_node_by_name_DFS(struct dtb_node *dtb_node,
  ...
  while (dtb_node != NULL) {
    ...
    dtb_node_child =
        dtb_node_get_dtb_node_by_name_DFS(dtb_node->child, nodename);
...
struct dtb_node *dtb_node_get_dtb_node_by_name_BFS(struct dtb_node *dtb_node,
  ...
  if (dtb_node != NULL) {
    ...
    while (dtb_node != NULL) {
      dtb_node_child =
          dtb_node_get_dtb_node_by_name_BFS(dtb_node->child, nodename);
...
struct dtb_node *dtb_node_get_dtb_node_by_path(struct dtb_node *dtb_node,
   ...
   */
  node_name =
      &((char *)pathname)[(pathname_sz - 1) - (&pathname_clone[i] - node_name)];
  free(pathname_clone);
...
struct dtb_node *dtb_node_get_dtb_node_by_phandle_DFS(struct dtb_node *dtb_node,
  ...
  while (dtb_node != NULL) {
    ...
    dtb_node_child =
        dtb_node_get_dtb_node_by_phandle_DFS(dtb_node->child, handle);
...
struct dtb_node *dtb_node_get_dtb_node_by_phandle_BFS(struct dtb_node *dtb_node,
  ...
  if (dtb_node != NULL) {
    ...
    while (dtb_node != NULL) {
      dtb_node_child =
          dtb_node_get_dtb_node_by_phandle_BFS(dtb_node->child, handle);
...
rt_bool_t dtb_node_get_dtb_node_status(const struct dtb_node *dtb_node) {
  if (dtb_node != NULL) {
    char *status =
        dtb_node_get_dtb_node_property_value(dtb_node, "status", NULL);
    if (status != NULL) {
      return (!strcmp(status, "okay") || !strcmp(status, "ok")) ? RT_TRUE
                                                                : RT_FALSE;
    }
...
uint8_t dtb_node_get_dtb_byte_value(void *value) { return *(uint8_t *)value; }
