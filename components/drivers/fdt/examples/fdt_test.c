...
int dtb_test() {
  ...
  if ((fdt = dtb_node_load_from_fs("vexpress-v2p-ca9.dtb")) != RT_NULL) {
    ...
    if (dtb_node_list != RT_NULL) {
      struct dtb_node *serial0 = dtb_node_get_dtb_node_by_path(
          dtb_node_list,
          "/smb@4000000/motherboard/iofpga@7,00000000/uart@9000");
      struct dtb_node *cpu =
          dtb_node_get_dtb_node_by_path(dtb_node_list, "/cpus");
      struct dtb_node *user1 = dtb_node_get_dtb_node_by_path(
          dtb_node_list, "/smb@4000000/motherboard/leds/user1");
      ...
      if (serial0 != RT_NULL) {
        ...
        rt_kputs("reg = <");
        for_each_property_cell(serial0, "reg", u32_value, u32_ptr,
                               property_size) {
        ...
        rt_kputs("compatible = ");
        for_each_property_string(serial0, "compatible", str_ptr,
                                 property_size) {
      ...
      if (user1 != RT_NULL) {
        ...
        rt_kprintf("\nname = %s, lable = %s\n", user1->name,
                   dtb_node_get_dtb_node_property(user1, "label", RT_NULL));

        for_each_node_sibling(user) {
          rt_kprintf("name = %s, lable = %s\n", user->name,
                     dtb_node_get_dtb_node_property(user, "label", RT_NULL));
        }
      }
    }
  ...
  if ((fdt = dtb_node_load_from_fs("bcm2711-rpi-4-b.dtb")) != RT_NULL) {
    ...
    if (dtb_node_list != RT_NULL) {
      ...
      bt_pins =
          dtb_node_get_dtb_node_by_phandle_DFS(dtb_node_list, bt_pins->handle);
      bt_pins =
          dtb_node_get_dtb_node_by_phandle_BFS(dtb_node_list, bt_pins->handle);
      ...
      if (memreserve_size > 0) {
        ...
        for (i = 0; i < memreserve_size; ++i) {
          rt_kputs("\n/memreserve/\t");
          if (IN_64BITS_MODE) {
            rt_kprintf("0x%016x 0x%016x;", memreserve[i].address,
                       memreserve[i].size);
          } else {
            rt_kprintf("0x%08x%08x 0x%08x%08x;", memreserve[i].address,
                       memreserve[i].size);
          }
        }
      }
      ...
      if (bt_pins != RT_NULL) {
        ...
        rt_kputs("brcm,pins = [");
        for_each_property_byte(bt_pins, "brcm,pins", u8_value, u8_ptr,
                               property_size) {
