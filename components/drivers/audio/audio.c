...
static rt_err_t _audio_send_replay_frame(struct rt_audio_device *audio) {
  ...
  if (rt_data_queue_peek(&audio->replay->queue, (const void **)&data,
                         &src_size) != RT_EOK) {
    /* ack stop event */
    ...
    while (index < dst_size) {
      result = rt_data_queue_peek(&audio->replay->queue, (const void **)&data,
                                  &src_size);
      if (result != RT_EOK) {
      ...
      remain_bytes =
          MIN((dst_size - index), (src_size - audio->replay->read_index));
      ...
      if (audio->replay->read_index == src_size) {
        /* free memory */
        audio->replay->read_index = 0;
        rt_data_queue_pop(&audio->replay->queue, (const void **)&data,
                          &src_size, RT_WAITING_NO);
        rt_mp_free(data);
  ...
  if (audio->ops->transmit != RT_NULL) {
    if (audio->ops->transmit(audio, &buf_info->buffer[position], RT_NULL,
                             dst_size) != dst_size)
      result = -RT_ERROR;
  }
...
static rt_err_t _audio_flush_replay_frame(struct rt_audio_device *audio) {
  ...
  if (audio->replay->write_index) {
    result = rt_data_queue_push(&audio->replay->queue,
                                ...
                                audio->replay->write_index, RT_WAITING_FOREVER);
...
static rt_err_t _audio_dev_init(struct rt_device *dev) {
  ...
  if (dev->flag & RT_DEVICE_FLAG_WRONLY) {
    struct rt_audio_replay *replay =
        (struct rt_audio_replay *)rt_malloc(sizeof(struct rt_audio_replay));
    ...
    replay->mp = rt_mp_create("adu_mp", RT_AUDIO_REPLAY_MP_BLOCK_COUNT,
    ...
    rt_data_queue_init(&replay->queue, CFG_AUDIO_REPLAY_QUEUE_COUNT, 0,
                       RT_NULL);
  ...
  if (dev->flag & RT_DEVICE_FLAG_RDONLY) {
    struct rt_audio_record *record =
        (struct rt_audio_record *)rt_malloc(sizeof(struct rt_audio_record));
    rt_uint8_t *buffer;
    ...
    rt_audio_pipe_init(
        &record->pipe, "record",
        (rt_int32_t)(RT_PIPE_FLAG_FORCE_WR | RT_PIPE_FLAG_BLOCK_RD), buffer,
...
static rt_ssize_t _audio_dev_read(struct rt_device *dev, rt_off_t pos,
...
static rt_ssize_t _audio_dev_write(struct rt_device *dev, rt_off_t pos,
  ...
  while (index < size) {
    ...
    if (audio->replay->write_index % block_size == 0) {
      audio->replay->write_data =
          rt_mp_alloc(audio->replay->mp, RT_WAITING_FOREVER);
      rt_memset(audio->replay->write_data, 0, block_size);
    }

    /* copy data to replay memory pool */
    remain_bytes =
        MIN((block_size - audio->replay->write_index), (size - index));
    rt_memcpy(&audio->replay->write_data[audio->replay->write_index],
              &ptr[index], remain_bytes);
    ...
    if (audio->replay->write_index == 0) {
      rt_data_queue_push(&audio->replay->queue, audio->replay->write_data,
                         block_size, RT_WAITING_FOREVER);
    }
  }
...
static rt_err_t _audio_dev_control(struct rt_device *dev, int cmd, void *args) {
  ...
  case AUDIO_CTL_GETCAPS: {
    ...
    LOG_D("AUDIO_CTL_GETCAPS: main_type = %d,sub_type = %d", caps->main_type,
          caps->sub_type);
  ...
  case AUDIO_CTL_CONFIGURE: {
    ...
    LOG_D("AUDIO_CTL_CONFIGURE: main_type = %d,sub_type = %d", caps->main_type,
          caps->sub_type);
...
const static struct rt_device_ops audio_ops = {
    _audio_dev_init, _audio_dev_open,  _audio_dev_close,
    _audio_dev_read, _audio_dev_write, _audio_dev_control};
#endif

rt_err_t rt_audio_register(struct rt_audio_device *audio, const char *name,
...
void rt_audio_rx_done(struct rt_audio_device *audio, rt_uint8_t *pbuf,
                      rt_size_t len) {
