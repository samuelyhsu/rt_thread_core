...
static void _rt_pipe_resume_writer(struct rt_audio_pipe *pipe) {
  if (!rt_list_isempty(&pipe->suspended_write_list)) {
    ...
    thread =
        rt_list_entry(pipe->suspended_write_list.next, struct rt_thread, tlist);
...
static rt_ssize_t rt_pipe_read(rt_device_t dev, rt_off_t pos, void *buffer,
                               rt_size_t size) {
  rt_base_t level;
  rt_thread_t thread;
  ...
  if (!(pipe->flag & RT_PIPE_FLAG_BLOCK_RD)) {
    ...
    read_nbytes =
        rt_ringbuffer_get(&(pipe->ringbuffer), (rt_uint8_t *)buffer, size);
...
static void _rt_pipe_resume_reader(struct rt_audio_pipe *pipe) {
  ...
  if (!rt_list_isempty(&pipe->suspended_read_list)) {
    ...
    /* get suspended thread */
    thread =
        rt_list_entry(pipe->suspended_read_list.next, struct rt_thread, tlist);
...
static rt_ssize_t rt_pipe_write(rt_device_t dev, rt_off_t pos,
...
const static struct rt_device_ops audio_pipe_ops = {
    RT_NULL, RT_NULL, RT_NULL, rt_pipe_read, rt_pipe_write, rt_pipe_control};
#endif
 ...
 */
rt_err_t rt_audio_pipe_init(struct rt_audio_pipe *pipe, const char *name,
                            rt_int32_t flag, rt_uint8_t *buf, rt_size_t size) {
  RT_ASSERT(pipe);
  RT_ASSERT(buf);
...
#ifdef RT_USING_HEAP
rt_err_t rt_audio_pipe_create(const char *name, rt_int32_t flag,
                              rt_size_t size) {
