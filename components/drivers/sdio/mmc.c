...
static const rt_uint32_t tran_unit[] = {10000, 100000, 1000000, 10000000,
                                        0,     0,      0,       0};
...
rt_inline rt_uint32_t GET_BITS(rt_uint32_t *resp, rt_uint32_t start,
                               rt_uint32_t size) {
...
static rt_int32_t mmcsd_parse_csd(struct rt_mmcsd_card *card) {
  ...
  card->tacc_ns =
      (tacc_uint[csd->taac & 0x07] * tacc_value[(csd->taac & 0x78) >> 3] + 9) /
      10;
  card->max_data_rate = tran_unit[csd->tran_speed & 0x07] *
                        tran_value[(csd->tran_speed & 0x78) >> 3];
  if (csd->wr_blk_len >= 9) {
 ...
 * Read extended CSD.
 */
static int mmc_get_ext_csd(struct rt_mmcsd_card *card,
                           rt_uint8_t **new_ext_csd) {
  void *ext_csd;
...
static int mmc_parse_ext_csd(struct rt_mmcsd_card *card, rt_uint8_t *ext_csd) {
  ...
  LOG_I("emmc card capacity %d KB, card sec count:%d.", card->card_capacity,
        card->card_sec_cnt);
...
static int mmc_switch(struct rt_mmcsd_card *card, rt_uint8_t set,
  ...
  cmd.cmd_code = SWITCH;
  cmd.arg =
      (MMC_SWITCH_MODE_WRITE_BYTE << 24) | (index << 16) | (value << 8) | set;
...
static int mmc_compare_ext_csds(struct rt_mmcsd_card *card, rt_uint8_t *ext_csd,
 ...
 */
static int mmc_select_bus_width(struct rt_mmcsd_card *card,
                                rt_uint8_t *ext_csd) {
  ...
  rt_uint32_t bus_widths[] = {MMCSD_BUS_WIDTH_8, MMCSD_BUS_WIDTH_4,
                              MMCSD_BUS_WIDTH_1};
  struct rt_mmcsd_host *host = card->host;
...
rt_err_t mmc_send_op_cond(struct rt_mmcsd_host *host, rt_uint32_t ocr,
                          rt_uint32_t *rocr) {
  struct rt_mmcsd_cmd cmd;
  rt_uint32_t i;
  rt_err_t err = RT_EOK;
...
static int mmc_select_hs200(struct rt_mmcsd_card *card) {
  ...
  ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING,
