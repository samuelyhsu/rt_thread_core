...
check_hybrid:
  ...
  if (ret == GPT_MBR_PROTECTIVE) {
    ...
    if (sz != (uint32_t)total_sectors - 1 && sz != 0xFFFFFFFF) {
      LOG_I("GPT: mbr size in lba (%u) different than whole disk (%u).", sz,
...
static gpt_entry *alloc_read_gpt_entries(struct rt_mmcsd_card *card,
                                         gpt_header *gpt) {
  size_t count;
  gpt_entry *pte;
  ...
  if (read_lba(card, (size_t)(gpt->partition_entry_lba), (uint8_t *)pte,
               count / 512) != RT_EOK) {
    rt_free(pte);
    return RT_NULL;
  }
...
static gpt_header *alloc_read_gpt_header(struct rt_mmcsd_card *card,
                                         size_t lba) {
  gpt_header *gpt;
  void *buf;
...
static int is_gpt_valid(struct rt_mmcsd_card *card, size_t lba,
  ...
  if ((uint64_t)((*gpt)->start_lba) != lba) {
    LOG_E("GPT start_lba incorrect: %ld != %ld", (uint64_t)((*gpt)->start_lba),
          (uint64_t)lba);
    goto fail;
  }
  ...
  if ((uint64_t)((*gpt)->first_usable_lba) > lastlba) {
    LOG_E("GPT: first_usable_lba incorrect: %ld > %ld",
          ((uint64_t)((*gpt)->first_usable_lba)), (size_t)lastlba);
    goto fail;
  }

  if ((uint64_t)((*gpt)->last_usable_lba) > lastlba) {
    LOG_E("GPT: last_usable_lba incorrect: %ld > %ld",
          (uint64_t)((*gpt)->last_usable_lba), (size_t)lastlba);
    goto fail;
  }

  if ((uint64_t)((*gpt)->last_usable_lba) <
...
static void compare_gpts(gpt_header *pgpt, gpt_header *agpt, size_t lastlba) {
  ...
  if ((uint64_t)(pgpt->start_lba) != (uint64_t)(agpt->alternate_lba)) {
    ...
    LOG_I("GPT:%lld != %lld", (uint64_t)(pgpt->start_lba),
  ...
  if ((uint64_t)(pgpt->alternate_lba) != (uint64_t)(agpt->start_lba)) {
    ...
    LOG_I("GPT:%lld != %lld", (uint64_t)(pgpt->alternate_lba),
          (uint64_t)(agpt->start_lba));
    error_found++;
  }

  if ((uint64_t)(pgpt->first_usable_lba) !=
      (uint64_t)(agpt->first_usable_lba)) {
    LOG_I("GPT:first_usable_lbas don't match.");
    LOG_I("GPT:%lld != %lld", (uint64_t)(pgpt->first_usable_lba),
          (uint64_t)(agpt->first_usable_lba));
    error_found++;
  }
  ...
  if ((uint64_t)(pgpt->last_usable_lba) != (uint64_t)(agpt->last_usable_lba)) {
    ...
    LOG_I("GPT:%lld != %lld", (uint64_t)(pgpt->last_usable_lba),
  ...
  if ((pgpt->num_partition_entries) != (agpt->num_partition_entries)) {
    LOG_I("GPT:num_partition_entries don't match: "
          "0x%x != 0x%x",
          (pgpt->num_partition_entries), (agpt->num_partition_entries));
    error_found++;
  }
  ...
  if ((pgpt->sizeof_partition_entry) != (agpt->sizeof_partition_entry)) {
    LOG_I("GPT:sizeof_partition_entry values don't match: "
          "0x%x != 0x%x",
          (pgpt->sizeof_partition_entry), (agpt->sizeof_partition_entry));
    error_found++;
  }
  ...
  if ((pgpt->alternate_lba) != lastlba) {
    ...
    LOG_I("GPT:%lld != %lld", (uint64_t)(pgpt->alternate_lba), (size_t)lastlba);
    error_found++;
  }

  if ((agpt->start_lba) != lastlba) {
    LOG_I("GPT:Alternate GPT header not at the end of the disk.");
    LOG_I("GPT:%lld != %lld", (uint64_t)(agpt->start_lba), (size_t)lastlba);
    error_found++;
  }
...
static int find_valid_gpt(struct rt_mmcsd_card *card, gpt_header **gpt,
  ...
  good_pgpt =
      is_gpt_valid(card, GPT_PRIMARY_PARTITION_TABLE_LBA, &pgpt, &pptes);
  if (good_pgpt) {
...
int gpt_get_partition_param(struct rt_mmcsd_card *card,
                            struct dfs_partition *part, uint32_t pindex) {
  ...
  rt_kprintf("found part[%d], begin(sector): %d, end(sector):%d size: ", pindex,
             _ptes[pindex].starting_lba, _ptes[pindex].ending_lba);
