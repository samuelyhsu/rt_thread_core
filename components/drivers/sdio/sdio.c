...
static const rt_uint8_t speed_value[16] = {0,  10, 12, 13, 15, 20, 25, 30,
                                           35, 40, 45, 50, 55, 60, 70, 80};

static const rt_uint32_t speed_unit[8] = {10000, 100000, 1000000, 10000000,
                                          0,     0,      0,       0};

rt_inline rt_int32_t sdio_match_card(struct rt_mmcsd_card *card,
                                     const struct rt_sdio_device_id *id);

rt_int32_t sdio_io_send_op_cond(struct rt_mmcsd_host *host, rt_uint32_t ocr,
...
rt_int32_t sdio_io_rw_direct(struct rt_mmcsd_card *card, rt_int32_t rw,
                             rt_uint32_t fn, rt_uint32_t reg_addr,
                             rt_uint8_t *pdata, rt_uint8_t raw) {
  struct rt_mmcsd_cmd cmd;
  rt_int32_t err;
...
rt_int32_t sdio_io_rw_extended(struct rt_mmcsd_card *card, rt_int32_t rw,
                               rt_uint32_t fn, rt_uint32_t addr,
                               rt_int32_t op_code, rt_uint8_t *buf,
                               rt_uint32_t blocks, rt_uint32_t blksize) {
...
rt_inline rt_uint32_t sdio_max_block_size(struct rt_sdio_function *func) {
  rt_uint32_t size =
      MIN(func->card->host->max_seg_size, func->card->host->max_blk_size);
...
rt_int32_t sdio_io_rw_extended_block(struct rt_sdio_function *func,
                                     rt_int32_t rw, rt_uint32_t addr,
                                     rt_int32_t op_code, rt_uint8_t *buf,
                                     rt_uint32_t len) {
  rt_int32_t ret;
  ...
  if (func->card->cccr.multi_block && (len > sdio_max_block_size(func))) {
    ...
    while (left_size > func->cur_blk_size) {
      ...
      ret = sdio_io_rw_extended(func->card, rw, func->num, addr, op_code, buf,
  ...
  while (left_size > 0) {
    ...
    ret = sdio_io_rw_extended(func->card, rw, func->num, addr, op_code, buf, 1,
                              len);
    if (ret)
      return ret;
...
rt_uint8_t sdio_io_readb(struct rt_sdio_function *func, rt_uint32_t reg,
                         rt_int32_t *err) {
  rt_uint8_t data = 0;
  rt_int32_t ret;
...
rt_int32_t sdio_io_writeb(struct rt_sdio_function *func, rt_uint32_t reg,
                          rt_uint8_t data) {
  return sdio_io_rw_direct(func->card, 1, func->num, reg, &data, 0);
}

rt_uint16_t sdio_io_readw(struct rt_sdio_function *func, rt_uint32_t addr,
                          rt_int32_t *err) {
  rt_int32_t ret;
  rt_uint32_t dmabuf;
...
rt_int32_t sdio_io_writew(struct rt_sdio_function *func, rt_uint16_t data,
                          rt_uint32_t addr) {
...
rt_uint32_t sdio_io_readl(struct rt_sdio_function *func, rt_uint32_t addr,
                          rt_int32_t *err) {
  rt_int32_t ret;
  rt_uint32_t dmabuf;
...
rt_int32_t sdio_io_writel(struct rt_sdio_function *func, rt_uint32_t data,
                          rt_uint32_t addr) {
  rt_uint32_t dmabuf = data;
...
rt_int32_t sdio_io_read_multi_fifo_b(struct rt_sdio_function *func,
                                     rt_uint32_t addr, rt_uint8_t *buf,
                                     rt_uint32_t len) {
...
static rt_int32_t sdio_read_cccr(struct rt_mmcsd_card *card) {
  ...
  if (cccr_version >= SDIO_CCCR_REV_1_10) {
    data =
        sdio_io_readb(card->sdio_function[0], SDIO_REG_CCCR_POWER_CTRL, &ret);
    if (ret)
      goto out;
...
static rt_int32_t cistpl_funce_func0(struct rt_mmcsd_card *card,
  ...
  card->cis.max_tran_speed =
      speed_value[(buf[3] >> 3) & 15] * speed_unit[buf[3] & 7];
...
static rt_int32_t sdio_read_cis(struct rt_sdio_function *func) {
  ...
  for (i = 0; i < 3; i++) {
    data = sdio_io_readb(
        func0, SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_CIS + i, &ret);
    if (ret)
      return ret;
    cisptr |= data << (i * 8);
  }
  ...
  do {
    ...
    default:
      ...
      *prev = curr;
      prev = &curr->next;
      LOG_D("function %d, CIS tuple code %#x, length %d", func->num, tpl_code,
            tpl_link);
      break;
    }
...
static rt_int32_t sdio_read_fbr(struct rt_sdio_function *func) {
  ...
  data = sdio_io_readb(
      func0, SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_STD_FUNC_IF, &ret);
  if (ret)
    goto err;
...
static rt_int32_t sdio_init_card(struct rt_mmcsd_host *host, rt_uint32_t ocr) {
  ...
  if (card->flags & CARD_FLAG_HIGHSPEED) {
    mmcsd_set_clock(host, card->host->freq_max > 50000000
                              ? 50000000
                              : card->host->freq_max);
  } else {
...
static rt_int32_t sdio_irq_thread_create(struct rt_mmcsd_card *card) {
  ...
  if (!host->sdio_irq_num) {
    ...
    host->sdio_irq_thread =
        rt_thread_create("sdio_irq", sdio_irq_thread, host, RT_SDIO_STACK_SIZE,
...
rt_int32_t sdio_enable_func(struct rt_sdio_function *func) {
  ...
  timeout =
      rt_tick_get() + func->enable_timeout_val * RT_TICK_PER_SECOND / 1000;
...
void *sdio_get_drvdata(struct rt_sdio_function *func) { return func->priv; }
...
rt_int32_t sdio_set_block_size(struct rt_sdio_function *func,
  ...
  ret =
      sdio_io_writeb(func0, SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_BLKSIZE,
                     blksize & 0xff);
  if (ret)
    return ret;
  ret = sdio_io_writeb(func0,
                       SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_BLKSIZE + 1,
                       (blksize >> 8) & 0xff);
  if (ret)
    return ret;
