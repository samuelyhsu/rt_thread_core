...
static int __send_status(struct rt_mmcsd_card *card, rt_uint32_t *status,
                         unsigned retries) {
  int err;
...
static rt_err_t rt_mmcsd_req_blk(struct rt_mmcsd_card *card, rt_uint32_t sector,
  ...
  if (cmd.err || data.err || stop.err) {
    ...
    LOG_E("%d,%d,%d, 0x%08x,0x%08x", cmd.err, data.err, stop.err, data.flags,
          sector);
...
static rt_err_t rt_mmcsd_init(rt_device_t dev) { return RT_EOK; }

static rt_err_t rt_mmcsd_open(rt_device_t dev, rt_uint16_t oflag) {
  return RT_EOK;
}

static rt_err_t rt_mmcsd_close(rt_device_t dev) { return RT_EOK; }
...
static rt_ssize_t rt_mmcsd_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                rt_size_t size) {
  rt_err_t err = 0;
  rt_size_t offset = 0;
  ...
  while (remain_size) {
    req_size = (remain_size > blk_dev->max_req_size) ? blk_dev->max_req_size
                                                     : remain_size;
    err = rt_mmcsd_req_blk(blk_dev->card, part->offset + pos + offset, rd_ptr,
                           req_size, 0);
    if (err)
      break;
    offset += req_size;
...
static rt_ssize_t rt_mmcsd_write(rt_device_t dev, rt_off_t pos,
  ...
  while (remain_size) {
    req_size = (remain_size > blk_dev->max_req_size) ? blk_dev->max_req_size
                                                     : remain_size;
    err = rt_mmcsd_req_blk(blk_dev->card, part->offset + pos + offset, wr_ptr,
                           req_size, 1);
    if (err)
      break;
    offset += req_size;
...
rt_int32_t read_lba(struct rt_mmcsd_card *card, size_t lba, uint8_t *buffer,
                    size_t count) {
  rt_uint8_t status = 0;
...
const static struct rt_device_ops mmcsd_blk_ops = {
    rt_mmcsd_init, rt_mmcsd_open,  rt_mmcsd_close,
    rt_mmcsd_read, rt_mmcsd_write, rt_mmcsd_control};
#endif
...
rt_int32_t gpt_device_probe(struct rt_mmcsd_card *card) {
  ...
  blk_dev->max_req_size =
      BLK_MIN((card->host->max_dma_segs * card->host->max_seg_size) >> 9,
  ...
  rt_device_register(&(blk_dev->dev), card->host->name, RT_DEVICE_FLAG_RDWR);
  ...
  for (i = 0; i < RT_GPT_PARTITION_MAX; i++) {
    ...
    if (status == RT_EOK) {
      ...
      rt_snprintf(sname, sizeof(sname) - 1, "sem_%s%d", card->host->name,
                  i + 1);
      blk_dev->part.lock = rt_sem_create(sname, 1, RT_IPC_FLAG_FIFO);
...
void rt_mmcsd_blk_remove(struct rt_mmcsd_card *card) {
  ...
  for (l = (&blk_devices)->next, n = l->next; l != &blk_devices;
       l = n, n = n->next) {
