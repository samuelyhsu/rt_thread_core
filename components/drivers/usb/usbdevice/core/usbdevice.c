...
const static char *ustring[] = {
    "Language",      "RT-Thread Team.", "RTT Composite Device", "320219198301",
    "Configuration", "Interface",       USB_STRING_OS};
...
rt_err_t rt_usbd_class_register(udclass_t udclass) {
  ...
  if (!rt_list_isempty(&class_list)) {
    rt_kprintf("[D/USBD] If you want to use usb composite device please define "
               "RT_USB_DEVICE_COMPOSITE\n");
    return RT_ERROR;
  }
#endif
