...
static rt_ssize_t rt_usbd_ep_write(udevice_t device, uep_t ep, void *buffer,
                                   rt_size_t size);
static rt_ssize_t rt_usbd_ep_read_prepare(udevice_t device, uep_t ep,
...
static rt_err_t _get_device_descriptor(struct udevice *device, ureq_t setup) {
  ...
  size = (setup->wLength > USB_DESC_LENGTH_DEVICE) ? USB_DESC_LENGTH_DEVICE
                                                   : setup->wLength;
...
static rt_err_t _get_config_descriptor(struct udevice *device, ureq_t setup) {
  ...
  size = (setup->wLength > cfg_desc->wTotalLength) ? cfg_desc->wTotalLength
                                                   : setup->wLength;
...
static rt_err_t _get_qualifier_descriptor(struct udevice *device,
                                          ureq_t setup) {
...
static rt_err_t _get_descriptor(struct udevice *device, ureq_t setup) {
  ...
  if (setup->request_type == USB_REQ_TYPE_DIR_IN) {
    ...
    case USB_DESC_TYPE_DEVICEQUALIFIER:
      /* If a full-speed only device (with a device descriptor version number
      equal to 0200H) receives a GetDescriptor() request for a device_qualifier,
      it must respond with a request error. The host must not make a request for
      an other_speed_configuration descriptor unless it first successfully
  ...
  case USB_REQ_TYPE_ENDPOINT:
    ...
    case USB_REQ_CLEAR_FEATURE: {
      ...
      if (USB_EP_HALT == setup->wValue && ep->stalled == RT_TRUE) {
        ...
        for (node = ep->request_list.next; node != &ep->request_list;
             node = node->next) {
    ...
    case 0x04:
      ...
      if (usb_comp_id_desc == RT_NULL) {
        ...
        usb_comp_id_desc_size =
            ...
            (sizeof(struct usb_os_function_comp_id_descriptor) -
             sizeof(rt_list_t)) *
        ...
        for (p = device->os_comp_id_desc->func_desc.next;
             p != &device->os_comp_id_desc->func_desc; p = p->next) {
          func_comp_id_desc =
              rt_list_entry(p, struct usb_os_function_comp_id_descriptor, list);
          rt_memcpy(pusb_comp_id_desc,
...
static rt_err_t _ep0_out_notify(udevice_t device, struct ep_msg *ep_msg) {
  ...
  if (ep0->request.remain_size == 0) {
    ...
    rt_usbd_ep0_read(device, ep0->request.buffer, ep0->request.remain_size,
                     ep0->rx_indicate);
  }
...
static rt_err_t _sof_notify(udevice_t device) {
  ...
  for (i = device->curr_cfg->func_list.next; i != &device->curr_cfg->func_list;
       i = i->next) {
...
static rt_ssize_t rt_usbd_ep_read_prepare(udevice_t device, uep_t ep,
  ...
  return dcd_ep_read_prepare(device->dcd, EP_ADDRESS(ep), buffer,
                             size > EP_MAXPACKET(ep) ? EP_MAXPACKET(ep) : size);
}
 ...
 * @return RT_EOK.
 */
rt_err_t rt_usbd_device_set_interface_string(udevice_t device, int index,
                                             const char *string) {
  /* parameter check */
  RT_ASSERT(device != RT_NULL);
...
rt_err_t
rt_usbd_device_set_os_comp_id_desc(udevice_t device,
...
rt_err_t
rt_usbd_device_set_qualifier(udevice_t device,
...
rt_err_t rt_usbd_device_set_descriptor(udevice_t device, udev_desc_t dev_desc) {
  ...
  rt_memcpy((void *)&device->dev_desc, (void *)dev_desc,
...
uconfig_t rt_usbd_find_config(udevice_t device, rt_uint8_t value) {
  ...
  for (node = device->cfg_list.next; node != &device->cfg_list;
       node = node->next) {
 ...
 *
 * @return an usb interface alternate setting object on found or RT_NULL on not
 * found.
 */
 ...
 */
uep_t rt_usbd_find_endpoint(udevice_t device, ufunction_t *pfunc,
                            rt_uint8_t ep_addr) {
  uep_t ep;
  struct rt_list_node *i, *j, *k;
  ufunction_t func;
  ...
  for (i = cfg->func_list.next; i != &cfg->func_list; i = i->next) {
    ...
    for (j = func->intf_list.next; j != &func->intf_list; j = j->next) {
      ...
      for (k = intf->setting_list.next; k != &intf->setting_list; k = k->next) {
        ...
        /* allocate address for every endpoint in the interface alternate
         * setting */
        for (m = altsetting->ep_list.next; m != &altsetting->ep_list;
             m = m->next) {
        ...
        rt_memcpy((void *)&cfg->cfg_desc.data[cfg->cfg_desc.wTotalLength -
                  ...
                  (void *)altsetting->desc, altsetting->desc_size);
...
rt_err_t rt_usbd_os_comp_id_desc_add_os_func_comp_id_desc(
    usb_os_comp_id_desc_t os_comp_id_desc,
    usb_os_func_comp_id_desc_t os_func_comp_id_desc) {
  RT_ASSERT(os_comp_id_desc != RT_NULL);
  RT_ASSERT(os_func_comp_id_desc != RT_NULL);
  rt_list_insert_before(&os_comp_id_desc->func_desc,
                        &os_func_comp_id_desc->list);
 ...
 * @return RT_EOK.
 */
rt_err_t rt_usbd_set_feature(udevice_t device, rt_uint16_t value,
                             rt_uint16_t index) {
  RT_ASSERT(device != RT_NULL);
 ...
 * @return RT_EOK.
 */
rt_err_t rt_usbd_clear_feature(udevice_t device, rt_uint16_t value,
                               rt_uint16_t index) {
  RT_ASSERT(device != RT_NULL);
...
rt_err_t rt_usbd_ep0_setup_handler(udcd_t dcd, struct urequest *setup) {
  ...
  if (setup == RT_NULL) {
    ...
    rt_memcpy((void *)&msg.content.setup, (void *)setup,
...
rt_err_t rt_usbd_ep_out_handler(udcd_t dcd, rt_uint8_t address,
                                rt_size_t size) {
  struct udev_msg msg;
...
rt_size_t rt_usbd_ep0_read(udevice_t device, void *buffer, rt_size_t size,
                           rt_err_t (*rx_ind)(udevice_t device,
                                              rt_size_t size)) {
  uep_t ep0;
...
static rt_uint8_t
    usb_mq_pool[(USBD_MQ_MSG_SZ + sizeof(void *)) * USBD_MQ_MAX_MSG];
...
rt_err_t rt_usbd_core_init(void) {
  ...
  rt_mq_init(&usb_mq, "usbd", usb_mq_pool, USBD_MQ_MSG_SZ, sizeof(usb_mq_pool),
             RT_IPC_FLAG_FIFO);

  /* init usb device thread */
  rt_thread_init(&usb_thread, "usbd", rt_usbd_thread_entry, RT_NULL,
                 usb_thread_stack, RT_USBD_THREAD_STACK_SZ, RT_USBD_THREAD_PRIO,
                 20);
