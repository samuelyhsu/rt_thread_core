...
rt_align(4) struct winusb_descriptor _winusb_desc = {
...
rt_align(4) const static char *_ustring[] = {
    "Language",       "RT-Thread Team.", "RTT Win USB",
    "32021919830108", "Configuration",   "Interface",
    USB_STRING_OS // must be
};

rt_align(4) struct usb_os_proerty winusb_proerty[] = {
    USB_OS_PROPERTY_DESC(USB_OS_PROPERTY_TYPE_REG_SZ, "DeviceInterfaceGUID",
                         RT_WINUSB_GUID),
};

rt_align(4) struct usb_os_function_comp_id_descriptor winusb_func_comp_id_desc =
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size) {
  ...
  if (winusb_device->parent.tx_complete != RT_NULL) {
    winusb_device->parent.tx_complete(&winusb_device->parent,
                                      winusb_device->ep_in->buffer);
  }
  return RT_EOK;
}
...
static rt_err_t _ep0_cmd_read(ufunction_t func, ureq_t setup) {
  winusb_device_t winusb_device = (winusb_device_t)func->user_data;
  cmd_func = func;
  rt_usbd_ep0_read(func->device, winusb_device->cmd_buff, setup->wLength,
                   _ep0_cmd_handler);
  return RT_EOK;
}
...
static rt_ssize_t win_usb_read(rt_device_t dev, rt_off_t pos, void *buffer,
                               rt_size_t size) {
  ...
  rt_usbd_io_request(((ufunction_t)dev->user_data)->device,
                     winusb_device->ep_out, &winusb_device->ep_out->request);
  return size;
}
static rt_ssize_t win_usb_write(rt_device_t dev, rt_off_t pos,
  ...
  rt_usbd_io_request(((ufunction_t)dev->user_data)->device,
                     winusb_device->ep_in, &winusb_device->ep_in->request);
  return size;
}
...
const static struct rt_device_ops winusb_device_ops = {
    RT_NULL, RT_NULL, RT_NULL, win_usb_read, win_usb_write, win_usb_control,
};
#endif
...
static rt_err_t rt_usb_winusb_init(ufunction_t func) {
  ...
  return rt_device_register(&winusb_device->parent, "winusb",
                            RT_DEVICE_FLAG_RDWR);
}
...
ufunction_t rt_usbd_function_winusb_create(udevice_t device) {
...
#ifdef RT_USB_DEVICE_COMPOSITE
  rt_usbd_device_set_interface_string(device, WINUSB_INTF_STR_INDEX,
                                      _ustring[2]);
#else
  ...
  rt_usbd_altsetting_config_descriptor(winusb_setting, &_winusb_desc,
                                       (rt_off_t) &
                                           ((winusb_desc_t)0)->intf_desc);
  ...
  winusb_device->ep_out =
      rt_usbd_endpoint_new(&winusb_desc->ep_out_desc, _ep_out_handler);
  winusb_device->ep_in =
      rt_usbd_endpoint_new(&winusb_desc->ep_in_desc, _ep_in_handler);
...
struct udclass winusb_class = {.rt_usbd_function_create =
