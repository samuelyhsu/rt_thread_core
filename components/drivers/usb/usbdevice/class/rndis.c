 ...
 * 2013-07-18     aozima            re-initial respone chain list when RNDIS
 * restart. 2017-11-25     ZYH               fix it and add OS descriptor
...
struct rt_rndis_eth {
  ...
  rt_align(4) rt_uint8_t rx_pool[512];
  rt_align(4) rt_uint8_t tx_pool[512];

  rt_uint32_t cmd_pool[2];
  rt_align(
      4) char rx_buffer[sizeof(struct rndis_packet_msg) + USB_ETH_MTU + 14];
  rt_size_t rx_offset;
  rt_size_t rx_length;
  rt_bool_t rx_flag;
  rt_bool_t rx_frist;
  ...
  rt_align(
      4) char tx_buffer[sizeof(struct rndis_packet_msg) + USB_ETH_MTU + 14];
...
rt_align(4) static struct udevice_descriptor _dev_desc = {
...
rt_align(4) const static struct ucdc_comm_descriptor _comm_desc = {
...
rt_align(4) const static struct ucdc_data_descriptor _data_desc = {
    /* interface descriptor */
    {
        USB_DESC_LENGTH_INTERFACE,
        USB_DESC_TYPE_INTERFACE,
...
rt_align(4) const
    static char *_ustring[] = {"Language",               /* LANGID */
                               "RT-Thread Team.",        /* MANU */
...
rt_align(4) struct usb_os_function_comp_id_descriptor rndis_func_comp_id_desc =
...
// FS and HS needed
rt_align(4) static struct usb_qualifier_descriptor dev_qualifier = {
...
/* supported OIDs */
rt_align(4) const static rt_uint32_t oid_supported_list[] = {
    /* General OIDs */
...
static void _rndis_response_available(ufunction_t func) {
  ...
  if (device->need_notify == RT_TRUE) {
    ...
    rt_usbd_io_request(func->device, device->eps.ep_cmd,
                       &device->eps.ep_cmd->request);
  }
}
...
static rt_err_t _rndis_init_response(ufunction_t func, rndis_init_msg_t msg) {
  ...
  resp->MaxTransferSize = USB_ETH_MTU + 58; /* Space for 1280 IP buffer,
                                            Ethernet Header, RNDIS messages */
...

  {
    ...
    rt_list_insert_before(&((rt_rndis_eth_t)func->user_data)->response_list,
                          &response->list);
...
static void _set_resp(rndis_query_cmplt_t resp, rt_uint32_t value) {
  rt_uint32_t *response =
      (rt_uint32_t *)((char *)resp + sizeof(struct rndis_query_cmplt));
  *response = value;
}
...
static rt_err_t _rndis_query_response(ufunction_t func, rndis_query_msg_t msg) {
  ...
  case OID_GEN_LINK_SPEED:
    ...
    if (resp == RT_NULL)
      break;
    _set_resp(resp, (func->device->dcd->device_is_hs ? (480UL * 1000 * 1000)
                                                     : (12UL * 1000 * 1000)) /
                        100);
    break;
  ...
  resp->Status = RNDIS_STATUS_SUCCESS;
  resp->MessageLength =
      sizeof(struct rndis_query_cmplt) + resp->InformationBufferLength;
...
static rt_err_t _rndis_keepalive_response(ufunction_t func,
                                          rndis_keepalive_msg_t msg) {
...
static rt_err_t send_encapsulated_command_done(udevice_t device,
                                               rt_size_t size) {
  if (function != RT_NULL) {
...
static rt_err_t _rndis_send_encapsulated_command(ufunction_t func,
                                                 ureq_t setup) {
  RT_ASSERT(setup->wLength <= sizeof(rndis_message_buffer));
  function = func;
  rt_usbd_ep0_read(func->device, rndis_message_buffer, setup->wLength,
                   send_encapsulated_command_done);

  return RT_EOK;
}

static rt_err_t _rndis_get_encapsulated_response(ufunction_t func,
                                                 ureq_t setup) {
  rndis_gen_msg_t msg;
  struct rt_rndis_response *response;
  ...
  response = (struct rt_rndis_response *)((rt_rndis_eth_t)func->user_data)
                 ->response_list.next;
  ...
  if (!rt_list_isempty(&((rt_rndis_eth_t)func->user_data)->response_list)) {
    ...
    data[0] = RESPONSE_AVAILABLE;
    data[1] = 0;
    ((rt_rndis_eth_t)func->user_data)->eps.ep_cmd->request.buffer =
        ((rt_rndis_eth_t)func->user_data)->eps.ep_cmd->buffer;
    ((rt_rndis_eth_t)func->user_data)->eps.ep_cmd->request.size = 8;
    ((rt_rndis_eth_t)func->user_data)->eps.ep_cmd->request.req_type =
        UIO_REQUEST_WRITE;
    rt_usbd_io_request(func->device,
                       ((rt_rndis_eth_t)func->user_data)->eps.ep_cmd,
                       &((rt_rndis_eth_t)func->user_data)->eps.ep_cmd->request);
  } else {
    ((rt_rndis_eth_t)func->user_data)->need_notify = RT_TRUE;
  }
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size) {
  ...
  if (((rt_rndis_eth_t)func->user_data)->rx_frist == RT_TRUE) {
    ...
    if (size >= 44) {
      ...
      rt_memcpy(&((rt_rndis_eth_t)func->user_data)
                     ->rx_buffer[((rt_rndis_eth_t)func->user_data)->rx_offset],
                data, size);
  ...
  if (((rt_rndis_eth_t)func->user_data)->rx_offset >=
...
/**
 * This function will run cdc class, it will be called on handle set
...
static rt_err_t _function_enable(ufunction_t func) {
  ...
  eps->ep_cmd->buffer =
      (rt_uint8_t *)((rt_rndis_eth_t)func->user_data)->cmd_pool;
  ...
  /* clean resp chain list. */
  {
    ...
    while (
        !rt_list_isempty(&((rt_rndis_eth_t)func->user_data)->response_list)) {
      response = (struct rt_rndis_response *)((rt_rndis_eth_t)func->user_data)
                     ->response_list.next;
...
/**
 * This function will stop cdc class, it will be called on handle set
 * configuration request.
 *
 * @param device the usb device object.
 *
...
static rt_err_t rt_rndis_eth_init(rt_device_t dev) { return RT_EOK; }

static rt_err_t rt_rndis_eth_open(rt_device_t dev, rt_uint16_t oflag) {
  return RT_EOK;
}

static rt_err_t rt_rndis_eth_close(rt_device_t dev) { return RT_EOK; }

static rt_ssize_t rt_rndis_eth_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                    rt_size_t size) {
  rt_set_errno(-RT_ENOSYS);
  return 0;
}

static rt_ssize_t rt_rndis_eth_write(rt_device_t dev, rt_off_t pos,
...
struct pbuf *rt_rndis_eth_rx(rt_device_t dev) {
  ...
  if (device->rx_length != 0) {
    ...
    if (p != RT_NULL) {
      ...
      for (q = p; q != RT_NULL; q = q->next) {
        ...
        rt_memcpy(q->payload, (rt_uint8_t *)((device->rx_buffer) + offset),
                  q->len);
        offset += q->len;
      }
    }
...

  {
    ...
    rt_usbd_io_request(device->func->device, device->eps.ep_out,
                       &device->eps.ep_out->request);
  }
...
rt_err_t rt_rndis_eth_tx(rt_device_t dev, struct pbuf *p) {
  ...
  /* send */
  {
    ...
    rt_usbd_io_request(device->func->device, device->eps.ep_in,
                       &device->eps.ep_in->request);
  }
...
const static struct rt_device_ops rndis_device_ops = {
    rt_rndis_eth_init, rt_rndis_eth_open,  rt_rndis_eth_close,
    rt_rndis_eth_read, rt_rndis_eth_write, rt_rndis_eth_control};
#endif
 ...
 * @return RT_EOK on successful.
 */
static rt_err_t _rndis_indicate_status_msg(ufunction_t func,
                                           rt_uint32_t status) {
...
ufunction_t rt_usbd_function_rndis_create(udevice_t device) {
...
#ifdef RT_USB_DEVICE_COMPOSITE
  rt_usbd_device_set_interface_string(device, RNDIS_INTF_STR_INDEX,
                                      _ustring[2]);
#else
  ...
  rt_usbd_altsetting_config_descriptor(
      comm_setting, &_comm_desc, (rt_off_t) & ((ucdc_comm_desc_t)0)->intf_desc);
  rt_usbd_altsetting_config_descriptor(data_setting, &_data_desc, 0);
  /* configure the cdc interface descriptor */
  _cdc_descriptor_config(comm_setting->desc, intf_comm->intf_num,
                         data_setting->desc, intf_data->intf_num,
  ...
  rt_usbd_os_comp_id_desc_add_os_func_comp_id_desc(device->os_comp_id_desc,
                                                   &rndis_func_comp_id_desc);
...
#ifdef RNDIS_DELAY_LINK_UP
  rt_timer_init(&_rndis->timer, "RNDIS", timer_timeout, _rndis,
...
struct udclass rndis_class = {.rt_usbd_function_create =
