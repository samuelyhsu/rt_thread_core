...
rt_align(4) static struct udevice_descriptor dev_desc = {
...
rt_align(4) static char serno[_SER_NO_LEN + 1] = {'\0'};
rt_weak rt_err_t vcom_get_stored_serno(char *serno, int size);

rt_err_t vcom_get_stored_serno(char *serno, int size) { return RT_ERROR; }
rt_align(4) const static char *_ustring[] = {
    "Language", "RT-Thread Team.", "RTT Virtual Serial",
    serno,      "Configuration",   "Interface",
};
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size) {
  ...
  if ((request_size != 0) &&
      ((request_size % EP_MAXPACKET(data->ep_in)) == 0)) {
...
static rt_err_t _cdc_set_line_coding_callback(udevice_t device,
                                              rt_size_t size) {
...
static rt_err_t _cdc_set_line_coding(udevice_t device, ureq_t setup) {
  ...
  rt_usbd_ep0_read(device, (void *)&line_coding,
                   sizeof(struct ucdc_line_coding),
...
/**
 * This function will run cdc function, it will be called on handle set
...
/**
 * This function will stop cdc function, it will be called on handle set
 * configuration request.
 *
 * @param func the usb function object.
 *
...
static rt_err_t _vcom_control(struct rt_serial_device *serial, int cmd,
                              void *arg) {
...
static rt_ssize_t _vcom_rb_block_put(struct vcom *data, const rt_uint8_t *buf,
                                     rt_size_t size) {
  rt_base_t level;
  rt_size_t put_len = 0;
  rt_size_t w_ptr = 0;
  ...
  while (remain_size) {
    level = rt_hw_interrupt_disable();
    put_len = rt_ringbuffer_put(&data->tx_ringbuffer,
                                (const rt_uint8_t *)&buf[w_ptr], remain_size);
    rt_hw_interrupt_enable(level);
    w_ptr += put_len;
    remain_size -= put_len;
    if (put_len == 0) {
      rt_event_recv(&data->tx_event, CDC_TX_HAS_SPACE,
                    RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, VCOM_TX_TIMEOUT,
                    &res);
    } else {
...
static rt_ssize_t _vcom_tx(struct rt_serial_device *serial, rt_uint8_t *buf,
  ...
  if (data->connected) {
    if ((serial->parent.open_flag & RT_DEVICE_FLAG_STREAM)) {
      while (send_size < size) {
        ...
        if (ptr < size) {
          send_size += _vcom_rb_block_put(
              data, (const rt_uint8_t *)&buf[send_size], ptr - send_size);
      ...
      while (send_size < size) {
        send_size += _vcom_rb_block_put(data, (rt_uint8_t *)&buf[send_size],
                                        size - send_size);
      }
    }
  } else {
...
static const struct rt_uart_ops usb_vcom_ops = {
    _vcom_configure, _vcom_control, _vcom_putc, _vcom_getc, _vcom_tx};
...
static void vcom_tx_thread_entry(void *parameter) {
  ...
  while (1) {
    if ((rt_event_recv(&data->tx_event, CDC_TX_HAS_DATE,
...
static void rt_usb_vcom_init(struct ufunction *func) {
  ...
  rt_thread_init(&vcom_thread, "vcom", vcom_tx_thread_entry, func,
                 vcom_thread_stack, VCOM_TASK_STK_SIZE, 16, 20);
  result = rt_thread_startup(&vcom_thread);
  RT_ASSERT(result == RT_EOK);
}
struct udclass vcom_class = {.rt_usbd_function_create =
