...
rt_align(4) const rt_uint8_t _report_desc[] = {
...
rt_align(4) const static struct uhid_comm_descriptor _hid_comm_desc = {
...
#endif
#if !defined(RT_USB_DEVICE_HID_KEYBOARD) ||                                    \
    !defined(RT_USB_DEVICE_HID_MOUSE) || !defined(RT_USB_DEVICE_HID_MEDIA)
        USB_HID_PROTOCOL_NONE, /* nInterfaceProtocol : 0=none, 1=keyboard,
                                  2=mouse */
#elif !defined(RT_USB_DEVICE_HID_MOUSE)
        USB_HID_PROTOCOL_KEYBOARD, /* nInterfaceProtocol : 0=none, 1=keyboard,
                                      2=mouse */
#else
        USB_HID_PROTOCOL_MOUSE, /* nInterfaceProtocol : 0=none, 1=keyboard,
                                   2=mouse */
#endif
        0x00,
    },
    ...
    {
        USB_DESC_LENGTH_INTERFACE, USB_DESC_TYPE_INTERFACE,
        ...
        {
            {
                ...
                sizeof(_report_desc), /* wItemLength: Total length of Report
                                         descriptor */
            },
        },
    },
...
rt_align(4) const static char *_ustring[] = {
    "Language",       "RT-Thread Team.", "RTT HID-Device",
    "32021919830108", "Configuration",   "Interface",
};
...
static rt_err_t _interface_handler(ufunction_t func, ureq_t setup) {
  ...
  case USB_REQ_GET_DESCRIPTOR:
    if ((setup->wValue >> 8) == USB_DESC_TYPE_REPORT) {
      rt_usbd_ep0_write(func->device, (void *)(&_report_desc[0]),
                        sizeof(_report_desc));
    } else if ((setup->wValue >> 8) == USB_DESC_TYPE_HID) {

      rt_usbd_ep0_write(func->device, (void *)(&_hid_comm_desc.hid_desc),
                        sizeof(struct uhid_descriptor));
    }
    break;
  ...
  case USB_HID_REQ_SET_REPORT:
    ...
    rt_usbd_ep0_read(func->device, data->report_buf, setup->wLength,
 ...
 * @return RT_EOK on successful.
 */
static rt_err_t _hid_descriptor_config(uhid_comm_desc_t hid,
                                       rt_uint8_t cintf_nr) {
#ifdef RT_USB_DEVICE_COMPOSITE
...
static rt_ssize_t _hid_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  ...
  if (hiddev->func->device->state == USB_STATE_CONFIGURED) {
    ...
    rt_usbd_io_request(hiddev->func->device, hiddev->ep_in,
                       &hiddev->ep_in->request);
    return size;
  }

  return 0;
}
rt_weak void HID_Report_Received(hid_report_t report) { dump_report(report); }
rt_align(RT_ALIGN_SIZE) static rt_uint8_t hid_thread_stack[512];
...
static void hid_thread_entry(void *parameter) {
  ...
  while (1) {
    if (rt_mq_recv(&hiddev->hid_mq, &report, sizeof(report),
...
const static struct rt_device_ops hid_device_ops = {
    RT_NULL, RT_NULL, RT_NULL, RT_NULL, _hid_write, RT_NULL,
};
#endif
...
ufunction_t rt_usbd_function_hid_create(udevice_t device) {
  ...
  rt_usbd_altsetting_config_descriptor(hid_setting, &_hid_comm_desc,
                                       (rt_off_t) &
                                           ((uhid_comm_desc_t)0)->intf_desc);
...
struct udclass hid_class = {.rt_usbd_function_create =
