...
rt_align(4) const static struct umass_descriptor _mass_desc = {
...
rt_align(4) const static char *_ustring[] = {
    "Language",     "RT-Thread Team.", "RTT Mass Storage",
    "320219198301", "Configuration",   "Interface",
};
...
rt_align(4) static struct scsi_cmd cmd_data[] = {
...
static rt_ssize_t _read_capacity(ufunction_t func, ustorage_cbw_t cbw) {
  ...
  buf = data->ep_in->buffer;
  sector_count =
      data->geometry.sector_count - 1; /* Last Logical Block Address */
  sector_size = data->geometry.bytes_per_sector;
...
static rt_ssize_t _read_10(ufunction_t func, ustorage_cbw_t cbw) {
  ...
  data = (struct mstorage *)func->user_data;
  data->block =
      cbw->cb[2] << 24 | cbw->cb[3] << 16 | cbw->cb[4] << 8 | cbw->cb[5] << 0;
...
static rt_ssize_t _write_10(ufunction_t func, ustorage_cbw_t cbw) {
  ...
  RT_DEBUG_LOG(RT_DEBUG_USB,
               ("_write_10 count 0x%x block 0x%x 0x%x\n", data->count,
                data->block, data->geometry.sector_count));
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size) {
  ...
  case STAT_SEND:
    ...
    if (data->count > 0 && data->csw_response.data_reside > 0) {
      if (rt_device_read(data->disk, data->block, data->ep_in->buffer, 1) ==
          0) {
...
static void _cb_len_calc(ufunction_t func, struct scsi_cmd *cmd,
  ...
  if (cmd->cmd_len == 6) {
    ...
    case BLOCK_COUNT:
      data->cb_data_size =
          (cbw->cb[7] << 8 | cbw->cb[8]) * data->geometry.bytes_per_sector;
      break;
    case FIXED:
      data->cb_data_size = cmd->data_size;
      break;
...
/**
 * This function will run mass storage function, it will be called on handle set
...
static rt_err_t _function_enable(ufunction_t func) {
  ...
  data->ep_in->buffer =
      (rt_uint8_t *)rt_malloc(data->geometry.bytes_per_sector);
...
/**
 * This function will stop mass storage function, it will be called on handle
...
static rt_err_t _mstorage_descriptor_config(umass_desc_t desc,
                                            rt_uint8_t cintf_nr,
...
ufunction_t rt_usbd_function_mstorage_create(udevice_t device) {
...
#ifdef RT_USB_DEVICE_COMPOSITE
  rt_usbd_device_set_interface_string(device, MSTRORAGE_INTF_STR_INDEX,
                                      _ustring[2]);
#else
  ...
  rt_usbd_altsetting_config_descriptor(
      setting, &_mass_desc, (rt_off_t) & ((umass_desc_t)0)->intf_desc);
...
struct udclass msc_class = {.rt_usbd_function_create =
