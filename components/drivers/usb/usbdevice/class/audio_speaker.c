...
#define AUDIO_PER_MS_SZ                                                        \
  ((AUDIO_SAMPLERATE * AUDIO_CHANNEL * RESOLUTION_BYTE) / 1000)
...
rt_align(4) const static char *_ustring[] = {
    "Language",       "RT-Thread Team.", "RT-Thread Audio Speaker",
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (e & EVENT_AUDIO_DATA) {
        index = (speaker.buffer_index >= AUDIO_BUFFER_SZ / 2)
                    ? 0
                    : (AUDIO_BUFFER_SZ / 2);
        rt_device_write(speaker.dev, 0, speaker.buffer + index,
                        AUDIO_BUFFER_SZ / 2);
...
int audio_speaker_init(void) {
  ...
  speaker_tid =
      rt_thread_create("speaker_thread", speaker_entry, RT_NULL, 1024, 5, 10);
