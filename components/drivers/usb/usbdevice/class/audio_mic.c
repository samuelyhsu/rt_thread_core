...
#define RECORD_PER_MS_SZ                                                       \
  ((RECORD_SAMPLERATE * RECORD_CHANNEL * RESOLUTION_BYTE) / 1000)
...
rt_align(4) const static char *_ustring[] = {
    "Language",       "RT-Thread Team.", "RT-Thread Audio Microphone",
    "32021919830108", "Configuration",   "Interface",
};

rt_align(4) static struct uac_ac_descriptor ac_desc = {
#ifdef RT_USB_DEVICE_COMPOSITE
    ...
    {
        sizeof(struct usb_audio_input_terminal), UAC_CS_INTERFACE,
        UDESCSUB_AC_INPUT, 0x01, /* Terminal ID: 1 */
    ...
    {
        sizeof(struct usb_audio_output_terminal), UAC_CS_INTERFACE,
        UDESCSUB_AC_OUTPUT, 0x02, /* Terminal ID: 2 */
...
rt_align(4) static struct uinterface_descriptor as_desc0 = {
    USB_DESC_LENGTH_INTERFACE,
    USB_DESC_TYPE_INTERFACE,
    USB_DYNAMIC,
    0x00,
...
rt_align(4) static struct uac_as_descriptor as_desc = {
    /* Interface Descriptor */
    {
        USB_DESC_LENGTH_INTERFACE,
        USB_DESC_TYPE_INTERFACE,
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (e & EVENT_RECORD_DATA) {
        index = (mic.buffer_index >= RECORD_BUFFER_SZ / 2)
                    ? 0
                    : (RECORD_BUFFER_SZ / 2);
...
static rt_err_t _uac_samplerate_config(struct uac_as_descriptor *as,
...
ufunction_t rt_usbd_function_uac_mic_create(udevice_t device) {
  ...
  _uac_descriptor_config(setting_ac->desc, intf_ac->intf_num, setting_as->desc,
                         intf_as->intf_num);
  ...
  /* add the alternate setting to the interface, then set default setting of the
   * interface */
...
int audio_mic_init(void) {
  ...
  mic_tid = rt_thread_create("mic_thread", mic_entry, RT_NULL, 1024, 5, 10);
 ...
 *  register uac class
 */
struct udclass uac_class = {.rt_usbd_function_create =
