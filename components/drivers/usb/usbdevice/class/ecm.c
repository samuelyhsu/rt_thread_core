...
struct rt_ecm_eth {
  ...
  rt_align(4) rt_size_t rx_size;
  rt_align(4) rt_size_t rx_offset;
  rt_align(4) char rx_buffer[USB_ETH_MTU];
...
rt_align(4) const static struct ucdc_eth_descriptor _comm_desc = {
...
rt_align(4) const static char *_ustring[] = {
    "Language",             /* LANGID */
    "RT-Thread Team.",      /* MANU */
...
static rt_err_t _cdc_send_notifi(ufunction_t func,
                                 ucdc_notification_code_t notifi,
                                 rt_uint16_t wValue, rt_uint16_t wLength) {
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size) {
  rt_ecm_eth_t ecm_device = (rt_ecm_eth_t)func->user_data;
  rt_memcpy((void *)(ecm_device->rx_buffer + ecm_device->rx_offset),
  ...
  if (size < EP_MAXPACKET(ecm_device->eps.ep_out)) {
    ...
    rt_usbd_io_request(ecm_device->func->device, ecm_device->eps.ep_out,
                       &ecm_device->eps.ep_out->request);
  }

  return RT_EOK;
}
static rt_err_t rt_ecm_eth_init(rt_device_t dev) { return RT_EOK; }

static rt_err_t rt_ecm_eth_open(rt_device_t dev, rt_uint16_t oflag) {
  return RT_EOK;
}

static rt_err_t rt_ecm_eth_close(rt_device_t dev) { return RT_EOK; }

static rt_ssize_t rt_ecm_eth_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                  rt_size_t size) {
  rt_set_errno(-RT_ENOSYS);
  return 0;
}

static rt_ssize_t rt_ecm_eth_write(rt_device_t dev, rt_off_t pos,
...
const static struct rt_device_ops ecm_device_ops = {
    rt_ecm_eth_init, rt_ecm_eth_open,  rt_ecm_eth_close,
    rt_ecm_eth_read, rt_ecm_eth_write, rt_ecm_eth_control};
#endif
...
struct pbuf *rt_ecm_eth_rx(rt_device_t dev) {
  ...
  if (ecm_eth_dev->rx_size != 0) {
    ...
    if (p != RT_NULL) {
      ...
      for (q = p; q != RT_NULL; q = q->next) {
        ...
        rt_memcpy(q->payload, (rt_uint8_t *)((ecm_eth_dev->rx_buffer) + offset),
                  q->len);
        offset += q->len;
      }
    }
...

  {
    if (ecm_eth_dev->func->device->state == USB_STATE_CONFIGURED) {
      ...
      ecm_eth_dev->eps.ep_out->request.size =
          EP_MAXPACKET(ecm_eth_dev->eps.ep_out);
      ecm_eth_dev->eps.ep_out->request.req_type = UIO_REQUEST_READ_BEST;
      rt_usbd_io_request(ecm_eth_dev->func->device, ecm_eth_dev->eps.ep_out,
                         &ecm_eth_dev->eps.ep_out->request);
    }
  }
...
rt_err_t rt_ecm_eth_tx(rt_device_t dev, struct pbuf *p) {
  ...
  if (p->tot_len > USB_ETH_MTU) {
    LOG_W("ECM MTU is:%d, but the send packet size is %d", USB_ETH_MTU,
          p->tot_len);
    p->tot_len = USB_ETH_MTU;
  }

  result =
      rt_sem_take(&ecm_eth_dev->tx_buffer_free, rt_tick_from_millisecond(1000));
  if (result != RT_EOK) {
...

  {
    if (ecm_eth_dev->func->device->state == USB_STATE_CONFIGURED) {
      ...
      rt_usbd_io_request(ecm_eth_dev->func->device, ecm_eth_dev->eps.ep_in,
                         &ecm_eth_dev->eps.ep_in->request);
    }
  }
...
ufunction_t rt_usbd_function_ecm_create(udevice_t device) {
  ...
  rt_usbd_altsetting_config_descriptor(
      comm_setting, &_comm_desc, (rt_off_t) & ((ucdc_eth_desc_t)0)->intf_desc);
...
struct udclass ecm_class = {.rt_usbd_function_create =
