...
/**
 * This function will do USB_REQ_SET_IDLE request to set idle period to the usb
 * hid device
 *
 ...
 */
rt_err_t rt_usbh_hid_set_idle(struct uhintf *intf, int duration,
                              int report_id) {
...
/**
 * This function will do USB_REQ_GET_REPORT request to get report from the usb
 * hid device
 *
 * @param intf the interface instance.
...
rt_err_t rt_usbh_hid_get_report(struct uhintf *intf, rt_uint8_t type,
                                rt_uint8_t id, rt_uint8_t *buffer,
                                rt_size_t size) {
  struct urequest setup;
  struct uinstance *device;
  int timeout = USB_TIMEOUT_BASIC;
...
/**
 * This function will do USB_REQ_SET_REPORT request to set report to the usb hid
 * device
 *
 * @param intf the interface instance.
 * @buffer the data buffer to save usb report descriptor.
...
/**
 * This function will do USB_REQ_SET_PROTOCOL request to set protocal to the usb
 * hid device.
 *
 * @param intf the interface instance.
...
rt_err_t rt_usbh_hid_get_report_descriptor(struct uhintf *intf,
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_STANDARD | USB_REQ_TYPE_INTERFACE;
...
/**
 * This function is the callback function of hid's int endpoint, it is invoked
 * when data comes.
 *
...
/**
 * This function will run hid class driver when usb device is detected and
 * identified as a hid class device, it will continue the enumulate process.
 *
...
static rt_err_t rt_usbh_hid_enable(void *arg) {
  ...
  for (i = 0; i < intf->intf_desc->bNumEndpoints; i++) {
    ...
    ret = rt_usb_hcd_alloc_pipe(intf->device->hcd, &hid->pipe_in, intf->device,
                                ep_desc);
    if (ret != RT_EOK)
      return ret;
  }
