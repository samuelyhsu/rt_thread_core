...
/**
 * This function will do USB_REQ_GET_PROTOCOL request to set idle period to the
 * usb adk device
 *
 ...
 */
static rt_err_t rt_usbh_adk_get_protocol(struct uintf *intf,
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_VENDOR | USB_REQ_TYPE_DEVICE;
...
/**
 * This function will do USB_REQ_SEND_STRING request to set idle period to the
 * usb adk device
 *
 * @param intf the interface instance.
 * @duration the idle period of requesting data.
...
static rt_err_t rt_usbh_adk_send_string(struct uintf *intf, rt_uint16_t index,
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_VENDOR | USB_REQ_TYPE_DEVICE;
...
/**
 * This function will do USB_REQ_START request to set idle period to the usb adk
 * device
 *
 * @param intf the interface instance.
 * @duration the idle period of requesting data.
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_ssize_t rt_usbh_adk_read(rt_device_t device, rt_off_t pos,
  ...
  length =
      rt_usb_hcd_bulk_xfer(intf->device->hcd, adk->pipe_in, buffer, size, 300);
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_ssize_t rt_usbh_adk_write(rt_device_t device, rt_off_t pos,
...
const static struct rt_device_ops adk_device_ops = {RT_NULL;
RT_NULL;
RT_NULL;
rt_usbh_adk_read;
rt_usbh_adk_write;
...
/**
 * This function will run adk class driver when usb device is detected and
 * identified as a adk class device, it will continue the enumulate process.
 *
...
static rt_err_t rt_usbh_adk_enable(void *arg) {
  ...
  if (dev_desc->idVendor == USB_ACCESSORY_VENDOR_ID &&
    ...
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_MANUFACTURER,
                            _adk_manufacturer);
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_MODEL, _adk_model);
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_DESCRIPTION,
                            _adk_description);
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_VERSION, _adk_version);
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_URI, _adk_uri);
    rt_usbh_adk_send_string(intf, ACCESSORY_STRING_SERIAL, _adk_serial);
  ...
  for (i = 0; i < intf->intf_desc->bNumEndpoints; i++) {
    ...
    if (ep_desc->bEndpointAddress & USB_DIR_IN) {
      ...
      ret = rt_usb_hcd_alloc_pipe(intf->device->hcd, &adk->pipe_in, intf,
                                  ep_desc, RT_NULL);
      if (ret != RT_EOK)
        return ret;
    } else {
      /* allocate an output pipe for the adk instance */
      ret = rt_usb_hcd_alloc_pipe(intf->device->hcd, &adk->pipe_out, intf,
                                  ep_desc, RT_NULL);
      if (ret != RT_EOK)
        return ret;
    }
...
/**
 * This function will be invoked when usb device plug out is detected and it
 * would clean and release all hub class related resources.
 *
 * @param arg the argument.
 *
 * @return the error code, RT_EOK on successfully.
