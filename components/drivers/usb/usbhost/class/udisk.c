 ...
 * @return RT_EOK
 */
static rt_err_t rt_udisk_init(rt_device_t dev) { return RT_EOK; }
 ...
 */
static rt_ssize_t rt_udisk_write(rt_device_t dev, rt_off_t pos,
...
const static struct rt_device_ops udisk_device_ops = {
    rt_udisk_init, RT_NULL,        RT_NULL,
    rt_udisk_read, rt_udisk_write, rt_udisk_control};
#endif
...
rt_err_t rt_udisk_run(struct uhintf *intf) {
  ...
  /* set interface */
  //    ret = rt_usbh_set_interface(intf->device,
  //    intf->intf_desc->bInterfaceNumber); if(ret != RT_EOK)
  ...
  if (stor->pipe_in->status == UPIPE_STATUS_STALL) {
    ret =
        rt_usbh_clear_feature(intf->device, stor->pipe_in->ep.bEndpointAddress,
  ...
  if (stor->pipe_out->status == UPIPE_STATUS_STALL) {
    ret =
        rt_usbh_clear_feature(intf->device, stor->pipe_out->ep.bEndpointAddress,
                              USB_FEATURE_ENDPOINT_HALT);
    if (ret != RT_EOK)
      return ret;
  }
  ...
  while ((ret = rt_usbh_storage_get_capacity(
              intf, (rt_uint8_t *)stor->capicity)) != RT_EOK) {
    if (ret == -RT_EIO)
      return ret;
  ...
  for (i = 0; i < MAX_PARTITION_COUNT; i++) {
    ...
    if (ret == RT_EOK) {
      ...
      rt_device_register(&stor->dev[i], dname,
                         RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_REMOVABLE |
                             RT_DEVICE_FLAG_STANDALONE);

      stor->dev_cnt++;
      if (dfs_mount(stor->dev[i].parent.name, UDISK_MOUNTPOINT, "elm", 0, 0) ==
          0) {
      ...
      if (i == 0) {
        ...
        stor->dev_cnt++;
        if (dfs_mount(stor->dev[0].parent.name, UDISK_MOUNTPOINT, "elm", 0,
                      0) == 0) {
...
/**
 * This function will be invoked when usb disk plug out is detected and it would
 * clean and release all udisk related resources.
 *
 * @param intf the usb interface instance.
 *
 * @return the error code, RT_EOK on successfully.
