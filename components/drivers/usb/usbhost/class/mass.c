...
/**
 * This function will do USBREQ_GET_MAX_LUN request for the usb interface
 * instance.
 *
...
static rt_err_t _pipe_check(struct uhintf *intf, upipe_t pipe) {
  ...
  size = rt_usb_hcd_pipe_xfer(stor->pipe_in->inst->hcd, stor->pipe_in, &csw,
                              SIZEOF_CSW, 100);
  if (size != SIZEOF_CSW) {
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_err_t rt_usb_bulk_only_xfer(struct uhintf *intf, ustorage_cbw_t cmd,
                                      rt_uint8_t *buffer, int timeout) {
  rt_size_t size;
  rt_err_t ret;
  upipe_t pipe;
  struct ustorage_csw csw;
  ...
  do {
    /* send the cbw */
    size = rt_usb_hcd_pipe_xfer(stor->pipe_out->inst->hcd, stor->pipe_out, cmd,
                                SIZEOF_CBW, timeout);
    if (size != SIZEOF_CBW) {
...
rt_err_t rt_usbh_storage_get_max_lun(struct uhintf *intf, rt_uint8_t *max_lun) {
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_TYPE_INTERFACE;
  ...
  if (rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_in, max_lun, 1,
                           timeout) != 1) {
    return -RT_EIO;
  }
...
/**
 * This function will do USBREQ_MASS_STORAGE_RESET request for the usb interface
 * instance.
 *
 * @param intf the interface instance.
 *
...
rt_err_t rt_usbh_storage_reset(struct uhintf *intf) {
  ...
  /* construct the request */
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_CLASS | USB_REQ_TYPE_INTERFACE;
...
/**
 * This function will execute SCSI_READ_10 command to read data from the usb
 * device.
 *
 * @param intf the interface instance.
...
rt_err_t rt_usbh_storage_read10(struct uhintf *intf, rt_uint8_t *buffer,
                                rt_uint32_t sector, rt_size_t count,
                                int timeout) {
...
/**
 * This function will execute SCSI_WRITE_10 command to write data to the usb
 * device.
 *
 * @param intf the interface instance.
 ...
 * @return the error code, RT_EOK on successfully.
 */
rt_err_t rt_usbh_storage_request_sense(struct uhintf *intf,
                                       rt_uint8_t *buffer) {
  struct ustorage_cbw cmd;
...
/**
 * This function will execute SCSI_TEST_UNIT_READY command to get unit ready
 * status.
 *
 * @param intf the interface instance.
 *
 ...
 * and identified as a mass storage class device, it will continue to do the
 * enumulate process.
 *
...
static rt_err_t rt_usbh_storage_enable(void *arg) {
  ...
  for (i = 0; i < intf->intf_desc->bNumEndpoints; i++) {
    ...
    if (ep_desc->bEndpointAddress & USB_DIR_IN) {
      ...
      stor->pipe_in =
          rt_usb_instance_find_pipe(intf->device, ep_desc->bEndpointAddress);
    } else {
 ...
 * would clean and release all mass storage class related resources.
 *
 * @param arg the argument.
 *
 * @return the error code, RT_EOK on successfully.
...
/**
 * This function will register mass storage class driver to the usb class driver
 * manager. and it should be invoked in the usb system initialization.
 *
 * @return the error code, RT_EOK on successfully.
 */
