...
rt_err_t rt_usbh_class_driver_register(ucd_t drv) {
  ...
  if (rt_usbh_class_driver_find(drv->class_code, drv->subclass_code) ==
      RT_NULL) {
...
/**
 * This function finds a usb class driver by specified class code and subclass
 * code.
 *
...
ucd_t rt_usbh_class_driver_find(int class_code, int subclass_code) {
  ...
  for (node = _driver_list.next; node != &_driver_list; node = node->next) {
    ucd_t drv = (ucd_t)rt_list_entry(node, struct uclass_driver, list);
