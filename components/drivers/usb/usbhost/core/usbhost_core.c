...
rt_err_t rt_usbh_attatch_instance(uinst_t device) {
  ...
  rt_usb_hcd_alloc_pipe(device->hcd, &device->pipe_ep0_out, device,
                        &ep0_out_desc);
  rt_usb_hcd_alloc_pipe(device->hcd, &device->pipe_ep0_in, device,
                        &ep0_in_desc);
  ...
  ret =
      rt_usbh_get_descriptor(device, USB_DESC_TYPE_DEVICE, (void *)dev_desc, 8);
  if (ret != RT_EOK) {
  ...
  rt_thread_delay(2);
  rt_usbh_hub_clear_port_feature(device->parent_hub, i + 1,
  ...
  ret = rt_usbh_get_descriptor(device, USB_DESC_TYPE_DEVICE, (void *)dev_desc,
                               dev_desc->bLength);
  if (ret != RT_EOK) {
  ...
  ret = rt_usbh_get_descriptor(device, USB_DESC_TYPE_CONFIGURATION, &cfg_desc,
                               18);
  if (ret != RT_EOK) {
  ...
  for (i = 0; i < device->cfg_desc->bNumInterfaces; i++) {
    ...
    for (ep_index = 0; ep_index < intf_desc->bNumEndpoints; ep_index++) {
      ...
      if (ep_desc != RT_NULL) {
        if (rt_usb_hcd_alloc_pipe(device->hcd, &pipe, device, ep_desc) !=
            RT_EOK) {
...
rt_err_t rt_usbh_detach_instance(uinst_t device) {
  ...
  if (device->cfg_desc) {
    for (i = 0; i < device->cfg_desc->bNumInterfaces; i++) {
      ...
      rt_usbh_class_driver_disable(device->intf[i]->drv,
                                   (void *)device->intf[i]);
...
/**
 * This function will do USB_REQ_GET_DESCRIPTO' bRequest for the usb device
 * instance,
 *
 * @param device the usb device instance.
...
rt_err_t rt_usbh_get_descriptor(uinst_t device, rt_uint8_t type, void *buffer,
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_STANDARD | USB_REQ_TYPE_DEVICE;
...
rt_err_t rt_usbh_set_address(uinst_t device) {
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_STANDARD | USB_REQ_TYPE_DEVICE;
...
rt_err_t rt_usbh_set_interface(uinst_t device, int intf) {
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_STANDARD | USB_REQ_TYPE_INTERFACE;
...
rt_err_t rt_usbh_clear_feature(uinst_t device, int endpoint, int feature) {
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_STANDARD | USB_REQ_TYPE_ENDPOINT;
...
/**
 * This function will get an interface descriptor from the configuration
 * descriptor.
 *
...
int rt_usb_hcd_pipe_xfer(uhcd_t hcd, upipe_t pipe, void *buffer, int nbytes,
                         int timeout) {
  ...
  do {
    RT_DEBUG_LOG(RT_DEBUG_USB,
                 ("pipe transform remain size,: %d\n", remain_size));
    send_size = (remain_size > pipe->ep.wMaxPacketSize)
                    ? pipe->ep.wMaxPacketSize
                    : remain_size;
    if (hcd->ops->pipe_xfer(pipe, USBH_PID_DATA, pbuffer, send_size, timeout) ==
        send_size) {
