...
static rt_err_t root_hub_ctrl(struct uhcd *hcd, rt_uint16_t port,
...
rt_err_t rt_usbh_hub_get_descriptor(struct uinstance *device,
                                    rt_uint8_t *buffer, rt_size_t nbytes) {
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_TYPE_DEVICE;
  ...
  if (rt_usb_hcd_setup_xfer(device->hcd, device->pipe_ep0_out, &setup,
                            timeout) == 8) {
    if (rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_in, buffer, nbytes,
                             timeout) == nbytes) {
      return RT_EOK;
    }
  }
...
rt_err_t rt_usbh_hub_get_status(struct uinstance *device, rt_uint32_t *buffer) {
  ...
  if (rt_usb_hcd_setup_xfer(device->hcd, device->pipe_ep0_out, &setup,
                            timeout) == 8) {
    if (rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_in, buffer, 4,
                             timeout) == 4) {
      return RT_EOK;
    }
  }
...
rt_err_t rt_usbh_hub_get_port_status(uhub_t hub, rt_uint16_t port,
  ...
  if (hub->is_roothub) {
    root_hub_ctrl(hub->hcd, port, RH_GET_PORT_STATUS, (void *)buffer);
    return RT_EOK;
  }

  setup.request_type =
      USB_REQ_TYPE_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_TYPE_OTHER;
  setup.bRequest = USB_REQ_GET_STATUS;
  setup.wIndex = port;
  setup.wLength = 4;
  setup.wValue = 0;

  if (rt_usb_hcd_setup_xfer(hub->hcd, hub->self->pipe_ep0_out, &setup,
                            timeout) == 8) {
    if (rt_usb_hcd_pipe_xfer(hub->hcd, hub->self->pipe_ep0_in, buffer, 4,
                             timeout) == 4) {
      return RT_EOK;
    }
  }
...
rt_err_t rt_usbh_hub_clear_port_feature(uhub_t hub, rt_uint16_t port,
  ...
  setup.request_type =
      USB_REQ_TYPE_DIR_OUT | USB_REQ_TYPE_CLASS | USB_REQ_TYPE_OTHER;
...
/**
 * This function will rest hub port, it is invoked when sub device attached to
 * the hub port.
 *
 * @param intf the interface instance.
...
/**
 * This function will do debouce, it is invoked when sub device attached to the
 * hub port.
 *
...
/**
 * This function will poll all the hub ports to detect port status, especially
  ...
  for (i = 0; i < hub->num_ports; i++) {
    ...
    if (reconnect) {
      if (hub->child[i] != RT_NULL &&
          hub->child[i]->status != DEV_STATUS_IDLE) {
...
/**
 * This function is the callback function of hub's int endpoint, it is invoked
 * when data comes.
 *
...
static void rt_usbh_hub_irq(void *context) {
  ...
  rt_usb_hcd_pipe_xfer(hub->self->hcd, pipe, hub->buffer,
                       pipe->ep.wMaxPacketSize, timeout);
}

/**
 * This function will run usb hub class driver when usb hub is detected and
 * identified as a hub class device, it will continue to do the enumulate
 * process.
 *
...
static rt_err_t rt_usbh_hub_enable(void *arg) {
  ...
  /* If hub device supported ports over USB_HUB_PORT_NUM(Ex: 8 port hub). Set
   * hub->num_ports to USB_HUB_PORT_NUM */
  ...
  for (i = 0; i < hub->num_ports; i++) {
    ...
    rt_thread_delay(hub->hub_desc.pwron_to_good * 2 * RT_TICK_PER_SECOND /
                    1000);
  }
...
/**
 * This function will be invoked when usb hub plug out is detected and it would
 * clean and release all hub class related resources.
 *
 * @param arg the argument.
 *
 * @return the error code, RT_EOK on successfully.
...
void rt_usbh_hub_init(uhcd_t hcd) {
  ...
  thread = rt_thread_create(hcd->parent.parent.name, rt_usbh_hub_thread_entry,
                            hcd, USB_THREAD_STACK_SIZE, 8, 20);
  if (thread != RT_NULL) {
