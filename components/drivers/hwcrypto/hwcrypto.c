...
rt_err_t rt_hwcrypto_set_type(struct rt_hwcrypto_ctx *ctx, hwcrypto_type type) {
  if (ctx) {
    ...
    if ((ctx->type & HWCRYPTO_MAIN_TYPE_MASK) ==
        (type & HWCRYPTO_MAIN_TYPE_MASK)) {
      ctx->type = type;
      return RT_EOK;
    }
...
rt_err_t rt_hwcrypto_ctx_cpy(struct rt_hwcrypto_ctx *des,
  ...
  if (des->device != src->device || (des->type & HWCRYPTO_MAIN_TYPE_MASK) !=
                                        (src->type & HWCRYPTO_MAIN_TYPE_MASK)) {
    return -RT_EINVAL;
  }
  des->type = src->type;
...
struct rt_hwcrypto_device *rt_hwcrypto_dev_default(void) {
  ...
  hwcrypto_dev =
      (struct rt_hwcrypto_device *)rt_device_find(RT_HWCRYPTO_DEFAULT_NAME);
  return hwcrypto_dev;
}
...
const static struct rt_device_ops hwcrypto_ops = {RT_NULL, RT_NULL, RT_NULL,
