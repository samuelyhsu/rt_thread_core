...
#define HWCRYPTO_CRC8_CFG                                                      \
  { .last_val = 0x00, .poly = 0x07, .width = 8, .xorout = 0x00, .flags = 0, }

#define HWCRYPTO_CRC16_CFG                                                     \
  {                                                                            \
    .last_val = 0x0000, .poly = 0x8005, .width = 16, .xorout = 0x0000,         \
    .flags = 0,                                                                \
  }

#define HWCRYPTO_CRC32_CFG                                                     \
  {                                                                            \
    .last_val = 0x00000000, .poly = 0x04C11DB7, .width = 32,                   \
    .xorout = 0x00000000, .flags = 0,                                          \
  }

#define HWCRYPTO_CRC_CCITT_CFG                                                 \
  {                                                                            \
    .last_val = 0x0000, .poly = 0x1021, .width = 16, .xorout = 0x0000,         \
    .flags = CRC_FLAG_REFIN | CRC_FLAG_REFOUT,                                 \
  }

#define HWCRYPTO_CRC_DNP_CFG                                                   \
  {                                                                            \
    .last_val = 0x0000, .poly = 0x3D65, .width = 16, .xorout = 0xffff,         \
    .flags = CRC_FLAG_REFIN | CRC_FLAG_REFOUT,                                 \
  }
...
struct hwcrypto_crc_cfg {
  ...
  rt_uint16_t flags;    /**< Input or output data reverse. CRC_FLAG_REFIN or
                           CRC_FLAG_REFOUT */
};

struct hwcrypto_crc_ops {
  rt_uint32_t (*update)(
      struct hwcrypto_crc *ctx, const rt_uint8_t *in,
      rt_size_t length); /**< Perform a CRC calculation. return CRC value */
};
...
rt_hwcrypto_crc_create(struct rt_hwcrypto_device *device,
