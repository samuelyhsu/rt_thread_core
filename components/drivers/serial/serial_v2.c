 ...
 */
rt_size_t _serial_poll_rx(struct rt_device *dev, rt_off_t pos, void *buffer,
                          rt_size_t size) {
  struct rt_serial_device *serial;
  rt_size_t getc_size;
 ...
 * @return Return the final length of data received.
 */
rt_size_t _serial_poll_tx(struct rt_device *dev, rt_off_t pos,
  ...
  while (size) {
    if (serial->parent.open_flag & RT_DEVICE_FLAG_STREAM) {
      /* If open_flag satisfies RT_DEVICE_FLAG_STREAM and the received character
       * is '\n', inserts '\r' character before '\n' character for the effect of
 ...
 * @return Return the final length of data received.
 */
static rt_ssize_t _serial_fifo_rx(struct rt_device *dev, rt_off_t pos,
...
static rt_ssize_t _serial_fifo_tx_blocking_nbuf(struct rt_device *dev,
  ...
  serial->ops->transmit(serial, (rt_uint8_t *)buffer, size,
...
static rt_ssize_t _serial_fifo_tx_blocking_buf(struct rt_device *dev,
                                               rt_off_t pos, const void *buffer,
                                               rt_size_t size) {
  struct rt_serial_device *serial;
  struct rt_serial_tx_fifo *tx_fifo = RT_NULL;
  ...
  while (size) {
    ...
    tx_fifo->put_size =
        rt_ringbuffer_put(&(tx_fifo->rb), (rt_uint8_t *)buffer + offset, size);

    offset += tx_fifo->put_size;
    size -= tx_fifo->put_size;
    /* Call the transmit interface for transmission */
    serial->ops->transmit(serial, (rt_uint8_t *)buffer + offset,
                          tx_fifo->put_size, RT_SERIAL_TX_BLOCKING);
    /* Waiting for the transmission to complete */
    rt_completion_wait(&(tx_fifo->tx_cpt), RT_WAITING_FOREVER);
  }
...
static rt_ssize_t _serial_fifo_tx_nonblocking(struct rt_device *dev,
  ...
  if (tx_fifo->activated == RT_FALSE) {
    /* When serial transmit in tx_non_blocking mode, if the activated mode is
     * RT_FALSE, start copying data into the ringbuffer */
    tx_fifo->activated = RT_TRUE;
    ...
    /* Call the transmit interface for transmission */
    serial->ops->transmit(serial, put_ptr, tx_fifo->put_size,
                          RT_SERIAL_TX_NON_BLOCKING);
    /* In tx_nonblocking mode, there is no need to call rt_completion_wait()
     * APIs to wait for the rt_current_thread to resume */
    return length;
  }

  /* If the activated mode is RT_FALSE, it means that serial device is
   * transmitting, where only the data in the ringbuffer and there is no need to
   * call the transmit() API. Note that this part of the code requires disable
   * interrupts to prevent multi thread reentrant */
...
static rt_err_t rt_serial_tx_enable(struct rt_device *dev,
  ...
  if (tx_oflag == RT_SERIAL_TX_BLOCKING) {
    ...
    if (optmode == RT_SERIAL_TX_BLOCKING_BUFFER) {
      ...
      rt_ringbuffer_init(&(tx_fifo->rb), tx_fifo->buffer,
      ...
      /* Call the control() API to configure the serial device by
       * RT_SERIAL_TX_BLOCKING*/
  ...
  tx_fifo->activated = RT_FALSE;
  tx_fifo->put_size = 0;
  rt_ringbuffer_init(&(tx_fifo->rb), tx_fifo->buffer, serial->config.tx_bufsz);
  serial->serial_tx = tx_fifo;
...
static rt_err_t rt_serial_rx_enable(struct rt_device *dev,
  ...
  /* When using RT_SERIAL_RX_BLOCKING, rt_completion_init() and rx_cpt_index are
   * initialized */
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag) {
  ...
  if ((dev->open_flag) & (15 << 12)) {
    LOG_D("(%s) serial device has already been opened, it will run in its "
  ...
  /* By default, the receive mode of a serial devide is
   * RT_SERIAL_RX_NON_BLOCKING */
  ...
  if (serial->serial_rx == RT_NULL)
    rt_serial_rx_enable(dev, dev->open_flag & (RT_SERIAL_RX_BLOCKING |
                                               RT_SERIAL_RX_NON_BLOCKING));

  /* initialize the Tx structure according to open flag */
  if (serial->serial_tx == RT_NULL)
    rt_serial_tx_enable(dev, dev->open_flag & (RT_SERIAL_TX_BLOCKING |
                                               RT_SERIAL_TX_NON_BLOCKING));
...
static rt_err_t rt_serial_close(struct rt_device *dev) {
  ...
  rt_serial_rx_disable(dev, dev->open_flag & (RT_SERIAL_RX_BLOCKING |
                                              RT_SERIAL_RX_NON_BLOCKING));
  /* Disable serial tranmit mode. */
  rt_serial_tx_disable(dev, dev->open_flag & (RT_SERIAL_TX_BLOCKING |
                                              RT_SERIAL_TX_NON_BLOCKING));
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event) {
  ...
  case RT_SERIAL_EVENT_RX_DMADONE: {
    ...
    if (rx_length) { /* RT_SERIAL_EVENT_RX_DMADONE MODE */
      level = rt_hw_interrupt_disable();
  ...
  case RT_SERIAL_EVENT_TX_DONE: {
    ...
    serial->ops->transmit(
        serial, tx_fifo->buffer, tx_length,
