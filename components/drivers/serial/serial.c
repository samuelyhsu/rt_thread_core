 ...
 * 2015-05-19     Quintin      fix DMA tx mod tx_dma->activated flag !=RT_FALSE
 * BUG in open function. 2015-11-10     bernard      fix the poll rx issue when
 * there is no data. 2016-05-10     armink       add fifo mode to DMA rx when
 * serial->config.bufsz != 0. 2017-01-19     aubr.cool    prevent change serial
 * rx bufsz when serial is opened. 2017-11-07     JasonJia     fix data bits
 * error issue when using tcsetattr. 2017-11-15     JasonJia     fix poll rx
 * issue when data is full. add TCFLSH and FIONREAD support. 2018-12-08 Ernest
 * Chen  add DMA choice 2020-09-14     WillianChan  add a line feed to the
 * carriage return character when using interrupt tx 2020-12-14     Meco Man
 * implement function of setting window's size(TIOCSWINSZ) 2021-08-22     Meco
 * Man     implement function of getting window's size(TIOCGWINSZ)
 */
...
static int serial_fops_read(struct dfs_fd *fd, void *buf, size_t count) {
  ...
  do {
    ...
    if (size <= 0) {
      ...
      wait_ret = rt_wqueue_wait_interruptible(&(device->wait_queue), 0,
                                              RT_WAITING_FOREVER);
      if (wait_ret != RT_EOK) {
        break;
      }
...
static const struct dfs_file_ops _serial_fops = {
    serial_fops_open, serial_fops_close, serial_fops_ioctl,
    serial_fops_read, serial_fops_write, RT_NULL, /* flush */
    RT_NULL,                                      /* lseek */
    RT_NULL,                                      /* getdents */
    serial_fops_poll,
};
 ...
 */
rt_inline int _serial_poll_rx(struct rt_serial_device *serial, rt_uint8_t *data,
                              int length) {
  int ch;
  int size;
 ...
 */
rt_inline int _serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data,
                             int length) {
  int size;
  struct rt_serial_rx_fifo *rx_fifo;
...
static rt_ssize_t
_serial_fifo_calc_recved_len(struct rt_serial_device *serial) {
  struct rt_serial_rx_fifo *rx_fifo =
      (struct rt_serial_rx_fifo *)serial->serial_rx;
 ...
 */
static void rt_dma_recv_update_get_index(struct rt_serial_device *serial,
                                         rt_size_t len) {
  struct rt_serial_rx_fifo *rx_fifo =
      (struct rt_serial_rx_fifo *)serial->serial_rx;
 ...
 */
static void rt_dma_recv_update_put_index(struct rt_serial_device *serial,
                                         rt_size_t len) {
  struct rt_serial_rx_fifo *rx_fifo =
      (struct rt_serial_rx_fifo *)serial->serial_rx;
 ...
 * Serial DMA routines
 */
rt_inline int _serial_dma_rx(struct rt_serial_device *serial, rt_uint8_t *data,
                             int length) {
  rt_base_t level;
  ...
  if (serial->config.bufsz == 0) {
    ...
    if (rx_fifo->get_index + recv_len < serial->config.bufsz)
      ...
      rt_memcpy(data + serial->config.bufsz - rx_fifo->get_index,
                rx_fifo->buffer,
...
rt_inline int _serial_dma_tx(struct rt_serial_device *serial,
                             const rt_uint8_t *data, int length) {
  rt_base_t level;
  rt_err_t result;
  ...
  result = rt_data_queue_push(&(tx_dma->data_queue), data, length,
                              RT_WAITING_FOREVER);
  if (result == RT_EOK) {
    level = rt_hw_interrupt_disable();
    if (tx_dma->activated != RT_TRUE) {
      ...
      /* make a DMA transfer */
      serial->ops->dma_transmit(serial, (rt_uint8_t *)data, length,
                                RT_SERIAL_DMA_TX);
    } else {
      rt_hw_interrupt_enable(level);
    }
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag) {
  ...
  if (serial->serial_rx == RT_NULL) {
    if (oflag & RT_DEVICE_FLAG_INT_RX) {
      ...
      serial->ops->control(serial, RT_DEVICE_CTRL_SET_INT,
      ...
      if (serial->config.bufsz == 0) {
        ...
        rx_dma = (struct rt_serial_rx_dma *)rt_malloc(
            sizeof(struct rt_serial_rx_dma));
        RT_ASSERT(rx_dma != RT_NULL);
  ...
  if (serial->serial_tx == RT_NULL) {
    if (oflag & RT_DEVICE_FLAG_INT_TX) {
      ...
      tx_dma =
          (struct rt_serial_tx_dma *)rt_malloc(sizeof(struct rt_serial_tx_dma));
...
static rt_err_t rt_serial_close(struct rt_device *dev) {
  ...
  if (dev->open_flag & RT_DEVICE_FLAG_INT_RX) {
    ...
    /* configure low level device */
    serial->ops->control(serial, RT_DEVICE_CTRL_CLR_INT,
                         (void *)RT_DEVICE_FLAG_INT_RX);
    dev->open_flag &= ~RT_DEVICE_FLAG_INT_RX;
...
static rt_ssize_t rt_serial_read(struct rt_device *dev, rt_off_t pos,
...
static rt_ssize_t rt_serial_write(struct rt_device *dev, rt_off_t pos,
...
static const struct speed_baudrate_item _tbl[] = {
    {B2400, BAUD_RATE_2400},       {B4800, BAUD_RATE_4800},
    {B9600, BAUD_RATE_9600},       {B19200, BAUD_RATE_19200},
    {B38400, BAUD_RATE_38400},     {B57600, BAUD_RATE_57600},
    {B115200, BAUD_RATE_115200},   {B230400, BAUD_RATE_230400},
    {B460800, BAUD_RATE_460800},   {B500000, BAUD_RATE_500000},
    {B921600, BAUD_RATE_921600},   {B2000000, BAUD_RATE_2000000},
...
static void _tc_flush(struct rt_serial_device *serial, int queue) {
  ...
  case TCIOFLUSH:
    ...
    if ((device->open_flag & RT_DEVICE_FLAG_INT_RX) ||
        (device->open_flag & RT_DEVICE_FLAG_DMA_RX)) {
...
#define _TIO_BUFLEN 20
      ...
      /* send the command to terminal for getting the window size of the
       * terminal */
      rt_kprintf("\033[18t");
      ...
      /* interpreting data eg: "\033[8;1;15t" which means row is 1 and col is 15
       * (unit: size of ONE character) */
      rt_memset(row_s, 0, 4);
      rt_memset(col_s, 0, 4);
      cnt1 = 0;
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event) {
  ...
  case RT_SERIAL_EVENT_TX_DMADONE: {
    ...
    if (rt_data_queue_peek(&(tx_dma->data_queue), &data_ptr, &data_size) ==
        RT_EOK) {
      /* transmit next data node */
      tx_dma->activated = RT_TRUE;
      serial->ops->dma_transmit(serial, (rt_uint8_t *)data_ptr, data_size,
                                RT_SERIAL_DMA_TX);
    } else {
      tx_dma->activated = RT_FALSE;
    }
