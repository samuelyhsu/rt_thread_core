...
static int _pm_device_suspend(rt_uint8_t mode) {
  ...
  for (index = 0; index < _pm.device_pm_number; index++) {
    if (_pm.device_pm[index].ops->suspend != RT_NULL) {
      ret =
          _pm.device_pm[index].ops->suspend(_pm.device_pm[index].device, mode);
      if (ret != RT_EOK)
        break;
    }
  }
...
/**
 * This function selects the sleep mode according to the
 * rt_pm_request/rt_pm_release count.
 */
...
static rt_bool_t _pm_device_check_idle(void) {
  ...
  for (int i = 0; i < PM_MODULE_MAX_ID; i++) {
    if (pm->module_status[i].busy_flag == RT_TRUE) {
      if (rt_tick_get() - pm->module_status[i].start_time >
          pm->module_status[i].timeout) {
 ...
 * @return none
 */
rt_weak rt_uint8_t pm_get_sleep_threshold_mode(rt_uint8_t cur_mode,
...
static void _pm_change_sleep_mode(struct rt_pm *pm) {
  ...
  if (_pm.sleep_mode == PM_SLEEP_MODE_NONE) {
    ...
    if (pm->timer_mask & (0x01 << pm->sleep_mode)) {
      ...
      pm->sleep_mode =
          pm_get_sleep_threshold_mode(pm->sleep_mode, timeout_tick);
...
void rt_pm_device_register(struct rt_device *device,
  ...
  device_pm = (struct rt_device_pm *)RT_KERNEL_REALLOC(
      _pm.device_pm, (_pm.device_pm_number + 1) * sizeof(struct rt_device_pm));
 ...
 */
void rt_pm_notify_set(void (*notify)(rt_uint8_t event, rt_uint8_t mode,
                                     void *data),
                      void *data) {
 ...
 */
static rt_ssize_t _rt_pm_device_read(rt_device_t dev, rt_off_t pos,
...
static rt_ssize_t _rt_pm_device_write(rt_device_t dev, rt_off_t pos,
...
static rt_err_t _rt_pm_device_control(rt_device_t dev, int cmd, void *args) {
  rt_uint32_t mode;
...
MSH_CMD_EXPORT_ALIAS(rt_pm_release_mode, pm_release,
                     release power management mode);
...
MSH_CMD_EXPORT_ALIAS(rt_pm_release_mode_all, pm_release_all,
                     release power management mode count);
...
MSH_CMD_EXPORT_ALIAS(rt_pm_request_mode, pm_request,
                     request power management mode);
...
MSH_CMD_EXPORT_ALIAS(rt_module_release_mode, pm_module_release,
                     release module power mode);
...
MSH_CMD_EXPORT_ALIAS(rt_module_release_mode_all, pm_module_release_all,
                     release power management mode count);
...
MSH_CMD_EXPORT_ALIAS(rt_module_request_mode, pm_module_request,
                     request power management mode);
...
MSH_CMD_EXPORT_ALIAS(rt_module_delay_sleep, pm_module_delay,
                     module request delay sleep);
...
MSH_CMD_EXPORT_ALIAS(rt_pm_run_mode_switch, pm_run,
                     switch power management run mode);
...
struct rt_pm *rt_pm_get_handle(void) { return &_pm; }
  ...
  for (index = 0; index < PM_MODULE_MAX_ID; index++) {
    if ((pm->module_status[index].busy_flag == RT_TRUE) ||
      ...
      rt_kprintf("|  %04d  |  %d   | 0x%08x | 0x%08x |\n", index,
                 pm->module_status[index].busy_flag,
...
FINSH_FUNCTION_EXPORT_ALIAS(rt_pm_dump_status, pm_dump,
                            dump power management status);
