...
static void sensor_show_data(rt_size_t num, rt_sensor_t sensor,
                             struct rt_sensor_data *sensor_data) {
  const char *unit_name = sensor_get_unit_name(&sensor->info);
  switch (sensor->info.type) {
  case RT_SENSOR_TYPE_ACCE:
    rt_kprintf("num:%d, x:%f, y:%f, z:%f %s, timestamp:%u\n", num,
               sensor_data->data.acce.x, sensor_data->data.acce.y,
               sensor_data->data.acce.z, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_GYRO:
    rt_kprintf("num:%d, x:%f, y:%f, z:%f %s, timestamp:%u\n", num,
               sensor_data->data.gyro.x, sensor_data->data.gyro.y,
               sensor_data->data.gyro.z, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_MAG:
    rt_kprintf("num:%d, x:%f, y:%f, z:%f %s, timestamp:%u\n", num,
               sensor_data->data.mag.x, sensor_data->data.mag.y,
               sensor_data->data.mag.z, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_GNSS:
    rt_kprintf("num:%d, lon:%f, lat:%f %s, timestamp:%u\n", num,
               sensor_data->data.coord.longitude,
               sensor_data->data.coord.latitude, unit_name,
               sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_TEMP:
    rt_kprintf("num:%d, temp:%f%s, timestamp:%u\n", num, sensor_data->data.temp,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_HUMI:
    rt_kprintf("num:%d, humi:%f%s, timestamp:%u\n", num, sensor_data->data.humi,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_BARO:
    rt_kprintf("num:%d, press:%f%s, timestamp:%u\n", num,
               sensor_data->data.baro, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_LIGHT:
    rt_kprintf("num:%d, light:%f%s, timestamp:%u\n", num,
               sensor_data->data.light, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_PROXIMITY:
  case RT_SENSOR_TYPE_TOF:
    rt_kprintf("num:%d, distance:%f%s, timestamp:%u\n", num,
               sensor_data->data.proximity, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_HR:
    rt_kprintf("num:%d, heart rate:%f%s, timestamp:%u\n", num,
               sensor_data->data.hr, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_TVOC:
    rt_kprintf("num:%d, tvoc:%f%s, timestamp:%u\n", num, sensor_data->data.tvoc,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_NOISE:
    rt_kprintf("num:%d, noise:%f%s, timestamp:%u\n", num,
               sensor_data->data.noise, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_STEP:
    rt_kprintf("num:%d, step:%f%s, timestamp:%u\n", num, sensor_data->data.step,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_FORCE:
    rt_kprintf("num:%d, force:%f%s, timestamp:%u\n", num,
               sensor_data->data.force, unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_DUST:
    rt_kprintf("num:%d, dust:%f%s, timestamp:%u\n", num, sensor_data->data.dust,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_ECO2:
    rt_kprintf("num:%d, eco2:%f%s, timestamp:%u\n", num, sensor_data->data.eco2,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_IAQ:
    rt_kprintf("num:%d, IAQ:%f%s, timestamp:%u\n", num, sensor_data->data.iaq,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_ETOH:
    rt_kprintf("num:%d, EtOH:%f%s, timestamp:%u\n", num, sensor_data->data.etoh,
               unit_name, sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_BP:
    rt_kprintf("num:%d, bp.sbp:%f, bp.dbp:%f %s, timestamp:%u\n", num,
               sensor_data->data.bp.sbp, sensor_data->data.bp.dbp, unit_name,
               sensor_data->timestamp);
    break;
  case RT_SENSOR_TYPE_NONE:
  default:
...
static void sensor_fifo_rx_entry(void *parameter) {
  ...
  data = (struct rt_sensor_data *)rt_calloc(sensor->info.fifo_max,
                                            sizeof(struct rt_sensor_data));
  if (data == RT_NULL) {
...
static void sensor_fifo(int argc, char **argv) {
  ...
  tid1 = rt_thread_create("sen_rx_thread", sensor_fifo_rx_entry, sensor, 1024,
                          15, 5);
...
static void sensor_int(int argc, char **argv) {
  ...
  tid1 = rt_thread_create("sen_rx_thread", sensor_irq_rx_entry, sensor, 1024,
                          15, 5);
...
static void sensor(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    rt_kprintf(
        "device name sensor name      sensor type     mode resolution range\n");
    rt_kprintf("----------- ------------- ------------------ ---- ---------- "
               "----------\n");
    for (node = information->object_list.next;
      ...
      rt_kprintf(
          "%-*.*s %-*s %-*s %u%u%u  %-*f %.*f - %.*f%-*s\n", RT_NAME_MAX + 3,
          RT_NAME_MAX, sensor_dev->parent.parent.name, 13,
          sensor_dev->info.name, 18, sensor_get_type_name(&sensor_dev->info),
          RT_SENSOR_MODE_GET_ACCURACY(sensor_dev->info.mode),
          RT_SENSOR_MODE_GET_POWER(sensor_dev->info.mode),
          RT_SENSOR_MODE_GET_FETCH(sensor_dev->info.mode), 10,
          sensor_dev->info.accuracy.resolution, 2,
          sensor_dev->info.scale.range_min, 2, sensor_dev->info.scale.range_max,
          5, sensor_get_unit_name(&sensor_dev->info));
    }
    ...
    if (argc == 2) {
      ...
      mode = atoi(argv[2]);
      if (rt_device_control(dev, RT_SENSOR_CTRL_SET_POWER_MODE, (void *)mode) ==
          RT_EOK) {
    ...
    if (argc == 2) {
      ...
      mode = atoi(argv[2]);
      if (rt_device_control(dev, RT_SENSOR_CTRL_SET_ACCURACY_MODE,
                            (void *)mode) == RT_EOK) {
    ...
    if (argc == 2) {
      ...
      mode = atoi(argv[2]);
      if (rt_device_control(dev, RT_SENSOR_CTRL_SET_FETCH_MODE, (void *)mode) ==
          RT_EOK) {
