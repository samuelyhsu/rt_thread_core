...
static void _sensor_cb(rt_sensor_t sen) {
  ...
  if (sen->data_len > 0) {
    sen->parent.rx_indicate(&sen->parent,
                            sen->data_len / sizeof(struct rt_sensor_data));
  } else if (RT_SENSOR_MODE_GET_FETCH(sen->info.mode) ==
...
static rt_err_t _sensor_irq_init(rt_sensor_t sensor) {
  ...
  if (sensor->config.irq_pin.mode == PIN_MODE_INPUT_PULLDOWN) {
    rt_pin_attach_irq(sensor->config.irq_pin.pin, PIN_IRQ_MODE_RISING,
                      _irq_callback, (void *)sensor);
  } else if (sensor->config.irq_pin.mode == PIN_MODE_INPUT_PULLUP) {
    rt_pin_attach_irq(sensor->config.irq_pin.pin, PIN_IRQ_MODE_FALLING,
                      _irq_callback, (void *)sensor);
  } else if (sensor->config.irq_pin.mode == PIN_MODE_INPUT) {
    rt_pin_attach_irq(sensor->config.irq_pin.pin, PIN_IRQ_MODE_RISING_FALLING,
                      _irq_callback, (void *)sensor);
  }
...
/* sensor local ops */
static rt_ssize_t _local_fetch_data(rt_sensor_t sensor, rt_sensor_data_t buf,
                                    rt_size_t len) {
...
static struct rt_sensor_ops local_ops = {.fetch_data = _local_fetch_data,
...
static rt_err_t _sensor_open(rt_device_t dev, rt_uint16_t oflag) {
  ...
  rt_err_t res = RT_EOK;
  rt_err_t (*local_ctrl)(rt_sensor_t sensor, int cmd, void *arg) =
      _local_control;
  ...
  if (sensor->module != RT_NULL && sensor->info.fifo_max > 0 &&
      sensor->data_buf == RT_NULL) {
    /* Allocate memory for the sensor buffer */
    sensor->data_buf =
        rt_malloc(sizeof(struct rt_sensor_data) * sensor->info.fifo_max);
  ...
  if (oflag & RT_DEVICE_FLAG_RDONLY && dev->flag & RT_DEVICE_FLAG_RDONLY) {
    ...
    if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_FETCH_MODE,
                   (void *)RT_SENSOR_MODE_FETCH_POLLING) == RT_EOK) {
      RT_SENSOR_MODE_SET_FETCH(sensor->info.mode, RT_SENSOR_MODE_FETCH_POLLING);
    }
  } else if (oflag & RT_DEVICE_FLAG_INT_RX &&
             dev->flag & RT_DEVICE_FLAG_INT_RX) {
    /* If interrupt mode is supported, configure it to interrupt mode */
    if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_FETCH_MODE,
                   (void *)RT_SENSOR_MODE_FETCH_INT) == RT_EOK) {
    ...
    if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_FETCH_MODE,
                   (void *)RT_SENSOR_MODE_FETCH_FIFO) == RT_EOK) {
      /* Initialization sensor interrupt */
      _sensor_irq_init(sensor);
  ...
  if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_POWER_MODE,
                 (void *)RT_SENSOR_MODE_POWER_HIGHEST) == RT_EOK) {
    RT_SENSOR_MODE_SET_POWER(sensor->info.mode, RT_SENSOR_MODE_POWER_HIGHEST);
  }

  /* Configure accuracy mode to highest mode */
  if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_ACCURACY_MODE,
                 (void *)RT_SENSOR_MODE_ACCURACY_HIGHEST) == RT_EOK) {
    RT_SENSOR_MODE_SET_ACCURACY(sensor->info.mode,
...
static rt_err_t _sensor_close(rt_device_t dev) {
  ...
  if (local_ctrl(sensor, RT_SENSOR_CTRL_SET_POWER_MODE,
                 (void *)RT_SENSOR_MODE_POWER_DOWN) == RT_EOK) {
  ...
  if (RT_SENSOR_MODE_GET_FETCH(sensor->info.mode) !=
...
static rt_ssize_t _sensor_read(rt_device_t dev, rt_off_t pos, void *buf,
                               rt_size_t len) {
  rt_sensor_t sensor = (rt_sensor_t)dev;
  rt_size_t result = 0;
  RT_ASSERT(dev != RT_NULL);
...
static rt_err_t _sensor_control(rt_device_t dev, int cmd, void *args) {
  ...
  case RT_SENSOR_CTRL_SET_ACCURACY_MODE:
    ...
    if (!(mode == RT_SENSOR_MODE_ACCURACY_HIGHEST ||
          mode == RT_SENSOR_MODE_ACCURACY_HIGH ||
          mode == RT_SENSOR_MODE_ACCURACY_MEDIUM ||
          mode == RT_SENSOR_MODE_ACCURACY_LOW ||
          mode == RT_SENSOR_MODE_ACCURACY_LOWEST ||
          mode == RT_SENSOR_MODE_ACCURACY_NOTRUST)) {
    ...
    if (result == RT_EOK) {
      ...
      LOG_D("set accuracy mode code: %d",
            RT_SENSOR_MODE_GET_ACCURACY(sensor->info.mode));
    }
    break;
  case RT_SENSOR_CTRL_SET_POWER_MODE:
    /* Configuration sensor power mode */
    mode = (rt_uint32_t)args & 0x000F;
    if (!(mode == RT_SENSOR_MODE_POWER_HIGHEST ||
          mode == RT_SENSOR_MODE_POWER_HIGH ||
          mode == RT_SENSOR_MODE_POWER_MEDIUM ||
          mode == RT_SENSOR_MODE_POWER_LOW ||
          mode == RT_SENSOR_MODE_POWER_LOWEST ||
          mode == RT_SENSOR_MODE_POWER_DOWN)) {
    ...
    if (result == RT_EOK) {
      ...
      LOG_D("set power mode code: %d",
            RT_SENSOR_MODE_GET_POWER(sensor->info.mode));
    }
    break;
  case RT_SENSOR_CTRL_SET_FETCH_MODE:
    /* Configuration sensor power mode */
    mode = (rt_uint32_t)args & 0x000F;
    if (!(mode == RT_SENSOR_MODE_FETCH_POLLING ||
          mode == RT_SENSOR_MODE_FETCH_INT ||
    ...
    if (result == RT_EOK) {
      ...
      LOG_D("set fetch mode code: %d",
            RT_SENSOR_MODE_GET_FETCH(sensor->info.mode));
    }
    break;
...
const static struct rt_device_ops rt_sensor_ops = {
    RT_NULL,      _sensor_open, _sensor_close,
    _sensor_read, RT_NULL,      _sensor_control};
#endif
...
int rt_hw_sensor_register(rt_sensor_t sensor, const char *name,
  ...
  device_name =
      (char *)rt_calloc(1, rt_strlen(sensor_name) + 1 + rt_strlen(name));
  ...
  result =
      rt_device_register(device, device_name, flag | RT_DEVICE_FLAG_STANDALONE);
  if (result != RT_EOK) {
