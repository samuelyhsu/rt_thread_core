...
/**
 * @brief    This function will initialize the data queue. Calling this function
 * will initialize the data queue control block and set the notification
 * callback function.
 *
 ...
 *           When the number of data in the data queue is less than this value,
 * this function will wake up the thread waiting for write data.
 *
 ...
 * initialization is successful. When the return value is RT_ENOMEM, it means
 * insufficient memory allocation failed.
 */
rt_err_t rt_data_queue_init(struct rt_data_queue *queue, rt_uint16_t size,
                            rt_uint16_t lwm,
                            void (*evt_notify)(struct rt_data_queue *queue,
                                               rt_uint32_t event)) {
  ...
  queue->queue =
      (struct rt_data_item *)rt_malloc(sizeof(struct rt_data_item) * size);
...
/**
 * @brief    This function will write data to the data queue. If the data queue
 * is full, the thread will suspend for the specified amount of time.
 *
 * @param    queue is a pointer to the data queue object.
 * .
 ...
 *
 * @return   Return the operation status. When the return value is RT_EOK, the
 * operation is successful. When the return value is RT_ETIMEOUT, it means the
 * specified time out.
 */
rt_err_t rt_data_queue_push(struct rt_data_queue *queue, const void *data_ptr,
                            rt_size_t data_size, rt_int32_t timeout) {
  rt_base_t level;
  rt_thread_t thread;
  rt_err_t result;
  ...
  if (!rt_list_isempty(&(queue->suspended_pop_list))) {
    /* get thread entry */
    thread =
        rt_list_entry(queue->suspended_pop_list.next, struct rt_thread, tlist);
...
/**
 * @brief    This function will pop data from the data queue. If the data queue
 * is empty,the thread will suspend for the specified amount of time.
 *
 * @note     When the number of data in the data queue is less than lwm(low
 ...
 * specified time out.
 */
rt_err_t rt_data_queue_pop(struct rt_data_queue *queue, const void **data_ptr,
  ...
  *size = queue->queue[queue->get_index].data_size;
  ...
  if (rt_data_queue_len(queue) <= queue->lwm) {
    ...
    if (!rt_list_isempty(&(queue->suspended_push_list))) {
      /* get thread entry */
      thread = rt_list_entry(queue->suspended_push_list.next, struct rt_thread,
                             tlist);
 ...
 *
 * @return   Return the operation status. When the return value is RT_EOK, the
 * operation is successful. When the return value is -RT_EEMPTY, it means the
 * data queue is empty.
 */
rt_err_t rt_data_queue_peek(struct rt_data_queue *queue, const void **data_ptr,
                            rt_size_t *size) {
  rt_base_t level;
...
void rt_data_queue_reset(struct rt_data_queue *queue) {
  ...
  while (!rt_list_isempty(&(queue->suspended_push_list))) {
    ...
    /* get next suspend thread */
    thread =
        rt_list_entry(queue->suspended_push_list.next, struct rt_thread, tlist);
    /* set error code to RT_ERROR */
    thread->error = -RT_ERROR;
