...
static resource_id_t id_mgr =
    RESOURCE_ID_INIT(RT_UNAMED_PIPE_NUMBER, resoure_id);
 ...
 *           When the return value is -1, it means the file descriptor is
 * invalid. When the return value is -RT_ENOMEM, it means insufficient memory
 * allocation failed.
 */
 ...
 *
 *               FIONREAD        The command to get the number of bytes in the
 * pipe.
 *
 *               FIONWRITE       The command to get the number of bytes can be
 * written to the pipe.
 *
 ...
 *           When the return value is 0, it means O_NONBLOCK is enabled and
 * there is no thread that has the pipe open for writing. When the return value
 * is -EAGAIN, it means there are no data to be read.
 */
 ...
 *           When the return value is -EAGAIN, it means O_NONBLOCK is enabled
 * and there are no space to be written. When the return value is -EPIPE, it
 * means there is no thread that has the pipe open for reading.
 */
 ...
 *           POLLHUP means there is no thread that occupied the pipe to open for
 * writing. POLLOUT means there is space to be written. POLLERR means there is
 * no thread that occupied the pipe to open for reading.
 */
...
static const struct dfs_file_ops pipe_fops = {
    pipe_fops_open, pipe_fops_close, pipe_fops_ioctl,
    pipe_fops_read, pipe_fops_write, RT_NULL,
    RT_NULL,        RT_NULL,         pipe_fops_poll,
};
#endif /* defined(RT_USING_POSIX_DEVIO) && defined(RT_USING_POSIX_PIPE) */

/**
 * @brief    This function will open the pipe and actually creates the pipe
 * buffer.
 *
 ...
 *           When the return value is -RT_EINVAL, it means the device handle is
 * empty. When the return value is -RT_ENOMEM, it means insufficient memory
 * allocation failed.
 */
 ...
 * @param    device is a pointer to the pipe device descriptor.
 *
 * @param    pos is a parameter compatible with POSIX standard interface
 * (currently meaningless, just pass in 0).
 *
 ...
 *
 * @return   Return the length of data read.
 *           When the return value is 0, it means the pipe device handle is
 * empty or the count is 0.
 */
rt_ssize_t rt_pipe_read(rt_device_t device, rt_off_t pos, void *buffer,
                        rt_size_t count) {
  uint8_t *pbuf;
  ...
  while (read_bytes < count) {
    int len =
        rt_ringbuffer_get(pipe->fifo, &pbuf[read_bytes], count - read_bytes);
    if (len <= 0) {
      break;
    }
 ...
 * empty or the count is 0.
 */
rt_ssize_t rt_pipe_write(rt_device_t device, rt_off_t pos, const void *buffer,
                         rt_size_t count) {
  uint8_t *pbuf;
  ...
  while (write_bytes < count) {
    int len =
        rt_ringbuffer_put(pipe->fifo, &pbuf[write_bytes], count - write_bytes);
    if (len <= 0) {
      break;
    }
...
const static struct rt_device_ops pipe_ops = {
    RT_NULL,      rt_pipe_open,  rt_pipe_close,
    rt_pipe_read, rt_pipe_write, rt_pipe_control,
};
#endif /* RT_USING_DEVICE_OPS */

/**
 * @brief    This function will initialize a pipe device.
 *           The system allocates a pipe handle from dynamic heap memory,
 * initializes the pipe handle with the specified value, and registers the pipe
 ...
 *           When the return value is RT_NULL, it means the initialization
 * failed.
 */
rt_pipe_t *rt_pipe_create(const char *name, int bufsz) {
  ...
  if (rt_device_register(&pipe->parent, name,
                         RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_REMOVABLE) != 0) {
 ...
 *           The system will release the pipe handle and unregister the pipe
 ...
 * @return   Return the operation status.
 *           When the return value is 0, it means the operation is successful.
 *           When the return value is -RT_EINVAL, it means the pipe device is
 * not found or the device isn't a pipe. When the return value is -RT_EBUSY, it
 * means the pipe device is busy.
 */
