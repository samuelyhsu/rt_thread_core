...
static rt_err_t _workqueue_submit_work(struct rt_workqueue *queue,
  ...
  if (ticks == 0) {
    ...
    if (queue->work_current == RT_NULL &&
        ((queue->work_thread->stat & RT_THREAD_SUSPEND_MASK) ==
    ...
    if (work->flags & RT_WORK_STATE_SUBMITTING) {
      ...
      rt_timer_init(&(work->timer), "work", _delayed_work_timeout_handler, work,
                    ticks, RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
...
static rt_err_t _workqueue_cancel_work(struct rt_workqueue *queue,
                                       struct rt_work *work) {
  rt_base_t level;
  rt_err_t err;
 ...
 *
 * @param work_func is a callback function that will be called when this work
 * item is executed.
 *
 * @param work_data is a user data passed to the callback function as the second
 * parameter.
 */
 ...
 *
 * @return Return a pointer to the workqueue object. It will return RT_NULL if
 * failed.
 */
struct rt_workqueue *rt_workqueue_create(const char *name,
  ...
  if (queue != RT_NULL) {
    ...
    queue->work_thread = rt_thread_create(name, _workqueue_thread_entry, queue,
 ...
 * @param work is a pointer to the work item object.
 *
 * @param ticks is the delay ticks for the work item to be submitted to the work
 * queue.
 *
 ...
 */
rt_err_t rt_workqueue_submit_work(struct rt_workqueue *queue,
                                  struct rt_work *work, rt_tick_t ticks) {
  RT_ASSERT(queue != RT_NULL);
  RT_ASSERT(work != RT_NULL);
...
/**
 * @brief Submit a work item to the work queue without delay. This work item
 * will be executed after the current work item.
 *
 * @param queue is a pointer to the workqueue object.
 *
 * @param work is a pointer to the work item object.
 ...
 * @return RT_EOK   Success.
 */
rt_err_t rt_workqueue_urgent_work(struct rt_workqueue *queue,
                                  struct rt_work *work) {
  rt_base_t level;
...
/**
 * @brief Cancel a work item in the work queue. If the work item is executing,
 * this function will block until it is done.
 *
 * @param queue is a pointer to the workqueue object.
 *
 * @param work is a pointer to the work item object.
 ...
 * @param work is a pointer to the work item object.
 *
 * @param ticks is the delay OS ticks for the work item to be submitted to the
 * work queue.
 *
 *             NOTE: The max timeout tick should be no more than (RT_TICK_MAX/2
 * - 1)
...
/**
 * @brief Submit a work item to the system work queue without delay. This work
 * item will be executed after the current work item.
 *
 * @param work is a pointer to the work item object.
 *
 * @return RT_EOK   Success.
