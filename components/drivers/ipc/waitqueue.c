 ...
 *                             to blocked thread.
 * 2022-01-24     THEWON       let rt_wqueue_wait return thread->error when
 * using signal
 */
...
/**
 * @brief    This function is the default wakeup function, but it doesn't do
 * anything in actual. It always return 0, user should define their own wakeup
 * function.
 *
...
/**
 * @brief    This function will join a thread to the specified waiting queue,
 * the thread will holds a wait or timeout return on the specified wait queue.
 *
 * @param    queue is a pointer to the wait queue.
 *
 * @param    condition is parameters compatible with POSIX standard interface
 ...
 */
static int _rt_wqueue_wait(rt_wqueue_t *queue, int condition, int msec,
                           int suspend_flag) {
  int tick;
  ...
  if (tick != RT_WAITING_FOREVER) {
    rt_timer_control(tmr, RT_TIMER_CTRL_SET_TIME, &tick);
