 ...
 * 2021-07-20     arminker     fix write_index bug in function
 * rt_ringbuffer_put_force 2021-08-14     Jackistang   add comments for function
 * interface.
 */
...
/**
 * @brief Put a block of data into the ring buffer. If the capacity of ring
 * buffer is insufficient, it will discard out-of-range data.
 *
 * @param rb            A pointer to the ring buffer object.
...
rt_size_t rt_ringbuffer_put(struct rt_ringbuffer *rb, const rt_uint8_t *ptr,
  ...
  rt_memcpy(&rb->buffer_ptr[rb->write_index], &ptr[0],
...
/**
 * @brief Put a block of data into the ring buffer. If the capacity of ring
 * buffer is insufficient, it will overwrite the existing data in the ring
 * buffer.
 *
 * @param rb            A pointer to the ring buffer object.
 * @param ptr           A pointer to the data buffer.
...
rt_size_t rt_ringbuffer_put_force(struct rt_ringbuffer *rb,
                                  const rt_uint8_t *ptr, rt_uint32_t length) {
 ...
 * @param ptr           A pointer to the data buffer.
 * @param length        The size of the data we want to read from the ring
 * buffer.
 *
...
rt_size_t rt_ringbuffer_get(struct rt_ringbuffer *rb, rt_uint8_t *ptr,
  ...
  rt_memcpy(&ptr[0], &rb->buffer_ptr[rb->read_index],
 ...
 * readable byte of the ring buffer.
 *
 * @note It is recommended to read only one byte, otherwise it may cause buffer
 * overflow.
 *
...
/**
 * @brief Put a byte into the ring buffer. If ring buffer is full, this
 * operation will fail.
 *
 * @param rb        A pointer to the ring buffer object.
 * @param ch        A byte put into the ring buffer.
 *
 * @return Return the data size we put into the ring buffer. The ring buffer is
 * full if returns 0. Otherwise, it will return 1.
 */
...
/**
 * @brief Put a byte into the ring buffer. If ring buffer is full, it will
 * discard an old data and put into a new data.
 *
 * @param rb        A pointer to the ring buffer object.
 * @param ch        A byte put into the ring buffer.
 *
 ...
 * @param size      The size of the buffer in bytes.
 *
 * @return Return a pointer to ring buffer object. When the return value is
 * RT_NULL, it means this creation failed.
 */
...
/**
 * @brief Destroy the ring buffer object, which is created by
