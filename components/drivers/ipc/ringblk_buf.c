 ...
 *
 * @note When your application need align access, please make the buffer address
 * is aligned.
 */
...
/**
 * Allocate a block by given size. The block will add to blk_list when allocate
 * success.
 *
 * @param rbb ring block buffer object
 * @param blk_size block size
 *
 * @note When your application need align access, please make the blk_szie is
 * aligned.
 *
...
rt_rbb_blk_t rt_rbb_blk_get(rt_rbb_t rbb) {
  ...
  for (node = rt_slist_first(&rbb->blk_list); node;
 ...
 *
 * @param rbb ring block buffer object
 * @param queue_data_len The max queue data size, and the return size must less
 * then it.
