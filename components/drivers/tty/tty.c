...
struct termios tty_std_termios = {/* for the benefit of tty drivers  */
                                  .c_iflag =
                                      IMAXBEL | IUCLC | INLCR | ICRNL | IGNPAR,
                                  .c_oflag = OPOST,
...
static int tty_open(struct dfs_fd *fd) {
  ...
  if (!noctty && current->leader && !current->tty && tty->session == -1) {
    current->tty = tty;
...
static const struct dfs_file_ops tty_fops = {
    tty_open, tty_close, tty_ioctl, tty_read, tty_write, RT_NULL, /* flush */
    RT_NULL,                                                      /* lseek */
    RT_NULL,                                                      /* getdents */
    tty_poll,
};
...
void console_init() { n_tty_init(); }

void tty_set_fops(struct dfs_file_ops *fops) { *fops = tty_fops; }

void console_set_fops(struct dfs_file_ops *fops) { *fops = console_fops; }
