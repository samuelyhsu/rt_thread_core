...
static ssize_t process_output_block(struct tty_struct *tty, const char *buf,
                                    unsigned int nr) {
  struct n_tty_data *ldata = tty->disc_data;
  int i = 0;
  ssize_t size = 0;
...
static void n_tty_set_termios(struct tty_struct *tty, struct termios *old) {
  ...
  if (!old || (old->c_lflag ^ tty->init_termios.c_lflag) & (ICANON | EXTPROC)) {
    ...
    if (!L_ICANON(tty) || !read_cnt(ldata)) {
      ...
      set_bit((ldata->read_head - 1) & (RT_TTY_BUF - 1),
              (int *)ldata->read_flags);
  ...
  if (I_ISTRIP(tty) || I_IUCLC(tty) || I_IGNCR(tty) || I_ICRNL(tty) ||
      I_INLCR(tty) || L_ICANON(tty) || I_IXON(tty) || L_ISIG(tty) ||
      L_ECHO(tty) || I_PARMRK(tty)) {
    ...
    if (L_ICANON(tty)) {
      ...
      if (L_IEXTEN(tty)) {
        ...
        if (L_ECHO(tty)) {
          set_bit(REPRINT_CHAR(tty), (int *)ldata->char_map);
        }
      }
    }
    if (I_IXON(tty)) {
...
static int n_tty_open(struct dfs_fd *fd) {
  ...
  ldata->atomic_read_lock =
      rt_mutex_create("atomic_read_lock", RT_IPC_FLAG_FIFO);
...
static int canon_copy_from_read_buf(struct tty_struct *tty, char *b,
                                    size_t nr) {
  struct n_tty_data *ldata = tty->disc_data;
...
static inline void n_tty_receive_char_inline(struct tty_struct *tty,
                                             unsigned char c) {
  struct n_tty_data *ldata = tty->disc_data;
...
static void n_tty_receive_char_lnext(struct tty_struct *tty, unsigned char c,
                                     char flag) {
  struct n_tty_data *ldata = tty->disc_data;
...
static void n_tty_receive_buf_real_raw(struct tty_struct *tty, char *cp,
                                       int count) {
  struct n_tty_data *ldata = tty->disc_data;
  size_t n = 0, head = 0;
...
static void n_tty_receive_buf_standard(struct tty_struct *tty, char *cp,
                                       int count) {
  struct n_tty_data *ldata = tty->disc_data;
  char flag = TTY_NORMAL;
...
static inline void n_tty_receive_char_fast(struct tty_struct *tty,
                                           unsigned char c) {
  struct n_tty_data *ldata = tty->disc_data;
...
static void n_tty_receive_buf_fast(struct tty_struct *tty, char *cp,
                                   int count) {
  struct n_tty_data *ldata = tty->disc_data;
  char flag = TTY_NORMAL;
...
static int n_tty_flush(struct dfs_fd *fd) { return 0; }

static int n_tty_lseek(struct dfs_fd *fd, off_t offset) { return 0; }

static int n_tty_getdents(struct dfs_fd *fd, struct dirent *dirp,
                          uint32_t count) {
  return 0;
}
...
static struct tty_ldisc_ops n_tty_ops = {
    "n_tty",           0,           n_tty_open,
    n_tty_close,       n_tty_ioctl, n_tty_read,
    n_tty_write,       n_tty_flush, n_tty_lseek,
    n_tty_getdents,    n_tty_poll,  n_tty_set_termios,
    n_tty_receive_buf, 0,
};

void n_tty_init(void) { tty_register_ldisc(N_TTY, &n_tty_ops); }
