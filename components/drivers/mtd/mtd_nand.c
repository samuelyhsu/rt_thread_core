...
static rt_err_t _mtd_open(rt_device_t dev, rt_uint16_t oflag) { return RT_EOK; }

static rt_err_t _mtd_close(rt_device_t dev) { return RT_EOK; }

static rt_ssize_t _mtd_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
  return size;
}

static rt_ssize_t _mtd_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  return size;
}
...
const static struct rt_device_ops mtd_nand_ops = {
    _mtd_init, _mtd_open, _mtd_close, _mtd_read, _mtd_write, _mtd_control};
#endif
...
rt_err_t rt_mtd_nand_write(struct rt_mtd_nand_device *device, rt_off_t page,
  ...
  return device->ops->write_page(device, page, data, data_len, spare,
                                 spare_len);
}
...
int mtd_nand_read(const char *name, int block, int page) {
  ...
  result = rt_mtd_nand_read(nand, page, page_ptr, nand->page_size, oob_ptr,
                            nand->oob_size);
...
int mtd_nand_readoob(const char *name, int block, int page) {
  ...
  /* calculate the page number */
  page = block * nand->pages_per_block + page;
  rt_mtd_nand_read(nand, page, RT_NULL, nand->page_size, oob_ptr,
                   nand->oob_size);
  mtd_dump_hex(oob_ptr, nand->oob_size);
...
int mtd_nand_write(const char *name, int block, int page) {
  ...
  /* calculate the page number */
  page = block * nand->pages_per_block + page;
  result = rt_mtd_nand_write(nand, page, page_ptr, nand->page_size, oob_ptr,
                             nand->oob_size);
...
static void mtd_nand(int argc, char **argv) {
  ...
  help:
    ...
    rt_kprintf("         read     <name> <bn> <pn>  Read data on page <pn> of "
               "block <bn> of device <name>\n");
    rt_kprintf("         readoob  <name> <bn> <pn>  Read oob  on page <pn> of "
               "block <bn> of device <name>\n");
    rt_kprintf("         write    <name> <bn> <pn>  Run write test on page "
               "<pn> of block <bn> of device <name>\n");
    rt_kprintf("         erase    <name> <bn>       Erase on block <bn> of "
               "device <name>\n");
    rt_kprintf("         eraseall <name>            Erase all block on device "
               "<name>\n");
    return;
...
FINSH_FUNCTION_EXPORT_ALIAS(mtd_nand_read, nand_read,
                            read page in nand - nand_read(name, block, page));
FINSH_FUNCTION_EXPORT_ALIAS(mtd_nand_readoob, nand_readoob,
                            read spare data in nand -
                                nand_readoob(name, block, page));
FINSH_FUNCTION_EXPORT_ALIAS(mtd_nand_write, nand_write,
                            write dump data to nand -
                                nand_write(name, block, page));
FINSH_FUNCTION_EXPORT_ALIAS(mtd_nand_erase, nand_erase,
                            nand_erase(name, block));
FINSH_FUNCTION_EXPORT_ALIAS(mtd_nand_erase_all, nand_erase_all,
                            erase all of nand device -
                                nand_erase_all(name, block));
