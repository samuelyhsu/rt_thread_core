...
rt_err_t rt_wlan_dev_init(struct rt_wlan_device *device, rt_wlan_mode_t mode) {
  ...
  /* init wlan device */
  LOG_D("F:%s L:%d is run device:0x%08x mode:%d", __FUNCTION__, __LINE__,
        device, mode);
  if ((device == RT_NULL) || (mode >= RT_WLAN_MODE_MAX)) {
    LOG_E("F:%s L:%d Parameter Wrongful device:0x%08x mode:%d", __FUNCTION__,
          __LINE__, device, mode);
    return -RT_ERROR;
  }

  if (mode == RT_WLAN_AP && device->flags & RT_WLAN_FLAG_STA_ONLY) {
    LOG_E("F:%s L:%d This wlan device can only be set to sta mode!",
          __FUNCTION__, __LINE__);
    return -RT_ERROR;
  } else if (mode == RT_WLAN_STATION && device->flags & RT_WLAN_FLAG_AP_ONLY) {
    LOG_E("F:%s L:%d This wlan device can only be set to ap mode!",
          __FUNCTION__, __LINE__);
    return -RT_ERROR;
  }
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_MODE, (void *)&mode);
  if (result != RT_EOK) {
...
rt_err_t rt_wlan_dev_connect(struct rt_wlan_device *device,
                             struct rt_wlan_info *info, const char *password,
                             int password_len) {
  rt_err_t result = RT_EOK;
...
rt_err_t rt_wlan_dev_fast_connect(struct rt_wlan_device *device,
  ...
  buff.len = rt_device_control(RT_DEVICE(device),
                               RT_WLAN_CMD_GET_FAST_CONNECT_INFO, buff.data);
  if (buff.len < 0) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_FAST_CONNECT, &buff);
...
rt_err_t rt_wlan_dev_disconnect(struct rt_wlan_device *device) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_DISCONNECT, RT_NULL);
  return result;
}

rt_err_t rt_wlan_dev_ap_start(struct rt_wlan_device *device,
                              struct rt_wlan_info *info, const char *password,
                              int password_len) {
  rt_err_t result = RT_EOK;
...
rt_err_t rt_wlan_dev_ap_deauth(struct rt_wlan_device *device,
                               rt_uint8_t mac[6]) {
  rt_err_t result = RT_EOK;
...
rt_err_t rt_wlan_dev_set_powersave(struct rt_wlan_device *device, int level) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_SET_POWERSAVE, &level);
  return result;
}
...
rt_err_t rt_wlan_dev_register_event_handler(struct rt_wlan_device *device,
...
rt_err_t
rt_wlan_dev_unregister_event_handler(struct rt_wlan_device *device,
                                     rt_wlan_dev_event_t event,
                                     rt_wlan_dev_event_handler handler) {
  int i = 0;
  rt_base_t level;
  ...
  for (i = 0; i < RT_WLAN_DEV_EVENT_NUM; i++) {
    if (device->handler_table[event][i].handler == handler) {
      rt_memset(&device->handler_table[event][i], 0,
                sizeof(struct rt_wlan_dev_event_desc));
      rt_hw_interrupt_enable(level);
      return RT_EOK;
    }
  }
...
void rt_wlan_dev_indicate_event_handle(struct rt_wlan_device *device,
                                       rt_wlan_dev_event_t event,
                                       struct rt_wlan_buff *buff) {
...
rt_err_t rt_wlan_dev_enter_promisc(struct rt_wlan_device *device) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_CFG_PROMISC, &enable);
  return result;
}
...
rt_err_t rt_wlan_dev_set_promisc_callback(struct rt_wlan_device *device,
...
void rt_wlan_dev_promisc_handler(struct rt_wlan_device *device, void *data,
                                 int len) {
  rt_wlan_pormisc_callback_t callback;
...
rt_err_t rt_wlan_dev_cfg_filter(struct rt_wlan_device *device,
...
rt_err_t rt_wlan_dev_set_channel(struct rt_wlan_device *device, int channel) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_SET_CHANNEL, &channel);
  return result;
}
...
int rt_wlan_dev_get_channel(struct rt_wlan_device *device) {
  ...
  result =
      rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_GET_CHANNEL, &channel);
  if (result != RT_EOK) {
    rt_set_errno(result);
    return -1;
  }
...
rt_err_t rt_wlan_dev_set_country(struct rt_wlan_device *device,
                                 rt_country_code_t country_code) {
  int result = RT_EOK;
  ...
  result = rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_SET_COUNTRY,
                             &country_code);
  return result;
}
...
rt_country_code_t rt_wlan_dev_get_country(struct rt_wlan_device *device) {
  ...
  result = rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_GET_COUNTRY,
                             &country_code);
  if (result != RT_EOK) {
    rt_set_errno(result);
    return RT_COUNTRY_UNKNOWN;
...
rt_err_t rt_wlan_dev_report_data(struct rt_wlan_device *device, void *buff,
                                 int len) {
...
rt_err_t rt_wlan_dev_enter_mgnt_filter(struct rt_wlan_device *device) {
  ...
  result = rt_device_control(RT_DEVICE(device), RT_WLAN_CMD_CFG_MGNT_FILTER,
                             &enable);
  return result;
}
...
rt_err_t
rt_wlan_dev_set_mgnt_filter_callback(struct rt_wlan_device *device,
...
void rt_wlan_dev_mgnt_filter_handler(struct rt_wlan_device *device, void *data,
                                     int len) {
  rt_wlan_mgnt_filter_callback_t callback;
...
int rt_wlan_dev_send_raw_frame(struct rt_wlan_device *device, void *buff,
                               int len) {
  if (device == RT_NULL) {
    return -RT_EIO;
  }
...
static rt_err_t _rt_wlan_dev_control(rt_device_t dev, int cmd, void *args) {
  ...
  case RT_WLAN_CMD_MODE: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_MODE, "RT_WLAN_CMD_MODE");
  ...
  case RT_WLAN_CMD_SCAN: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SCAN, "RT_WLAN_CMD_SCAN");
  ...
  case RT_WLAN_CMD_JOIN: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_JOIN, "RT_WLAN_CMD_JOIN");
  ...
  case RT_WLAN_CMD_SOFTAP: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SOFTAP, "RT_WLAN_CMD_SOFTAP");
  ...
  case RT_WLAN_CMD_DISCONNECT: {
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_DISCONNECT, "RT_WLAN_CMD_DISCONNECT");
  ...
  case RT_WLAN_CMD_AP_STOP: {
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_AP_STOP, "RT_WLAN_CMD_AP_STOP");
  ...
  case RT_WLAN_CMD_AP_DEAUTH: {
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_AP_DEAUTH, "RT_WLAN_CMD_AP_DEAUTH");
  ...
  case RT_WLAN_CMD_SCAN_STOP: {
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SCAN_STOP, "RT_WLAN_CMD_SCAN_STOP");
  ...
  case RT_WLAN_CMD_GET_RSSI: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_RSSI, "RT_WLAN_CMD_GET_RSSI");
  ...
  case RT_WLAN_CMD_SET_POWERSAVE: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SET_POWERSAVE, "RT_WLAN_CMD_SET_POWERSAVE");
  ...
  case RT_WLAN_CMD_GET_POWERSAVE: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_POWERSAVE, "RT_WLAN_CMD_GET_POWERSAVE");
  ...
  case RT_WLAN_CMD_CFG_PROMISC: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_CFG_PROMISC, "RT_WLAN_CMD_CFG_PROMISC");
  ...
  case RT_WLAN_CMD_CFG_FILTER: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_CFG_FILTER, "RT_WLAN_CMD_CFG_FILTER");
  ...
  case RT_WLAN_CMD_CFG_MGNT_FILTER: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_CFG_MGNT_FILTER, "RT_WLAN_CMD_CFG_MGNT_FILTER");
  ...
  case RT_WLAN_CMD_SET_CHANNEL: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SET_CHANNEL, "RT_WLAN_CMD_SET_CHANNEL");
  ...
  case RT_WLAN_CMD_GET_CHANNEL: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_CHANNEL, "RT_WLAN_CMD_GET_CHANNEL");
  ...
  case RT_WLAN_CMD_SET_COUNTRY: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SET_COUNTRY, "RT_WLAN_CMD_SET_COUNTRY");
  ...
  case RT_WLAN_CMD_GET_COUNTRY: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_COUNTRY, "RT_WLAN_CMD_GET_COUNTRY");
  ...
  case RT_WLAN_CMD_SET_MAC: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_SET_MAC, "RT_WLAN_CMD_SET_MAC");
  ...
  case RT_WLAN_CMD_GET_MAC: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_MAC, "RT_WLAN_CMD_GET_MAC");
  ...
  case RT_WLAN_CMD_GET_FAST_CONNECT_INFO: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_GET_FAST_INFO, "RT_WLAN_CMD_GET_FAST_INFO");
  ...
  case RT_WLAN_CMD_FAST_CONNECT: {
    ...
    LOG_D("%s %d cmd[%d]:%s  run......", __FUNCTION__, __LINE__,
          RT_WLAN_CMD_FAST_CONNECT, "RT_WLAN_CMD_FAST_CONNECT");
    if (wlan->ops->wlan_get_fast_info) {
...
const static struct rt_device_ops wlan_ops = {
    _rt_wlan_dev_init, RT_NULL, RT_NULL,
    RT_NULL,           RT_NULL, _rt_wlan_dev_control};
#endif
