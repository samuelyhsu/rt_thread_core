...
#define RT_WLAN_LOG_D(_fmt, ...)                                               \
  LOG_D("L:%d "_fmt                                                            \
        "",                                                                    \
        __LINE__, ##__VA_ARGS__)
#define RT_WLAN_LOG_I(...) LOG_I(__VA_ARGS__)
#define RT_WLAN_LOG_W(_fmt, ...)                                               \
  LOG_W("F:%s L:%d "_fmt                                                       \
        "",                                                                    \
        __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define RT_WLAN_LOG_E(_fmt, ...)                                               \
  LOG_E("F:%s L:%d "_fmt                                                       \
        "",                                                                    \
        __FUNCTION__, __LINE__, ##__VA_ARGS__)
...
static void rt_wlan_mgnt_work(void *parameter) {
  ...
  case RT_WLAN_EVT_STA_CONNECTED: {
    ...
    if (rt_wlan_is_connected() == RT_TRUE) {
      ...
      rt_exit_critical();
      RT_WLAN_LOG_D("run save config! ssid:%s len%d", _sta_mgnt.info.ssid.val,
                    _sta_mgnt.info.ssid.len);
...
static rt_err_t rt_wlan_send_to_thread(rt_wlan_event_t event, void *buff,
                                       int len) {
...
static rt_err_t rt_wlan_sta_info_del(struct rt_wlan_info *info, int timeout) {
  ...
  if (err == RT_EOK) {
    ...
    for (sta_list = sta_info.node, sta_prve = RT_NULL; sta_list != RT_NULL;
      ...
      /* find mac addr */
      if (rt_memcmp(&sta_list->info.bssid[0], &info->bssid[0],
                    RT_WLAN_BSSID_MAX_LENGTH) == 0) {
        if (sta_prve == RT_NULL) {
    ...
    rt_mutex_release(&sta_info_mutex);
    RT_WLAN_LOG_I("sta exit mac:%02x:%02x:%02x:%02x:%02x:%02x", info->bssid[0],
...
static void rt_wlan_auto_connect_run(struct rt_work *work, void *parameter) {
  ...
  if ((cfg_info.key.len > 0) &&
      (cfg_info.key.len <= RT_WLAN_PASSWORD_MAX_LENGTH)) {
...
static void rt_wlan_event_dispatch(struct rt_wlan_device *device,
  ...
  for (i = 0; i < sizeof(complete_tab) / sizeof(complete_tab[0]); i++) {
    if ((complete_tab[i] != RT_NULL)) {
      ...
      RT_WLAN_LOG_D("&complete_tab[i]->complete:0x%08x",
...
static rt_err_t rt_wlan_complete_wait(struct rt_wlan_complete_des *complete,
                                      rt_uint32_t event, rt_uint32_t timeout,
                                      rt_uint32_t *recved) {
  if (complete == RT_NULL) {
    return -RT_ERROR;
  }
    ...
    if (handler) {
      if (mode == RT_WLAN_STATION) {
        rt_wlan_dev_register_event_handler(
            RT_WLAN_DEVICE(device), RT_WLAN_DEV_EVT_CONNECT, handler, RT_NULL);
        rt_wlan_dev_register_event_handler(RT_WLAN_DEVICE(device),
                                           RT_WLAN_DEV_EVT_CONNECT_FAIL,
                                           handler, RT_NULL);
        rt_wlan_dev_register_event_handler(RT_WLAN_DEVICE(device),
                                           RT_WLAN_DEV_EVT_DISCONNECT, handler,
                                           RT_NULL);
        rt_wlan_dev_register_event_handler(RT_WLAN_DEVICE(device),
                                           RT_WLAN_DEV_EVT_SCAN_REPORT, handler,
                                           RT_NULL);
        rt_wlan_dev_register_event_handler(RT_WLAN_DEVICE(device),
                                           RT_WLAN_DEV_EVT_SCAN_DONE, handler,
                                           RT_NULL);
      } else if (mode == RT_WLAN_AP) {
        rt_wlan_dev_register_event_handler(
            RT_WLAN_DEVICE(device), RT_WLAN_DEV_EVT_AP_START, handler, RT_NULL);
        rt_wlan_dev_register_event_handler(
            RT_WLAN_DEVICE(device), RT_WLAN_DEV_EVT_AP_STOP, handler, RT_NULL);
        rt_wlan_dev_register_event_handler(RT_WLAN_DEVICE(device),
      ...
      rt_wlan_dev_event_t event;
      handler = rt_wlan_event_dispatch;
      for (event = RT_WLAN_DEV_EVT_INIT_DONE; event < RT_WLAN_DEV_EVT_MAX;
           event++) {
        rt_wlan_dev_unregister_event_handler(RT_WLAN_DEVICE(device), event,
                                             handler);
      }
    }
  }
...
static void rt_wlan_join_scan_callback(int event, struct rt_wlan_buff *buff,
                                       void *parameter) {
  ...
  RT_WLAN_LOG_D("%s info len:%d tgt info len:%d", __FUNCTION__, info->ssid.len,
                tgt_info->ssid.len);
  RT_WLAN_LOG_D("%s info ssid:%s tgt info ssid:%s", __FUNCTION__,
                &info->ssid.val[0], &tgt_info->ssid.val[0]);

  if (rt_memcmp(&info->ssid.val[0], &tgt_info->ssid.val[0], info->ssid.len) ==
          0 &&
...
rt_err_t rt_wlan_connect(const char *ssid, const char *password) {
  ...
  err = rt_wlan_register_event_handler(RT_WLAN_EVT_SCAN_REPORT,
                                       rt_wlan_join_scan_callback, &info);
  if (err != RT_EOK) {
  ...
  RT_WLAN_LOG_D("find best info ssid:%s mac: %02x %02x %02x %02x %02x %02x",
                info.ssid.val, info.bssid[0], info.bssid[1], info.bssid[2],
                info.bssid[3], info.bssid[4], info.bssid[5]);
#endif
...
rt_err_t rt_wlan_connect_adv(struct rt_wlan_info *info, const char *password) {
  ...
  RT_WLAN_LOG_D("%s is run ssid:%s password:%s", __FUNCTION__, info->ssid.val,
                password);
  /* Parameter checking */
  if (password != RT_NULL) {
  ...
  if (info->ssid.len == 0 || info->ssid.len > RT_WLAN_SSID_MAX_LENGTH) {
    RT_WLAN_LOG_E("ssid is zero or to long! ssid:%s len:%d", info->ssid.val,
                  info->ssid.len);
    return -RT_EINVAL;
  }
  /* is connect ? */
  ...
  if (rt_wlan_is_connected()) {
    if ((_sta_mgnt.info.ssid.len == info->ssid.len) &&
        ...
        (rt_memcmp(&_sta_mgnt.info.ssid.val[0], &info->ssid.val[0],
                   info->ssid.len) == 0) &&
        (rt_memcmp(&_sta_mgnt.info.bssid[0], &info->bssid[0],
                   RT_WLAN_BSSID_MAX_LENGTH) == 0) &&
  ...
  err =
      rt_wlan_dev_fast_connect(_sta_mgnt.device, info, password, password_len);
  if (err != RT_EOK) {
...
rt_bool_t rt_wlan_is_connected(void) {
  ...
  RT_WLAN_LOG_D("%s is run : %s", __FUNCTION__,
                _connect ? "connect" : "disconnect");
  return _connect;
}
...
rt_err_t rt_wlan_set_mac(rt_uint8_t mac[6]) {
  ...
  RT_WLAN_LOG_D("%s is run mac: %02x:%02x:%02x:%02x:%02x:%02x", __FUNCTION__,
...
rt_err_t rt_wlan_start_ap_adv(struct rt_wlan_info *info, const char *password) {
  ...
  if (rt_wlan_ap_is_active()) {
    if ((_ap_mgnt.info.ssid.len == info->ssid.len) &&
        ...
        (rt_memcmp(&_ap_mgnt.info.ssid.val[0], &info->ssid.val[0],
                   info->ssid.len) == 0) &&
...
rt_bool_t rt_wlan_ap_is_active(void) {
  ...
  RT_WLAN_LOG_D("%s is run active:%s", __FUNCTION__,
...
int rt_wlan_ap_get_sta_info(struct rt_wlan_info *info, int num) {
  ...
  for (sta_list = sta_info.node; sta_list != RT_NULL && i < sta_num;
...
rt_err_t rt_wlan_ap_deauth_sta(rt_uint8_t *mac) {
  ...
  RT_WLAN_LOG_D("%s is run mac: %02x:%02x:%02x:%02x:%02x:%02x:%d", __FUNCTION__,
                mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ...
  for (sta_list = sta_info.node; sta_list != RT_NULL;
       sta_list = sta_list->next) {
    if (rt_memcmp(&sta_list->info.bssid[0], &mac[0],
                  RT_WLAN_BSSID_MAX_LENGTH) == 0) {
      find_flag = RT_TRUE;
      break;
    }
...
void rt_wlan_mgnt_lock(void) { MGNT_LOCK(); }

void rt_wlan_mgnt_unlock(void) { MGNT_UNLOCK(); }
