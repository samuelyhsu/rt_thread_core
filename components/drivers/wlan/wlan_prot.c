...
static struct rt_wlan_prot_event_des prot_event_tab[RT_WLAN_PROT_EVT_MAX]
                                                   [RT_WLAN_PROT_MAX];

static void rt_wlan_prot_event_handle(struct rt_wlan_device *wlan,
...
static struct rt_wlan_device *rt_wlan_prot_find_by_name(const char *name) {
  ...
  if (device == RT_NULL) {
    LOG_E("F:%s L:%d not find wlan dev!! name:%s", __FUNCTION__, __LINE__,
          name);
    return RT_NULL;
  }
...
rt_err_t rt_wlan_prot_attach_dev(struct rt_wlan_device *wlan,
  ...
  if (prot != RT_NULL && (rt_strcmp(prot->name, prot_name) == 0)) {
  ...
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_CONNECT, handler,
                                     RT_NULL);
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_DISCONNECT, handler,
                                     RT_NULL);
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_AP_START, handler,
                                     RT_NULL);
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_AP_STOP, handler,
                                     RT_NULL);
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_AP_ASSOCIATED,
                                     handler, RT_NULL);
  rt_wlan_dev_register_event_handler(wlan, RT_WLAN_DEV_EVT_AP_DISASSOCIATED,
                                     handler, RT_NULL);
...
rt_err_t rt_wlan_prot_detach_dev(struct rt_wlan_device *wlan) {
  ...
  for (event = RT_WLAN_DEV_EVT_INIT_DONE; event < RT_WLAN_DEV_EVT_MAX;
       event++) {
    rt_wlan_dev_unregister_event_handler(wlan, event,
                                         rt_wlan_prot_event_handle);
  }
...
rt_err_t rt_wlan_prot_event_register(struct rt_wlan_prot *prot,
...
rt_err_t rt_wlan_prot_event_unregister(struct rt_wlan_prot *prot,
                                       rt_wlan_prot_event_t event) {
  int i;
  ...
  for (i = 0; i < RT_WLAN_PROT_MAX; i++) {
    if ((prot_event_tab[event][i].handler != RT_NULL) &&
        (prot_event_tab[event][i].prot == prot)) {
      rt_memset(&prot_event_tab[event][i], 0,
                sizeof(struct rt_wlan_prot_event_des));
      return RT_EOK;
    }
  }
...
rt_err_t rt_wlan_prot_transfer_dev(struct rt_wlan_device *wlan, void *buff,
                                   int len) {
...
rt_err_t rt_wlan_dev_transfer_prot(struct rt_wlan_device *wlan, void *buff,
                                   int len) {
  struct rt_wlan_prot *prot = wlan->prot;
