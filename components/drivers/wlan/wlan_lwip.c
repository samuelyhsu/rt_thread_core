  ...
  if (lwip_prot->connected_flag) {
    if (wlan->mode == RT_WLAN_STATION) {
...
#ifdef LWIP_USING_DHCPD
      {
        ...
        rt_memcpy(netif_name, eth_dev->netif->name,
                  sizeof(eth_dev->netif->name));
...
#ifdef LWIP_USING_DHCPD
    {
      char netif_name[RT_NAME_MAX];
      rt_memset(netif_name, 0, sizeof(netif_name));
      rt_memcpy(netif_name, lwip_prot->eth.netif->name,
                sizeof(lwip_prot->eth.netif->name));
...
static void rt_wlan_lwip_event_handle(struct rt_wlan_prot *port,
                                      struct rt_wlan_device *wlan, int event) {
  struct lwip_prot_des *lwip_prot = (struct lwip_prot_des *)wlan->prot;
  rt_bool_t flag_old;
...
static rt_err_t rt_wlan_lwip_protocol_control(rt_device_t device, int cmd,
                                              void *args) {
  ...
  LOG_D("F:%s L:%d device:0x%08x user_data:0x%08x", __FUNCTION__, __LINE__,
        eth_dev, eth_dev->parent.user_data);
...
static rt_err_t rt_wlan_lwip_protocol_recv(struct rt_wlan_device *wlan,
                                           void *buff, int len) {
...
const static struct rt_device_ops wlan_lwip_ops = {
    RT_NULL, RT_NULL, RT_NULL, RT_NULL, RT_NULL, rt_wlan_lwip_protocol_control};
#endif

static struct rt_wlan_prot *
rt_wlan_lwip_protocol_register(struct rt_wlan_prot *prot,
                               struct rt_wlan_device *wlan) {
...
static void rt_wlan_lwip_protocol_unregister(struct rt_wlan_prot *prot,
                                             struct rt_wlan_device *wlan) {
...
static struct rt_wlan_prot_ops ops = {rt_wlan_lwip_protocol_recv,
...
int rt_wlan_lwip_init(void) {
  ...
  for (event = RT_WLAN_PROT_EVT_INIT_DONE; event < RT_WLAN_PROT_EVT_MAX;
       event++) {
