...
#define MAX_SPI_PACKET_SIZE                                                    \
  (member_offset(struct spi_data_packet, buffer) + SPI_MAX_DATA_LEN)
...
struct rw009_wifi {
  ...
  rt_align(4) rt_uint8_t
      spi_tx_mempool[(sizeof(struct spi_data_packet) + 4) * SPI_TX_POOL_SIZE];
  rt_align(4) rt_uint8_t
      spi_rx_mempool[(sizeof(struct spi_data_packet) + 4) * SPI_RX_POOL_SIZE];

  rt_align(4) uint8_t spi_hw_rx_buffer[MAX_SPI_BUFFER_SIZE];
...
static void resp_handler(struct rw009_wifi *wifi_device,
                         struct rw009_resp *resp) {
  ...
  case RW009_CMD_INIT:
    ...
    resp_return =
        (struct rw009_resp *)rt_malloc(member_offset(struct rw009_resp, resp) +
                                       sizeof(rw009_resp_init)); // TODO:
    if (resp_return == RT_NULL)
      break;
    rt_memcpy(resp_return, resp,
              member_offset(struct rw009_resp, resp) + sizeof(rw009_resp_init));

    WIFI_DEBUG("sn:%-*.*s\n", sizeof(resp->resp.init.sn),
               sizeof(resp->resp.init.sn), resp->resp.init.sn);
    WIFI_DEBUG("version:%-*.*s\n", sizeof(resp->resp.init.version),
               sizeof(resp->resp.init.version), resp->resp.init.version);
  ...
  case RW009_CMD_SCAN:
    if (resp->len == sizeof(rw009_ap_info)) {
      rw009_ap_info *ap_scan =
          rt_realloc(wifi_device->ap_scan,
                     sizeof(rw009_ap_info) * (wifi_device->ap_scan_count + 1));
      if (ap_scan != RT_NULL) {
        rt_memcpy(&ap_scan[wifi_device->ap_scan_count], &resp->resp.ap_info,
                  sizeof(rw009_ap_info));
  ...
  case RW009_CMD_EASY_JOIN:
    ...
    resp_return =
        (struct rw009_resp *)rt_malloc(member_offset(struct rw009_resp, resp) +
                                       sizeof(rw009_resp_join)); // TODO:
    if (resp_return == RT_NULL)
      break;
    rt_memcpy(resp_return, resp,
              member_offset(struct rw009_resp, resp) + sizeof(rw009_resp_join));

    if (resp->result == 0) {
      rt_memcpy(&wifi_device->ap_info, &resp_return->resp.ap_info,
                sizeof(rw009_resp_join));
...
static rt_err_t rw009_cmd(struct rw009_wifi *wifi_device, uint32_t cmd,
                          void *args) {
  ...
  data_packet = (struct spi_data_packet *)rt_mp_alloc(&wifi_device->spi_tx_mp,
                                                      RT_WAITING_FOREVER);
  ...
  data_packet->data_len =
      member_offset(struct rw009_cmd, params) + wifi_cmd->len;

  rt_mb_send(&wifi_device->spi_tx_mb, (rt_uint32_t)data_packet);
  rt_event_send(&spi_wifi_data_event, 1);

  result =
      rt_mb_recv(&wifi_device->rw009_cmd_mb, (rt_uint32_t *)&resp, timeout);
...
static rt_err_t spi_wifi_transfer(struct rw009_wifi *dev) {
  ...
  result = rt_mb_recv(&wifi_device->spi_tx_mb, (rt_uint32_t *)&data_packet, 0);
  if ((result == RT_EOK) && (data_packet != RT_NULL) &&
      (data_packet->data_len > 0)) {
    cmd.M2S_len =
        data_packet->data_len + member_offset(struct spi_data_packet, buffer);
    ...
    if ((resp.S2M_len) && (resp.S2M_len <= MAX_SPI_PACKET_SIZE)) {
      ...
      if (data_packet->data_type == data_type_eth_data) {
        ...
        if (wifi_device->active) {
          ...
          pbuf_take(p, (rt_uint8_t *)data_packet->buffer,
                    data_packet->data_len);
 ...
 * **************************************/
static rt_err_t rw009_wifi_init(rt_device_t dev) { return RT_EOK; }

static rt_err_t rw009_wifi_open(rt_device_t dev, rt_uint16_t oflag) {
  return RT_EOK;
}

static rt_err_t rw009_wifi_close(rt_device_t dev) { return RT_EOK; }

static rt_ssize_t rw009_wifi_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                  rt_size_t size) {
  rt_set_errno(-RT_ENOSYS);
  return 0;
}

static rt_ssize_t rw009_wifi_write(rt_device_t dev, rt_off_t pos,
...
static void spi_wifi_data_thread_entry(void *parameter) {
  ...
  while (1) {
    /* receive first event */
    if (rt_event_recv(&spi_wifi_data_event, 1,
...
const static struct rt_device_ops rw009_ops = {
    rw009_wifi_init, rw009_wifi_open,  rw009_wifi_close,
    rw009_wifi_read, rw009_wifi_write, rw009_wifi_control};
#endif
...
rt_err_t rt_hw_wifi_init(const char *spi_device_name, wifi_mode_t mode) {
  ...
  rw009_wifi_device.rt_spi_device =
      (struct rt_spi_device *)rt_device_find(spi_device_name);
  ...
  rt_mp_init(&rw009_wifi_device.spi_tx_mp, "spi_tx",
             &rw009_wifi_device.spi_tx_mempool[0],
             sizeof(rw009_wifi_device.spi_tx_mempool),
             sizeof(struct spi_data_packet));

  rt_mp_init(&rw009_wifi_device.spi_rx_mp, "spi_rx",
             &rw009_wifi_device.spi_rx_mempool[0],
             sizeof(rw009_wifi_device.spi_rx_mempool),
             sizeof(struct spi_data_packet));

  rt_mb_init(&rw009_wifi_device.spi_tx_mb, "spi_tx",
             &rw009_wifi_device.spi_tx_mb_pool[0], SPI_TX_POOL_SIZE,
             RT_IPC_FLAG_PRIO);

  rt_mb_init(&rw009_wifi_device.eth_rx_mb, "eth_rx",
             &rw009_wifi_device.eth_rx_mb_pool[0], SPI_TX_POOL_SIZE,
             RT_IPC_FLAG_PRIO);

  rt_mb_init(&rw009_wifi_device.rw009_cmd_mb, "wifi_cmd",
             &rw009_wifi_device.rw009_cmd_mb_pool[0],
             sizeof(rw009_wifi_device.rw009_cmd_mb_pool) / 4, RT_IPC_FLAG_PRIO);
...

  {
    ...
    tid = rt_thread_create("wifi", spi_wifi_data_thread_entry, RT_NULL, 2048,
  ...
  /* init: get mac address */
  {
    ...
    rw009_wifi_control((rt_device_t)&rw009_wifi_device, RW009_CMD_INIT,
...
rt_err_t rw009_join(const char *SSID, const char *passwd) {
  ...
  result =
      rt_device_control(wifi_device, RW009_CMD_EASY_JOIN, (void *)&easy_join);

  return result;
}

rt_err_t rw009_softap(const char *SSID, const char *passwd, uint32_t security,
                      uint32_t channel) {
  rt_err_t result;
  rt_device_t wifi_device;
  ...
  result = rt_device_control(wifi_device, RW009_CMD_SOFTAP, (void *)&softap);
...
int32_t rw009_rssi(void) {
  ...
  // SCAN
  result = rt_device_control((rt_device_t)wifi_device, RW009_CMD_RSSI, RT_NULL);
...
static rt_err_t rw009_scan(void) {
  ...
  rt_kprintf("\nCMD RW009_CMD_SCAN \n");
  result = rt_device_control((rt_device_t)wifi_device, RW009_CMD_SCAN, RT_NULL);
