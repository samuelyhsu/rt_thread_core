...
#define RT_SFUD_DEFAULT_SPI_CFG                                                \
  {                                                                            \
    .mode = RT_SPI_MODE_0 | RT_SPI_MSB, .data_width = 8,                       \
...
#define RT_SFUD_DEFAULT_QSPI_CFG                                               \
  {                                                                            \
    RT_SFUD_DEFAULT_SPI_CFG, .medium_size = 0x800000, .ddr_mode = 0,           \
                             .qspi_dl_width = 4,                               \
  }
...
static rt_err_t rt_sfud_control(rt_device_t dev, int cmd, void *args) {
  ...
  case RT_DEVICE_CTRL_BLK_GETGEOME: {
    ...
    struct spi_flash_device *rtt_dev =
        (struct spi_flash_device *)(dev->user_data);
  ...
  case RT_DEVICE_CTRL_BLK_ERASE: {
    rt_uint32_t *addrs = (rt_uint32_t *)args, start_addr = addrs[0],
                end_addr = addrs[1], phy_start_addr;
    struct spi_flash_device *rtt_dev =
        (struct spi_flash_device *)(dev->user_data);
    sfud_flash *sfud_dev = (sfud_flash *)(rtt_dev->user_data);
    rt_size_t phy_size;

    if (addrs == RT_NULL || start_addr > end_addr || rtt_dev == RT_NULL ||
        sfud_dev == RT_NULL) {
      return -RT_ERROR;
    }
...
static rt_ssize_t rt_sfud_read(rt_device_t dev, rt_off_t pos, void *buffer,
                               rt_size_t size) {
  struct spi_flash_device *rtt_dev =
      (struct spi_flash_device *)(dev->user_data);
  sfud_flash *sfud_dev = (sfud_flash *)(rtt_dev->user_data);
...
static rt_ssize_t rt_sfud_write(rt_device_t dev, rt_off_t pos,
...
static sfud_err spi_write_read(const sfud_spi *spi, const uint8_t *write_buf,
  ...
  struct spi_flash_device *rtt_dev =
      (struct spi_flash_device *)(sfud_dev->user_data);
  ...
  if (rtt_dev->rt_spi_device->bus->mode & RT_SPI_BUS_MODE_QSPI) {
    ...
    if (write_size && read_size) {
      if (rt_qspi_send_then_recv(qspi_dev, write_buf, write_size, read_buf,
                                 read_size) <= 0) {
  ...
  {
    if (write_size && read_size) {
      if (rt_spi_send_then_recv(rtt_dev->rt_spi_device, write_buf, write_size,
                                read_buf, read_size) != RT_EOK) {
        result = SFUD_ERR_TIMEOUT;
      }
    } else if (write_size) {
...
static sfud_err qspi_read(const struct __sfud_spi *spi, uint32_t addr,
  ...
  struct rt_qspi_device *qspi_dev =
      (struct rt_qspi_device *)(rtt_dev->rt_spi_device);
...
sfud_err sfud_spi_port_init(sfud_flash *flash) {
  ...
  if (RT_TICK_PER_SECOND < 1000) {
    LOG_W("[SFUD] Warning: The OS tick(%d) is less than 1000. So the flash "
          "write will take more time.",
          RT_TICK_PER_SECOND);
  }
  /* 100 microsecond delay */
...
const static struct rt_device_ops flash_device_ops = {
    RT_NULL, RT_NULL, RT_NULL, rt_sfud_read, rt_sfud_write, rt_sfud_control};
#endif

/**
 * Probe SPI flash by SFUD (Serial Flash Universal Driver) driver library and
 * though SPI device by specified configuration.
 *
...
rt_sfud_flash_probe_ex(const char *spi_flash_dev_name, const char *spi_dev_name,
  ...
  if (rtt_dev && sfud_dev && spi_flash_dev_name_bak && spi_dev_name_bak) {
    ...
    rt_strncpy(spi_flash_dev_name_bak, spi_flash_dev_name,
    ...
    /* SPI configure */
    {
      ...
      rtt_dev->rt_spi_device =
          (struct rt_spi_device *)rt_device_find(spi_dev_name);
    ...
    /* SFUD flash device initialize */
    {
      ...
      /* when initialize success, then copy SFUD flash device's geometry to
       * RT-Thread SPI flash device */
    ...
    LOG_I("Probe SPI flash %s by SPI device %s success.", spi_flash_dev_name,
          spi_dev_name);
    return rtt_dev;
  } else {
...
#define CMD_BENCH_INDEX 5
  ...
  const char *sf_help_info[] = {
      [CMD_PROBE_INDEX] = "sf probe [spi_device]           - probe and init "
                          "SPI flash by given 'spi_device'",
      [CMD_READ_INDEX] = "sf read addr size               - read 'size' bytes "
                         "starting at 'addr'",
      [CMD_WRITE_INDEX] = "sf write addr data1 ... dataN   - write some bytes "
                          "'data' to flash starting at 'addr'",
      [CMD_ERASE_INDEX] = "sf erase addr size              - erase 'size' "
                          "bytes starting at 'addr'",
      [CMD_RW_STATUS_INDEX] = "sf status [<volatile> <status>] - read or write "
                              "'1:volatile|0:non-volatile' 'status'",
      [CMD_BENCH_INDEX] = "sf bench                        - full chip "
                          "benchmark. DANGER: It will erase full chip!",
  };
      ...
      if (argc < 3) {
        ...
        if (sfud_dev->chip.capacity < 1024 * 1024) {
          ...
          rt_kprintf("%d MB %s is current selected device.\n",
                     sfud_dev->chip.capacity / 1024 / 1024, sfud_dev->name);
        }
      }
    } else {
      if (!sfud_dev) {
      ...
      if (!rt_strcmp(operator, "read")) {
        if (argc < 4) {
          ...
          if (data) {
            ...
            if (result == SFUD_SUCCESS) {
              rt_kprintf("Read the %s flash data success. Start from 0x%08X, "
                         "size is %ld. The data is:\n",
                         sfud_dev->name, addr, size);
              rt_kprintf("Offset (h) 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D "
                         "0E 0F\n");
        ...
        if (argc < 4) {
          ...
          if (data) {
            ...
            if (result == SFUD_SUCCESS) {
              rt_kprintf("Write the %s flash data success. Start from 0x%08X, "
                         "size is %ld.\n",
                         sfud_dev->name, addr, size);
        ...
        if (argc < 4) {
          ...
          if (result == SFUD_SUCCESS) {
            rt_kprintf("Erase the %s flash data success. Start from 0x%08X, "
                       "size is %ld.\n",
                       sfud_dev->name, addr, size);
          }
        }
        ...
        if ((argc > 2 && rt_strcmp(argv[2], "yes")) || argc < 3) {
          rt_kprintf(
              "DANGER: It will erase full chip! Please run 'sf bench yes'.\n");
          return;
        }
        ...
        size_t write_size = SFUD_WRITE_MAX_PAGE_SIZE,
               read_size = SFUD_WRITE_MAX_PAGE_SIZE, cur_op_size;
        uint8_t *write_data = rt_malloc(write_size),
        ...
        if (write_data && read_data) {
          ...
          for (i = 0; i < size; i += write_size) {
            ...
            if (result != SFUD_SUCCESS) {
              rt_kprintf("Writing %s failed, already wr for %lu bytes, write "
                         "%d each time\n",
          ...
          for (i = 0; i < size; i += read_size) {
            ...
            if (memcmp(write_data, read_data, cur_op_size)) {
              rt_kprintf("Data check ERROR! Please check you flash by other "
                         "command.\n");
              result = SFUD_ERR_READ;
            }

            if (result != SFUD_SUCCESS) {
              rt_kprintf("Read %s failed, already rd for %lu bytes, read %d "
                         "each time\n",
