...
static rt_ssize_t rt_msd_write(rt_device_t dev, rt_off_t pos,
                               const void *buffer, rt_size_t size);
static rt_ssize_t rt_msd_read(rt_device_t dev, rt_off_t pos, void *buffer,
                              rt_size_t size);
static rt_ssize_t rt_msd_sdhc_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                   rt_size_t size);
static rt_ssize_t rt_msd_sdhc_write(rt_device_t dev, rt_off_t pos,
                                    const void *buffer, rt_size_t size);
...
const static struct rt_device_ops msd_ops = {rt_msd_init,  rt_msd_open,
...
static rt_err_t rt_msd_init(rt_device_t dev) {
  ...
  /* init SD card */
  {
    ...
    /* The host shall supply power to the card so that the voltage is reached to
       Vdd_min within 250ms and start to supply at least 74 SD clocks to the SD
      ...
      do {
        ...
        if (result == RT_EOK) {
          ...
          if ((0xAA == response[4]) && (0x00 == response[3])) {
            ...
            MSD_DEBUG("[err] VCA = 0, SD2.0 not surpport current operation "
                      "voltage range\r\n");
            result = RT_ERROR;
            goto _exit;
          }
    ...
    if (msd->card_type == MSD_CARD_TYPE_SD_V1_X) {
      ...
      while (1) {
        ...
        result = _send_cmd(msd->spi_device, READ_OCR, 0x00, 0x00, response_r3,
                           response);
        if (result != RT_EOK) {
          rt_spi_release(msd->spi_device);
          MSD_DEBUG("[info] It maybe SD1.x or MMC But it is Not response to "
                    "CMD58!\r\n");
          goto _exit;
        }
        ...
        if (0 != (response[0] & 0xFE)) {
          rt_spi_release(msd->spi_device);
          MSD_DEBUG("[info] It look CMD58 as illegal command so it is not SD "
                    "card!\r\n");
          break;
        }
        rt_spi_release(msd->spi_device);
        ...
        while (1) {
          ...
          result = _send_cmd(msd->spi_device, SD_SEND_OP_COND, 0x00, 0x00,
                             response_r1, response);
          if (result != RT_EOK) {
            rt_spi_release(msd->spi_device);
            continue;
      ...
      if (is_sd_v1_x != RT_TRUE) {
        ...
        while (1) {
          rt_spi_take(msd->spi_device);
          result = _send_cmd(msd->spi_device, SEND_OP_COND, 0x00, 0x00,
                             response_r1, response);
          rt_spi_release(msd->spi_device);
      ...
      do {
        rt_spi_take(msd->spi_device);
        if (rt_tick_timeout(tick_start,
                            rt_tick_from_millisecond(CARD_TRY_TIMES_ACMD41))) {
          rt_spi_release(msd->spi_device);
        ...
        /* CMD55 APP_CMD */
        result = _send_cmd(msd->spi_device, APP_CMD, 0x00, 0x65, response_r1,
                           response);
        ...
        /* ACMD41 SD_SEND_OP_COND */
        result = _send_cmd(msd->spi_device, SD_SEND_OP_COND, 0x40000000, 0x77,
                           response_r1, response);
        if (result != RT_EOK) {
          rt_spi_release(msd->spi_device);
      ...
      if ((response[0] & 0xFE) != 0) {
        rt_spi_release(msd->spi_device);
        MSD_DEBUG("[err] It look 2nd CMD58 as illegal command so it is not SD "
                  "card!\r\n");
        result = RT_ERROR;
        goto _exit;
      }
  ...
  /* read CSD */
  {
    ...
    //        response_r1, response);
    result =
        _send_cmd(msd->spi_device, SEND_CSD, 0x00, 0x00, response_r1, response);
    ...
    /* Analyze CSD */
    {
      ...
      if (msd->card_type == MSD_CARD_TYPE_MMC) {
        ...
        card_capacity =
            (1 << READ_BL_LEN) * ((C_SIZE + 1) * (1 << (C_SIZE_MULT + 2)));
...
static rt_ssize_t rt_msd_read(rt_device_t dev, rt_off_t pos, void *buffer,
  ...
  if (size == 1) {
    ...
    result = _send_cmd(msd->spi_device, READ_SINGLE_BLOCK,
                       pos * msd->geometry.bytes_per_sector, 0x00, response_r1,
                       response);
    if ((result != RT_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR)) {
    ...
    result =
        _read_block(msd->spi_device, buffer, msd->geometry.bytes_per_sector);
    if (result != RT_EOK) {
      MSD_DEBUG("[err] read SINGLE_BLOCK #%d fail!\r\n", pos);
      size = 0;
    }
...
static rt_ssize_t rt_msd_write(rt_device_t dev, rt_off_t pos,
  ...
  if (size == 1) {
    ...
    result =
        _write_block(msd->spi_device, buffer, msd->geometry.bytes_per_sector,
...
static rt_ssize_t rt_msd_sdhc_write(rt_device_t dev, rt_off_t pos,
  ...
  if (size == 1) {
    ...
    /* CMD55 APP_CMD */
    result =
        _send_cmd(msd->spi_device, APP_CMD, 0x00, 0x00, response_r1, response);
    if ((result != RT_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR)) {
      MSD_DEBUG("[err] CMD55 APP_CMD fail!\r\n");
      size = 0;
      goto _exit;
