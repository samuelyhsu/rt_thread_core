...
static uint8_t spi_read_op(struct rt_spi_device *spi_device, uint8_t op,
                           uint8_t address);
static void spi_write_op(struct rt_spi_device *spi_device, uint8_t op,
                         uint8_t address, uint8_t data);

static uint8_t spi_read(struct rt_spi_device *spi_device, uint8_t address);
static void spi_write(struct rt_spi_device *spi_device, rt_uint8_t address,
                      rt_uint8_t data);

static void enc28j60_clkout(struct rt_spi_device *spi_device, rt_uint8_t clk);
static void enc28j60_set_bank(struct rt_spi_device *spi_device,
                              uint8_t address);
static uint32_t enc28j60_interrupt_disable(struct rt_spi_device *spi_device);
static void enc28j60_interrupt_enable(struct rt_spi_device *spi_device,
                                      uint32_t level);

static uint16_t enc28j60_phy_read(struct rt_spi_device *spi_device,
                                  rt_uint8_t address);
static void enc28j60_phy_write(struct rt_spi_device *spi_device,
                               rt_uint8_t address, uint16_t data);
static rt_bool_t enc28j60_check_link_status(struct rt_spi_device *spi_device);

#define enc28j60_lock(dev)                                                     \
  rt_mutex_take(&((struct net_device *)dev)->lock, RT_WAITING_FOREVER);
#define enc28j60_unlock(dev)                                                   \
  rt_mutex_release(&((struct net_device *)dev)->lock);
...
static void enc28j60_set_bank(struct rt_spi_device *spi_device,
  ...
  if ((address & BANK_MASK) != Enc28j60Bank) {
    /* set the bank. */
    spi_write_op(spi_device, ENC28J60_BIT_FIELD_CLR, ECON1,
                 (ECON1_BSEL1 | ECON1_BSEL0));
    spi_write_op(spi_device, ENC28J60_BIT_FIELD_SET, ECON1,
                 (address & BANK_MASK) >> 5);
...
static rt_err_t enc28j60_init(rt_device_t dev) {
  ...
  spi_write_op(spi_device, ENC28J60_BIT_FIELD_SET, MACON3,
               MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN |
                   MACON3_FULDPX);
...
/* Close the interface */
static rt_err_t enc28j60_close(rt_device_t dev) { return RT_EOK; }

/* Read */
static rt_ssize_t enc28j60_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                rt_size_t size) {
...
/* Write */
static rt_ssize_t enc28j60_write(rt_device_t dev, rt_off_t pos,
...
static struct pbuf *enc28j60_rx(rt_device_t dev) {
  ...
  while (eir & ~EIR_PKTIF) {
    ...
    if (eir & EIR_PKTIF) {
      ...
      //            spi_write_op(spi_device, ENC28J60_BIT_FIELD_CLR, EIR,
      //            EIR_PKTIF);
    }
  ...
  if (pk_counter) {
    ...
    rxstat |= ((rt_uint16_t)spi_read_op(spi_device, ENC28J60_READ_BUF_MEM, 0))
              << 8;
...
const static struct rt_device_ops enc28j60_ops = {
    enc28j60_init, enc28j60_open,  enc28j60_close,
    enc28j60_read, enc28j60_write, enc28j60_control};
#endif
...
rt_err_t enc28j60_attach(const char *spi_device_name) {
  ...
  /* config spi */
  {
    ...
    cfg.max_hz =
        20 * 1000 * 1000; /* SPI Interface with Clock Speeds Up to 20 MHz */
  ...
  /* detect device */
  {
    ...
    enc28j60_dev.phy_id =
        (enc28j60_phy_read(spi_device, PHHID1) | ((value >> 10) << 16)) << 3;
...
static void enc28j60(void) {
  ...
  rt_kprintf("       0x%02X  0x%02X  0x%02X  0x%02X  0x%02X\n",
             spi_read(spi_device, ECON1), spi_read(spi_device, ECON2),
             spi_read(spi_device, ESTAT), spi_read(spi_device, EIR),
             spi_read(spi_device, EIE));

  rt_kprintf("MAC  : MACON1 MACON3 MACON4\n");
  rt_kprintf("       0x%02X   0x%02X   0x%02X\n", spi_read(spi_device, MACON1),
             spi_read(spi_device, MACON3), spi_read(spi_device, MACON4));
  ...
  rt_kprintf("0x%02X    0x%02X    0x%04X\n", spi_read(spi_device, ERXFCON),
             ...
             (spi_read(spi_device, MAMXFLH) << 8) |
                 spi_read(spi_device, MAMXFLL));
  ...
  rt_kprintf("       0x%04X 0x%04X 0x%02X     0x%02X     0x%02X\n",
             ...
             spi_read(spi_device, MACLCON1), spi_read(spi_device, MACLCON2),
