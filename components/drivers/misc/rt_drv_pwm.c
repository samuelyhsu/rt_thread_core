 ...
 * 2022-07-25     liYony       fix complementary outputs and add usage
 * information in finsh 2022-08-31     liYony       Add complementary output
 * section to framework for management 2022-09-24     qiyu         Add dead-time
...
*/
static rt_ssize_t _pwm_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
  rt_err_t result = RT_EOK;
  struct rt_device_pwm *pwm = (struct rt_device_pwm *)dev;
...
size : number of pulse, only set to sizeof(rt_uint32_t).
*/
static rt_ssize_t _pwm_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  rt_err_t result = RT_EOK;
  struct rt_device_pwm *pwm = (struct rt_device_pwm *)dev;
  rt_uint32_t *pulse = (rt_uint32_t *)buffer;
...
static const struct rt_device_ops pwm_device_ops = {
    RT_NULL, RT_NULL, RT_NULL, _pwm_read, _pwm_write, _pwm_control};
...
rt_err_t rt_pwm_enable(struct rt_device_pwm *device, int channel) {
   ...
   * If channel is a negative number (0 ~ -n), it means using complementary
   * output pin. */
  if (channel > 0) {
    result =
        rt_device_control(&device->parent, PWMN_CMD_DISABLE, &configuration);
  } else {
    result =
...
rt_err_t rt_pwm_set_period(struct rt_device_pwm *device, int channel,
                           rt_uint32_t period) {
  rt_err_t result = RT_EOK;
  struct rt_pwm_configuration configuration = {0};
...
rt_err_t rt_pwm_set_pulse(struct rt_device_pwm *device, int channel,
                          rt_uint32_t pulse) {
  rt_err_t result = RT_EOK;
  struct rt_pwm_configuration configuration = {0};
...
rt_err_t rt_pwm_set_dead_time(struct rt_device_pwm *device, int channel,
  ...
  result =
      rt_device_control(&device->parent, PWM_CMD_SET_DEAD_TIME, &configuration);

  return result;
}

rt_err_t rt_pwm_set_phase(struct rt_device_pwm *device, int channel,
                          rt_uint32_t phase) {
  rt_err_t result = RT_EOK;
  struct rt_pwm_configuration configuration = {0};
    ...
    if (!strcmp(argv[1], "probe")) {
      ...
      if (!strcmp(argv[1], "enable")) {
        if (argc == 3) {
          ...
          rt_kprintf("pwm enable <channel>                     - enable pwm "
                     "channel\n");
          rt_kprintf(
              "    e.g. MSH >pwm enable  1              - PWM_CH1  nomal\n");
          rt_kprintf("    e.g. MSH >pwm enable -1              - PWM_CH1N "
                     "complememtary\n");
        }
        ...
        if (argc == 3) {
          ...
          rt_kprintf("pwm disable <channel>                    - disable pwm "
                     "channel\n");
        }
        ...
        if (argc == 5) {
          result = rt_pwm_set(pwm_device, atoi(argv[2]), atoi(argv[3]),
                              atoi(argv[4]));
          rt_kprintf("pwm info set on %s at channel %d\n", pwm_device,
                     (rt_base_t)atoi(argv[2]));
        } else {
        ...
        if (argc == 4) {
          ...
          rt_kprintf("%s phase is set %d \n", pwm_device->parent.parent.name,
                     (rt_base_t)atoi(argv[3]));
        }
      } else if (!strcmp(argv[1], "dead_time")) {
        if (argc == 4) {
          result =
              rt_pwm_set_dead_time(pwm_device, atoi(argv[2]), atoi(argv[3]));
          result_str = (result == RT_EOK) ? "success" : "failure";
        ...
        rt_kprintf("Usage: \n");
        rt_kprintf("pwm probe      <device name>                - probe pwm by "
                   "name\n");
        rt_kprintf("pwm enable     <channel>                    - enable pwm "
                   "channel\n");
        rt_kprintf("pwm disable    <channel>                    - disable pwm "
                   "channel\n");
        rt_kprintf("pwm get        <channel>                    - get pwm "
                   "channel info\n");
        rt_kprintf("pwm set        <channel> <period> <pulse>   - set pwm "
                   "channel info\n");
        rt_kprintf(
            "pwm phase      <channel> <phase>            - set pwm phase\n");
        rt_kprintf("pwm dead_time  <channel> <dead_time>        - set pwm dead "
                   "time\n");
        result = -RT_ERROR;
      }
    }
