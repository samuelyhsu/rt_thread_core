...
static rt_ssize_t _pin_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
...
static rt_ssize_t _pin_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  struct rt_device_pin_status *status;
  struct rt_device_pin *pin = (struct rt_device_pin *)dev;
...
const static struct rt_device_ops pin_ops = {
    RT_NULL, RT_NULL, RT_NULL, _pin_read, _pin_write, _pin_control};
#endif
...
static void _pin_cmd_print_usage(void) {
  ...
  rt_kprintf("  mode: set pin mode to "
             "output/input/input_pullup/input_pulldown/output_od\n    e.g. MSH "
...
static void _pin_cmd_write(int argc, char *argv[]) {
  ...
  if ((0 == rt_strcmp("high", argv[3])) || (0 == rt_strcmp("on", argv[3]))) {
    value = PIN_HIGH;
  } else if ((0 == rt_strcmp("low", argv[3])) ||
             (0 == rt_strcmp("off", argv[3]))) {
    value = PIN_LOW;
  } else {
    _pin_cmd_print_usage();
    return;
