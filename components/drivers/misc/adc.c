...
static rt_ssize_t _adc_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
...
const static struct rt_device_ops adc_ops = {
    RT_NULL, RT_NULL, RT_NULL, _adc_read, RT_NULL, _adc_control,
};
#endif

rt_err_t rt_hw_adc_register(rt_adc_device_t device, const char *name,
...
rt_int16_t rt_adc_voltage(rt_adc_device_t dev, rt_uint32_t channel) {
  ...
  if (_adc_control((rt_device_t)dev, RT_ADC_CMD_GET_RESOLUTION, &resolution) !=
      RT_EOK) {
    goto _voltage_exit;
  }
