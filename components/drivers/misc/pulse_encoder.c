...
static rt_err_t rt_pulse_encoder_open(struct rt_device *dev,
                                      rt_uint16_t oflag) {
  struct rt_pulse_encoder_device *pulse_encoder;

  pulse_encoder = (struct rt_pulse_encoder_device *)dev;
  if (pulse_encoder->ops->control) {
    return pulse_encoder->ops->control(pulse_encoder, PULSE_ENCODER_CMD_ENABLE,
                                       RT_NULL);
  } else {
    return -RT_ENOSYS;
  }
...
static rt_err_t rt_pulse_encoder_close(struct rt_device *dev) {
  ...
  pulse_encoder = (struct rt_pulse_encoder_device *)dev;
  if (pulse_encoder->ops->control) {
    return pulse_encoder->ops->control(pulse_encoder, PULSE_ENCODER_CMD_DISABLE,
                                       RT_NULL);
  } else {
    return -RT_ENOSYS;
  }
...
static rt_ssize_t rt_pulse_encoder_read(struct rt_device *dev, rt_off_t pos,
...
static rt_err_t rt_pulse_encoder_control(struct rt_device *dev, int cmd,
                                         void *args) {
  rt_err_t result;
  struct rt_pulse_encoder_device *pulse_encoder;
...
rt_err_t
rt_device_pulse_encoder_register(struct rt_pulse_encoder_device *pulse_encoder,
