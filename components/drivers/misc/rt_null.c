...
static rt_ssize_t null_read(rt_device_t dev, rt_off_t pos, void *buffer,
                            rt_size_t size) {
  return 0;
}

static rt_ssize_t null_write(rt_device_t dev, rt_off_t pos, const void *buffer,
                             rt_size_t size) {
  return size;
}
...
const static struct rt_device_ops null_ops = {
    RT_NULL, RT_NULL, RT_NULL, null_read, null_write, null_control};
#endif
