...
static rt_err_t rt_inputcapture_open(struct rt_device *dev, rt_uint16_t oflag) {
  ...
  if (inputcapture->ringbuff == RT_NULL) {
    inputcapture->ringbuff = rt_ringbuffer_create(
        sizeof(struct rt_inputcapture_data) * RT_INPUT_CAPTURE_RB_SIZE);
  }
...
static rt_ssize_t rt_inputcapture_read(struct rt_device *dev, rt_off_t pos,
  ...
  inputcapture = (struct rt_inputcapture_device *)dev;
  receive_size = rt_ringbuffer_get(inputcapture->ringbuff, (rt_uint8_t *)buffer,
                                   sizeof(struct rt_inputcapture_data) * size);

  return receive_size / sizeof(struct rt_inputcapture_data);
}

static rt_err_t rt_inputcapture_control(struct rt_device *dev, int cmd,
                                        void *args) {
  rt_err_t result;
  struct rt_inputcapture_device *inputcapture;
...
const static struct rt_device_ops inputcapture_ops = {rt_inputcapture_init,
...
rt_err_t
rt_device_inputcapture_register(struct rt_inputcapture_device *inputcapture,
 ...
 * level: RT_TRUE denotes high level pulse, and RT_FALSE denotes low level
 * pulse.
 */
void rt_hw_inputcapture_isr(struct rt_inputcapture_device *inputcapture,
  ...
  rt_size_t receive_size;
  if (inputcapture->ops->get_pulsewidth(inputcapture, &data.pulsewidth_us) !=
      RT_EOK) {
    return;
  }

  data.is_high = level;
  if (rt_ringbuffer_put(inputcapture->ringbuff, (rt_uint8_t *)&data,
                        sizeof(struct rt_inputcapture_data)) == 0) {
    LOG_W("inputcapture ringbuffer doesn't have enough space.");
  }

  receive_size = rt_ringbuffer_data_len(inputcapture->ringbuff) /
                 sizeof(struct rt_inputcapture_data);
