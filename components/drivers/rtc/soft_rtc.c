...
static void alarm_timeout(void *param) { rt_alarm_update(param, 1); }
...
const static struct rt_device_ops soft_rtc_ops = {
    RT_NULL, RT_NULL, RT_NULL, RT_NULL, RT_NULL, soft_rtc_control};
#endif
...
#ifdef RT_USING_ALARM
  rt_timer_init(&alarm_time, "alarm", alarm_timeout, &soft_rtc_dev, 0,
