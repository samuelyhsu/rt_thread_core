...
const static struct rt_device_ops rtc_core_ops = {
    rt_rtc_init, rt_rtc_open, rt_rtc_close, RT_NULL, RT_NULL, rt_rtc_control,
};
...
rt_err_t set_date(rt_uint32_t year, rt_uint32_t month, rt_uint32_t day) {
  ...
  /* get current time */
  ret = rt_device_control(_rtc_device, RT_DEVICE_CTRL_RTC_GET_TIME,
                          &old_timestamp);
  if (ret != RT_EOK) {
    return ret;
  }
...
rt_err_t set_timestamp(time_t timestamp) {
  ...
  /* update to RTC device. */
  return rt_device_control(_rtc_device, RT_DEVICE_CTRL_RTC_SET_TIME,
                           &timestamp);
}
...
MSH_CMD_EXPORT(date, get date and time or
                         set(local timezone)[year month day hour min sec])
