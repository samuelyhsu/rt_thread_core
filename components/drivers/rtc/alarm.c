...
static void alarm_update(rt_uint32_t event) {
  ...
  if (!rt_list_isempty(&_container.head)) {
    ...
    for (next = _container.head.next; next != &_container.head;
         next = next->next) {
...
static rt_err_t alarm_setup(rt_alarm_t alarm, struct tm *wktime) {
  ...
  *setup = *wktime;
  ...
  if ((setup->tm_hour == 23) && (setup->tm_min == 59) &&
      (setup->tm_sec == 59)) {
    /*
...
static rt_uint8_t _alarm_flag_tbl_size =
    sizeof(_alarm_flag_tbl) / sizeof(_alarm_flag_tbl[0]);
...
void rt_alarm_dump(void) {
  ...
  for (next = _container.head.next; next != &_container.head;
    ...
    rt_kprintf(
        "| %02d:%02d:%02d |  %2d  |  %2s  | %2d |\n", alarm->wktime.tm_hour,
        alarm->wktime.tm_min, alarm->wktime.tm_sec, alarm->wktime.tm_wday,
        _alarm_flag_tbl[flag_index].name, alarm->flag & RT_ALARM_STATE_START);
  }
  rt_kprintf("+----------+------+------+----+\n");
}
...
int rt_alarm_system_init(void) {
  ...
  tid = rt_thread_create("alarmsvc", rt_alarmsvc_thread_init, RT_NULL, 2048, 10,
                         5);
  if (tid != RT_NULL)
