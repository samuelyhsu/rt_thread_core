 ...
 */
rt_inline int _can_int_rx(struct rt_can_device *can, struct rt_can_msg *data,
                          int msgs) {
  int size;
  ...
  while (msgs) {
    ...
    if (hdr >= 0 && can->hdr && hdr < can->config.maxhdr &&
      ...
      listmsg = rt_list_entry(can->hdr[hdr].list.next, struct rt_can_msg_list,
                              hdrlist);
...
#endif /*RT_CAN_USING_HDR*/
    {
      if (!rt_list_isempty(&rx_fifo->uselist)) {
        listmsg =
            rt_list_entry(rx_fifo->uselist.next, struct rt_can_msg_list, list);
        rt_list_remove(&listmsg->list);
#ifdef RT_CAN_USING_HDR
        rt_list_remove(&listmsg->hdrlist);
...
rt_inline int _can_int_tx(struct rt_can_device *can,
                          const struct rt_can_msg *data, int msgs) {
  int size;
  ...
  while (msgs) {
    ...
    level = rt_hw_interrupt_disable();
    tx_tosnd = rt_list_entry(tx_fifo->freelist.next,
                             struct rt_can_sndbxinx_list, list);
    RT_ASSERT(tx_tosnd != RT_NULL);
    rt_list_remove(&tx_tosnd->list);
    rt_hw_interrupt_enable(level);

    no = ((rt_uint32_t)tx_tosnd - (rt_uint32_t)tx_fifo->buffer) /
         sizeof(struct rt_can_sndbxinx_list);
...
rt_inline int _can_int_tx_priv(struct rt_can_device *can,
                               const struct rt_can_msg *data, int msgs) {
  int size;
  rt_base_t level;
...
static rt_err_t rt_can_open(struct rt_device *dev, rt_uint16_t oflag) {
  ...
  if (can->can_rx == RT_NULL) {
    if (oflag & RT_DEVICE_FLAG_INT_RX) {
      ...
      can->ops->control(can, RT_DEVICE_CTRL_SET_INT,
  ...
  if (can->can_tx == RT_NULL) {
    if (oflag & RT_DEVICE_FLAG_INT_TX) {
      ...
      rt_sem_init(&(tx_fifo->sem), tmpname, can->config.sndboxnumber,
                  RT_IPC_FLAG_FIFO);
      can->can_tx = tx_fifo;
  ...
  if (can->hdr == RT_NULL) {
    ...
    phdr = (struct rt_can_hdr *)rt_malloc(can->config.maxhdr *
                                          sizeof(struct rt_can_hdr));
...
static rt_err_t rt_can_close(struct rt_device *dev) {
  ...
  if (dev->open_flag & RT_DEVICE_FLAG_INT_RX) {
    ...
    can->ops->control(can, RT_DEVICE_CTRL_CLR_INT,
                      (void *)RT_DEVICE_FLAG_INT_RX);
  }
...
static rt_ssize_t rt_can_read(struct rt_device *dev, rt_off_t pos, void *buffer,
                              rt_size_t size) {
  struct rt_can_device *can;
...
static void cantimeout(void *arg) {
  ...
  can = (rt_can_t)arg;
  RT_ASSERT(can);
  rt_device_control((rt_device_t)can, RT_CAN_CMD_GET_STATUS,
                    (void *)&can->status);
...
void rt_hw_can_isr(struct rt_can_device *can, int event) {
  ...
  case RT_CAN_EVENT_RX_IND: {
    ...
    if (!rt_list_isempty(&rx_fifo->freelist)) {
      listmsg =
          rt_list_entry(rx_fifo->freelist.next, struct rt_can_msg_list, list);
      rt_list_remove(&listmsg->list);
#ifdef RT_CAN_USING_HDR
      rt_list_remove(&listmsg->hdrlist);
      if (listmsg->owner != RT_NULL && listmsg->owner->msgs) {
    ...
    /* invoke callback */
#ifdef RT_CAN_USING_HDR
    if (can->hdr != RT_NULL && can->hdr[hdr].connected &&
...
int cmd_canstat(int argc, void **argv) {
  static const char *ErrCode[] = {"No Error!", "Warning !", "Passive !",
                                  "Bus Off !"};
  ...
  if (argc >= 2) {
    ...
    rt_kprintf("\n Total..send...packages: %010ld. Dropped...send..packages: "
               "%010ld.\n",
               status.sndpkg + status.dropedsndpkg, status.dropedsndpkg);
  } else {
    rt_kprintf(" Invalid Call %s\n", argv[0]);
    rt_kprintf(" Please using %s cannamex .Here canname is driver name and x "
               "is candrive number.\n",
               argv[0]);
  }
  return 0;
