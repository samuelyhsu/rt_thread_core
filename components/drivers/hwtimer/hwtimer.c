...
static rt_ssize_t rt_hwtimer_read(struct rt_device *dev, rt_off_t pos,
...
static rt_ssize_t rt_hwtimer_write(struct rt_device *dev, rt_off_t pos,
...
static rt_err_t rt_hwtimer_control(struct rt_device *dev, int cmd, void *args) {
  ...
  case HWTIMER_CTRL_FREQ_SET: {
    ...
    if ((*f > timer->info->maxfreq) || (*f < timer->info->minfreq)) {
      LOG_W("frequency setting out of range! It will maintain at %d Hz",
            timer->freq);
      result = -RT_EINVAL;
      break;
    }
...
const static struct rt_device_ops hwtimer_ops = {
    rt_hwtimer_init, rt_hwtimer_open,  rt_hwtimer_close,
    rt_hwtimer_read, rt_hwtimer_write, rt_hwtimer_control};
#endif
