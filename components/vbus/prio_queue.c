...
static void _do_push(struct rt_prio_queue *que, rt_uint8_t prio,
...
rt_err_t rt_prio_queue_init(struct rt_prio_queue *que, const char *name,
                            void *buf, rt_size_t bufsz, rt_size_t itemsz) {
  RT_ASSERT(que);
...
void rt_prio_queue_detach(struct rt_prio_queue *que) {
  ...
  while (!rt_list_isempty(&(que->suspended_pop_list))) {
    ...
    thread =
        rt_list_entry(que->suspended_pop_list.next, struct rt_thread, tlist);
...
#ifdef RT_USING_HEAP
struct rt_prio_queue *rt_prio_queue_create(const char *name, rt_size_t item_nr,
                                           rt_size_t item_sz) {
  struct rt_prio_queue *que;
  rt_size_t bufsz;

  bufsz =
      item_nr * (sizeof(struct rt_prio_queue_item) + item_sz + sizeof(void *));
...
rt_err_t rt_prio_queue_push(struct rt_prio_queue *que, rt_uint8_t prio,
                            void *data, rt_int32_t timeout) {
  rt_base_t level;
  struct rt_prio_queue_item *item;
...
rt_err_t rt_prio_queue_pop(struct rt_prio_queue *que, void *data,
                           rt_int32_t timeout) {
  rt_base_t level;
  struct rt_prio_queue_item *item;
  ...
  level = rt_hw_interrupt_disable();
  for (item = _do_pop(que); item == RT_NULL; item = _do_pop(que)) {
    rt_thread_t thread;
...
void rt_prio_queue_dump(struct rt_prio_queue *que) {
  ...
  for (level = 0; level < RT_PRIO_QUEUE_PRIO_MAX; level++) {
    ...
    rt_kprintf("%2d: ", level);
    for (item = que->head[level]; item; item = item->next) {
      rt_kprintf("%p, ", item);
    }
    rt_kprintf("\n");
  }
