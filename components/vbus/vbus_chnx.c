...
static rt_ssize_t _read(rt_device_t dev, rt_off_t pos, void *buffer,
                        rt_size_t size) {
  rt_size_t outsz = 0;
  struct rt_vbus_dev *vdev = dev->user_data;
  ...
  while (1) {
    ...
    while (vdev->act) {
      ...
      rt_memcpy((char *)buffer + outsz, ((char *)(vdev->act + 1)) + vdev->pos,
                cpysz);
      vdev->pos += cpysz;
...
static rt_ssize_t _write(rt_device_t dev, rt_off_t pos, const void *buffer,
                         rt_size_t size) {
  rt_err_t err;
  struct rt_vbus_dev *vdev = dev->user_data;
  ...
  if (rt_interrupt_get_nest() == 0) {
    /* Thread context. */
    err = rt_vbus_post(vdev->chnr, vdev->req.prio, buffer, size,
                       RT_WAITING_FOREVER);
  } else {
    /* Interrupt context. */
    err = rt_vbus_post(vdev->chnr, vdev->req.prio, buffer, size, 0);
  }
...
rt_err_t _control(rt_device_t dev, int cmd, void *args) {
  ...
  case VBUS_IOC_LISCFG: {
    ...
    rt_vbus_register_listener(vdev->chnr, liscfg->event, liscfg->listener,
                              liscfg->ctx);
    return RT_EOK;
  } break;
...
void rt_vbus_chnx_register_disconn(rt_device_t dev, rt_vbus_event_listener indi,
  ...
  if (vdev)
    rt_vbus_register_listener(vdev->chnr, RT_VBUS_EVENT_ID_DISCONN, indi, ctx);
}
...
rt_err_t rt_vbus_chnx_init(void) {
  ...
  for (i = 0, p = rt_vbus_chn_devx; i < ARRAY_SIZE(_devx) && p->req.name;
       i++, p++) {
