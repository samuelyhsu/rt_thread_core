...
const char *rt_vbus_chn_st2str[] = {
    "available",   "closed",    "establishing",
    "established", "suspended", "closing",
};
...
const char *rt_vbus_cmd2str[] = {
    "ENABLE", "DISABLE", "SET", "ACK", "NAK", "SUSPEND", "RESUME",
};
...
#define LEN2BNR(len)                                                           \
  ((len + RT_VBUS_BLK_HEAD_SZ + sizeof(struct rt_vbus_blk) - 1) /              \
   sizeof(struct rt_vbus_blk))

rt_inline void _ring_add_get_bnr(struct rt_vbus_ring *ring, rt_size_t bnr) {
...
static void _bus_out_entry(void *param) {
  ...
  _bus_out_que =
      rt_prio_queue_create("vbus", _BUS_OUT_PKG_NR, sizeof(struct rt_vbus_pkg));
  ...
  while (rt_prio_queue_pop(_bus_out_que, &dpkg, RT_WAITING_FOREVER) == RT_EOK) {
    int sp;
    rt_uint32_t nxtidx;
    ...
    if (nxtidx >= RT_VMM_RB_BLK_NR) {
      ...
      tailsz = (RT_VMM_RB_BLK_NR - RT_VBUS_OUT_RING->put_idx) *
                   sizeof(RT_VBUS_OUT_RING->blks[0]) -
               RT_VBUS_BLK_HEAD_SZ;
      ...
      rt_memcpy(&RT_VBUS_OUT_RING->blks[RT_VBUS_OUT_RING->put_idx].data,
                dpkg.data, tailsz);
      rt_memcpy(&RT_VBUS_OUT_RING->blks[0], ((char *)dpkg.data) + tailsz,
                dpkg.len - tailsz);
...
rt_err_t rt_vbus_post(rt_uint8_t id, rt_uint8_t prio, const void *data,
  ...
  for (putsz = 0; size; size -= putsz) {
    ...
    vbus_debug("post (data: %p(%d), size: %d, finshed: %d, timeout: %d)\n",
               pkg.data, ((unsigned char *)pkg.data)[0], pkg.len, pkg.finished,
               timeout);
...
void _chn0_tx_listener(void *p) { rt_completion_done(&_chn0_post_cmp); }

/* Posts in channel0 should be sync. */
static rt_err_t _chn0_post(const void *data, rt_size_t size, int timeout) {
  rt_err_t err;
...
static void rt_vbus_notify_chn(unsigned char chnr, rt_err_t err) {
  ...
  while (!rt_list_isempty(&_chn_suspended_threads[chnr])) {
    ...
    thread = rt_list_entry(_chn_suspended_threads[chnr].next, struct rt_thread,
                           tlist);
    thread->error = err;
...
rt_err_t rt_vbus_listen_on(rt_uint8_t chnr, rt_int32_t timeout) {
  ...
  return rt_event_recv(&_bus_in_event, 1 << chnr,
                       RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, timeout,
                       &notuse);
}
...
static size_t __dump_naked_cmd(char *dst, size_t lsize, unsigned char *dp,
                               size_t dsize) {
  size_t len;
  if (dp[0] == RT_VBUS_CHN0_CMD_DISABLE || dp[0] == RT_VBUS_CHN0_CMD_SUSPEND ||
      dp[0] == RT_VBUS_CHN0_CMD_RESUME) {
    len = rt_snprintf(dst, lsize, "%s %d", rt_vbus_cmd2str[dp[0]], dp[1]);
  } else if (dp[0] == RT_VBUS_CHN0_CMD_ENABLE) {
    len = rt_snprintf(dst, lsize, "%s %s", rt_vbus_cmd2str[dp[0]], dp + 1);
  } else if (dp[0] < RT_VBUS_CHN0_CMD_MAX) {
    len = rt_snprintf(dst, lsize, "%s %s %d", rt_vbus_cmd2str[dp[0]], dp + 1,
                      dp[2 + rt_strlen((char *)dp + 1)]);
  } else {
    len = rt_snprintf(dst, lsize, "(invalid)%d %d", dp[0], dp[1]);
  }
  return len;
}
...
static char *dump_cmd_pkt(unsigned char *dp, size_t dsize) {
  ...
  if (dp[0] == RT_VBUS_CHN0_CMD_ACK || dp[0] == RT_VBUS_CHN0_CMD_NAK) {
    len = rt_snprintf(_cmd_dump_buf, sizeof(_cmd_dump_buf), "%s ",
                      rt_vbus_cmd2str[dp[0]]);
    len += __dump_naked_cmd(_cmd_dump_buf + len, sizeof(_cmd_dump_buf) - len,
                            dp + 1, dsize - 1);
  } else {
    len = __dump_naked_cmd(_cmd_dump_buf, sizeof(_cmd_dump_buf), dp, dsize);
  }
...
static rt_err_t _chn0_echo_with(rt_uint8_t prefix, rt_uint32_t dsize,
                                unsigned char *dp) {
  rt_err_t err;
  unsigned char *resp;
...
static int _sess_find(const unsigned char *name, enum _vbus_session_st st) {
  int i;
...
static int _chn0_actor(unsigned char *dp, size_t dsize) {
  ...
  case RT_VBUS_CHN0_CMD_ENABLE: {
    ...
    *resp = RT_VBUS_CHN0_CMD_SET;
    ...
    rt_vbus_set_recv_wm(chnr, _sess[i].req->recv_wm.low,
                        _sess[i].req->recv_wm.high);
    rt_vbus_set_post_wm(chnr, _sess[i].req->post_wm.low,
                        _sess[i].req->post_wm.high);
    ...
    if (err == RT_EOK) {
      ...
      vbus_debug("set sess %d st: %s\n", i, rt_vbus_sess_st2str[_sess[i].st]);
      _sess[i].chnr = chnr;
...
static void _bus_in_entry(void *param) {
  ...
  while (rt_sem_take(&_bus_in_sem, RT_WAITING_FOREVER) == RT_EOK) {
    ...
    while (RT_VBUS_IN_RING->get_idx != RT_VBUS_IN_RING->put_idx) {
      ...
      if (id == 0) {
        if (size > 60)
          ...
          _chn0_actor(RT_VBUS_IN_RING->blks[RT_VBUS_IN_RING->get_idx].data,
                      size);
        _ring_add_get_bnr(RT_VBUS_IN_RING, LEN2BNR(size));
        continue;
      }
      ...
      if (nxtidx >= RT_VMM_RB_BLK_NR) {
        ...
        rt_memcpy(act + 1,
                  &RT_VBUS_IN_RING->blks[RT_VBUS_IN_RING->get_idx].data,
                  tailsz);
        rt_memcpy((char *)(act + 1) + tailsz, &RT_VBUS_IN_RING->blks[0],
                  size - tailsz);
        ...
        rt_memcpy(act + 1,
                  &RT_VBUS_IN_RING->blks[RT_VBUS_IN_RING->get_idx].data, size);
...
int rt_vbus_init(void *outr, void *inr) {
  ...
  for (i = 0; i < ARRAY_SIZE(_chn_wm_que); i++) {
    rt_wm_que_init(&_chn_wm_que[i], RT_VMM_RB_BLK_NR / 3,
  ...
  rt_thread_init(&_bus_out_thread, "vbusout", _bus_out_entry, RT_NULL,
                 _bus_out_thread_stack, sizeof(_bus_out_thread_stack),
                 _BUS_OUT_THRD_PRIO, 20);
  rt_thread_startup(&_bus_out_thread);

  rt_thread_init(&_bus_in_thread, "vbusin", _bus_in_entry, RT_NULL,
...
void rt_vbus_data_pkt_dump(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(_bus_in_action); i++) {
    ...
    rt_kprintf("%2d: ", i);
#endif
    for (dp = _bus_in_action[i][_IN_ACT_HEAD]; dp; dp = dp->next) {
...
void rt_vbus_chm_wm_dump(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(_chn_recv_wm); i++) {
    rt_kprintf("%8x, %8x, %8x, %8x\n", _chn_recv_wm[i].low_mark,
               _chn_recv_wm[i].high_mark, _chn_recv_wm[i].level,
               _chn_recv_wm[i].last_warn);
  }
}
#endif
...
FINSH_FUNCTION_EXPORT_ALIAS(rt_vbus_que_dump, vbque,
                            dump vbus out queue status);
...
#ifdef RT_VBUS_USING_FLOW_CONTROL
FINSH_FUNCTION_EXPORT_ALIAS(rt_vbus_chm_wm_dump, vbwm,
                            dump vbus water mark status);
#endif
#endif
