...
int dfs_romfs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
                    const void *data) {
...
int dfs_romfs_unmount(struct dfs_filesystem *fs) { return RT_EOK; }
...
struct romfs_dirent *dfs_romfs_lookup(struct romfs_dirent *root_dirent,
  ...
  while (dirent != NULL) {
    ...
    for (index = 0; index < dirent_size; index++) {
      ...
      if (rt_strlen(dirent[index].name) == (rt_size_t)(subpath_end - subpath) &&
          rt_strncmp(dirent[index].name, subpath, (subpath_end - subpath)) ==
              0) {
...
int dfs_romfs_stat(struct dfs_filesystem *fs, const char *path,
                   struct stat *st) {
  rt_size_t size;
  struct romfs_dirent *dirent;
  struct romfs_dirent *root_dirent;
...
int dfs_romfs_getdents(struct dfs_fd *file, struct dirent *dirp,
                       uint32_t count) {
  rt_size_t index;
  rt_size_t len;
  const char *name;
...
static const struct dfs_file_ops _rom_fops = {
    dfs_romfs_open, dfs_romfs_close, dfs_romfs_ioctl,    dfs_romfs_read, NULL,
    NULL,           dfs_romfs_lseek, dfs_romfs_getdents, NULL,
};
