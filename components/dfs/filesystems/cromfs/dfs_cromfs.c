...
#define CROMFS_ALIGN_SIZE                                                      \
  (1UL << CROMFS_ALIGN_SIZE_BIT) /* must be same as sizeof cromfs_dirent */
...
typedef struct st_cromfs_info {
  ...
  uint32_t (*read_bytes)(struct st_cromfs_info *ci, uint32_t pos, void *buf,
                         uint32_t size);
...
static void cromfs_avl_remove(struct cromfs_avl_struct *node_to_delete,
                              struct cromfs_avl_struct **ptree);
static void cromfs_avl_insert(struct cromfs_avl_struct *new_node,
                              struct cromfs_avl_struct **ptree);
static struct cromfs_avl_struct *
cromfs_avl_find(avl_key_t key, struct cromfs_avl_struct *ptree);

static void cromfs_avl_rebalance(struct cromfs_avl_struct ***nodeplaces_ptr,
                                 int count) {
  for (; count > 0; count--) {
...
static uint32_t cromfs_read_bytes(cromfs_info *ci, uint32_t pos, void *buf,
                                  uint32_t size) {
...
static uint32_t cromfs_noblk_read_bytes(cromfs_info *ci, uint32_t pos,
...
static uint32_t cromfs_blk_read_bytes(cromfs_info *ci, uint32_t pos, void *buf,
                                      uint32_t size) {
  uint32_t ret = 0;
  ...
  if (sector_nr < 2) {
    ...
    if (sector_nr) {
      ret_len =
          rt_device_read(ci->device, start_blk, (char *)buf + off_s, sector_nr);
...
static uint8_t *cromfs_dirent_cache_get(cromfs_info *ci, uint32_t pos,
                                        uint32_t size) {
  rt_list_t *l = NULL;
  ...
  /* find */
  for (l = ci->cromfs_dirent_cache_head.next;
       l != &ci->cromfs_dirent_cache_head; l = l->next) {
...
static void cromfs_dirent_cache_destroy(cromfs_info *ci) {
  ...
  while ((l = ci->cromfs_dirent_cache_head.next) !=
         &ci->cromfs_dirent_cache_head) {
    rt_list_remove(l);
    dir = (cromfs_dirent_cache *)l;
    free(dir->buff);
    free(dir);
...
static int dfs_cromfs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
                            const void *data) {
  ...
  if (len != sizeof ci->part_info || memcmp(ci->part_info.magic, CROMFS_MAGIC,
                                            sizeof ci->part_info.magic) != 0) {
    free(ci);
    return -RT_ERROR;
  }
...
static uint32_t cromfs_lookup(cromfs_info *ci, const char *path, int *is_dir,
                              uint32_t *size, uint32_t *osize) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      name_block = (di_iter->dirent.name_size + CROMFS_ALIGN_SIZE_MASK) >>
...
static uint32_t dfs_cromfs_lookup(cromfs_info *ci, const char *path,
...
static int fill_file_data(file_info *fi) {
  ...
  if (!fi->data_valid) {
    ...
    if (cromfs_read_bytes(ci, fi->partition_pos, compressed_file_buff, size) !=
        size) {
      goto end;
    }
    if (uncompress((uint8_t *)fi->buff, (uLongf *)&osize,
                   (uint8_t *)compressed_file_buff, size) != Z_OK) {
      goto end;
    }
    fi->data_valid = 1;
  }
...
static file_info *get_file_info(cromfs_info *ci, uint32_t partition_pos,
                                int inc_ref) {
  struct cromfs_avl_struct *node =
      cromfs_avl_find(partition_pos, ci->cromfs_avl_root);
...
static file_info *inset_file_info(cromfs_info *ci, uint32_t partition_pos,
                                  int is_dir, uint32_t size, uint32_t osize) {
  file_info *fi = NULL;
  void *file_buff = NULL;
...
static int dfs_cromfs_stat(struct dfs_filesystem *fs, const char *path,
                           struct stat *st) {
  uint32_t size = 0, osize = 0;
  int is_dir = 0;
  cromfs_info *ci = NULL;
...
static int dfs_cromfs_getdents(struct dfs_fd *file, struct dirent *dirp,
                               uint32_t count) {
  uint32_t index = 0;
  uint8_t *name = NULL;
  ...
  for (index = 0; index < count && file->pos < file->vnode->size; index++) {
    ...
    name_size = (sub_dirent->dirent.name_size + CROMFS_ALIGN_SIZE_MASK) &
