...
static struct auth_ops ops = {authnone_verf, authnone_marshal,
...
AUTH *authnone_create(void) {
  ...
  buf_len =
      ((str_len) + (sizeof(rt_uint32_t)) - 1) & ~((sizeof(rt_uint32_t)) - 1);
...
static bool_t authnone_refresh(AUTH *x) { return (FALSE); }
