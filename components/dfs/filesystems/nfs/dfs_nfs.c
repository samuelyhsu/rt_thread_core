...
static int nfs_parse_host_export(const char *host_export, char *host,
                                 size_t host_len, char *export,
                                 size_t export_len) {
  int index;
...
int nfs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
              const void *data) {
  mountres3 res;
  nfs_filesystem *nfs;
  ...
  nfs->mount_client =
      clnt_create((char *)nfs->host, MOUNT_PROGRAM, MOUNT_V3, "udp");
  ...
  if (mountproc3_mnt_3((char *)nfs->export, &res, nfs->mount_client) !=
      RPC_SUCCESS) {
...
int nfs_unmount(struct dfs_filesystem *fs) {
  ...
  if (nfs->mount_client != NULL &&
      mountproc3_umnt_3((char *)nfs->export, NULL, nfs->mount_client) !=
          RPC_SUCCESS) {
...
int nfs_ioctl(struct dfs_fd *file, int cmd, void *args) { return -ENOSYS; }
...
int nfs_read(struct dfs_fd *file, void *buf, size_t count) {
  ...
  struct dfs_filesystem *dfs_nfs =
      ((struct dfs_filesystem *)(file->vnode->data));
...
char *nfs_readdir(nfs_filesystem *nfs, nfs_dir *dir) {
  ...
  if (dir->entry == NULL) {
    ...
    memcpy(&dir->cookieverf, &dir->res.READDIR3res_u.resok.cookieverf,
           sizeof(cookieverf3));
...
static const struct dfs_file_ops nfs_fops = {
    nfs_open,  nfs_close,    nfs_ioctl, nfs_read, nfs_write, NULL, /* flush */
    nfs_lseek, nfs_getdents, NULL,                                 /* poll */
};

static const struct dfs_filesystem_ops _nfs = {
    "nfs",       DFS_FS_FLAG_DEFAULT,
    &nfs_fops,   nfs_mount,
    nfs_unmount, NULL, /* mkfs */
    NULL,              /* statfs */
    nfs_unlink,  nfs_stat,
    nfs_rename,
};
