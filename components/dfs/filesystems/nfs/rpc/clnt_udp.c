 ...
 * sent and received.
 */
CLIENT *clntudp_bufcreate(struct sockaddr_in *raddr, unsigned long program,
                          unsigned long version, struct timeval wait,
  ...
  if (raddr->sin_port == 0) {
    ...
    extern unsigned short pmap_getport(
        struct sockaddr_in * address, unsigned long program,
  ...
  cu->cu_sendsz = sendsz;
  cu->cu_recvsz = recvsz;
  call_msg.rm_xid = ((unsigned long)rt_thread_self()) ^
                    ((unsigned long)rt_tick_get()) ^ (xid_count++);
...
CLIENT *clntudp_create(struct sockaddr_in *raddr, unsigned long program,
                       unsigned long version, struct timeval wait, int *sockp) {
...
send_again:
  ...
  do {
    ...
    inlen = recvfrom(cu->cu_sock, cu->cu_inbuf, (int)cu->cu_recvsz, 0,
...
static bool_t clntudp_control(CLIENT *cl, int request, char *info) {
  ...
  case CLSET_TIMEOUT: {
    ...
    mtimeout =
        ((cu->cu_total.tv_sec * 1000) + ((cu->cu_total.tv_usec + 500) / 1000));

    /* set socket option, note: lwip only support msecond timeout */
    setsockopt(cu->cu_sock, SOL_SOCKET, SO_RCVTIMEO, &mtimeout,
               sizeof(mtimeout));
  } break;
  case CLGET_TIMEOUT:
