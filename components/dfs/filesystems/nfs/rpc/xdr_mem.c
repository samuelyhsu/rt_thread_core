...
static struct xdr_ops xdrmem_ops = {xdrmem_getlong,
                                    xdrmem_putlong,
                                    xdrmem_getbytes,
                                    xdrmem_putbytes,
                                    xdrmem_getpos,
 ...
 * memory buffer.
 */
void xdrmem_create(XDR *xdrs, const char *addr, unsigned int size,
                   enum xdr_op op) {
  xdrs->x_op = op;
...
static bool_t xdrmem_putbytes(XDR *xdrs, const char *addr, unsigned int len) {
  if (xdrs->x_handy < len)
    return FALSE;
  xdrs->x_handy -= len;
...
static int32_t *xdrmem_inline(XDR *xdrs, unsigned int len) {
  int32_t *buf = 0;
