...
int dfs_elm_mount(struct dfs_filesystem *fs, unsigned long rwflag,
  ...
  if (rt_device_control(fs->dev_id, RT_DEVICE_CTRL_BLK_GETGEOME, &geometry) ==
    ...
    if (geometry.bytes_per_sector > FF_MAX_SS) {
      rt_kprintf("The sector size of device is greater than the sector size of "
                 "FAT.\n");
      return -EINVAL;
    }
  }
...
void ff_rel_grant(FF_SYNC_t m) { rt_mutex_release(m); }
...
/* Free memory block */
void ff_memfree(void *mem) { rt_free(mem); }
#endif /* FF_USE_LFN == 3 */
