...
int dfs_ramfs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
                    const void *data) {
...
int dfs_ramfs_ioctl(struct dfs_fd *file, int cmd, void *args) { return -EIO; }

struct ramfs_dirent *dfs_ramfs_lookup(struct dfs_ramfs *ramfs, const char *path,
                                      rt_size_t *size) {
  const char *subpath;
  ...
  if (file->flags & O_DIRECTORY) {
    ...
    if (dirent == NULL) {
      if (file->flags & O_CREAT || file->flags & O_WRONLY) {
        ...
        /* create a file entry */
        dirent = (struct ramfs_dirent *)rt_memheap_alloc(
...
int dfs_ramfs_stat(struct dfs_filesystem *fs, const char *path,
                   struct stat *st) {
  rt_size_t size;
  struct ramfs_dirent *dirent;
  struct dfs_ramfs *ramfs;
...
int dfs_ramfs_getdents(struct dfs_fd *file, struct dirent *dirp,
                       uint32_t count) {
  rt_size_t index, end;
  struct dirent *d;
  struct ramfs_dirent *dirent;
...
int dfs_ramfs_rename(struct dfs_filesystem *fs, const char *oldpath,
                     const char *newpath) {
  struct ramfs_dirent *dirent;
  struct dfs_ramfs *ramfs;
  rt_size_t size;
...
static const struct dfs_file_ops _ram_fops = {
    dfs_ramfs_open,  dfs_ramfs_close,    dfs_ramfs_ioctl,
    dfs_ramfs_read,  dfs_ramfs_write,    NULL, /* flush */
    dfs_ramfs_lseek, dfs_ramfs_getdents,
};
