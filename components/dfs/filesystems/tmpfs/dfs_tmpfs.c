 ...
 * 2023-02-01     xqyjlj       fix cannot open the same file repeatedly in 'w'
 * mode
 */
...
static int _path_separate(const char *path, char *parent_path,
                          char *file_name) {
...
int dfs_tmpfs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
                    const void *data) {
...
int dfs_tmpfs_ioctl(struct dfs_fd *file, int cmd, void *args) {
  ...
  case RT_FIOMMAP2: {
    ...
    if (mmap2) {
      ...
      mmap2->ret =
          lwp_map_user_phy(lwp_self(), RT_NULL, d_file->data, mmap2->length, 0);
    }
    return RT_EOK;
    break;
  }
...
struct tmpfs_file *dfs_tmpfs_lookup(struct tmpfs_sb *superblock,
                                    const char *path, rt_size_t *size) {
...
int dfs_tmpfs_stat(struct dfs_filesystem *fs, const char *path,
                   struct stat *st) {
  rt_size_t size;
  struct tmpfs_file *d_file;
  struct tmpfs_sb *superblock;
...
int dfs_tmpfs_getdents(struct dfs_fd *file, struct dirent *dirp,
                       uint32_t count) {
  rt_size_t index, end;
  struct dirent *d;
...
int dfs_tmpfs_rename(struct dfs_filesystem *fs, const char *oldpath,
                     const char *newpath) {
  struct tmpfs_file *d_file, *p_file;
  struct tmpfs_sb *superblock;
  rt_size_t size;
...
static const struct dfs_file_ops _tmp_fops = {
    dfs_tmpfs_open,  dfs_tmpfs_close,    dfs_tmpfs_ioctl,
    dfs_tmpfs_read,  dfs_tmpfs_write,    NULL, /* flush */
    dfs_tmpfs_lseek, dfs_tmpfs_getdents,
};
