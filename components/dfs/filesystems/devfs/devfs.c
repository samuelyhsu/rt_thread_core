...
int dfs_device_fs_mount(struct dfs_filesystem *fs, unsigned long rwflag,
                        const void *data) {
  return RT_EOK;
}
...
int dfs_device_fs_open(struct dfs_fd *file) {
  ...
  if ((file->vnode->path[0] == '/') && (file->vnode->path[1] == '\0') &&
    ...
    root_dirent = (struct device_dirent *)rt_malloc(
        sizeof(struct device_dirent) + count * sizeof(rt_device_t));
    if (root_dirent != RT_NULL) {
      ...
      for (node = information->object_list.next;
        ...
        if (count == root_dirent->device_count) {
          rt_kprintf(
              "warning: There are newly added devices that are not displayed!");
          break;
        }
...
int dfs_device_fs_stat(struct dfs_filesystem *fs, const char *path,
                       struct stat *st) {
  /* stat root directory */
  if ((path[0] == '/') && (path[1] == '\0')) {
    st->st_dev = 0;
...
int dfs_device_fs_getdents(struct dfs_fd *file, struct dirent *dirp,
                           uint32_t count) {
  rt_uint32_t index;
  rt_object_t object;
  struct dirent *d;
  ...
  for (index = 0; index < count &&
                  index + root_dirent->read_index < root_dirent->device_count;
       index++) {
