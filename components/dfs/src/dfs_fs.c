 ...
 */
int dfs_filesystem_get_partition(struct dfs_partition *part, uint8_t *buf,
                                 uint32_t pindex) {
...
#define DPT_ITEM_SIZE 16  /* partition item size */
  ...
  part->type = type;
  part->offset =
      *(dpt + 8) | *(dpt + 9) << 8 | *(dpt + 10) << 16 | *(dpt + 11) << 24;
  part->size =
      *(dpt + 12) | *(dpt + 13) << 8 | *(dpt + 14) << 16 | *(dpt + 15) << 24;
...
int dfs_mount(const char *device_name, const char *path,
  ...
  for (ops = &filesystem_operation_table[0];
    ...
    if ((*ops != NULL) &&
        (strncmp((*ops)->name, filesystemtype, strlen((*ops)->name)) == 0))
      break;
...
int dfs_mount_table(void) {
  ...
  while (1) {
    ...
    if (dfs_mount(mount_table[index].device_name, mount_table[index].path,
                  mount_table[index].filesystemtype, mount_table[index].rwflag,
...
int dfs_mount_device(rt_device_t dev) {
  ...
  while (1) {
    ...
    if (strcmp(mount_table[index].device_name, dev->parent.name) == 0) {
      if (dfs_mount(mount_table[index].device_name, mount_table[index].path,
                    mount_table[index].filesystemtype,
                    mount_table[index].rwflag, mount_table[index].data) != 0) {
...
int df(const char *path) {
  ...
  rt_kprintf("disk free: %d.%d %s [ %d block, %d bytes per block ]\n",
             (unsigned long)cap, minor, unit_str[unit_index], buffer.f_bfree,
             buffer.f_bsize);
  return 0;
}
