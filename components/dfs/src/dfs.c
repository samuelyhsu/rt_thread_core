...
const struct dfs_filesystem_ops
    *filesystem_operation_table[DFS_FILESYSTEM_TYPES_MAX];
...
int dfs_init(void) {
  ...
  rt_memset((void *)filesystem_operation_table, 0,
            sizeof(filesystem_operation_table));
 ...
 * @note please don't invoke it on ISR.
 */
void dfs_unlock(void) { rt_mutex_release(&fslock); }

#ifdef DFS_USING_POSIX

void dfs_fd_unlock(void) { rt_mutex_release(&fdlock); }
...
int fd_is_open(const char *pathname) {
  ...
  if (fullpath != NULL) {
    ...
    for (index = 0; index < fdt->maxfd; index++) {
      fd = fdt->fds[index];
      if (fd == NULL || fd->vnode->fops == NULL || fd->vnode->path == NULL)
        continue;
...
static int fd_get_fd_index_form_fdt(struct dfs_fdtable *fdt,
                                    struct dfs_fd *file) {
  int fd = -1;
...
char *dfs_normalize_path(const char *directory, const char *filename) {
  ...
  if (filename[0] != '/') /* it's a absolute path, use it directly */
  {
    ...
    rt_snprintf(fullpath, strlen(directory) + strlen(filename) + 2, "%s/%s",
                directory, filename);
  } else {
...
struct dfs_fdtable *dfs_fdtable_get_global(void) { return &_fdtab; }
  ...
  for (index = 0; index < (int)fd_table->maxfd; index++) {
    ...
    if (fd && fd->vnode->fops) {
      ...
      if (fd->vnode == RT_NULL) {
        ...
        rt_kprintf("0x%.8x 0x%.8x ", (int)(size_t)(fd->vnode->data),
                   (int)(size_t)fd);
      }
...
int lsof(int argc, char *argv[]) {
  rt_kprintf(
      "PID fd type    fd-ref magic vnode      vnode/data addr       path  \n");
