...
void dfs_fm_lock(void) { rt_mutex_take(&dfs_fm.lock, RT_WAITING_FOREVER); }

void dfs_fm_unlock(void) { rt_mutex_release(&dfs_fm.lock); }
...
static struct dfs_fnode *dfs_fnode_find(const char *path,
                                        rt_list_t **hash_head) {
