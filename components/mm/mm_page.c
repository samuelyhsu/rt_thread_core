...
static const char *get_name(rt_varea_t varea) { return "master-page-record"; }
...
rt_weak int rt_hw_clz(unsigned long n) { return __builtin_clzl(n); }

rt_weak int rt_hw_ctz(unsigned long n) { return __builtin_ctzl(n); }
...
void *rt_page_page2addr(struct rt_page *p) { return page_to_addr(p); }
...
void rt_page_init(rt_region_t reg) {
  ...
  while (reg.start != reg.end) {
    ...
    /* insert to list */
    _early_page_insert((void *)p - early_offset, size_bits - ARCH_PAGE_SHIFT);
    reg.start += (1UL << size_bits);
  }

  pages_alloc_handler = _early_pages_alloc;
  /* doing the page table bushiness */
  if (rt_aspace_load_page(&rt_kernel_space, (void *)init_mpr_align_start,
                          init_mpr_npage)) {
