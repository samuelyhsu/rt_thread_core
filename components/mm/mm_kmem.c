...
static void list_kernel_space(void) { rt_aspace_print_all(&rt_kernel_space); }
...
rt_ubase_t rt_kmem_pvoff(void) { return rt_pv_offset; }

void rt_kmem_pvoff_set(rt_ubase_t pvoff) { rt_pv_offset = pvoff; }
...
int rt_kmem_map_phy(void *va, void *pa, rt_size_t length, rt_size_t attr) {
  ...
  if (va == RT_NULL) {
    ...
    struct rt_mm_va_hint hint = {.flags = MMF_MAP_FIXED,
                                 ...
                                 .prefer = va,
                                 .map_size =
                                     RT_ALIGN(length + lo_off, ARCH_PAGE_SIZE)};

    err =
        rt_aspace_map_phy(&rt_kernel_space, &hint, attr, MM_PA_TO_OFF(pa), &va);

    if (err) {
      LOG_W("%s: map %p to %p (%p bytes) failed(err %d)", __func__, pa, va,
            length, err);
    }
  }
