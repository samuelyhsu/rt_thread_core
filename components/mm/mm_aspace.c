...
static int _do_prefetch(rt_aspace_t aspace, rt_varea_t varea, void *start,
  ...
  while (vaddr != end) {
    ...
    if (msg.response.status == MM_FAULT_STATUS_OK) {
      ...
      void *map = rt_hw_mmu_map(aspace, vaddr, store + PV_OFFSET, store_sz,
                                varea->attr);

      if (!map) {
        LOG_W("%s: MMU mapping failed for va %p to %p of %lx", __func__, vaddr,
              store + PV_OFFSET, store_sz);
      } else {
...
int _varea_install(rt_aspace_t aspace, rt_varea_t varea, rt_mm_va_hint_t hint) {
   ...
   */
  alloc_va = _find_free(aspace, hint->prefer, hint->map_size, hint->limit_start,
...
static inline int _not_in_range(void *start, rt_size_t length,
                                void *limit_start, rt_size_t limit_size) {
  if (start != RT_NULL)
    LOG_D("%s: [%p : %p] [%p : %p]", __func__, start, length, limit_start,
          limit_size);
...
static inline int _not_align(void *start, rt_size_t length, rt_size_t mask) {
  return (start != RT_NULL) && (((uintptr_t)start & mask) || (length & mask));
}
...
int _mm_aspace_map_phy(rt_aspace_t aspace, rt_varea_t varea,
  ...
  if (!aspace || !hint || !hint->limit_range_size || !hint->map_size) {
    ...
    err = -RT_EINVAL;
  } else if (_not_in_range(hint->limit_start, hint->limit_range_size,
    ...
    if (err == RT_EOK) {
      ...
      err = _do_named_map(aspace, varea->start, varea->size, (rt_size_t)pa_off,
                          attr);
...
int rt_aspace_map_phy_static(rt_aspace_t aspace, rt_varea_t varea,
  ...
  if (varea && hint) {
    ...
    LOG_D("%s: start %p size %p phy at %p", __func__, varea->start, varea->size,
          pa_off << MM_PAGE_SHIFT);
    err = _mm_aspace_map_phy(aspace, varea, hint, attr, pa_off, ret_va);
  } else {
    err = -RT_EINVAL;
  }
...
static inline void *_lower(void *a, void *b) { return a < b ? a : b; }
...
static void *_ascending_search(rt_varea_t varea, rt_size_t req_size,
  ...
  while (varea && varea->start < limit.end) {
    ...
    if (nx_va) {
      rt_size_t gap_size = _lower(limit.end, nx_va->start - 1) - candidate + 1;
