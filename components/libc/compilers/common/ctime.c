 ...
 * 2021-02-11     Meco Man     fix bug #3183 - align days[] and months[] to 4
 * bytes 2021-02-12     Meco Man     add errno 2012-12-08     Bernard
 * <clock_time.c> fix the issue of _timevalue.tv_usec initialization, which
 * found by Rob <rdent@iinet.net.au> 2021-02-12     Meco Man     move all of the
 * functions located in <clock_time.c> to this file 2021-03-15     Meco Man
 * fixed a bug of leaking memory in asctime() 2021-05-01     Meco Man support
 * fixed timezone 2021-07-21     Meco Man     implement that change/set timezone
 * APIs
 */
...
char *asctime_r(const struct tm *t, char *buf) {
  ...
  if ((int)rt_strlen(days) <= (t->tm_wday << 2) ||
      (int)rt_strlen(months) <= (t->tm_mon << 2)) {
    LOG_W(
        "asctime_r: the input parameters exceeded the limit, please check it.");
    *(int *)buf = *(int *)days;
 ...
 *
 * @return The value ((time_t)-1) is returned if the calendar time is not
 * available. If timer is not a NULL pointer, the return value is also stored in
 * timer.
 *
 */
...
rt_weak clock_t clock(void) { return rt_tick_get(); }
RTM_EXPORT(clock);
...
time_t timegm(struct tm *const t) {
  ...
  if (t->tm_sec >
      60) /* seconds after the minute - [0, 60] including leap second */
  {
...
int nanosleep(const struct timespec *rqtp, struct timespec *rmtp) {
  if (rqtp->tv_sec < 0 || rqtp->tv_nsec < 0 ||
      rqtp->tv_nsec >= NANOSECOND_PER_SECOND) {
    rt_set_errno(EINVAL);
    return -1;
  }
  ...
  tick = rqtp->tv_sec * RT_TICK_PER_SECOND +
         ((uint64_t)rqtp->tv_nsec * RT_TICK_PER_SECOND) / NANOSECOND_PER_SECOND;
  ...
  if (rt_get_errno() == -RT_EINTR) {
    if (rmtp) {
      ...
      rmtp->tv_nsec = (tick % RT_TICK_PER_SECOND) *
...
static int _rt_clock_time_system_init(void) {
  ...
  if (device != RT_NULL) {
    ...
    if (rt_device_control(device, RT_DEVICE_CTRL_RTC_GET_TIME, &time) ==
        RT_EOK) {
...
int clock_gettime(clockid_t clockid, struct timespec *tp) {
  ...
  case CLOCK_REALTIME: {
    ...
    tp->tv_nsec = (_timevalue.tv_usec +
                   (tick % RT_TICK_PER_SECOND) * MICROSECOND_PER_TICK) *
                  1000;
    rt_hw_interrupt_enable(level);
  } break;
...
int clock_nanosleep(clockid_t clockid, int flags, const struct timespec *rqtp,
  ...
  case CLOCK_REALTIME: {
    ...
    if ((flags & TIMER_ABSTIME) == TIMER_ABSTIME) {
      ...
      rt_int64_t tns = (rqtp->tv_nsec - _timevalue.tv_usec * 1000) *
                       (RT_TICK_PER_SECOND / NANOSECOND_PER_SECOND);
      tick = ts + tns;
      rt_tick_t rt_tick = rt_tick_get();
      tick = tick < rt_tick ? 0 : tick - rt_tick;
    } else {
      tick = rqtp->tv_sec * RT_TICK_PER_SECOND +
             ((uint64_t)(rqtp->tv_nsec) * RT_TICK_PER_SECOND) /
...
int clock_settime(clockid_t clockid, const struct timespec *tp) {
  ...
  if (device != RT_NULL) {
    ...
    if (rt_device_control(device, RT_DEVICE_CTRL_RTC_SET_TIME, &second) ==
        RT_EOK) {
      return 0;
    }
  }
...
int rt_timespec_to_tick(const struct timespec *time) {
  ...
  if (time != NULL) {
    ...
    tick = second * RT_TICK_PER_SECOND +
           nsecond * RT_TICK_PER_SECOND / NANOSECOND_PER_SECOND;
    if (tick < 0)
      tick = 0;
  }
...
static void rtthread_timer_wrapper(void *timerobj) {
  ...
  if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout()) {
    timer->reload = (timer->interval.tv_sec * NANOSECOND_PER_SECOND +
                    ...
                    clock_cpu_getres();
    if (timer->reload)
      rt_cputimer_control(&timer->cputimer, RT_TIMER_CTRL_SET_TIME,
                          &(timer->reload));
  } else
#endif /* RT_USING_CPUTIME */
  {
    timer->reload =
        (timer->interval.tv_sec * RT_TICK_PER_SECOND) +
        (timer->interval.tv_nsec * RT_TICK_PER_SECOND) / NANOSECOND_PER_SECOND;
    if (timer->reload)
...
void timer_id_lock() { rt_hw_spin_lock(&_timer_id_lock); }

void timer_id_unlock() { rt_hw_spin_unlock(&_timer_id_lock); }
...
int timer_create(clockid_t clockid, struct sigevent *evp, timer_t *timerid) {
  ...
  if (clockid > CLOCK_ID_MAX ||
      (evp->sigev_notify != SIGEV_NONE && evp->sigev_notify != SIGEV_SIGNAL)) {
    rt_set_errno(EINVAL);
    return -1;
  }
...
#ifdef RT_USING_CPUTIME
  if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout()) {
    rt_cputimer_init(&timer->cputimer, timername, rtthread_timer_wrapper, timer,
                     0, RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
  } else
#endif /* RT_USING_CPUTIME */
  {
    if (evp->sigev_notify == SIGEV_NONE)
      rt_timer_init(&timer->timer, timername, RT_NULL, RT_NULL, 0,
                    RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
    else
      rt_timer_init(&timer->timer, timername, rtthread_timer_wrapper, timer, 0,
                    RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
  }
...
int timer_gettime(timer_t timerid, struct itimerspec *its) {
  ...
  if (timer->status == ACTIVE) {
    ...
    if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout()) {
      ...
      rt_cputimer_control(&timer->cputimer, RT_TIMER_CTRL_GET_REMAIN_TIME,
                          &remain_tick);
...
#endif /* RT_USING_CPUTIME */
    {
      ...
      /* calculate 'nanosecond';  To avoid lost of accuracy, because
       * "RT_TICK_PER_SECOND" maybe 100, 1000, 1024 and so on.
       *
       *        remain_tick                  millisecond remain_tick *
       * MILLISECOND_PER_SECOND
       *  ------------------------- = --------------------------  --->
       * millisecond = -------------------------------------------
       *    RT_TICK_PER_SECOND          MILLISECOND_PER_SECOND
       * RT_TICK_PER_SECOND
       *
       *                    remain_tick * MILLISECOND_PER_SECOND remain_tick *
       * MILLISECOND_PER_SECOND * MICROSECOND_PER_SECOND millisecond =
       * ----------------------------------------  ---> nanosecond =
       * -------------------------------------------------------------------
       ...
       */
      nanoseconds =
          (((remaining % RT_TICK_PER_SECOND) * MILLISECOND_PER_SECOND) *
...
int timer_settime(timer_t timerid, int flags, const struct itimerspec *value,
  ...
  if (timer == NULL || value->it_interval.tv_nsec < 0 ||
      ...
      value->it_interval.tv_sec < 0 || value->it_value.tv_nsec < 0 ||
  ...
  /* calculate timer period(tick);  To avoid lost of accuracy, because
   * "RT_TICK_PER_SECOND" maybe 100, 1000, 1024 and so on.
   *
   *          tick                        nanosecond nanosecond *
   * RT_TICK_PER_SECOND
   *  ------------------------- = --------------------------  --->  tick =
   * ------------------------------------- RT_TICK_PER_SECOND
   * NANOSECOND_PER_SECOND                         NANOSECOND_PER_SECOND
   *
   */
#ifdef RT_USING_CPUTIME
  if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout()) {
    ...
    tick = (value->it_value.tv_sec * NANOSECOND_PER_SECOND +
...
#endif /* RT_USING_CPUTIME */
  {
    if ((flags & TIMER_ABSTIME) == TIMER_ABSTIME) {
...
#else
      rt_int64_t ts =
          ((value->it_value.tv_sec - _timevalue.tv_sec) * RT_TICK_PER_SECOND);
      rt_int64_t tns = (value->it_value.tv_nsec - _timevalue.tv_usec * 1000) *
                       (RT_TICK_PER_SECOND / NANOSECOND_PER_SECOND);
      ...
      timer->reload = (value->it_value.tv_sec * RT_TICK_PER_SECOND) +
                      value->it_value.tv_nsec *
                          (RT_TICK_PER_SECOND / NANOSECOND_PER_SECOND);
  }
  ...
  if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout()) {
    if ((value->it_interval.tv_sec == 0) && (value->it_interval.tv_nsec == 0))
      ...
      rt_cputimer_control(&timer->cputimer, RT_TIMER_CTRL_SET_PERIODIC,
                          RT_NULL);
...
int8_t tz_get(void) { return _current_timezone; }

int8_t tz_is_dst(void) { return 0; }
