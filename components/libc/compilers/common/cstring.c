 ...
 *        location pointed to by s, by writing zeros (bytes containing '\0') to
 * that area.
 *
 * @note  The bzero() function is deprecated (marked as LEGACY in POSIX.
 * 1-2001).
 */
void bzero(void *s, size_t n) { rt_memset(s, 0, n); }

void bcopy(const void *src, void *dest, size_t n) { rt_memcpy(dest, src, n); }
