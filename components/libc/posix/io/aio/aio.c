...
/**
 * The aio_cancel() function shall attempt to cancel one or more asynchronous
 * I/O requests currently outstanding against file descriptor fildes. The aiocbp
 * argument points to the asynchronous I/O control block for a particular
 * request to be canceled. If aiocbp is NULL, then all outstanding cancelable
 ...
 * canceled, then the normal asynchronous completion process shall take place
 ...
 * asynchronous I/O operation is the errno value that would be set by the
 ...
 * operation, as specified by the op argument, for I/O operations associated
 ...
 *
 * option: If op is O_DSYNC, all currently queued I/O operations shall be
 * completed as if by a call to fdatasync(); that is, as defined for
 * synchronized I/O data integrity completion.
 *
 * option: If op is O_SYNC, all currently queued I/O operations shall be
 * completed as if by a call to fsync(); that is, as defined for synchronized
 * I/O file integrity completion. If the aio_fsync() function fails, or if the
 * operation queued by aio_fsync() fails, then outstanding I/O operations are
 ...
 * success or failure of the operation. If the operation does not complete
 * successfully, the error status for the operation shall be set to indicate the
 * error. The aio_sigevent member determines the asynchronous notification to
 * occur as specified in Signal Generation and Delivery when all operations have
 * achieved synchronized I/O completion. All other members of the structure
 * referenced by aiocbp are ignored. If the control block referenced by aiocbp
 ...
 * aiocbp->aio_buf. The function call shall return when the read request has
 * been initiated or queued to the file or device (even when the data cannot be
 ...
 * priority minus aiocbp->aio_reqprio. If Thread Execution Scheduling is not
 * supported, then the base scheduling priority is that of the calling process;
 *
 ...
 * absolute position in the file as given by aio_offset, as if lseek() were
 * called immediately prior to the operation with an offset equal to aio_offset
 * and a whence equal to SEEK_SET. After a successful call to enqueue an
 * asynchronous I/O operation, the value of the file offset for the file is
 * unspecified.
 *
 ...
 * aiocbp->aio_fildes, the behavior of this function shall be according to the
 ...
 * corresponding read(), write(), or fsync() function call. If the error status
 * for the operation is equal to [EINPROGRESS], then the return status for the
 * operation is undefined. The aio_return() function may be called exactly once
 * to retrieve the return status of a given asynchronous operation; thereafter,
 * if the same aiocb structure is used in a call to aio_return() or aio_error(),
 * an error may be returned. When the aiocb structure referred to by aiocbp is
 * used to submit another asynchronous operation, then aio_return() may be
 ...
 * completed, until a signal interrupts the function, or, if timeout is not
 * NULL, until the time interval specified by timeout has passed. If any of the
 * aiocb structures in the list correspond to completed asynchronous I/O
 * operations (that is, the error status for the operation is not equal to
 * [EINPROGRESS]) at the time of the call, the function shall return without
 * suspending the calling thread. The list argument is an array of pointers to
 * asynchronous I/O control blocks. The nent argument indicates the number of
 * elements in the array. Each aiocb structure pointed to has been used in
 * initiating an asynchronous I/O request via aio_read(), aio_write(), or
 * lio_listio(). This array may contain null pointers, which are ignored. If
 * this array contains pointers that refer to aiocb structures that have not
 ...
 * aiocbp->aio_buf. The function shall return when the write request has been
 ...
 * set for the file descriptor, or if aio_fildes is associated with a device
 * that is incapable of seeking, write operations append to the file in the same
 * order as the calls were made, except under circumstances described in
 * Asynchronous I/O. After a successful call to enqueue an asynchronous I/O
 ...
 *
 * If the mode argument is LIO_NOWAIT, the function shall return immediately,
 * and asynchronous notification shall occur, according to the sig argument,
 * when all the I/O operations complete. If sig is NULL, then no asynchronous
 * notification shall occur. If sig is not NULL, asynchronous notification
 * occurs as specified in Signal Generation and Delivery when all the requests
 * in list have completed.
 *
 * The I/O requests enumerated by list are submitted in an unspecified order.
 *
 * The list argument is an array of pointers to aiocb structures. The array
 * contains nent elements. The array may contain NULL elements, which shall be
 * ignored.
 *
 * If the buffer pointed to by list or the aiocb structures pointed to by the
 * elements of the array list become illegal addresses before all asynchronous
 * I/O completed and, if necessary, the notification is sent, then the behavior
 * is undefined. If the buffers pointed to by the aio_buf member of the aiocb
 * structure pointed to by the elements of the array list become illegal
 * addresses prior to the asynchronous I/O associated with that aiocb structure
 * being completed, the behavior is undefined.
 *
 * The aio_lio_opcode field of each aiocb structure specifies the operation to
 * be performed. The supported operations are LIO_READ, LIO_WRITE, and LIO_NOP;
 * these symbols are defined in <aio.h>. The LIO_NOP operation causes the list
 * entry to be ignored. If the aio_lio_opcode element is equal to LIO_READ, then
 * an I/O operation is submitted as if by a call to aio_read() with the aiocbp
 * equal to the address of the aiocb structure. If the aio_lio_opcode element is
 * equal to LIO_WRITE, then an I/O operation is submitted as if by a call to
 * aio_write() with the aiocbp equal to the address of the aiocb structure.
 *
 * The aio_fildes member specifies the file descriptor on which the operation is
 * to be performed.
 *
 * The aio_buf member specifies the address of the buffer to or from which the
 * data is transferred.
 *
 ...
 *
 * The nent argument specifies how many elements are members of the list; that
