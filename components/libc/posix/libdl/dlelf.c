...
rt_err_t dlmodule_load_shared_object(struct rt_dlmodule *module,
                                     void *module_ptr) {
  ...
  for (index = 0; index < elf_module->e_phnum; index++) {
    ...
    LOG_D("LOAD segment: %d, 0x%p, 0x%08x", index, phdr[index].p_vaddr,
          phdr[index].p_memsz);

    if (phdr[index].p_memsz < phdr[index].p_filesz) {
      rt_kprintf("invalid elf: segment %d: p_memsz: %d, p_filesz: %d\n", index,
                 phdr[index].p_memsz, phdr[index].p_filesz);
      return RT_NULL;
    }
    if (!has_vstart) {
...
#endif
      LOG_D("relocate symbol %s shndx %d", strtab + sym->st_name,
            sym->st_shndx);
  ...
  for (index = 0; index < elf_module->e_shnum; index++) {
    ...
    if (rt_strcmp((const char *)(shstrab + shdr[index].sh_name), ELF_DYNSYM) ==
        0)
      break;
  }
  ...
  if (index != elf_module->e_shnum) {
    ...
    for (i = 0, count = 0; i < shdr[index].sh_size / sizeof(Elf_Sym); i++) {
      ...
      module->symtab[count].addr =
          (void *)(module->mem_space + symtab[i].st_value -
                   module->vstart_addr);
      module->symtab[count].name = rt_malloc(length);
      rt_memset((void *)module->symtab[count].name, 0, length);
      rt_memcpy((void *)module->symtab[count].name, strtab + symtab[i].st_name,
                length);
      count++;
    }
    ...
    for (i = 0; i < shdr[index].sh_size / sizeof(Elf_Sym); i++) {
      if (((flag & 0x01) == 0) &&
          (rt_strcmp((const char *)(strtab + symtab[i].st_name),
                     "dlmodule_thread_priority") == 0)) {
        flag |= 0x01;
        priority = *(rt_uint16_t *)(module->mem_space + symtab[i].st_value -
                                    module->vstart_addr);
      ...
      if (((flag & 0x02) == 0) &&
          (rt_strcmp((const char *)(strtab + symtab[i].st_name),
                     "dlmodule_thread_stacksize") == 0)) {
        flag |= 0x02;
        stacksize = *(rt_uint32_t *)(module->mem_space + symtab[i].st_value -
                                     module->vstart_addr);
...
rt_err_t dlmodule_load_relocated_object(struct rt_dlmodule *module,
                                        void *module_ptr) {
  ...
  for (index = 0; index < elf_module->e_shnum; index++) {
    ...
    if (IS_PROG(shdr[index]) && IS_ALLOC(shdr[index])) {
      ...
      LOG_D("load rodata 0x%x, size %d, rodata 0x%x", ptr, shdr[index].sh_size,
    ...
    if (IS_PROG(shdr[index]) && IS_AW(shdr[index])) {
      ...
      LOG_D("load data 0x%x, size %d, data 0x%x", ptr, shdr[index].sh_size,
            *(rt_uint32_t *)data_addr);
      ptr += shdr[index].sh_size;
    }
  ...
  /* set module entry */
  module->entry_addr =
      (rt_dlmodule_entry_func_t)((rt_uint8_t *)module->mem_space +
    ...
    for (i = 0; i < nr_reloc; i++) {
      ...
      if (sym->st_shndx != STN_UNDEF) {
        ...
        if ((ELF_ST_TYPE(sym->st_info) == STT_SECTION) ||
          ...
          if (rt_strncmp((const char *)(shstrab + shdr[sym->st_shndx].sh_name),
                         ELF_RODATA, 8) == 0) {
            /* relocate rodata section */
            LOG_D("rodata");
            addr = (Elf_Addr)(rodata_addr + sym->st_value);
          } else if (rt_strncmp(
                         (const char *)(shstrab + shdr[sym->st_shndx].sh_name),
                         ELF_BSS, 5) == 0) {
          ...
          addr = (Elf_Addr)((rt_uint8_t *)module->mem_space - module_addr +
                            sym->st_value);
        ...
        dlmodule_relocate(module, rel,
                          (Elf_Addr)((rt_uint8_t *)module->mem_space -
