...
static void _dlmodule_thread_entry(void *parameter) {
  ...
  if (module->cmd_line) {
    ...
    argc = _rt_module_split_arg((char *)module->cmd_line,
                                rt_strlen(module->cmd_line), argv);
    if (argc == 0)
      goto __exit;
  }
  ...
  LOG_D("run main entry: 0x%p with %s", module->entry_addr, module->cmd_line);
...
struct rt_dlmodule *dlmodule_create(void) {
  ...
  module = (struct rt_dlmodule *)rt_object_allocate(RT_Object_Class_Module,
                                                    "module");
  if (module) {
 ...
 */
struct rt_dlmodule *rt_module_self(void) { return dlmodule_self(); }
...
struct rt_dlmodule *dlmodule_load(const char *filename) {
  ...
  if ((elf_module->e_ident[EI_CLASS] != ELFCLASS32) &&
      (elf_module->e_ident[EI_CLASS] != ELFCLASS64)) {
...
struct rt_dlmodule *dlmodule_exec(const char *pgname, const char *cmd,
  ...
  if (module) {
    if (module->entry_addr) {
      ...
      tid = rt_thread_create(module->parent.name, _dlmodule_thread_entry,
                             (void *)module, module->stack_size,
                             module->priority, 10);
      if (tid) {
...
struct rt_dlmodule *dlmodule_load_custom(const char *filename,
...
struct rt_dlmodule *dlmodule_exec_custom(const char *pgname, const char *cmd,
                                         int cmd_size,
                                         struct rt_dlmodule_ops *ops) {
  struct rt_dlmodule *module = RT_NULL;
...
rt_uint32_t dlmodule_symbol_find(const char *sym_str) {
  ...
  for (index = _rt_module_symtab_begin; index != _rt_module_symtab_end;
       index++) {
...
int list_module(void) {
  ...
  for (node = list->next; node != list; node = node->next) {
    module =
        (struct rt_dlmodule *)(rt_list_entry(node, struct rt_object, list));
