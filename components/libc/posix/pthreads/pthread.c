 ...
 * 2022-05-10     xiangxistu   Modify the recycle logic about resource of
 * pthread.
 */
...
static void pthread_entry_stub(void *parameter) {
  ...
  /* According to "detachstate" to whether or not to recycle resource
   * immediately */
...
int pthread_create(pthread_t *pid, const pthread_attr_t *attr,
  ...
  if (rt_thread_init(ptd->tid, name, pthread_entry_stub, ptd, stack,
                     ptd->attr.stacksize, ptd->attr.schedparam.sched_priority,
                     20) != RT_EOK) {
    ret = EINVAL;
    goto __exit;
  }
...
int pthread_getconcurrency(void) { return concurrency_level; }
