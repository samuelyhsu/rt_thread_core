...
Thread::Thread(rt_uint32_t stack_size, rt_uint8_t priority, rt_uint32_t tick,
               const char *name)
    : _entry(RT_NULL), _param(RT_NULL), started(false) {
  rt_event_init(&_event, name, 0);

  _thread = rt_thread_create(name, (thread_func_t)func, this, stack_size,
                             priority, tick);
}

Thread::Thread(void (*entry)(void *p), void *p, rt_uint32_t stack_size,
               rt_uint8_t priority, rt_uint32_t tick, const char *name)
...
void Thread::run(void *parameter) { /* please overload this method */
}

rt_err_t Thread::wait(int32_t millisec) { return join(millisec); }
...
rt_err_t Thread::join(int32_t millisec) {
  if (started) {
    ...
    return rt_event_recv(&_event, 1, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                         tick, RT_NULL);
  } else {
    return -RT_ENOSYS;
  }
