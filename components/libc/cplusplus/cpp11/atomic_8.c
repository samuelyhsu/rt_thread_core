...
/*
 * override gcc builtin atomic function for std::atomic<int64_t>,
 * std::atomic<uint64_t>
...
bool __atomic_compare_exchange_8(volatile void *ptr, volatile void *expected,
...
#define __atomic_fetch_op_8(OPNAME, OP)                                        \
  uint64_t __atomic_fetch_##OPNAME##_8(volatile void *ptr, uint64_t val,       \
                                       int memorder) {                         \
    volatile uint64_t *val_ptr = (volatile uint64_t *)ptr;                     \
    rt_base_t level;                                                           \
    uint64_t tmp;                                                              \
...
__atomic_fetch_op_8(add, +) __atomic_fetch_op_8(sub, -)
    __atomic_fetch_op_8(and, &) __atomic_fetch_op_8(or, |)
