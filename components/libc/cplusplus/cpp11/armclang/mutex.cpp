...
static int check_create(volatile __ARM_TPL_mutex_t *__vm,
                        bool recursive = false) {
  if (__vm->data == 0) {
    uintptr_t mut_null = 0;
    arm_tpl_mutex_struct *mutex_p =
        (arm_tpl_mutex_struct *)rt_malloc(sizeof(arm_tpl_mutex_struct));
    if (mutex_p == nullptr)
      return -1;
...
extern "C" int __ARM_TPL_mutex_lock(__ARM_TPL_mutex_t *__m) {
  ...
  if (check_create(__vm))
    return -1;
  while (mutexLock((arm_tpl_mutex_struct *)(__vm->data), ARM_TPL_MAX_DELAY) !=
         0)
    ;
  return 0;
}
