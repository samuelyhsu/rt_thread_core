...
extern "C" __ARM_TPL_thread_id
__ARM_TPL_thread_get_id(const __ARM_TPL_thread_t *__t) {
  return (
      __ARM_TPL_thread_id)(((pthread_t)__t->data - CPP11_DEFAULT_ID_OFFSET));
}
...
extern "C" int __ARM_TPL_thread_nanosleep(const __ARM_TPL_timespec_t *__req,
  ...
  tick = __req->tv_sec * RT_TICK_PER_SECOND +
         (__req->tv_nsec * RT_TICK_PER_SECOND) / 1000000000;
  ...
  if (__rem) {
    ...
    __rem->tv_nsec =
        (tick % RT_TICK_PER_SECOND) * (1000000000 / RT_TICK_PER_SECOND);
  }
  return 0;
}

extern "C" unsigned __ARM_TPL_thread_hw_concurrency() { return 1; }
