...
int arm_tpl_cv::timedwait(rt_mutex_t lock, bool recursive,
...
extern "C" int __ARM_TPL_condvar_wait(__ARM_TPL_condvar_t *__cv,
  ...
  if (check_create(__vcv) != 0)
    return -1;
  struct arm_tpl_mutex_struct *tmutex =
      (struct arm_tpl_mutex_struct *)(__m->data);
...
extern "C" int __ARM_TPL_condvar_timedwait(__ARM_TPL_condvar_t *__cv,
  ...
  struct arm_tpl_mutex_struct *tmutex =
      (struct arm_tpl_mutex_struct *)(__m->data);
  unsigned int timeout_ms = (__ts->tv_sec - now.tv_sec) * 1000 +
                            (__ts->tv_nsec - now.tv_nsec) / 1000000;
  if (((arm_tpl_cv *)__vcv->data)
          ->timedwait(tmutex->mutex, tmutex->type == RECURSIVE, timeout_ms) < 0)
    return -1;
  return 0;
}
