...
rt_size_t at_server_send(at_server_t server, const char *buf, rt_size_t size) {
  ...
  if (server == RT_NULL) {
    LOG_E("input AT Server object is NULL, please create or get AT Server "
          "object!");
    return 0;
  }
 ...
 *         =0: receive failed
 */
rt_size_t at_server_recv(at_server_t server, char *buf, rt_size_t size,
                         rt_int32_t timeout) {
...
static rt_err_t at_check_args(const char *args, const char *args_format) {
  ...
  if (left_sq_bracket_num != right_sq_bracket_num ||
      left_angle_bracket_num != right_angle_bracket_num ||
...
static rt_err_t at_cmd_process(at_cmd_t cmd, const char *cmd_args) {
  ...
  if (cmd_args[0] == AT_CMD_EQUAL_MARK && cmd_args[1] == AT_CMD_QUESTION_MARK &&
    ...
    at_server_print_result(result);
             ...
             (cmd_args[0] >= AT_CMD_CHAR_0 && cmd_args[0] <= AT_CMD_CHAR_9 &&
              cmd_args[1] == AT_CMD_CR)) {
...
static rt_err_t at_cmd_get_name(const char *cmd_buffer, char *cmd_name) {
  ...
  for (i = 0; i < strlen(cmd_buffer) + 1; i++) {
    if (*(cmd_buffer + i) == AT_CMD_QUESTION_MARK ||
        *(cmd_buffer + i) == AT_CMD_EQUAL_MARK ||
        *(cmd_buffer + i) == AT_CMD_CR ||
        (*(cmd_buffer + i) >= AT_CMD_CHAR_0 &&
         *(cmd_buffer + i) <= AT_CMD_CHAR_9)) {
      cmd_name_len = i;
...
int at_server_init(void) {
  ...
  if (!at_server_local) {
    result = -RT_ENOMEM;
    LOG_E("AT server session initialize failed! No memory for at_server "
          "structure !");
    goto __exit;
  }
  ...
  if (!at_server_local->rx_notice) {
    LOG_E("AT server session initialize failed! at_rx_notice semaphore create "
          "failed!");
    result = -RT_ENOMEM;
    goto __exit;
  }
  ...
  if (at_server_local->device) {
    ...
    open_result = rt_device_open(at_server_local->device,
                                 RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_DMA_RX);
    /* using interrupt mode when DMA mode not supported */
    if (open_result == -RT_EIO) {
      open_result =
          rt_device_open(at_server_local->device,
    ...
    LOG_E("AT device initialize failed! Not find the device : %s.",
          AT_SERVER_DEVICE);
    result = -RT_ERROR;
    goto __exit;
  }
  ...
  rt_memcpy(at_server_local->end_mark, AT_CMD_END_MARK,
            sizeof(AT_CMD_END_MARK));

  at_server_local->parser_entry = server_parser;
  at_server_local->parser = rt_thread_create(
      "at_svr", (void (*)(void *parameter))server_parser, at_server_local,
      2 * 1024, RT_THREAD_PRIORITY_MAX / 3 - 1, 5);
...
__exit:
  if (!result) {
    ...
    LOG_E("RT-Thread AT server (V%s) initialize failed(%d).", AT_SW_VERSION,
          result);
  }
