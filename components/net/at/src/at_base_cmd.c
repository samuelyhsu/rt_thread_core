...
static at_result_t at_exec(void) { return AT_RESULT_OK; }
...
static at_result_t at_uart_query(void) {
  struct rt_serial_device *serial =
      (struct rt_serial_device *)at_get_server()->device;

  at_server_printfln("AT+UART=%d,%d,%d,%d,%d", serial->config.baud_rate,
                     serial->config.data_bits, serial->config.stop_bits,
                     serial->config.parity, 1);
...
static at_result_t at_uart_setup(const char *args) {
  ...
  argc = at_req_parse_args(args, req_expr, &baudrate, &databits, &stopbits,
                           &parity, &flow_control);
  if (argc != 5) {
  ...
  if (rt_device_control(at_get_server()->device, RT_DEVICE_CTRL_CONFIG,
                        &config) != RT_EOK) {
    return AT_RESULT_FAILE;
  }
...
AT_CMD_EXPORT("AT+UART",
              "=<baudrate>,<databits>,<stopbits>,<parity>,<flow_control>",
              RT_NULL, at_uart_query, at_uart_setup, RT_NULL);
