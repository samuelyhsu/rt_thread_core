...
static rt_err_t at_server_console_getchar(struct at_server *server, char *ch,
                                          rt_int32_t timeout) {
  *ch = console_getchar();
  return RT_EOK;
}
...
static void server_cli_parser(void) {
  ...
  static rt_err_t (*getchar_bak)(struct at_server * server, char *ch,
                                 rt_int32_t timeout);
  ...
  if (server) {
    rt_kprintf(
        "======== Welcome to using RT-Thread AT command server cli ========\n");
...
#define DELECT_KEY 0x7F
  ...
  static rt_err_t (*client_odev_rx_ind)(rt_device_t dev, rt_size_t size) =
      RT_NULL;
  rt_base_t level;
  ...
  if (client) {
    ...
    if (client_rx_fifo && at_client) {
      rt_kprintf("======== Welcome to using RT-Thread AT command client cli "
                 "========\n");
      rt_kprintf("Cli will forward your command to server port(%s). Press "
                 "'ESC' to exit.\n",
...
MSH_CMD_EXPORT(at, RT - Thread AT component cli
               : at<server | client[dev_name]>);
