...
extern rt_size_t at_utils_send(rt_device_t dev, rt_off_t pos,
                               const void *buffer, rt_size_t size);
extern rt_size_t at_vprintfln(rt_device_t device, const char *format,
                              va_list args);
 ...
 *          = RT_NULL: no memory
 */
at_response_t at_create_resp(rt_size_t buf_size, rt_size_t line_num,
                             rt_int32_t timeout) {
 ...
 *           = RT_NULL: no memory
 */
at_response_t at_resp_set_info(at_response_t resp, rt_size_t buf_size,
                               rt_size_t line_num, rt_int32_t timeout) {
  char *p_temp;
  RT_ASSERT(resp);
...
const char *at_resp_get_line(at_response_t resp, rt_size_t resp_line) {
  ...
  if (resp_line > resp->line_counts || resp_line <= 0) {
    LOG_E("AT response get line failed! Input response line(%d) error!",
          resp_line);
    return RT_NULL;
  }
 ...
 *        -7 : enter AT CLI mode
 */
int at_obj_exec_cmd(at_client_t client, at_response_t resp,
                    const char *cmd_expr, ...) {
  va_list args;
  ...
  if (client == RT_NULL) {
    LOG_E("input AT Client object is NULL, please create or get AT Client "
          "object!");
    return -RT_ERROR;
  }
  ...
  if (resp != RT_NULL) {
    if (rt_sem_take(client->resp_notice, resp->timeout) != RT_EOK) {
      ...
      LOG_W("execute command (%.*s) timeout (%d ticks)!", cmd_size, cmd,
            resp->timeout);
...
int at_client_obj_wait_connect(at_client_t client, rt_uint32_t timeout) {
  ...
  while (1) {
    ...
    if (rt_tick_get() - start_time > rt_tick_from_millisecond(timeout)) {
      LOG_E("wait AT client(%s) connect timeout(%d tick).", client_name,
            timeout);
      result = -RT_ETIMEOUT;
      break;
    }
 ...
 *         =0: send failed
 */
rt_size_t at_client_obj_send(at_client_t client, const char *buf,
                             rt_size_t size) {
  rt_size_t len;
...
rt_size_t at_client_obj_recv(at_client_t client, char *buf, rt_size_t size,
  ...
  while (1) {
    ...
    if (rt_sem_take(client->rx_notice, rt_tick_from_millisecond(timeout)) !=
        RT_EOK)
      break;
  }
 ...
 */
int at_obj_set_urc_table(at_client_t client, const struct at_urc *urc_table,
                         rt_size_t table_sz) {
  rt_size_t idx;
  ...
  if (client->urc_table_size == 0) {
    client->urc_table =
        (struct at_urc_table *)rt_calloc(1, sizeof(struct at_urc_table));
    ...
    new_urc_table = (struct at_urc_table *)rt_realloc(
        client->urc_table,
        client->urc_table_size * sizeof(struct at_urc_table) +
            sizeof(struct at_urc_table));
  ...
  for (i = 0; i < client->urc_table_size; i++) {
    for (j = 0; j < client->urc_table[i].urc_size; j++) {
      ...
      if ((prefix_len ? !rt_strncmp(buffer, urc->cmd_prefix, prefix_len) : 1) &&
          (suffix_len ? !rt_strncmp(buffer + bufsz - suffix_len,
                                    urc->cmd_suffix, suffix_len)
                      : 1)) {
        return urc;
      }
    }
    ...
    if (at_recv_readline(client) > 0) {
      if ((urc = get_urc_obj(client)) != RT_NULL) {
        ...
        if (resp->buf_len + client->recv_line_len < resp->buf_size) {
          ...
          rt_memcpy(resp->buf + resp->buf_len, client->recv_line_buf,
          ...
          LOG_E("Read response buffer failed. The Response buffer size is out "
                "of buffer size(%d)!",
                resp->buf_size);
        }
        /* check response result */
        if ((client->end_sign != 0) && (end_ch == client->end_sign) &&
            (resp->line_num == 0)) {
          /* get the end sign, return response state END_OK.*/
          client->resp_status = AT_RESP_OK;
        } else if (rt_memcmp(client->recv_line_buf, AT_RESP_END_OK,
                             rt_strlen(AT_RESP_END_OK)) == 0 &&
                   resp->line_num == 0) {
          /* get the end data by response result, return response state END_OK.
           */
          client->resp_status = AT_RESP_OK;
        } else if (rt_strstr(client->recv_line_buf, AT_RESP_END_ERROR) ||
                   (rt_memcmp(client->recv_line_buf, AT_RESP_END_FAIL,
                              rt_strlen(AT_RESP_END_FAIL)) == 0)) {
...
#define AT_CLIENT_THREAD_NAME "at_clnt"
  ...
  if (client->rx_notice == RT_NULL) {
    LOG_E("AT client initialize failed! at_client_notice semaphore create "
          "failed!");
    result = -RT_ENOMEM;
    goto __exit;
  }
  ...
  if (client->resp_notice == RT_NULL) {
    LOG_E(
        "AT client initialize failed! at_client_resp semaphore create failed!");
    result = -RT_ENOMEM;
    goto __exit;
  }
  ...
  client->parser =
      rt_thread_create(name, (void (*)(void *parameter))client_parser, client,
                       1024 + 512, RT_THREAD_PRIORITY_MAX / 3 - 1, 5);
...
int at_client_init(const char *dev_name, rt_size_t recv_bufsz) {
  ...
  if (idx >= AT_CLIENT_NUM_MAX) {
    LOG_E("AT client initialize failed! Check the maximum number(%d) of AT "
          "client.",
          AT_CLIENT_NUM_MAX);
    result = -RT_EFULL;
    goto __exit;
  ...
  if (client->device) {
    ...
    open_result = rt_device_open(client->device,
...
__exit:
  if (result == RT_EOK) {
    ...
    LOG_I("AT client(V%s) on device %s initialize success.", AT_SW_VERSION,
          dev_name);
  } else {
    LOG_E("AT client(V%s) on device %s initialize failed(%d).", AT_SW_VERSION,
          dev_name, result);
  }
