...
typedef void (*at_evt_cb_t)(struct at_socket *socket, at_socket_evt_t event,
                            const char *buff, size_t bfsz);

/* A callback prototype to inform about events for AT socket */
typedef void (*at_socket_callback)(struct at_socket *conn, int event,
                                   uint16_t len);
...
struct at_socket_ops {
  ...
  int (*at_send)(struct at_socket *socket, const char *buff, size_t bfsz,
...
#define sendto(socket, data, size, flags, to, tolen)                           \
  at_sendto(socket, data, size, flags, to, tolen)
#define send(socket, data, size, flags) at_send(socket, data, size, flags)
#define recvfrom(socket, mem, len, flags, from, fromlen)                       \
  at_recvfrom(socket, mem, len, flags, from, fromlen)
...
#define getaddrinfo(nodename, servname, hints, res)                            \
  at_getaddrinfo(nodename, servname, hints, res)
