...
#ifdef AT_USING_SOCKET_SERVER
static void at_connect_notice_cb(struct at_socket *sock, at_socket_evt_t event,
...
struct at_socket *at_get_base_socket(int base_socket) {
  ...
  rt_slist_for_each(node, &_socket_list) {
    at_sock = rt_slist_entry(node, struct at_socket, list);
    if (base_socket == (int)at_sock->user_data &&
        at_sock->state != AT_SOCKET_LISTEN) {
      if (at_sock && at_sock->magic == AT_SOCKET_MAGIC) {
        rt_hw_interrupt_enable(level);
        return at_sock;
      }
...
static rt_err_t at_recvpkt_put(rt_slist_t *rlist, const char *ptr,
                               size_t length) {
...
static size_t at_recvpkt_get(rt_slist_t *rlist, char *mem, size_t len) {
  ...
  for (node = rt_slist_first(rlist); node;) {
    ...
    if (page_pos >= len - content_pos) {
      rt_memcpy((char *)mem + content_pos, pkt->buff + pkt->bfsz_index,
                len - content_pos);
...
static void at_do_event_changes(struct at_socket *sock, at_event_t event,
                                rt_bool_t is_plus) {
  switch (event) {
  case AT_EVENT_SEND: {
    if (is_plus) {
...
static struct at_socket *alloc_socket_by_device(struct at_device *device,
  ...
  if (device->class->socket_ops->at_socket != RT_NULL) {
    ...
    for (idx = 0; idx < device->class->socket_num && device->sockets[idx].magic;
         idx++)
      ;
  }
  ...
  if ((sock->recv_notice = rt_sem_create(name, 0, RT_IPC_FLAG_FIFO)) ==
      RT_NULL) {
...
static void at_recv_notice_cb(struct at_socket *sock, at_socket_evt_t event,
                              const char *buff, size_t bfsz);
static void at_closed_notice_cb(struct at_socket *sock, at_socket_evt_t event,
                                const char *buff, size_t bfsz);
...
static int socketaddr_to_ipaddr_port(const struct sockaddr *sockaddr,
...
static int ipaddr_port_to_socketaddr(struct sockaddr *sockaddr, ip_addr_t *addr,
                                     uint16_t *port) {
...
#ifdef AT_USING_SOCKET_SERVER
static int (*store_at_socket_temporary)(struct at_device *device,
                                        enum at_socket_type type);
static void at_connect_notice_cb(struct at_socket *sock, at_socket_evt_t event,
                                 const char *buff, size_t bfsz) {
  RT_ASSERT(buff);
  ...
  rt_slist_for_each(node, &_socket_list) {
    at_sock = rt_slist_entry(node, struct at_socket, list);
    if (at_sock && at_sock->magic == AT_SOCKET_MAGIC &&
        at_sock->listen.is_listen == RT_TRUE) {
      break;
    }
    at_sock = RT_NULL;
  }
  ...
  if (at_recvpkt_put(&(at_sock->recvpkt_list), socket_info,
                     AT_SOCKET_INFO_LEN) != RT_EOK) {
    rt_free((void *)buff);
...
int at_connect(int socket, const struct sockaddr *name, socklen_t namelen) {
  ...
  if (sock->ops->at_connect(sock, ipstr, remote_port, sock->type, RT_TRUE) <
      0) {
    result = -1;
    goto __exit;
  }
...
int at_accept(int socket, struct sockaddr *name, socklen_t *namelen) {
  ...
  if (rt_sem_take(sock->recv_notice, RT_WAITING_FOREVER) < 0) {
    ...
    rt_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
    at_recvpkt_get(&(sock->recvpkt_list), (char *)&receive_buff,
                   AT_SOCKET_INFO_LEN);
    rt_mutex_release(sock->recv_lock);
  }
  ...
  LOG_D("Accept: [socket :%d, base_socket:%d]", new_socket,
        (int)new_sock->user_data);
...
int at_recvfrom(int socket, void *mem, size_t len, int flags,
  ...
  rt_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
  if ((recv_len = at_recvpkt_get(&(sock->recvpkt_list), (char *)mem, len)) >
      0) {
    rt_mutex_release(sock->recv_lock);
    goto __exit;
  }
  ...
  if (sock->state == AT_SOCKET_CLOSED) {
    ...
    goto __exit;
             ...
             sock->state != AT_SOCKET_OPEN) {
    LOG_E("received data error, current socket (%d) state (%d) is error.",
          socket, sock->state);
    result = -1;
    goto __exit;
  }
...
int at_sendto(int socket, const void *data, size_t size, int flags,
  ...
  case AT_SOCKET_TCP:
    if (sock->state == AT_SOCKET_CLOSED) {
      ...
      goto __exit;
               ...
               sock->state != AT_SOCKET_OPEN) {
      LOG_E("send data error, current socket (%d) state (%d) is error.", socket,
            sock->state);
      result = -1;
      goto __exit;
    }

    if ((len = sock->ops->at_send(sock, (const char *)data, size, sock->type)) <
        0) {
      result = -1;
      goto __exit;
    }
...
int at_getsockopt(int socket, int level, int optname, void *optval,
                  socklen_t *optlen) {
...
int at_setsockopt(int socket, int level, int optname, const void *optval,
                  socklen_t optlen) {
  struct at_socket *sock;
...
int at_getaddrinfo(const char *nodename, const char *servname,
  ...
  *res = RT_NULL;
  ...
  if (hints != RT_NULL) {
    ...
    if (hints->ai_family != AF_AT && hints->ai_family != AF_INET &&
  ...
  if (nodename != RT_NULL) {
    ...
    if ((hints != RT_NULL) && (hints->ai_flags & AI_NUMERICHOST)) {
      ...
      if (idx < strlen(nodename)) {
        if (device->class->socket_ops->at_domain_resolve((char *)nodename,
                                                         ip_str) != 0) {
          return EAI_FAIL;
        }
      } else {
...
#error "not support IPV6."
  ...
  RT_ASSERT(total_size <= sizeof(struct addrinfo) +
  ...
  sa = (struct sockaddr_storage *)(void *)((uint8_t *)ai +
