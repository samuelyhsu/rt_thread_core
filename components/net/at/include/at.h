...
#define AT_CMD_EXPORT(_name_, _args_expr_, _test_, _query_, _setup_, _exec_)   \
  rt_used static const struct at_cmd                                           \
      __at_cmd_##_test_##_query_##_setup_##_exec_ rt_section("RtAtCmdTab") = { \
          _name_, _args_expr_, _test_, _query_, _setup_, _exec_,               \
  };
...
int at_obj_set_urc_table(at_client_t client, const struct at_urc *table,
                         rt_size_t size);
 ...
 * If there are multiple AT Client in the program, these functions can operate
 * on the first initialized AT client.
 */

#define at_exec_cmd(resp, ...)                                                 \
  at_obj_exec_cmd(at_client_get_first(), resp, __VA_ARGS__)
#define at_client_wait_connect(timeout)                                        \
  at_client_obj_wait_connect(at_client_get_first(), timeout)
#define at_client_send(buf, size)                                              \
...
#define at_set_urc_table(urc_table, table_sz)                                  \
