...
/* whether the network interface device is 'up' (set by the network interface
 * driver or application) */
#define NETDEV_FLAG_UP 0x01U
/* if set, the network interface device has broadcast capability, only supported
 * in the 'lwIP' stack */
#define NETDEV_FLAG_BROADCAST 0x02U
/* if set, the network interface device has an active link (set by the network
 * interface driver) */
#define NETDEV_FLAG_LINK_UP 0x04U
/* if set, the network interface device is an ethernet device using ARP, only
 * supported in the 'lwIP' stack */
#define NETDEV_FLAG_ETHARP 0x08U
/* if set, the network interface device is an ethernet device, only supported in
 * the 'lwIP' stack */
#define NETDEV_FLAG_ETHERNET 0x10U
/* if set, the network interface device has IGMP capability, only supported in
 * the 'lwIP' stack */
#define NETDEV_FLAG_IGMP 0x20U
/* if set, the network interface device has MLD6 capability, only supported in
 * the 'lwIP' stack */
#define NETDEV_FLAG_MLD6 0x40U
/* if set, the network interface device connected to internet successfully (set
 * by the network interface driver) */
#define NETDEV_FLAG_INTERNET_UP 0x80U
/* if set, the network interface device has DHCP capability (set by the network
 * interface device driver or application) */
...
/* function prototype for network interface device status or address change
 * callback functions */
typedef void (*netdev_callback_fn)(struct netdev *netdev,
                                   enum netdev_cb_type type);
...
struct netdev {
  ...
  netdev_callback_fn
      status_callback; /* network interface device flags change callback */
  netdev_callback_fn addr_callback; /* network interface device address
...
struct netdev_ops {
  ...
  int (*set_dns_server)(struct netdev *netdev, uint8_t dns_num,
...
#define netdev_is_up(netdev)                                                   \
  (((netdev)->flags & NETDEV_FLAG_UP) ? (uint8_t)1 : (uint8_t)0)
#define netdev_is_link_up(netdev)                                              \
  (((netdev)->flags & NETDEV_FLAG_LINK_UP) ? (uint8_t)1 : (uint8_t)0)
#define netdev_is_internet_up(netdev)                                          \
  (((netdev)->flags & NETDEV_FLAG_INTERNET_UP) ? (uint8_t)1 : (uint8_t)0)
#define netdev_is_dhcp_enabled(netdev)                                         \
  (((netdev)->flags & NETDEV_FLAG_DHCP) ? (uint8_t)1 : (uint8_t)0)
...
/* Set network interface device callback, it can be called when the status or
 * address changed */
...
/* Set network interface device status and address, this function can only be
 * called in the network interface device driver */
