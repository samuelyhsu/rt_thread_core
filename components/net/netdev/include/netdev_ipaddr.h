...
#define ip4_addr_isany(ipaddr)                                                 \
  ((ipaddr) == NULL || ip4_addr_isany_val(*(ipaddr)))
...
#define inet_aton(cp, addr)                                                    \
  ((IP_IS_V6_VAL(*addr)) ? netdev_ip6addr_aton(cp, ip_2_ip6(addr))             \
                         : netdev_ip4addr_aton(cp, ip_2_ip4(addr)))
#define inet_ntoa(addr)                                                        \
  ((IP_IS_V6_VAL(addr)) ? netdev_ip6addr_ntoa(ip_2_ip6(&addr))                 \
                        : netdev_ip4addr_ntoa(ip_2_ip4(&addr)))
#define inet_ntoa_r(addr, buf, buflen)                                         \
  ((IP_IS_V6_VAL(addr)) ? netdev_ip6addr_ntoa_r(ip_2_ip6(&addr), buf, buflen)  \
...
#define inet_ntoa_r(addr, buf, buflen)                                         \
  netdev_ip4addr_ntoa_r((const ip4_addr_t *)&(addr), buf, buflen)
#else /* NETDEV_IPV6 */
...
#define inet_ntoa_r(addr, buf, buflen)                                         \
  netdev_ip6addr_ntoa_r((const ip6_addr_t *)&(addr), buf, buflen)
