...
int netdev_register(struct netdev *netdev, const char *name, void *user_data) {
  ...
  flags_mask = NETDEV_FLAG_UP | NETDEV_FLAG_LINK_UP | NETDEV_FLAG_INTERNET_UP |
               NETDEV_FLAG_DHCP;
  ...
  if (rt_strlen(name) > RT_NAME_MAX) {
    ...
    LOG_E("netdev name[%s] length is so long that have been cut into [%s].",
          name, netdev_name);
  }
  ...
  for (node = &(netdev_list->list); node; node = rt_slist_next(node)) {
    ...
    if (cur_netdev == netdev) {
      ...
      if (netdev_list == netdev &&
          rt_slist_next(&netdev_list->list) == RT_NULL) {
        netdev_list = RT_NULL;
      } else {
...
struct netdev *netdev_get_by_name(const char *name) {
  ...
  for (node = &(netdev_list->list); node; node = rt_slist_next(node)) {
    netdev = rt_slist_entry(node, struct netdev, list);
    if (netdev && (rt_strncmp(netdev->name, name,
                              rt_strlen(netdev->name) < RT_NAME_MAX
                                  ? rt_strlen(netdev->name)
                                  : RT_NAME_MAX) == 0)) {
      rt_hw_interrupt_enable(level);
      return netdev;
    }
...
void netdev_set_default(struct netdev *netdev) {
  if (netdev) {
    ...
    LOG_D("Setting default network interface device name(%s) successfully.",
          netdev->name);
  }
}
...
int netdev_set_up(struct netdev *netdev) {
  ...
  if (!netdev->ops || !netdev->ops->set_up) {
    LOG_E("The network interface device(%s) not support to set status.",
          netdev->name);
    return -RT_ERROR;
  }
  ...
  /* execute enable network interface device operations by network interface
   * device driver */
...
int netdev_set_down(struct netdev *netdev) {
  ...
  /* execute disable network interface device operations by network interface
   * driver */
  return netdev->ops->set_down(netdev);
}

/**
 * This function will control network interface device DHCP capability enable or
 * disable.
 *
...
int netdev_dhcp_enabled(struct netdev *netdev, rt_bool_t is_enabled) {
  ...
  if (!netdev->ops || !netdev->ops->set_dhcp) {
    LOG_E("The network interface device(%s) not support to set DHCP status.",
          netdev->name);
    return -RT_ERROR;
  }
...
int netdev_set_ipaddr(struct netdev *netdev, const ip_addr_t *ip_addr) {
  ...
  if (!netdev->ops || !netdev->ops->set_addr_info) {
    LOG_E("The network interface device(%s) not support to set IP address.",
          netdev->name);
    return -RT_ERROR;
  }

  if (netdev_is_dhcp_enabled(netdev)) {
    LOG_E("The network interface device(%s) DHCP capability is enable, not "
          "support set IP address.",
          netdev->name);
    return -RT_ERROR;
  }

  /* execute network interface device set IP address operations */
  return netdev->ops->set_addr_info(netdev, (ip_addr_t *)ip_addr, RT_NULL,
                                    RT_NULL);
}
...
int netdev_set_netmask(struct netdev *netdev, const ip_addr_t *netmask) {
  ...
  if (!netdev->ops || !netdev->ops->set_addr_info) {
    LOG_E(
        "The network interface device(%s) not support to set netmask address.",
        netdev->name);
    return -RT_ERROR;
  }
  ...
  if (netdev_is_dhcp_enabled(netdev)) {
    LOG_E("The network interface device(%s) DHCP capability is enable, not "
          "support set netmask address.",
          netdev->name);
    return -RT_ERROR;
  }

  /* execute network interface device set netmask address operations */
  return netdev->ops->set_addr_info(netdev, RT_NULL, (ip_addr_t *)netmask,
                                    RT_NULL);
}
...
int netdev_set_gw(struct netdev *netdev, const ip_addr_t *gw) {
  ...
  if (!netdev->ops || !netdev->ops->set_addr_info) {
    LOG_E(
        "The network interface device(%s) not support to set gateway address.",
        netdev->name);
    return -RT_ERROR;
  }
 ...
 *         -1: set netmask address failed
 */
int netdev_set_dns_server(struct netdev *netdev, uint8_t dns_num,
                          const ip_addr_t *dns_server) {
  RT_ASSERT(netdev);
  RT_ASSERT(dns_server);

  if (dns_num >= NETDEV_DNS_SERVERS_NUM) {
    LOG_E("The number of DNS servers(%d) set exceeds the maximum number(%d).",
          dns_num + 1, NETDEV_DNS_SERVERS_NUM);
    return -RT_ERROR;
  }

  if (!netdev->ops || !netdev->ops->set_dns_server) {
    LOG_E("The network interface device(%s) not support to set DNS server "
          "address.",
          netdev->name);
    return -RT_ERROR;
  }
...
/**
 * This function will set callback to be called when the network interface
 * device status has been changed.
 *
 * @param netdev the network interface device to change
 * @param status_callback the callback be called when the status has been
 * changed.
 */
 ...
 *
 * @param netdev the network interface device to change
 * @param addr_callback the callback be called when the address has been
 * changed.
 */
 ...
 * @param ipaddr the new IP address
 */
void netdev_low_level_set_ipaddr(struct netdev *netdev,
 ...
 * @param netmask the new netmask address
 */
void netdev_low_level_set_netmask(struct netdev *netdev,
 ...
 *
 */
void netdev_low_level_set_dns_server(struct netdev *netdev, uint8_t dns_num,
                                     const ip_addr_t *dns_server) {
  unsigned int index;
...
void netdev_low_level_set_status(struct netdev *netdev, rt_bool_t is_up) {
  if (netdev && netdev_is_up(netdev) != is_up) {
    ...
    if (netdev->status_callback) {
      netdev->status_callback(netdev, is_up ? NETDEV_CB_STATUS_UP
...
void netdev_low_level_set_link_status(struct netdev *netdev, rt_bool_t is_up) {
  if (netdev && netdev_is_link_up(netdev) != is_up) {
    ...
    if (netdev->status_callback) {
      netdev->status_callback(netdev, is_up ? NETDEV_CB_STATUS_LINK_UP
 ...
 */
void netdev_low_level_set_internet_status(struct netdev *netdev,
                                          rt_bool_t is_up) {
  if (netdev && netdev_is_internet_up(netdev) != is_up) {
    ...
    /* execute  network interface device status change callback function */
    if (netdev->status_callback) {
      netdev->status_callback(netdev, is_up ? NETDEV_CB_STATUS_INTERNET_UP
 ...
 */
void netdev_low_level_set_dhcp_status(struct netdev *netdev,
                                      rt_bool_t is_enable) {
  if (netdev && netdev_is_dhcp_enabled(netdev) != is_enable) {
    ...
    if (netdev->status_callback) {
      netdev->status_callback(netdev, is_enable
  ...
  for (node = &(cur_netdev_list->list); node; node = rt_slist_next(node)) {
    ...
    for (index = 0; index < NETDEV_DNS_SERVERS_NUM; index++) {
      rt_kprintf("dns server #%d: %s\n", index,
                 inet_ntoa(netdev->dns_servers[index]));
    }
...
static void netdev_set_if(char *netdev_name, char *ip_addr, char *gw_addr,
                          char *nm_addr) {
  struct netdev *netdev = RT_NULL;
  ip_addr_t addr;
...
int netdev_ifconfig(int argc, char **argv) {
  if (argc == 1) {
    ...
    rt_kprintf("bad parameter! e.g: ifconfig e0 192.168.1.30 192.168.1.1 "
               "255.255.255.0\n");
#ifdef RT_LWIP_DHCP
...
MSH_CMD_EXPORT_ALIAS(netdev_ifconfig, ifconfig,
                     list the information of all network interfaces);
#endif /* NETDEV_USING_IFCONFIG */

#ifdef NETDEV_USING_PING
int netdev_cmd_ping(char *target_name, char *netdev_name, rt_uint32_t times,
                    rt_size_t size) {
...
#define NETDEV_PING_IS_COMMONICABLE(netdev)                                    \
  ((netdev) && (netdev)->ops && (netdev)->ops->ping && netdev_is_up(netdev) && \
  ...
  if (NETDEV_PING_IS_COMMONICABLE(netdev_default)) {
    ...
    if (netdev == RT_NULL) {
      ...
      rt_kprintf(
          "ping: network interface device(%s) not support ping feature.\n",
          netdev->name);
      return -RT_ERROR;
  ...
  for (index = 0; index < times; index++) {
    ...
    ret = netdev->ops->ping(netdev, (const char *)target_name, size,
                            NETDEV_PING_RECV_TIMEO, &ping_resp);
    if (ret == -RT_ETIMEOUT) {
      rt_kprintf("ping: from %s icmp_seq=%d timeout\n",
                 (ip_addr_isany(&(ping_resp.ip_addr)))
                     ? target_name
      ...
      if (ping_resp.ttl == 0) {
        rt_kprintf("%d bytes from %s icmp_seq=%d time=%d ms\n",
                   ping_resp.data_len, inet_ntoa(ping_resp.ip_addr), index,
                   ping_resp.ticks);
      } else {
        rt_kprintf("%d bytes from %s icmp_seq=%d ttl=%d time=%d ms\n",
                   ping_resp.data_len, inet_ntoa(ping_resp.ip_addr), index,
                   ping_resp.ttl, ping_resp.ticks);
      }
    }

    /* if the response time is more than NETDEV_PING_DELAY, no need to delay */
    delay_tick =
        ((rt_tick_get() - start_tick) > NETDEV_PING_DELAY) || (index == times)
            ? 0
            : NETDEV_PING_DELAY;
...
static void netdev_set_dns(char *netdev_name, uint8_t dns_num,
                           char *dns_server) {
  struct netdev *netdev = RT_NULL;
  ip_addr_t dns_addr;
