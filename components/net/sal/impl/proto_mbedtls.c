...
int mbedtls_net_send_cb(void *ctx, const unsigned char *buf, size_t len) {
  ...
  ret = pf->skt_ops->sendto((int)sock->user_data, (void *)buf, len, 0, RT_NULL,
                            RT_NULL);
  if (ret < 0) {
#ifdef RT_USING_DFS
...
int mbedtls_net_recv_cb(void *ctx, unsigned char *buf, size_t len) {
  ...
  ret = pf->skt_ops->recvfrom((int)sock->user_data, (void *)buf, len, 0,
                              RT_NULL, RT_NULL);
  if (ret < 0) {
#ifdef RT_USING_DFS
    if ((fcntl(socket, F_GETFL) & O_NONBLOCK) == O_NONBLOCK)
...
static int mbedtls_connect(void *sock) {
  ...
  mbedtls_ssl_set_bio(&session->ssl, &session->server_fd, mbedtls_net_send_cb,
                      mbedtls_net_recv_cb, RT_NULL);
  ...
  if (ret != 0) {
    ...
    mbedtls_x509_crt_verify_info((char *)session->buffer, session->buffer_len,
                                 "  ! ", ret);
    goto __exit;
  }
