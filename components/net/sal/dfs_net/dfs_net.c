...
int dfs_net_getsocket(int fd) {
  ...
  if (_dfs_fd->vnode->type != FT_SOCKET)
    socket = -1;
  else
    socket = (int)(size_t)_dfs_fd->vnode->data;
...
const struct dfs_file_ops _net_fops = {
    NULL, /* open     */
    dfs_net_close, dfs_net_ioctl, dfs_net_read,
    dfs_net_write, NULL,          NULL, /* lseek    */
    NULL,                               /* getdents */
    dfs_net_poll,
};

const struct dfs_file_ops *dfs_net_get_fops(void) { return &_net_fops; }
