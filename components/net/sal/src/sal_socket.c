...
int sal_getsockopt(int socket, int level, int optname, void *optval,
  ...
  return pf->skt_ops->getsockopt((int)(size_t)sock->user_data, level, optname,
                                 optval, optlen);
}
...
#else
  return pf->skt_ops->setsockopt((int)(size_t)sock->user_data, level, optname,
                                 optval, optlen);
#endif /* SAL_USING_TLS */
}
...
int sal_recvfrom(int socket, void *mem, size_t len, int flags,
  ...
  if (SAL_SOCKOPS_PROTO_TLS_VALID(sock, recv)) {
    ...
    return pf->skt_ops->recvfrom((int)(size_t)sock->user_data, mem, len, flags,
                                 from, fromlen);
  }
#else
  return pf->skt_ops->recvfrom((int)(size_t)sock->user_data, mem, len, flags,
...
#else
  return pf->skt_ops->sendto((int)(size_t)sock->user_data, dataptr, size, flags,
                             to, tolen);
#endif
}
