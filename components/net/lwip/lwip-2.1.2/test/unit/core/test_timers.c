...
static void do_test_cyclic_timers(u32_t offset) {
  ...
  fail_unless((*list_head)->time ==
              (u32_t)(lwip_sys_now + test_cyclic.interval_ms));
}
...
START_TEST(test_long_timer) {
  ...
  sys_timeout(LWIP_UINT32_MAX / 4, dummy_handler,
              LWIP_PTR_NUMERIC_CAST(void *, 0));
...
Suite *timers_suite(void) {
  ...
  return create_suite("TIMERS", tests, LWIP_ARRAYSIZE(tests), timers_setup,
                      timers_teardown);
}
