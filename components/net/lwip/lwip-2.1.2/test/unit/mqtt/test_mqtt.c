...
START_TEST(basic_connect) {
  ...
  struct netif netif;
  err_t err;
  struct mqtt_connect_client_info_t client_info = {"dumm", NULL, NULL, 10,
                                                   NULL,   NULL, 0,    0};
  struct pbuf *p;
  ...
  err = mqtt_client_connect(client, &test_mqtt_remote_ip, 1234,
                            test_mqtt_connection_cb, NULL, &client_info);
  ...
  if (client->conn->recv(client->conn->callback_arg, client->conn, p, ERR_OK) !=
      ERR_OK) {
    pbuf_free(p);
  }
...
Suite *mqtt_suite(void) {
  ...
  return create_suite("MQTT", tests, sizeof(tests) / sizeof(testfunc),
                      mqtt_setup, mqtt_teardown);
}
