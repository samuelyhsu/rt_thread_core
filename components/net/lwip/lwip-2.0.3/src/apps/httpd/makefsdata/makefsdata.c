...
int deflate_level =
    10; /* default compression level, can be changed via command line */
...
static void print_usage(void) {
  printf(
      " Usage: htmlgen [targetdir] [-s] [-e] [-i] [-11] [-nossi] [-c] "
      "[-f:<filename>] [-m] [-svr:<name>]" USAGE_ARG_DEFLATE NEWLINE NEWLINE);
...
int main(int argc, char *argv[]) {
  ...
  sprintf(curSubdir,
          ""); /* start off in web page's root directory - relative paths */
...
u8_t *get_file_data(const char *filename, int *file_size, int can_be_compressed,
                    int *is_compressed) {
  FILE *inFile;
  size_t fsize = 0;
  u8_t *buf;
...
int write_checksums(FILE *struct_file, const char *varname, u16_t hdr_len,
                    u16_t hdr_chksum, const u8_t *file_data, size_t file_size) {
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename) {
  ...
  fprintf(data_file, "/* %s (%d chars) */" NEWLINE, qualifiedName,
  ...
  file_data =
      get_file_data(filename, &file_size, includeHttpHeader && has_content_len,
                    &is_compressed);
