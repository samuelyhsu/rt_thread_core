...
/** Remove all pcbs on listen-, active- and time-wait-list (bound- isn't
 * exported). */
...
static struct pbuf *tcp_create_segment_wnd(ip_addr_t *src_ip, ip_addr_t *dst_ip,
  ...
  u16_t pbuf_len =
      (u16_t)(sizeof(struct ip_hdr) + sizeof(struct tcp_hdr) + data_len);
  ...
  tcphdr->chksum =
      inet_chksum_pseudo(p, src_ip, dst_ip, IP_PROTO_TCP, p->tot_len);
...
struct pbuf *tcp_create_segment(ip_addr_t *src_ip, ip_addr_t *dst_ip,
                                u16_t src_port, u16_t dst_port, void *data,
                                size_t data_len, u32_t seqno, u32_t ackno,
                                u8_t headerflags) {
 ...
 */
struct pbuf *tcp_create_rx_segment(struct tcp_pcb *pcb, void *data,
                                   size_t data_len, u32_t seqno_offset,
                                   u32_t ackno_offset, u8_t headerflags) {
  return tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, pcb->remote_port,
                            pcb->local_port, data, data_len,
 ...
 */
struct pbuf *tcp_create_rx_segment_wnd(struct tcp_pcb *pcb, void *data,
                                       size_t data_len, u32_t seqno_offset,
                                       u32_t ackno_offset, u8_t headerflags,
                                       u16_t wnd) {
  return tcp_create_segment_wnd(&pcb->remote_ip, &pcb->local_ip,
                                pcb->remote_port, pcb->local_port, data,
                                data_len, pcb->rcv_nxt + seqno_offset,
                                pcb->lastack + ackno_offset, headerflags, wnd);
}

/** Safely bring a tcp_pcb into the requested state */
void tcp_set_state(struct tcp_pcb *pcb, enum tcp_state state,
                   ip_addr_t *local_ip, ip_addr_t *remote_ip, u16_t local_port,
                   u16_t remote_port) {
...
static void test_tcp_counters_check_rxdata(struct test_tcp_counters *counters,
  ...
  EXPECT_RET(counters->recved_bytes + p->tot_len <=
             counters->expected_data_len);
...
err_t test_tcp_counters_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p,
                             err_t err) {
  struct test_tcp_counters *counters = arg;
  ...
  EXPECT(counters->recv_calls_after_close == 0 &&
         counters->recved_bytes_after_close == 0);
  return ERR_OK;
}
...
static err_t test_tcp_netif_output(struct netif *netif, struct pbuf *p,
  ...
  struct test_tcp_txcounters *txcounters =
      (struct test_tcp_txcounters *)netif->state;
