 ...
 * Application Module 2013-05-25     Bernard      port to v1.4.1 2017-03-26
 * HuangXiHans  port to v2.0.2 2017-11-15     Bernard      add lock for
 * init_done callback 2018-11-02     MurphyZhao   port to v2.1.0 2020-06-20
 * liuxianliang port to v2.1.2 2021-06-25     liuxianliang port to v2.0.3
...
void mem_overflow_check_raw(void *p, size_t size, const char *descr1,
  ...
  for (k = 0; k < MEM_SANITY_REGION_AFTER_ALIGNED; k++) {
    if (m[k] != 0xcd) {
      char errstr[128];
      rt_snprintf(errstr, sizeof(errstr), "detected mem overflow in %s%s",
                  descr1, descr2);
  ...
  for (k = 0; k < MEM_SANITY_REGION_BEFORE_ALIGNED; k++) {
    if (m[k] != 0xcd) {
      char errstr[128];
      rt_snprintf(errstr, sizeof(errstr), "detected mem underflow in %s%s",
                  descr1, descr2);
      LWIP_ASSERT(errstr, 0);
    }
  }
...
struct netif *lwip_ip4_route_src(const ip4_addr_t *dest,
