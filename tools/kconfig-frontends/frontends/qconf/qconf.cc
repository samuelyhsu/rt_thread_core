...
ConfigList::ConfigList(ConfigView *p, const char *name)
    : Parent(p), updateAll(false), symbolYesPix(xpm_symbol_yes),
      ...
      voidPix(xpm_void), showName(false), showRange(false), showData(false),
  ...
  setHeaderLabels(QStringList() << _("Option") << _("Name") << "N"
                                << "M"
                                << "Y" << _("Value"));
...
void ConfigList::mouseReleaseEvent(QMouseEvent *e) {
  ...
  case promptColIdx:
    ...
    if (!icon.isNull()) {
      int off = header()->sectionPosition(0) + visualRect(indexAt(p)).x() +
                4; // 4 is Hardcoded image offset. There might be a way to do it
                   // properly.
...
void ConfigInfoView::menuInfo(void) {
  ...
  if (sym) {
    ...
    if (showDebug()) {
      if (_menu->prompt->visible.expr) {
        debug += "&nbsp;&nbsp;dep: ";
        expr_print(_menu->prompt->visible.expr, expr_print_help, &debug,
                   E_NONE);
        debug += "<br><br>";
      }
    }
  ...
  if (showDebug())
    debug += QString().sprintf("defined at %s:%d<br><br>", _menu->file->name,
                               _menu->lineno);
...
ConfigSearchWindow::ConfigSearchWindow(ConfigMainWindow *parent,
  ...
  if (name) {
    ...
    height =
        configSettings->value("/window height", parent->height() / 2).toInt();
    resize(width, height);
...
ConfigMainWindow::ConfigMainWindow(void) : searchWindow(0) {
  ...
  singleViewAction =
      new QAction(QPixmap(xpm_single_view), _("Single View"), this);
