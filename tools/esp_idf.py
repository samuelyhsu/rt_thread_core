...
def GenerateCFiles(env, project):
    ...
    if cm_file:
        ...
        for group in project:
            for f in group['src']:
                path = _make_path_relative(
                    main_component_dir, os.path.normpath(f.rfile().abspath))
        ...
        if n:
            ...
            for i in range(n):
                cm_file.write("\tINTERFACE \"-u __rt_init_" +
                              init_export[i] + "\"\n")
            cm_file.write(")\n")
        cm_file.close()
    ...
    if cm_file:
        ...
        freertos_root = os.getcwd().replace('\\', '/') + \
            '/packages/FreeRTOS_Wrapper-latest/FreeRTOS'
